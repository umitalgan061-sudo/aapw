...
- `stable-2026-08-12-run292` — Pindex-05 promoted from the Run290 experiment candidate to the same `canonical-dev`-only activation path as Pindex-01..04 (ADR-RUN292); shipped `game3d.html`/`index.html` runtime source delta 0; 34-smoke/PWA/service-worker/world-reference/terrain-seat/road/technical-debt/additive-only/mobile-perf/world-event-determinism all PASS; mandatory DoD PASS.

- `stable-2026-08-12-run293` — Pindex-06 deterministic micro-surface detail, freshly surveyed and independently tuned (no pre-built candidate, ADR-RUN293); shipped `game3d.html`/`index.html` runtime source delta 0; 34-smoke/PWA/service-worker/world-reference/terrain-seat/road/technical-debt/additive-only/mobile-perf/world-event-determinism all PASS; mandatory DoD PASS.

- `stable-2026-08-12-run294` — Pindex-07 deterministic micro-surface detail, first pindex in this batch with real rock+snow cells (ADR-RUN294); shipped `game3d.html`/`index.html` runtime source delta 0; 34-smoke/PWA/service-worker/world-reference/terrain-seat/road/technical-debt/additive-only/mobile-perf/world-event-determinism all PASS; mandatory DoD PASS.
...
- `stable-2026-08-12-run318` — Pindex-10 deterministic micro-surface detail, completes the 10-pindex canonical-dev detail pass started at Run277 (ADR-RUN318); resolved a run-number collision against a concurrent session's unrelated Run317 Terrain3D height-field bridge (renumbered to Run318, no content lost either side, merge conflict in `3D_GAME_PROGRESS.md` resolved by keeping both sections). Shipped `game3d.html`/`index.html` runtime source delta 0 (`canonical-dev.html` preview path only). Full DoD re-verified fresh at commit time: `node --check` all changed files, `smokeTestGame3D.js` 34/34 PASS (zero uncaught page/console errors), `checkRun317Pindex10DetailBrowser.js` real headless-Chromium color-isolation proof (640 vertices touched, all Pindex-10, 0 outside it), `checkSmokeCheckRegistry`/`checkPwaInstallability`/`checkServiceWorkerCache`/`checkTechnicalDebt` (0 new debt)/`checkWorldReferenceMap`/`checkWorldReferenceAlignment`/`checkWorldEventDeterminism`/`checkMobilePerfBudget`/`terrainSeatSafetyCheck` (14/14)/`roadNetworkSafetyCheck` all PASS. Runtime adoption of the completed 10-pindex layer (promoting it from `canonical-dev.html` preview to the live default renderer) remains an open owner decision, not made by this run.

- `stable-2026-08-12-run321` — periodic platform control + rule consolidation (§8.12/§15, no code delta). Full DoD sweep fresh at commit time: `smokeTestGame3D.js` 34/34 PASS, `checkPwaInstallability`/`checkServiceWorkerCache`/`checkWorldReferenceMap`/`checkWorldReferenceAlignment`/`checkWorldReferenceHydrologyExtent`/`checkWorldReferenceWaterMask`/`checkWorldEventDeterminism`/`checkMobilePerfBudget`/`terrainSeatSafetyCheck` (14/14)/`roadNetworkSafetyCheck`/`checkTechnicalDebt`/`checkSmokeCheckRegistry` all PASS/OK; `analyzePerfTrend.js` no sustained drift over 220 rows. Concurrency clean, no resync needed. Governance rule set re-read end to end, no stale/conflicting rule found, deferred-rule activation conditions still unmet.

- `stable-2026-08-13-0055` — per-morph dragon scale shape (ADR-0269, Run 322): 7 dragon-scales palettes now resolve distinct scale geometry (row count/aspect/overlap/ridge/scar profile) instead of sharing one recoloured lattice; `dragon-black` keeps its original unmodified look. Full DoD fresh at commit time: `node --check` clean, `checkTexturePaletteLibrary.js` PASS (110/110 palettes, new baseline/distinctness/purity assertions for the 7 dragon profiles), full `smokeTestGame3D.js` 34/34 PASS (all 8 dragon-behaviour checks unaffected — texture-only change), `checkTechnicalDebt.js` PASS (0 new debt). Runtime/product delta: one file (`materials/dragonTextures.js`) + its test; no shipped gameplay/world/physics path touched. Concurrency clean (local `HEAD` matched `origin/main` at commit time; a detached-HEAD state found at session start from an unrelated automated CI commit was fast-forward-resolved first, no work lost).

- `stable-2026-08-13-run325` — water animation, two chained subtasks (ADR-0270 depth-tapered sea/lake swell, ADR-0271 downstream river/waterfall flow). The sea now really undulates (three dispersion-derived swell components, ~4.3m crest-to-trough over a ~200m dominant wavelength) without re-introducing ADR-0048's shallow-lake flicker: amplitude is tapered by baked bathymetry so the trough is provably always shallower than the water column (2.15m total amplitude vs. a 10m full-wave depth), and the ADR-0048 smoke guard was replaced with a stronger one asserting that inequality rather than banning wave maths outright. Rivers and their 13 waterfall curtains now show real downstream foam whose speed follows the local bed gradient (1.52-4.15 m/s, sqrt-of-slope as open-channel flow actually behaves). Full DoD fresh at commit time: `node --check` clean on all changed files, `smokeTestGame3D.js` 34/34 PASS exit 0 with zero console/page errors, `checkWaterVisualContract`/`checkSmokeCheckRegistry`/`checkTechnicalDebt` (0 new debt)/`checkPwaInstallability`/`checkServiceWorkerCache`/`checkWorldReferenceMap`/`checkWorldReferenceAlignment`/`checkWorldEventDeterminism`/`roadNetworkSafetyCheck`/`terrainSeatSafetyCheck` (14/14) all PASS, `checkMobilePerfBudget` PASS at 236,889 triangles vs the 500K budget. Two new evidence scripts each provide a real GPU pixel read-back (21.3% and 9.51% of rendered pixels change over time) plus 4 screenshots. Concurrency: `origin/main` had advanced 7 commits (Godot terrain-authoring track) during the session; fast-forwarded before publishing, zero file overlap.

- `stable-2026-08-13-run326` — procedurally rigged creature bodies (ADR-0272): 19 species (cat, dog, horse, elephant, wolf, deer, sheep, cow, goat, boar, rabbit, bear, lion, giraffe, human, soldier, raven, eagle, chicken) now build a real `THREE.Skeleton` + bound `SkinnedMesh` from parametric real-metre proportions, with no asset download — which is what finally unblocks `creatureSpeciesConfig.js`'s five-year-old `awaiting-model` state, since `assets_manifest.json` shows most of these species have no rigged model and ten have no model at all. Full DoD fresh at commit time: `node --check` clean, `smokeTestGame3D.js` 34/34 PASS exit 0 zero console/page errors, `checkRun326CreatureRig.js` PASS on all 19 (single root bone, skin weights sum to 1, indices in range, bind pose on the ground, proportions in band, same-species rigs bit-identical, single-bone rotation deforms only its own weighted vertices), `checkServiceWorkerCache`/`checkPwaInstallability`/`checkTechnicalDebt` (0 new debt)/`checkSmokeCheckRegistry` all PASS/OK. `perf_log.csv` sample identical to run325 on every render figure — nothing in the shipped scene graph imports these modules yet, so the live game is provably unaffected. Concurrency clean at commit time. Gait and behaviour AI are the declared next subtasks; this run is the rig only.

- `stable-2026-08-13-run327` — procedural gait driver for the run-326 creature rigs (ADR-0273): new `gameplay/creatureGait.js` poses any of the 19 species' bones through all ten named gaits (`walk`/`trot`/`pace`/`prowl`/`stride`/`hop`/`gallop`/`bound`/`sprint`/`flap`) as a pure, deterministic function of elapsed time and the plan's own `strideHz` — legs swing with a counter-bending ankle, a tail wag runs independent of gait, wings flap (with a lagging tip) only during `flap`, legs stay tucked mid-flight. Full DoD fresh at commit time: `node --check` clean, `smokeTestGame3D.js` 34/34 PASS exit 0 zero console/page errors, `checkRun327CreatureGait.js` PASS on 38 species x gait combinations (finite/bounded rotations, correct bones move per gait, deterministic, reset zeroes every touched bone) plus a real GPU pixel read-back proving visible before/after movement for one representative species per archetype, `checkServiceWorkerCache`/`checkPwaInstallability`/`checkTechnicalDebt` (0 new debt)/`checkSmokeCheckRegistry` all PASS/OK. `perf_log.csv` sample identical to run326/325 on drawCalls/triangles/geometries/textures — nothing in the shipped scene graph imports this module yet, so the live game is provably unaffected. Concurrency: `origin/main` had advanced 1 commit (Godot terrain-authoring track, G65 rock/snow) during the session; fast-forwarded before publishing, zero file overlap. Behaviour AI (`gameplay/creatureBrain.js`) is the declared next subtask; this run is the gait driver only, still unwired from any live tick.

- `stable-2026-08-13-run328` — no code shipped; verification + concurrency-collision handling run. Built a fully working `gameplay/creatureGait.js` independently (per-limb pivot bones added to `creatureRig.js` to fix a shared-`root`/`chest` limitation, 19/19 species PASS on its own check script) but a fresh `origin/main` fetch immediately before publishing found a concurrent session had already merged the identical task (ADR-0273, commit `b619f3b`) minutes earlier. Discarded the local duplicate, fast-forwarded cleanly, then re-verified the full baseline fresh at this exact commit: `smokeTestGame3D.js` 34/34 PASS exit 0 zero console/page errors, `checkSmokeCheckRegistry`/`checkWorldReferenceMap`/`checkSeededRandomPolicy`/`terrainSeatSafetyCheck` (14/14)/`roadNetworkSafetyCheck`/`checkCreatureSpeciesConfig`/`checkTechnicalDebt` (0 new)/`checkPwaInstallability`/`checkServiceWorkerCache` all PASS/OK, no regressions. Priority re-scan: item 1.5 (ground/grass color) reconfirmed already resolved (ADR-0073); item 1.7 (castle texturing) reconfirmed still asset-blocked (ADR-0131) with a new unblock idea logged for a future run (procedural castle geometry, same pattern as run 326's creature rigs). Next subtask unchanged from run 327: `gameplay/creatureBrain.js`.

- `stable-2026-08-13-run330-reverify` — Run330 checkpoint continuity repair only; no product/runtime source changed by this record. Fresh exact-main verification on `bea79c20e6b9426e80f640655657d78e3eb2629a` passed seeded/world-reference checks, 14/14 settlement safety, road connectivity/grade/river/visual/material safety, PWA installability/cache, technical-debt guard, mobile render budget and the full real-browser smoke suite with zero blocking errors. Fresh real renderer sample recorded by `collectPerfSnapshot.js`: `2026-08-13,run330-checkpoint-reverify,1,48,858912,46,16,391`.

- `stable-2026-08-13-run331` — Run331 village-collision current-main reverify; existing real renderer sample `2026-08-13,run331-village-collision,1,52,866460,50,24,347`; seeded/world-reference, 14/14 settlement safety, road safety, PWA/cache, technical-debt, mobile budget and full real-browser smoke all PASS on exact current main.

- `stable-2026-08-13-run332` — Run332 NPC/animal collision current-main reverify; existing real renderer sample `2026-08-13,run332-npc-animal-collision,0,57,904446,51,29,257`; seeded/world-reference, 14/14 settlement safety, road safety, PWA/cache, technical-debt, mobile budget and full real-browser smoke all PASS on exact current main.

- `stable-2026-08-13-run334` — Run334 bird flight locomotion (ADR-0280) checkpoint on commit `e68fc55`; real renderer sample `2026-08-13,run334-bird-flight,1,52,866460,50,24,273`; full 35/35 real-browser smoke, checkRun329CreatureBrain.js (16/16 species), new checkRun334BirdFlight.js, 14/14 settlement safety, road safety, PWA/cache, technical-debt guard all PASS. `git push origin <tag>` failed with the known/documented HTTP 403 (§8.11) — local tag + this line stand in for it.

- `stable-2026-08-13-run335` — Run335 FAZ 8 world-event catalog growth, 5 new curated events (ADR-0281) checkpoint on commit `0c230e6`; real renderer sample `2026-08-13,run335-worldevents-catalog-growth,1,52,866460,50,24,391`; full 35/35 real-browser smoke, checkWorldEventDeterminism.js PASS (regenerated fixture), checkTechnicalDebt.js, checkSmokeCheckRegistry.js, checkSeededRandomPolicy.js, checkServiceWorkerCache.js/checkPwaInstallability.js all PASS. `git push origin <tag>` outcome recorded below (§8.11 known constraint).

- `stable-2026-08-13-2306` — Run336 horse-drawn cart vehicle (ADR-0282), FAZ 6 complete, checkpoint on commit `ee90448`; real renderer sample `2026-08-13,run336-cart-vehicle,1,57,904446,51,29,347`; full 35/35 real-browser smoke, new checkRun336CartVehicle.js (deterministic spawn/pose, ping-pong bounds, wheel-spin, rendered pixel proof), checkTechnicalDebt.js, checkSmokeCheckRegistry.js, checkSeededRandomPolicy.js, checkServiceWorkerCache.js/checkPwaInstallability.js, terrainSeatSafetyCheck.js (14/14), roadNetworkSafetyCheck.js (20.24km, unaffected) all PASS.

- `stable-2026-08-14-run337` — Run337 player-cart collision (ADR-0283), closing run336's named gap; checkpoint on commit `89c5a1a`; real renderer sample `2026-08-14,run337-player-cart-collision,1,52,866460,50,24,391`; full 35/35 real-browser smoke including new `checkPlayerCartDynamicCollider` (live re-query + post-construction registerDynamicCollider proven), checkTechnicalDebt.js, checkSmokeCheckRegistry.js, checkSeededRandomPolicy.js, checkAssetsManifest.js, checkServiceWorkerCache.js/checkPwaInstallability.js, terrainSeatSafetyCheck.js (14/14), roadNetworkSafetyCheck.js (20.24km, unaffected) all PASS.

- `stable-2026-08-14-run338` — Run338 grade-aware cart speed (ADR-0284), closing run336/337's own repeatedly-named gap; checkpoint on commit `56ef656`; real renderer sample `2026-08-14,run338-cart-grade-speed,1,52,866460,50,24,347`; full 35/35 real-browser smoke (cold baseline before, full re-run after) including new `checkRun338CartGradeSpeed.js` (flat/gentle-grade regression guard, steep uphill/downhill directional speed proof, wheel-spin/translation proportionality, determinism, rendered pixel proof), checkTechnicalDebt.js, checkSmokeCheckRegistry.js, checkSeededRandomPolicy.js, checkAssetsManifest.js, checkServiceWorkerCache.js/checkPwaInstallability.js, terrainSeatSafetyCheck.js (14/14), roadNetworkSafetyCheck.js (20.24km, unaffected) all PASS.

- `stable-2026-08-14-run339` — Run339 menu/pause flow (ADR-0285), closing the last of §3's seven "what a full game needs" gaps to have zero code; checkpoint on commit `d68c858`; real renderer sample `2026-08-14,run339-pause-menu,1,57,904446,51,29,415`; full 37/37 real-browser smoke (run twice — before and after fixing a real CSS click-interception regression and a real checkServiceWorkerCache.js FAIL, both found and fixed in this same pass) including new `checkPauseMenu` (getComputedStyle display assertion, verified to fail on the bug/pass on the fix), real freeze proof (day/night clock frozen across 2.5 real seconds while paused), checkTechnicalDebt.js, checkSmokeCheckRegistry.js, checkSeededRandomPolicy.js, checkAssetsManifest.js, checkServiceWorkerCache.js/checkPwaInstallability.js, terrainSeatSafetyCheck.js (14/14), roadNetworkSafetyCheck.js (20.24km, unaffected) all PASS.

- `stable-2026-08-14-run340` — Run340 dialogue input paused-gate (ADR-0286), closing run 339's own disclosed `QUESTIONS_FOR_OWNER.md` gap; checkpoint on commit `677b1f0`; real renderer sample `2026-08-14,run340-dialogue-pause-gate,1,52,866460,50,24,391`; full 38/38 real-browser smoke (cold baseline before, full re-run after) including new `checkDialoguePauseGate` (proves choice-select/close/Escape are no-ops while `isPaused()` is true, then work again once false), checkTechnicalDebt.js, checkSmokeCheckRegistry.js, checkSeededRandomPolicy.js, checkAssetsManifest.js, checkServiceWorkerCache.js/checkPwaInstallability.js, terrainSeatSafetyCheck.js (14/14), roadNetworkSafetyCheck.js (20.24km, unaffected) all PASS.

- `stable-2026-08-14-run341` — Run341 pause-menu settings screen (graphics quality, ADR-0289), closing `GOVERNANCE_FULL_GAME_DIRECTIVE.md` §3's last half-open row; checkpoint on commit `c27008a`; real renderer sample `2026-08-14,run341-pause-settings-quality,1,58,816332,57,27,391`; full 39/39 real-browser smoke (cold baseline before, full re-run after) including new `checkPauseMenuSettings` (desktop 4-radio group, apply persists+reloads, fresh-instance re-read from storage, mobile note-only, `resolveRenderQuality()` manual-override resolution proven directly), plus a real pre-existing `checkServiceWorkerCache.js` FAIL found and closed in the same pass (two `src/3d` files + 10 animal models from other concurrent commits never registered; `SHELL_CACHE` v11->v12), checkTechnicalDebt.js, checkSmokeCheckRegistry.js, checkSeededRandomPolicy.js, checkAssetsManifest.js, checkPwaInstallability.js, terrainSeatSafetyCheck.js (14/14), roadNetworkSafetyCheck.js (20.24km, unaffected) all PASS. Tag created locally (`git tag -a`) but `git push origin stable-2026-08-14-run341` failed twice with HTTP 403 (the branch push itself succeeded cleanly to the same remote) — the remote also warned this repo has moved to `github.com/umitalgan061-sudo/aapw`, possibly related. Commit `c27008a` is on remote `main` regardless; only the tag ref itself is not yet mirrored — retry next run or once the move is resolved.

- `stable-2026-08-14-run342` — Run342 controls-help/pause-menu Escape coexistence (ADR-0290), closing run 339's own last disclosed `QUESTIONS_FOR_OWNER.md` scope edge; rebased cleanly onto a concurrent session's `c27008a`/`fb120b4` (pause-menu settings screen, ADR-0289) discovered mid-session, three-file text conflict resolved (both ADRs kept, both new smoke checks kept, this run's own duplicate service-worker fix discarded in favor of the already-landed upstream one — same category as run328's concurrency precedent); checkpoint on commit (see git log for exact hash at push time); real renderer sample `2026-08-14,run342-escape-coexistence-fix,1,58,816332,57,27,368` (byte-identical draw-call/triangle/geometry/texture figures to run341, as expected for a pure input-handling change); full 40/40 real-browser smoke run fresh against the final post-rebase merged tree (up from run341's 39 — this run's own check plus the settings-screen check both present and correctly wired), checkSmokeCheckRegistry.js (40 checks/15 modules), checkTechnicalDebt.js, checkSeededRandomPolicy.js, checkAssetsManifest.js, checkServiceWorkerCache.js/checkPwaInstallability.js, terrainSeatSafetyCheck.js (14/14), roadNetworkSafetyCheck.js (20.24km, unaffected) all PASS.

- `stable-2026-08-14-run343` — Run343 `pixelRatioCap` wired into the renderer (ADR-0291), narrowing run341's disclosed `QUALITY_PRESETS` gap from three unwired knobs to two (`drawDistance`/`textureSize` remain); checkpoint on commit `9041fad`; real renderer sample `2026-08-14,run343-pixelratio-cap,0,63,854318,57,31,273` (drift vs run342 within the project's own documented camera-frustum sampling variance); full `smokeTestGame3D.js` run twice — cold baseline (40/40 PASS, exit 0) and again after the change (40/40 PASS, exit 0, zero console/page errors both times), checkTechnicalDebt.js, checkSeededRandomPolicy.js, checkSmokeCheckRegistry.js (40 checks/15 modules, `sceneManager.js` 530->538/600), checkAssetsManifest.js, terrainSeatSafetyCheck.js (14/14), roadNetworkSafetyCheck.js (20.24km, unaffected) all PASS. `git push origin stable-2026-08-14-run343` failed with the same HTTP 403 run341 already hit (branch push itself succeeded cleanly to the same remote) — the remote again warned this repo has moved to `github.com/umitalgan061-sudo/aapw`; same known, already-flagged (run151/run322) constraint, not re-raised. Commit `9041fad` is on remote `main` regardless; only the tag ref is not yet mirrored.

- `stable-2026-08-19-0330` — Run345 (`6d99864`), follow-on to the concurrent Run344 LFS/repo-rename RCA (`f5c2e45`): applied that run's `add_repo(..., "aapw")` workaround, fetched all 508 real LFS objects directly (`git lfs pull` itself hangs in this sandbox even once authorized — worked around with a local batch-download script), found and fixed one new pre-existing bug (`*.FBX` uppercase files never matched `.gitattributes`' lowercase-only `*.fbx` pattern — one-line additive fix, verified inert on already-committed content), then ran the full `smokeTestGame3D.js` suite fresh against real assets for the first time since the rename: **42/42 PASS, 0 FAIL**. `checkTechnicalDebt.js`/`checkSeededRandomPolicy.js` PASS. `git push origin stable-2026-08-19-0330` failed with the same known `HTTP 403`/repo-moved-to-`aapw` constraint as every prior tag push (run151/322/341/343) — branch push (`main`) succeeded cleanly regardless; only the tag ref is not mirrored.

- `stable-2026-08-19-0430` — Run346 (`4fedbda`), first audio in the game (ADR-0292): `audio/audioManager.js` (real `THREE.AudioListener` + `playClick()`/`dispose()`) wired into `ui/pauseMenu.js`'s existing open/close gesture, playing one CC0 UI-click sound (Kenney, via the Calinou/kenney-ui-audio mirror). Non-browser gates all PASS fresh: `checkTechnicalDebt.js`, `checkSeededRandomPolicy.js`, `checkAssetsManifest.js` (499 entries), `checkServiceWorkerCache.js` (188 JS files, `SHELL_CACHE` v16->v17), `checkPwaInstallability.js`, `checkSmokeCheckRegistry.js` (43 checks/19 modules, no file over the 600-line cap), `terrainSeatSafetyCheck.js` (14/14, unaffected), `roadNetworkSafetyCheck.js` (17.84km, unaffected). Real-browser verification is narrower than a full clean suite pass this run, disclosed plainly: this session's checkout still has all `.fbx`/`.glb` files as git-lfs pointer stubs (the exact condition `RCA_RUN344_LFS_REPO_RENAME.md` already root-caused; `git lfs pull` hangs here too), so the full `smokeTestGame3D.js` 43-check suite could not complete a clean end-to-end pass — a standalone targeted run of the 5 checks touching this change's own surface (2D shell, the new audio check, and all three pause-menu checks) ran fresh against a real page load with a genuine trusted `page.click()` and all **5/5 PASS**, zero *new* console/page errors beyond the already-disclosed LFS-stub asset-load warnings every 3D-mode check in this session shows regardless of this diff. `collectPerfSnapshot.js run346-first-audio`: 55 draw calls/709,382 triangles/57 geometries/22 textures/202MB heap — no drawcall/triangle regression (this change adds zero renderable geometry). `git push origin stable-2026-08-19-0430` failed with the same known `HTTP 403`/repo-moved-to-`aapw` constraint as every prior tag push — branch push (`main`, commit `4fedbda`) succeeded cleanly regardless; only the tag ref is not mirrored.

- `stable-2026-08-19-0456` — Run347 (`48f0895`), mute control for the pause-menu click sound (ADR-0293): `audioManager.setMuted()`/`isMuted()` (real `THREE.AudioListener.setMasterVolume()`) + a live-apply checkbox in `ui/pauseMenu.js`'s settings screen (both desktop and mobile), persisted via new `STORAGE_KEYS.SOUND_MUTED`, no reload needed. Non-browser gates all PASS fresh: `checkTechnicalDebt.js`, `checkSeededRandomPolicy.js`, `checkAssetsManifest.js` (unchanged, no new asset), `checkSmokeCheckRegistry.js` (44 checks/18 modules, no file over the 600-line cap). Same pre-existing LFS-pointer-stub environment condition Runs 344/345/346 already root-caused meant the full `smokeTestGame3D.js` suite could not complete end-to-end here either (`timeout 280s` attempt produced zero output, consistent with Run 346's own `checkFreeCamera` hang finding) — worked around the same way Run 346 did: a standalone script running only this run's new `checkPauseMenuMute` + extended `checkAudioManager` directly against a real `game3d.html` page load, both **PASS**. `collectPerfSnapshot.js run347-mute-control`: 55 draw calls/709,382 triangles/57 geometries/22 textures/202MB heap — byte-identical to Run 346's own sample, zero rendering-cost regression. `git push origin stable-2026-08-19-0456` failed with the same known `HTTP 403`/repo-moved-to-`aapw` constraint as every prior tag push (run151/322/341/343/345/346) — branch push (`main`, commit `48f0895`) succeeded cleanly regardless; only the tag ref is not mirrored.

- `stable-2026-08-19-0503` — Run348 (`4bf8a1c`), same session as Run347 — second sound cue (ADR-0294): `audioManager.playDiscoveryChime()` reuses `ui-click.wav`'s already-loaded buffer at a distinct volume/pitch (`setPlaybackRate`, no new asset), wired to `ui/settlementDiscovery.js`'s existing `_discover()` event via a new `onDiscover` callback. Non-browser gates all PASS fresh: `checkTechnicalDebt.js`, `checkSeededRandomPolicy.js`, `checkAssetsManifest.js` (unchanged), `checkSmokeCheckRegistry.js` (still 44 checks/18 modules — two existing checks extended, not new; `game3d.js` now 596/600, flagged as this run's own disclosed near-cap risk for the next run touching that file). Same pre-existing LFS-pointer-stub environment condition Runs 344-347 already root-caused meant the full `smokeTestGame3D.js` suite still could not complete end-to-end this session — worked around the same way: a standalone script running only the two extended checks (`checkSettlementDiscovery`, `checkAudioManager`) directly against a real `game3d.html` page load, both **PASS**. `collectPerfSnapshot.js run348-discovery-chime`: 55 draw calls/709,382 triangles/57 geometries/22 textures/202MB heap — byte-identical to this session's own Run 347 sample, zero rendering-cost regression. `git push origin stable-2026-08-19-0503` failed with the same known `HTTP 403`/repo-moved-to-`aapw` constraint as every prior tag push — branch push (`main`, commit `4bf8a1c`) succeeded cleanly regardless; only the tag ref is not mirrored.

- `stable-2026-08-19-0511` — Run348b (`082b1c0`), same session as Run347/348 — periodic platform control + rule consolidation (§8.12, window 341-351): full non-browser gate sweep (PWA installability, service-worker cache, world-reference/hydrology/alignment/water-mask, terrain-seat/road-network/world-event-determinism/technical-debt/seeded-random/assets-manifest/additive-only guards, perf-trend drift) all PASS fresh; `npm audit` N/A (no `package.json`). Found and fixed one real bug: `checkCameraContract.js`'s hardcoded 15000ms nav timeout (below this project's own documented `game3d.html` boot cost) reproducibly failed twice, bumped to the same 30000ms convention every sibling check already uses, re-verified PASS. §16 deferred-rule activation conditions re-checked, all still unmet (`SaveSystem` absent, no public API/mod support, smoke suite/F2 panel still sufficient). Same pre-existing LFS-pointer-stub condition (Runs 344-348) still blocks a clean full `smokeTestGame3D.js`/`checkMobilePerfBudget.js` pass in this session — not new, not re-raised. `git push origin stable-2026-08-19-0511` failed with the same known `HTTP 403`/repo-moved-to-`aapw` constraint as every prior tag push — branch push (`main`, commit `082b1c0`) succeeded cleanly regardless; only the tag ref is not mirrored.

- `stable-2026-08-19-1142` — Run349 (`f02cc2f`), owner-requested visible geographic realism (ADR-0295): new `world/terrainBiomeShading.js` (slope/altitude biome albedo — shore sand, grass, forest patches, slope-driven rock, altitude+canonical snow that sheds off >40° faces, seabed) and `world/terrainReliefDetail.js` (coast domain-warp that breaks the canonical mask's ~138 m rectangular staircase + ridged land relief), both tapered to zero within 650 m of every kingdom seat; `overlay.png` neutralised at load into a unit-mean luminance multiplier (it is a saturated green photo texture, measured 0.42 mean saturation — hue authority moved to per-vertex shading); runtime `qualityBlend` 0.88→0.34; ocean palette retuned to the owner's reference. **§8.4 terrain gate run before AND after: 14/14 seats PASS with raw heights byte-identical to baseline**, roads PASS (13 edges, all grades <20°, 17.84→17.73 km), water-mask checksum `2ca2bed8d8a1…`, alignment 100%, hydrology extent, world-event determinism `ea2bd3bfff60…` all unchanged. `checkTerrainVisualContract` PASS incl. **65/65 seam vertices continuous** and rendered-height==sampler at every vertex; water/road/vegetation/lighting/fog visual contracts, `checkServiceWorkerCache` (191 JS, SHELL_CACHE v17→v18), `checkPwaInstallability`, `checkTechnicalDebt`, `checkSeededRandomPolicy`, `checkAssetsManifest`, `checkSmokeCheckRegistry` (44 checks, 545 files under the 600-line cap) all PASS. `terrain.js` crossed the cap so the micro-PBR block moved verbatim to `world/terrainMicroSurface.js` and is re-exported (626→491). `collectPerfSnapshot.js run349-terrain-realism`: 59 draw calls/718,672 triangles/60 geometries/28 textures/202MB — well inside the desktop <2500/<5M budget. Evidence in `artifacts/terrain-biome-shading/{before,after}/` (gitignored): 5 framings each, 613 chunks, zero console errors both runs. Also fixed a shared pre-existing defect (§8.2, same root cause as run 348b): 25 scripts used a 15 s `game3d.html` navigation timeout below the project's own ~9-13 s boot cost and had been timing out instead of asserting; raising it exposed two drifted assertions in `checkTerrainVisualContract.js`, both corrected. `checkSkyVisualContract.js` has further pre-existing drift, disclosed and deliberately not half-fixed. `git push origin stable-2026-08-19-1142` failed with the same known `HTTP 403`/repo-moved-to-`aapw` constraint as every prior tag push — branch push (`main`, `f02cc2f`) succeeded cleanly; only the tag ref is not mirrored.
