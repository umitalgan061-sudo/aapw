...
- `stable-2026-08-12-run297` — unified seam-free HD base-map sampling (ADR-RUN297): one parameterized layer replaces the nine per-pindex copies and completes the pass at 10/10 pindexes. Fixes five measured defects — nine amplitude seams (worst boundary step 5.00e-3 → 1.17e-4, 43x smoother), eight boundary-straddling mask columns, white-noise micro-detail (lag-1 autocorrelation 0.0405 → 0.767 at mask-cell sampling), 16-pixel staircase edges on every surface boundary (bilinear per-class weights, one-hot at every cell centre so canonical classification is untouched), and per-vertex BigInt decoding (~470 → ~50 ns/call, ~9x). Idempotent and supersede-not-compound, both proven at drift 0. Shipped `game3d.html`/`index.html` runtime source delta 0; 34-smoke/PWA/service-worker/world-reference/terrain-seat/road/technical-debt/additive-only/mobile-perf/world-event-determinism all PASS; mandatory DoD PASS.
