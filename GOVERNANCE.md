...
### Run348 platform-control + rule-consolidation superseding note (2026-08-19, scheduled routine)
The latest periodic platform control is now **run 348 (2026-08-19)**, superseding the historical run321 maintenance note without deleting it — run321's own window (~341-351) was due; concurrency was clean throughout this session (`git fetch origin main` re-run before every commit, matched exactly each time, no drift/resync needed). `package.json` remains absent, so `npm audit` remains N/A (`ENOLOCK`, same as every prior check). Full sweep re-run fresh: `checkPwaInstallability.js`/`checkServiceWorkerCache.js` (188 JS files) PASS; `checkWorldReferenceMap.js`/`checkWorldReferenceAlignment.js`/`checkWorldReferenceHydrologyExtent.js`/`checkWorldReferenceWaterMask.js` PASS; `checkWorldEventDeterminism.js` PASS (24-emission checksum match); `terrainSeatSafetyCheck.js` PASS 14/14; `roadNetworkSafetyCheck.js` PASS (mountain-avoidance + river non-collision, 17.84km network); `checkTechnicalDebt.js`/`checkSeededRandomPolicy.js`/`checkAssetsManifest.js`/`checkAdditiveOnlyDiff.js` (disabled since ADR-0263, always PASS) all PASS; `checkSmokeCheckRegistry.js` OK — 44 checks/18 modules, `game3d.js` now 596/600 (flagged this session as a real near-cap risk, see `3D_GAME_PROGRESS.md` Run348/ADR-0294); `analyzePerfTrend.js` — 247 rows, no sustained `jsHeapUsedMB` drift (first-half avg 320.7MB vs second-half avg 328.4MB, ratio 1.02x). **One real, small bug found and fixed in this same pass:** `checkCameraContract.js` used a hardcoded 15000ms `page.goto()` timeout — well under this project's own documented `game3d.html` boot cost (~9-13s typical, 20s+ outliers, see e.g. `game3dSmokeChecksSettlementDiscovery.js`'s own run-332 RCA) — and reproducibly timed out twice in a row this session while every other active `game3d.html`-navigating check already uses 30000ms; bumped to match, re-verified PASS immediately after (camera/OrbitControls contract itself unaffected — pure timeout value, no assertion logic touched). **`checkMobilePerfBudget.js` and the full `smokeTestGame3D.js` 44-check suite could not complete a clean pass this session** — the same pre-existing `RCA_RUN344_LFS_REPO_RENAME.md` condition (all `.fbx`/`.glb` files as ~130-byte LFS pointer stubs in this session's checkout, `git lfs` not even installed as a CLI here) already disclosed and notification-pushed by Run 344, re-confirmed independently by Runs 346/347/348's own work this window — not new information, no re-raised `QUESTIONS_FOR_OWNER.md` entry. `perf_log.csv` gained three real samples this session (`run347-mute-control`, `run348-discovery-chime`, `run348-platform-control`), all byte-identical on draw-calls/triangles/geometries/textures (55/709,382/57/22) — expected, none of this session's changes touch rendered geometry; `jsHeapUsedMB` varied (202/202/179) within this project's own already-documented run-to-run GC noise, not a regression signal. Rule-consolidation review (§8.12): every numbered section re-read, focused re-check on §16-18 (most likely to drift); no stale or internally-conflicting rule found. §16's deferred-rule activation conditions remain unmet (`SaveSystem` still absent — confirmed via `grep -rn SaveSystem src/`, only a comment referencing its future non-existence; no public API/mod support; smoke suite still judged sufficient at 44 checks with no sign of frequent regression escapes; F2 `renderer.info` still sufficient, no drift). This session's own two feature subtasks (ADR-0293 mute control, ADR-0294 discovery chime) are recorded separately in `3D_GAME_PROGRESS.md`/`DECISIONS.md`, not restated here. The next periodic platform-control **and** rule-consolidation window is approximately **run 368-378**, unless a phase completion or material platform change justifies an earlier check.
