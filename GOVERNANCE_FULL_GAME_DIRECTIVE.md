...
| 6 | Ses (müzik + efekt) | **Kısmen başladı (Run 346, genişledi Run 347/348).** `audio/audioManager.js` + tek bir CC0 UI tık sesi (`assets/audio/ui-click.wav`, Kenney) — duraklatma menüsü açılış/kapanışında çalıyor. **Run 347: sesi kapat/aç kontrolü eklendi** (`ui/pauseMenu.js`'nin Ayarlar sekmesinde, hem masaüstü hem mobilde görünen bir onay kutusu; canlı uygulanıyor, sayfa yeniden yüklemesi gerektirmiyor). **Run 348: ikinci bir ses ipucu eklendi** — yeni bir yerleşim keşfedildiğinde, aynı tık sesi dosyası farklı hacim/perdede (`playDiscoveryChime()`) çalıyor, yeni asset indirilmedi. Hâlâ yok: müzik, savaş/adım/ortam efekti, hacim kaydırıcısı (yalnız aç/kapa) — tek ses efektine oranla henüz gereksiz görülen bir sonraki dilim. |
