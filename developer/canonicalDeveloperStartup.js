 ...
 *
 * This module intentionally lives outside src/3d and is never imported by the default game3d
 * runtime. A developer/preflight harness must provide the already-created current runtime state
 * plus an explicit canonical bridge id; no owner/product choice is inferred here. Activation
 * composes the proven Run196 current-runtime ownership transaction with Run197's reversible real-
 * tick freeze and immediately enters canonical mode. Rollback/dispose return ownership to the
 * unchanged current runtime.
 *
 * The production service worker's existing same-origin network-first runtime-cache behavior can
 * cache this developer module after an explicit online import. No service-worker source change is
 * required, preserving the repository's strict additive-only guard.
 * @module developer/canonicalDeveloperStartup
 */
