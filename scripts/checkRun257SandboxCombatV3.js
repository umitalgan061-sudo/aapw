...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run257-sandbox-combat-v3');
...
async function installTransitionRecorder(page) {
  await page.evaluate(() => {
    ...
    if (!root) throw new Error('combat HUD unavailable');
    root.dataset.run257ObservedAttack = '';
    root.dataset.run257ObservedDodge = '';
    root.dataset.run257ObservedBlock = '';
    globalThis.__run257CombatObserver?.disconnect?.();
    const observe = () => {
      const stamina = Number(root.dataset.stamina);
      if (Number.isFinite(stamina) && stamina < 99 && !root.dataset.run257ObservedAttack) {
        root.dataset.run257ObservedAttack = String(stamina);
      }
      if (root.dataset.dodging === 'true') root.dataset.run257ObservedDodge = 'true';
      if (root.dataset.blocking === 'true') root.dataset.run257ObservedBlock = 'true';
    };
    const observer = new MutationObserver(observe);
    observer.observe(root, { attributes: true, attributeFilter: ['data-stamina', 'data-dodging', 'data-blocking'] });
    globalThis.__run257CombatObserver = observer;
    observe();
  });
}
async function transitionEvidence(page) {
  return page.locator('.g3d-sandbox-combat').evaluate((root) => ({
    attack: root.dataset.run257ObservedAttack,
    dodge: root.dataset.run257ObservedDodge,
    block: root.dataset.run257ObservedBlock,
  }));
}
...
async function main() {
  ...
  assert(playerSource.includes('STAMINA_REGEN_DELAY_SECONDS_RUN257_RCA'), 'Stamina readability delay missing');
  assert(npcSource.includes('SANDBOX_GUARD_COMBAT_CONFIG_RUN257'), 'Reactive guard combat contract missing');
  assert(npcSource.includes('EVENTS.PLAYER_DAMAGED'), 'Guard attacks are not wired to existing player health');
  assert(npcSource.includes('BLOCKED_ATTACK_DAMAGE') && npcSource.includes('playerDodging'), 'Block/dodge guard-response contract missing');
  assert(interactionSource.includes('sandboxCombatHostile') && interactionSource.includes('sandboxCombatDefeated'), 'Combatant dialogue suppression missing');
  ...
  try {
    ...
    await installTransitionRecorder(desktopPage);
    ...
    await desktopPage.waitForFunction(() => {
      const root = document.querySelector('.g3d-sandbox-combat');
      return Number(root?.dataset.run257ObservedAttack) < 99 && root?.dataset.run257ObservedBlock === 'true' && root?.dataset.run257ObservedDodge === 'true';
    }, null, { timeout: 8000 });
    const desktopEvidence = await transitionEvidence(desktopPage);
    assert(Number(desktopEvidence.attack) < 99 && desktopEvidence.block === 'true' && desktopEvidence.dodge === 'true', `Desktop transition evidence missing: ${JSON.stringify(desktopEvidence)}`);
    ...
    assert(mobileLayout.buttons.length === 3 && mobileLayout.buttons.every((button) => button.height >= 44 && button.display !== 'none'), `Mobile controls are not touch-safe: ${JSON.stringify(mobileLayout.buttons)}`);
    await installTransitionRecorder(mobilePage);
    ...
    const mobileEvidence = await transitionEvidence(mobilePage);
    assert(Number(mobileEvidence.attack) < 99 && mobileEvidence.block === 'true' && mobileEvidence.dodge === 'true', `Mobile transition evidence missing: ${JSON.stringify(mobileEvidence)}`);
    ...
    console.log(`[checkRun257SandboxCombatV3] PASS ${JSON.stringify({ arc, desktopEvidence, mobileEvidence, touchButtons: mobileLayout.buttons.length, persistenceKeysAdded: 0, screenshots: 2 })}`);
  } finally {
    await desktop.close();
    await mobile.close();
    await browser.close();
...
main().catch((error) => { console.error(`[checkRun257SandboxCombatV3] FAIL: ${error.stack || error}`); process.exitCode = 1; });