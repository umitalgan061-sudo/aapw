...
const OUT = path.join(ROOT, 'artifacts', 'run216-editor-bright-authoring-v2');
...
async function main() {
  ...
  try {
    ...
    console.log(`[checkRun216EditorBrightAuthoringBrowserV2] PASS: ${JSON.stringify({ first: first.snapshot, second })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun216EditorBrightAuthoringBrowserV2] FAIL: ${error?.stack || error}`);
  process.exit(1);
});
