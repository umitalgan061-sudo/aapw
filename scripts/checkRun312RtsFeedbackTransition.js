#!/usr/bin/env node
/** Run312: transition-based browser contract for the existing transient RTS destination marker. */
const fs = require('fs');
const http = require('http');
const path = require('path');

const ROOT = path.resolve(__dirname, '..');
const OUT = path.join(ROOT, 'artifacts', 'run312-rts-feedback-transition');
...
async function installTransitionRecorder(page) {
  await page.evaluate(() => {
    ...
    window.__RUN312_FEEDBACK_TRANSITIONS__ = {
      transitions,
      finish: () => {
        observer.disconnect();
        return {
...
async function desktopProof(browser, base) {
  ...
  try {
    ...
    const record = await page.evaluate(() => window.__RUN312_FEEDBACK_TRANSITIONS__.finish());
...
async function main() {
  ...
  try {
    ...
    const proof = {
      baseMain: 'd90939ac8d1eae7f327201576ba8f79507e96cce',
      desktop,
      mobile,
      rca: 'Run311 became stale after main advanced. Run312 republishes the same test-only transition contract from exact current main without modifying runtime RTS files or the stale branch.',
    ...
    console.log(`[checkRun312RtsFeedbackTransition] PROOF: ${JSON.stringify(proof)}`);
    console.log('[checkRun312RtsFeedbackTransition] PASS: desktop/mobile pre-input visible-transition proof + movement + budgets + zero console/page errors');
  } finally {
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun312RtsFeedbackTransition] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
