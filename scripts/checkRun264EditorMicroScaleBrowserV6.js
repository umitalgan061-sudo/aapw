...
const OUT = path.join(ROOT, 'artifacts', 'run264-editor-micro-scale-v6');
const MIN = 0.000001;
...
async function main() {
  ...
  try {
    ...
    console.log(`[checkRun264EditorMicroScaleBrowserV6] PASS ${JSON.stringify({ beforeCount: beforeIds.length, targetId, direct: { scale: direct.scale, text: direct.text, persisted: direct.persisted }, shrinkText, screenshots: 2 })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun264EditorMicroScaleBrowserV6] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
