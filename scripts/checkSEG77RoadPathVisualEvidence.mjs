...
try {
  ...
  const setup = await page.evaluate(async (guard) => {
    ...
    for (const mesh of roads.group.children) { const position = mesh.geometry.getAttribute('position'); renderedRoadVertices += position.count; for (let i = 0; i < position.count; i += 1) { const n = worldXZToNormalizedReference(position.getX(i), position.getZ(i), WORLD_SCALE.MAP_BOUNDS, WORLD_SCALE.METERS_PER_MAPUnit); if (n.x >= guard.xMin && n.x <= guard.xMax && n.y >= guard.yMin && n.y <= guard.yMax) renderedRoadVerticesInGuard += 1; } }
