#!/usr/bin/env node
/**
 * Run 184 full-reference migration dry-run.
 *
...
async function main() {
	...
	try {
		...
		data = await page.evaluate(async () => {
			...
			return {
				plan,
				seatResults,
				currentMst: currentMst.map((e) => `${e.fromId}->${e.toId}`),
				targetMst: targetMst.map((e) => `${e.fromId}->${e.toId}`),
				roadResults,
	...
	const runtimeFiles = ['src/3d/config.js', 'src/3d/sceneManager.js', 'src/3d/world/terrain.js', 'src/3d/world/water.js', 'src/3d/world/roads.js'];
