...
try {
  ...
  const result = await page.evaluate(async () => {
    ...
    for (const probe of probes) {
      maxPhysicsError = Math.max(maxPhysicsError, Math.abs(sampler(probe.x, probe.z) - collider.getGroundHeight(probe.x, probe.z)));
    }
    const scene = new THREE.Scene();
    const manager = new ChunkManager({ scene, chunkSizeMeters: 500, seed: WORLD_DEFAULTS.WORLD_SEED, flattenPads: [] });
    const g17 = probes[0];
    const chunkX = Math.round(g17.x / 500);
    const chunkZ = Math.round(g17.z / 500);
    ...
    const output = {
      ...
      g17Height: sampler(g17.x, g17.z),
      g77Height: sampler(probes[1].x, probes[1].z),
    };
    manager.disposeAll();
    return output;
  });
  if (pageErrors.length) throw new Error(`page errors: ${pageErrors.join(' | ')}`);
  if (!result.fullOwnerMapCoverage || result.legacyProceduralFallback) throw new Error(`invalid runtime policy ${JSON.stringify(result)}`);
