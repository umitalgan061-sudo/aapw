...
function steepGround(x, z) {
	return 25 + (x - dry.x) * 1.2 + (z - dry.z) * 0.9;
}
...
assert.deepEqual(
	sampleConfiguredNpcGeography(dry.x, dry.z, throwingGround),
	{ ok: false, error: 'ground-sample-failed' },
	'ground sampler exception did not fail closed',
);
assert.deepEqual(
	sampleConfiguredNpcGeography(NaN, dry.z, flatGround),
	{ ok: false, error: 'non-finite-position' },
	'non-finite world position did not fail closed',
);
...
for (const [start, target] of badRoutes) {
	assert.deepEqual(
		evaluateConfiguredNpcRoute(start, target, flatGround),
		{ ok: false, error: 'non-finite-route' },
		`malformed route did not fail closed: ${JSON.stringify({ start, target })}`,
	);
}

const seat = { id: 'failure-seat', x: dry.x, z: dry.z };
const tooCloseSpawn = {
	id: 'inside-keep', seatId: seat.id, offsetXMeters: 1, offsetZMeters: 1,
};
const tooFarSpawn = {
	id: 'outside-envelope', seatId: seat.id, offsetXMeters: 80, offsetZMeters: 80,
};
const allSteepSpawn = {
	id: 'all-steep', seatId: seat.id, offsetXMeters: 14, offsetZMeters: 0,
};
const closeResolution = resolveConfiguredNpcSpawnPlacement({ spawn: tooCloseSpawn, seat, sampleGroundHeight: flatGround });
assert.equal(closeResolution.ok, false, 'spawn inside keep clearance was silently accepted');
assert.equal(closeResolution.error, 'no-safe-settlement-ground');
const farResolution = resolveConfiguredNpcSpawnPlacement({ spawn: tooFarSpawn, seat, sampleGroundHeight: flatGround });
assert.equal(farResolution.ok, false, 'spawn outside settlement envelope was silently accepted');
assert.equal(farResolution.error, 'no-safe-settlement-ground');
const steepResolution = resolveConfiguredNpcSpawnPlacement({ spawn: allSteepSpawn, seat, sampleGroundHeight: steepGround });
assert.equal(steepResolution.ok, false, 'bounded relocation accepted a fully unsafe steep neighborhood');
assert.equal(steepResolution.error, 'no-safe-settlement-ground');

assert.deepEqual(
	resolveConfiguredNpcSpawnPlacement({ spawn: null, seat, sampleGroundHeight: flatGround }),
	{ ok: false, error: 'missing-spawn-or-seat' },
);
assert.deepEqual(
	resolveConfiguredNpcSpawnPlacement({ spawn: tooCloseSpawn, seat: null, sampleGroundHeight: flatGround }),
	{ ok: false, error: 'missing-spawn-or-seat' },
);
...
console.log('NPC_PLACEMENT_FAILURE_SAFETY_PASS', JSON.stringify({
	...
	closeError: closeResolution.error,
	farError: farResolution.error,
