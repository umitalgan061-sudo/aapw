...
import { buildG71Terrain3DReliefSource, measureG71Terrain3DRelief } from '../godot/terrain-authoring/geocells/ne/g71_relief.mjs';
import { buildG71Terrain3DBiomeSource } from '../godot/terrain-authoring/geocells/ne/g71_biome.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const arg = process.argv.find((v) => v.startsWith('--out-dir='));
const OUT = path.resolve(ROOT, arg ? arg.slice(10) : 'artifacts/ne-g71-relief-visual');
const requireOk = (ok, message) => { if (!ok) throw new Error(message); };
const sha256 = (bytes) => crypto.createHash('sha256').update(bytes).digest('hex');
const relief = buildG71Terrain3DReliefSource();
const biome = buildG71Terrain3DBiomeSource();
const authored = measureG71Terrain3DRelief();
requireOk(authored.canonicalSea === 96 && authored.heightSpan === 0 && authored.addedReliefAbsMax === 0, 'G71 authored geography drifted');
requireOk(relief.heights.length === biome.heights.length && relief.heights.every((h, i) => h === biome.heights[i]), 'Relief changed merged Biome heights');
...
try {
  ...
  const setup = await page.evaluate(async () => {
    ...
    window.__g71ReliefVisual = { render(kind) { state.renderer.render(state.scene, kind === 'near' ? near : kind === 'far' ? far : full); } };
  ...
  for (const kind of ['near', 'far', 'full-world']) {
    await page.evaluate((k) => window.__g71ReliefVisual.render(k), kind === 'full-world' ? 'full' : kind);
    const png = await page.locator('#proof').screenshot(); requireOk(png.length > 4096, `${kind} PNG too small`);
    fs.writeFileSync(path.join(OUT, `g71-relief-${kind}.png`), png); hashes[kind] = sha256(png);
  }
  ...
  fs.writeFileSync(path.join(OUT, 'g71-relief-visual-metrics.json'), `${JSON.stringify(report, null, 2)}\n`);
  console.log(`G71_RELIEF_VISUAL_METRICS=${JSON.stringify(report)}`); console.log('NE_G71_RELIEF_VISUAL_EVIDENCE_OK');
