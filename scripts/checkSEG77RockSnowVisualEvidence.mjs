...
for (let y = 0; y < size; y += 1) {
  ...
  for (let x = 0; x < size; x += 1) {
    ...
    let color = biome.color.slice();
    color = mix(color, [0.34, 0.33, 0.31], surface.rockWeight);
    ...
    samples.push([surface.height, color[0], color[1], color[2], surface.waterConfidence, surface.rockWeight, surface.snowWeight]);
  }
}
...
try {
  ...
  const render = await page.evaluate(async ({ samples, size, bounds }) => {
    ...
    for (let row = 0; row < size; row += 1) for (let col = 0; col < size; col += 1) {
      ...
      const s = samples[row * size + col];
      const map = normalizedReferenceToMapCanvas(nx, ny); const world = mapCanvasToPlannedWorldXZ(map.x, map.y); const py = s[0] * heightScale;
      ...
      positions.push(world.x - centerWorld.x, py, world.z - centerWorld.z); colors.push(s[1], s[2], s[3]);
    }
    for (let row = 0; row < size - 1; row += 1) for (let col = 0; col < size - 1; col += 1) {
      const a = row * size + col, b = a + 1, c = a + size, d = c + 1; indices.push(a, c, b, b, c, d);
    }
    ...
    const water = new THREE.Mesh(new THREE.PlaneGeometry(spanX * 1.15, spanZ * 1.15), new THREE.MeshPhysicalMaterial({ color: 0x417f98, transparent: true, opacity: 0.30, roughness: 0.2, depthWrite: false, side: THREE.DoubleSide }));
    water.rotation.x = -Math.PI / 2; scene.add(water);
    scene.add(new THREE.HemisphereLight(0xddefff, 0x392a20, 1.8)); const sun = new THREE.DirectionalLight(0xffe9c0, 2.6); sun.position.set(-spanX * 0.4, span * 0.8, -spanZ * 0.25); scene.add(sun);
    const camera = new THREE.PerspectiveCamera(48, 960 / 640, 1, span * 8);
    window.__g77RockSnow = { renderer, scene, camera, spanX, spanZ, span, minY, maxY, render(kind) {
  ...
  async function capture(kind, name) {
    await page.evaluate((value) => window.__g77RockSnow.render(value), kind);
  ...
  const metrics = { schema: 'se-g77-rock-snow-visual-v1', sourceMapSha256: G77_ROCK_SNOW_POLICY.sourceMapSha256, size,
    wetSamples: wet, drySamples: dry, fractionalRockSamples: fractionalRock, snowSamples, physicalHeightRange: [minHeight, maxHeight],
    render, visibleGeoCellOverlay: false, evidenceSha256: hashes, determinismPolicy: 'numeric source/import/seam gates are authoritative; GPU hashes are provenance only' };
