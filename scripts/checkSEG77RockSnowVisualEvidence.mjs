...
for (let y = 0; y < size; y += 1) {
  ...
  for (let x = 0; x < size; x += 1) {
    const nx = bounds.xMin + (bounds.xMax - bounds.xMin) * x / (size - 1), surface = sampleG77RockSnow(nx, ny), biome = sampleG77BiomeSurface(nx, ny);
    let color = mix(biome.color.slice(), [0.34, 0.33, 0.31], surface.rockWeight); color = mix(color, [0.88, 0.90, 0.92], surface.snowWeight);
    if (surface.waterConfidence >= 0.5) wet += 1; else dry += 1; if (surface.rockBlend > 0.001 && surface.rockBlend < 0.999) fractionalRock += 1; if (surface.snowWeight > 0.001) snowSamples += 1;
    minHeight = Math.min(minHeight, surface.height); maxHeight = Math.max(maxHeight, surface.height); samples.push([surface.height, color[0], color[1], color[2]]);
  }
}
need(wet > 0 && dry > 0, `mixed coast disappeared: wet=${wet} dry=${dry}`); need(fractionalRock > 1024, `rock material collapsed to blocks: ${fractionalRock}`); need(maxHeight - minHeight > 2, `relief span collapsed: ${maxHeight - minHeight}`);
const playwright = loadPlaywright(); need(Boolean(playwright), 'Playwright is required'); const server = await startStaticServer(); const { port } = server.address(); const browser = await playwright.chromium.launch({ headless: true });
try {
  const page = await browser.newPage({ viewport: { width: 960, height: 640 }, deviceScaleFactor: 1 }); const errors = []; page.on('pageerror', (error) => errors.push(String(error)));
  ...
  const render = await page.evaluate(async ({ samples, size, bounds }) => {
    const THREE = await import('/src/3d/vendor/three/three.module.js'); const { normalizedReferenceToMapCanvas } = await import('/src/3d/world/worldReferenceAlignment.js'); const { mapCanvasToPlannedWorldXZ } = await import('/src/3d/world/worldReferenceMigrationPlan.js');
    const canvas = document.getElementById('terrain-proof'); const renderer = new THREE.WebGLRenderer({ canvas, antialias: true, preserveDrawingBuffer: true }); renderer.setSize(960,640,false); renderer.setPixelRatio(1); renderer.outputColorSpace = THREE.SRGBColorSpace;
    const scene = new THREE.Scene(); scene.background = new THREE.Color(0x172229); const centerMap = normalizedReferenceToMapCanvas((bounds.xMin+bounds.xMax)*0.5,(bounds.yMin+bounds.yMax)*0.5); const centerWorld = mapCanvasToPlannedWorldXZ(centerMap.x,centerMap.y);
    ...
    window.__g77RockSnow={renderer,scene,camera,spanX,spanZ,span,minY,maxY,render(kind){ if(kind==='near')camera.position.set(-spanX*0.30,span*0.24,spanZ*0.32); else if(kind==='far')camera.position.set(0,span*0.72,spanZ*0.72); else camera.position.set(0,span*1.45,0.01); camera.lookAt(0,kind==='top'?0:(minY+maxY)*0.5,0); renderer.render(scene,camera); }}; return {vertices:positions.length/3,triangles:indices.length/3,worldSpanMeters:{x:spanX,z:spanZ},diagnosticHeightScale:heightScale};
  }, { samples, size, bounds });
  ...
  const hashes={near:await capture('near','g77-rock-snow-near.png'),far:await capture('far','g77-rock-snow-far.png'),top:await capture('top','g77-rock-snow-topdown.png')}; need(errors.length===0,`browser errors: ${errors.join(' | ')}`);
  const metrics={schema:'se-g77-rock-snow-visual-v4',sourceMapSha256:G77_ROCK_SNOW_POLICY.sourceMapSha256,size,wetSamples:wet,drySamples:dry,fractionalRockSamples:fractionalRock,snowSamples,physicalHeightRange:[minHeight,maxHeight],render,visibleGeoCellOverlay:false,evidenceSha256:hashes,determinismPolicy:'numeric source/import/seam gates are authoritative; GPU hashes are provenance only'};
