...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run296-editor-fbx-pack-transform');
...
async function main() {
  ...
  try {
    ...
    const setup = await page.evaluate(async () => {
      ...
      root.name = 'Run296 FBX Pack Test';
      root.userData.editorId = 'run296-fbx-root';
      root.userData.editorAssetId = 'peasant-girl';
      root.userData.editorFormat = 'fbx';
      root.position.set(0, 6, 0);

      function makePack(name, x) {
        const group = new THREE.Group();
        group.name = name;
        group.position.x = x;
        const mesh = new THREE.Mesh(
          new THREE.BoxGeometry(3, 3, 3),
          new THREE.MeshStandardMaterial({ color: name.endsWith('A') ? 0x9a7546 : 0x58779a, roughness: 0.72 })
        );
        mesh.name = `${name} Mesh`;
      ...
      root.add(makePack('Pack A', -4), makePack('Pack B', 4));
      root.traverse((node) => { if (node.isMesh) node.userData.editorRoot = root; });
      api.editableObjects.push(root);
      api.scene.add(root);
      api.refreshHierarchy();
      return { rootId: root.userData.editorId, childNames: root.children.map((child) => child.name) };
    });
    assert(setup.childNames.join(',') === 'Pack A,Pack B', `Synthetic FBX packs drifted: ${JSON.stringify(setup)}`);

    await page.locator('#we-hierarchy .we-hierarchy-item', { hasText: 'Run296 FBX Pack Test' }).click();
    await page.waitForFunction(() => {
      const root = window.__WESTEROS_WORLD_EDITOR__.getSelectedObject();
      return root?.userData?.editorId === 'run296-fbx-root' && window.__WESTEROS_EDITOR_FBX_PACKS__.listPacks(root).length === 2;
    });
    ...
    let snapshot = await page.evaluate(() => ({
      ...
      root: window.__WESTEROS_WORLD_EDITOR__.getSelectedObject().position.toArray(),
      a: window.__WESTEROS_WORLD_EDITOR__.getSelectedObject().children[0].position.toArray(),
      b: window.__WESTEROS_WORLD_EDITOR__.getSelectedObject().children[1].position.toArray()
    }));
    assert(snapshot.packs.activePackName === 'Pack B', `Pack B did not become active: ${JSON.stringify(snapshot)}`);
    assert(snapshot.transform.attachedEditorId === null && snapshot.packs.transformAttachedToPack === true, `TransformControls is not attached to child pack: ${JSON.stringify(snapshot)}`);
    ...
    snapshot = await page.evaluate(() => {
      ...
      return {
        root: { p: root.position.toArray(), r: [root.rotation.x, root.rotation.y, root.rotation.z], s: root.scale.toArray() },
        packA: { p: root.children[0].position.toArray(), s: root.children[0].scale.toArray() },
        packB: { p: pack.position.toArray(), r: [pack.rotation.x, pack.rotation.y, pack.rotation.z], s: pack.scale.toArray() },
        state: window.__WESTEROS_EDITOR_FBX_PACKS__.getSnapshot()
      };
    });
    assert(Math.abs(snapshot.packB.p[0] - 7.25) < 1e-9, `Pack B position did not change independently: ${JSON.stringify(snapshot)}`);
    assert(Math.abs(snapshot.packB.r[1] - (-37.5 * Math.PI / 180)) < 1e-9, `Pack B signed rotation drifted: ${JSON.stringify(snapshot)}`);
    assert(Math.abs(snapshot.packB.s[0] - 0.007) < 1e-9, `Pack B scale precision drifted: ${JSON.stringify(snapshot)}`);
    assert(snapshot.root.p.every((value) => value === 0 || value === 6) && snapshot.root.p[1] === 6, `FBX root moved with child pack: ${JSON.stringify(snapshot)}`);
    assert(snapshot.packA.p[0] === -4 && snapshot.packA.s.every((value) => value === 1), `Sibling pack changed with Pack B: ${JSON.stringify(snapshot)}`);
    ...
    const savedRoot = saved.objects.find((record) => record.id === 'run296-fbx-root');
    ...
    assert(savedPack.name === 'Pack B' && Math.abs(savedPack.transform.position[0] - 7.25) < 1e-9, `Saved pack transform drifted: ${JSON.stringify(savedPack)}`);
    assert(Math.abs(savedPack.transform.scale[0] - 0.007) < 1e-9, `Saved pack precision drifted: ${JSON.stringify(savedPack)}`);

    const restored = await page.evaluate(async (sceneData) => {
      const root = window.__WESTEROS_WORLD_EDITOR__.editableObjects.find((object) => object.userData?.editorId === 'run296-fbx-root');
    ...
    await page.evaluate(() => {
      const api = window.__WESTEROS_WORLD_EDITOR__;
      const root = api.editableObjects.find((object) => object.userData?.editorId === 'run296-fbx-root');
      const center = new window.THREE?.Vector3?.();
      void center;
      api.orbitControls.target.set(0, 6, 0);
      api.camera.position.set(22, 17, 24);
    ...
    assert(rootAttachment.selectedId === 'run296-fbx-root' && rootAttachment.transformId === 'run296-fbx-root', `Whole-FBX selection was not restored: ${JSON.stringify(rootAttachment)}`);

    assert(errors.length === 0, `Browser errors: ${errors.join(' | ')}`);
    console.log('[checkRun296EditorFbxPackTransform] PASS: FBX child packs select/move independently, root/sibling isolation holds, destructive root actions are guarded, pack transforms serialize/restore, and browser errors are zero.');
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error('[checkRun296EditorFbxPackTransform] FAIL:', error?.stack || error);
  process.exit(1);
});
