...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run297-editor-fbx-pack-transform');
...
async function main() {
  ...
  try {
    ...
    const setup = await page.evaluate(async () => {
      ...
      root.name = 'Run297 FBX Pack Test';
      root.userData.editorId = 'run297-fbx-root';
    ...
    await page.locator('#we-hierarchy .we-hierarchy-item', { hasText: 'Run297 FBX Pack Test' }).click();
    await page.waitForFunction(() => {
      const root = window.__WESTEROS_WORLD_EDITOR__.getSelectedObject();
      return root?.userData?.editorId === 'run297-fbx-root' && window.__WESTEROS_EDITOR_FBX_PACKS__.listPacks(root).length === 2;
    });
    ...
    assert(snapshot.root.p[0] === 0 && snapshot.root.p[1] === 6 && snapshot.root.p[2] === 0, `FBX root moved with child pack: ${JSON.stringify(snapshot)}`);
    ...
    const savedRoot = saved.objects.find((record) => record.id === 'run297-fbx-root');
    ...
    const restored = await page.evaluate((sceneData) => {
      const root = window.__WESTEROS_WORLD_EDITOR__.editableObjects.find((object) => object.userData?.editorId === 'run297-fbx-root');
    ...
    assert(rootAttachment.selectedId === 'run297-fbx-root' && rootAttachment.transformId === 'run297-fbx-root', `Whole-FBX selection was not restored: ${JSON.stringify(rootAttachment)}`);

    assert(errors.length === 0, `Browser errors: ${errors.join(' | ')}`);
    console.log('[checkRun297EditorFbxPackTransform] PASS: FBX child packs select/move independently, root/sibling isolation holds, destructive root actions are guarded, pack transforms serialize/restore, and browser errors are zero.');
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error('[checkRun297EditorFbxPackTransform] FAIL:', error?.stack || error);
  process.exit(1);
});
