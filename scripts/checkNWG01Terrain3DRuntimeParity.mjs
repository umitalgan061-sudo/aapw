...
function fail(message) { console.error(`[checkNWG01Terrain3DRuntimeParity] FAIL: ${message}`); process.exit(1); }
...
function buildSource() {
  ...
  const payload = {
    schema: 'westeros-g01-terrain3d-source-v1', policyId: G01_TERRAIN3D_RUNTIME_PARITY.id,
    sourceMapSha256: EXPECTED_SOURCE_SHA, terrain3dVersion: G01_TERRAIN3D_RUNTIME_PARITY.terrain3dVersion,
    width: SOURCE_SIZE, height: SOURCE_SIZE, normalizedBounds: bounds,
    heights: [], snowWeight: [], tintR: [], tintG: [], tintB: [], roughness: [], waterConfidence: [],
  };
  let waterDominantSamples = 0; let landDominantSamples = 0; let canonicalWaterSamples = 0;
  let maxCanonicalWaterSnowWeight = 0; let maxCanonicalWaterTintDelta = 0; let maxCanonicalWaterRoughnessDelta = 0;
  for (let y = 0; y < SOURCE_SIZE; y += 1) {
    ...
    for (let x = 0; x < SOURCE_SIZE; x += 1) {
      ...
      if (sample.waterConfidence >= 0.5) waterDominantSamples += 1; else landDominantSamples += 1;
      ...
      payload.heights.push(round(sample.heightMeters, 6)); payload.snowWeight.push(round(sample.snowWeight));
      payload.tintR.push(round(sample.tintR)); payload.tintG.push(round(sample.tintG)); payload.tintB.push(round(sample.tintB));
      payload.roughness.push(round(sample.roughness)); payload.waterConfidence.push(round(sample.waterConfidence));
    }
  }
  ...
  payload.semanticMetrics = { canonicalWaterCells: metrics.canonicalWaterCells, canonicalLandCells: metrics.canonicalLandCells,
    maxG00SharedSeamTintDelta: metrics.maxG00SharedSeamTintDelta, maxG00SharedSeamRoughnessDelta: metrics.maxG00SharedSeamRoughnessDelta,
    waterDominantSamples, landDominantSamples, canonicalWaterSamples, maxCanonicalWaterSnowWeight, maxCanonicalWaterTintDelta, maxCanonicalWaterRoughnessDelta };
  payload.sourceChecksum = checksumNumbers([...payload.heights, ...payload.snowWeight, ...payload.tintR, ...payload.tintG, ...payload.tintB, ...payload.roughness, ...payload.waterConfidence]);
  return payload;
}
...
function writeSource() {
  ...
  console.log(`G01_RUNTIME_SOURCE_METRICS=${JSON.stringify({ samples: source.width * source.height, sourceChecksum: source.sourceChecksum,
    minHeight: Math.min(...source.heights), maxHeight: Math.max(...source.heights), minSnowWeight: Math.min(...source.snowWeight), maxSnowWeight: Math.max(...source.snowWeight), ...source.semanticMetrics })}`);
...
function startStaticServer() {
  ...
  const server = http.createServer((req, res) => {
    try {
      ...
      res.writeHead(200, { 'Content-Type': mime[path.extname(filePath).toLowerCase()] || 'application/octet-stream' }); fs.createReadStream(filePath).pipe(res);
...
function compareBakeToSource(source, bake) {
  ...
  requireCondition(Number.isFinite(bake.boundaryProbeMaxHeightError) && bake.boundaryProbeMaxHeightError <= HEIGHT_TOLERANCE, `Terrain3D 255/256 boundary error ${bake.boundaryProbeMaxHeightError}`);
  ...
  let maxHeightError = 0; let maxSnowError = 0; let maxColorError = 0; let maxRoughnessError = 0;
...
async function verifyBrowserAdapter(bake) {
  const playwright = loadPlaywright(); requireCondition(Boolean(playwright), 'Playwright is required for G01 Three.js runtime parity');
  ...
  try {
    ...
    const result = await page.evaluate(async (payload) => {
      ...
      let maxNodeError = 0; let maxWorldVsNormalizedError = 0; let finiteSamples = 0; let checksum = 2166136261;
      for (let y = 0; y < payload.height; y += 1) {
        ...
        for (let x = 0; x < payload.width; x += 1) {
          ...
          const worldSample = sampleWorld(world.x, world.z); const normalizedSample = sampleNormalized(nx, ny); const i = y * payload.width + x;
          ...
          for (let c = 0; c < actual.length; c += 1) { maxNodeError = Math.max(maxNodeError, Math.abs(actual[c] - expected[c])); maxWorldVsNormalizedError = Math.max(maxWorldVsNormalizedError, Math.abs(actual[c] - direct[c])); }
          for (const value of actual) { const q = Math.round(value * 10000); checksum ^= q & 0xff; checksum = Math.imul(checksum, 16777619) >>> 0; }
          finiteSamples += 1;
        }
      }
      let outsideRejected = false; try { sampleNormalized(b.xMax + 0.001, b.yMin); } catch { outsideRejected = true; }
      return { finiteSamples, maxNodeError, maxWorldVsNormalizedError, checksum, outsideRejected };
    }, bake);
    requireCondition(!result.error, result.error ?? 'unknown G01 browser adapter failure'); requireCondition(errors.length === 0, `browser page errors: ${errors.join(' | ')}`);
    ...
    requireCondition(result.outsideRejected === true, 'G01 runtime adapter accepted an out-of-domain sample'); return result;
  } finally { await browser.close(); await new Promise((resolve) => server.close(resolve)); }
}

if (process.argv.includes('--verify')) {
  requireCondition(fs.existsSync(SOURCE_PATH), 'G01 runtime source proof missing'); requireCondition(fs.existsSync(BAKE_PATH), 'G01 Terrain3D bake proof missing');
  const source = JSON.parse(fs.readFileSync(SOURCE_PATH, 'utf8')); const bake = JSON.parse(fs.readFileSync(BAKE_PATH, 'utf8'));
  const terrain3d = compareBakeToSource(source, bake); const browser = await verifyBrowserAdapter(bake);
  console.log(`G01_RUNTIME_PARITY_METRICS=${JSON.stringify({ ...terrain3d, ...browser, bakeChecksum: bake.bakeChecksum })}`); console.log('NW_G01_TERRAIN3D_RUNTIME_PARITY_OK');
} else { writeSource(); }
