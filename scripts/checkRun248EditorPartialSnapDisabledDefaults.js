...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run248-editor-partial-snap-disabled-defaults');
...
async function main() {
  ...
  const fixturePath = path.join(ARTIFACT_DIR, 'partial-snap-disabled.scene.json');
  const expected = {
    id: 'run248-tree-0001',
    name: 'Run248 Partial Snap Disabled Tree',
    asset: 'marker-tree',
    position: [-17, 0, 24],
  ...
  try {
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '01-before-partial-snap-disabled-load.png'), fullPage: true });
    ...
    assert(after.snapEnabled === false, `Explicit snapEnabled=false was not preserved: ${JSON.stringify(after)}`);
    assert(after.snapSize === 1, `Missing snapSize did not fall back to default 1: ${JSON.stringify(after)}`);
    assert(after.gridChecked === false && after.gridDisabled === true && after.gridVisible === false, `Missing gridVisible or owner grid policy drifted: ${JSON.stringify(after)}`);
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '02-after-partial-snap-disabled-load.png'), fullPage: true });

    console.log(`[checkRun248EditorPartialSnapDisabledDefaults] PASS ${JSON.stringify({ object: after.object, snapEnabled: after.snapEnabled, snapSize: after.snapSize, gridVisible: after.gridVisible, unexpectedErrors: errors.length, screenshots: 2 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun248EditorPartialSnapDisabledDefaults] FAIL: ${error.stack || error}`); process.exitCode = 1; });
