...
if (metrics.canonicalWaterCells !== 44 || metrics.canonicalLandCells !== 52) {
  throw new Error(`G77 canonical hydrology drifted: ${metrics.canonicalWaterCells} water / ${metrics.canonicalLandCells} land`);
}
...
if (emitArg) {
  ...
  fs.writeFileSync(output, `${JSON.stringify(buildG77RockSnowProbe())}\n`);
}
