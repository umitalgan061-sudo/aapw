...
import {
  ...
  buildG65RoadPathProbe,
  canonicalRoadNetworkMaxNormalizedX,
  measureG65RoadPath,
} from '../godot/terrain-authoring/geocells/se/g65_road_path.mjs';
...
function fail(message) {
  console.error(`[checkSEG65RoadPath] FAIL: ${message}`);
  process.exit(1);
}
...
function loadPlaywright() {
  for (const id of ['playwright', '/opt/node22/lib/node_modules/playwright']) {
    try { return require(id); } catch { /* next supported runner location */ }
  }
  return null;
}
...
requireCondition(contract.maxSeatMapX === G65_ROAD_PATH_POLICY.canonicalMaxSeatMapX, `KINGDOM_SEATS eastern envelope changed: ${contract.maxSeatMapX}`);
requireCondition(contract.corridorPaddingMeters === G65_ROAD_PATH_POLICY.canonicalRouteCorridorPaddingMeters, `roadPathfinder corridor padding changed: ${contract.corridorPaddingMeters}`);
requireCondition(G65_ROAD_PATH_POLICY.sourceMapSha256 === EXPECTED_SOURCE_SHA, 'canonical map SHA changed');
requireCondition(canonicalRoadNetworkMaxNormalizedX() < G65_ROAD_PATH_POLICY.normalizedBounds.xMin, 'full-reference canonical road search envelope now reaches G65');
...
requireCondition(metrics.crossingEdges.length === 0, `real runtime road/path now intersects G65: ${JSON.stringify(metrics.crossingEdges)}`);
requireCondition(metrics.runtimeRoadReferenceEnvelope.maxX < G65_ROAD_PATH_POLICY.normalizedBounds.xMin, 'runtime road envelope reaches G65 east boundary');
requireCondition(metrics.canonicalWaterCells === 0 && metrics.canonicalLandCells === 96, 'G65 canonical 96-cell dry-land contract changed');
requireCondition(metrics.activeRoadSamples === 0, 'phantom cart-road paint appeared in road-free G65');
requireCondition(metrics.activePathSamples === 0, 'phantom footpath paint appeared in road-free G65');
requireCondition(metrics.maxAdjacentCoverageStep === 0, 'road-free G65 developed local coverage steps');
requireCondition(metrics.maxGuardBandCoverageDelta === 0, 'road/path guard-band discontinuity appeared');
requireCondition(metrics.maxHeightDeltaMeters === 0, 'Road/Path altered merged G65 Relief height without a real road');
requireCondition(metrics.maxSurfaceDelta === 0, 'Road/Path altered merged G65 Rock/Snow surface without a real road');
requireCondition(metrics.canonicalRoadExclusionMarginMeters > 100, `canonical road exclusion margin unexpectedly small: ${metrics.canonicalRoadExclusionMarginMeters}m`);
requireCondition(metrics.sourceSamples === 257 * 257, 'expected deterministic 257x257 road-coverage qualification grid');
requireCondition(metrics.terrain3dImportSize === 257 && metrics.terrain3dRegionSize === 256, 'G65 Road/Path must preserve 257 import over region_size 256');
...
const summary = {
  ...
  g65NormalizedXMin: G65_ROAD_PATH_POLICY.normalizedBounds.xMin,
...
console.log(`G65_ROAD_PATH_METRICS=${JSON.stringify(summary)}`);
console.log('SE_G65_ROAD_PATH_VALIDATION_OK');
...
if (emitArg) {
  ...
  fs.writeFileSync(absolute, `${JSON.stringify(buildG65RoadPathProbe(runtimeNetwork))}\n`, 'utf8');
}
