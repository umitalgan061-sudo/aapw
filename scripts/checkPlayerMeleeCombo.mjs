...
function requireFragments(source, label, fragments) {
	for (const fragment of fragments) assert.ok(source.includes(fragment), `missing ${label}: ${fragment}`);
}

const cfg = Object.freeze({
	lightCost: numberConstant('LIGHT_ATTACK_STAMINA_COST'),
	heavyCost: numberConstant('HEAVY_ATTACK_STAMINA_COST'),
	lightSeconds: numberConstant('LIGHT_ATTACK_SECONDS'),
	heavySeconds: numberConstant('HEAVY_ATTACK_SECONDS'),
	lightActiveStart: numberConstant('LIGHT_ATTACK_ACTIVE_START_SECONDS'),
	lightActiveEnd: numberConstant('LIGHT_ATTACK_ACTIVE_END_SECONDS'),
	heavyActiveStart: numberConstant('HEAVY_ATTACK_ACTIVE_START_SECONDS'),
	heavyActiveEnd: numberConstant('HEAVY_ATTACK_ACTIVE_END_SECONDS'),
	bufferSeconds: numberConstant('ATTACK_COMBO_BUFFER_SECONDS'),
	maxSteps: numberConstant('ATTACK_COMBO_MAX_STEPS'),
	lightReach: numberConstant('LIGHT_ATTACK_REACH_METERS'),
	heavyReach: numberConstant('HEAVY_ATTACK_REACH_METERS'),
	lightDamageScale: numberConstant('LIGHT_ATTACK_DAMAGE_SCALE'),
	heavyDamageScale: numberConstant('HEAVY_ATTACK_DAMAGE_SCALE'),
});
assert.ok(cfg.lightCost > 0 && cfg.heavyCost > cfg.lightCost && cfg.heavyCost < 40);
assert.ok(cfg.lightSeconds > 0.25 && cfg.lightSeconds < cfg.heavySeconds && cfg.heavySeconds < 1);
assert.ok(cfg.lightActiveStart > 0 && cfg.lightActiveEnd > cfg.lightActiveStart && cfg.lightActiveEnd < cfg.lightSeconds);
assert.ok(cfg.heavyActiveStart > 0 && cfg.heavyActiveEnd > cfg.heavyActiveStart && cfg.heavyActiveEnd < cfg.heavySeconds);
assert.ok(cfg.bufferSeconds > 0.1 && cfg.bufferSeconds < cfg.lightSeconds);
assert.equal(cfg.maxSteps, 3);
assert.ok(cfg.heavyReach > cfg.lightReach && cfg.heavyDamageScale > cfg.lightDamageScale);

requireFragments(player, 'player melee contract', [
	...
	'attackRemaining <= 0 && !guarding',
	'globalThis.removeEventListener?.(COMBAT_INPUT_EVENT, onCombatInput)',
]);
...
requireFragments(input, 'input parity/lifecycle contract', [
	...
	'this._visibilityTarget = globalThis.document?.addEventListener ? globalThis.document : target',
	'_pollGamepad(inputBlocked = readPlayerGameplayInputBlocked(this._isInputBlocked))',
	'if (inputBlocked) {',
]);
assert.match(
	input,
	/if \(readPlayerGameplayInputBlocked\(this\._isInputBlocked\) \|\| isInteractiveTarget\(event\.target\)\) return;/,
	'keyboard/pointer handlers must fail closed when gameplay is blocked or the originating DOM target is interactive',
);
...
requireFragments(touch, 'mobile melee/pause/lifecycle contract', [
	...
	"setAttribute('aria-label', 'Hafif saldırı')",
...
globalThis.dispatchEvent = (event) => {
	if (event?.type === 'aapw:player-combat-input') keyboardIntents.push(event.detail);
	return true;
};
...
function dispatchPointer(target, type, button, pointerId = 1, clientX = 0, clientY = 0) {
	const event = new Event(type, { cancelable: true });
	Object.defineProperties(event, { button: { value: button }, pointerId: { value: pointerId }, clientX: { value: clientX }, clientY: { value: clientY } });
	target.dispatchEvent(event);
	return event;
}
...
const worldTarget = new FakeKeyTarget(false);
const interactiveTarget = new FakeKeyTarget(true);
const pausedTarget = new FakeKeyTarget(false);
let pauseBlocked = true;
const worldInput = new KeyboardInput(worldTarget);
const interactiveInput = new KeyboardInput(interactiveTarget);
const pausedInput = new KeyboardInput(pausedTarget, { isInputBlocked: () => pauseBlocked });
let touchInput = null;
...
try {
	...
	for (const code of ['KeyW', 'Space', 'KeyQ', 'Tab', 'KeyC', 'KeyR']) {
		dispatchKey(interactiveTarget, 'keydown', code);
		dispatchKey(interactiveTarget, 'keyup', code);
	}
	dispatchPointer(interactiveTarget, 'pointerdown', 0);
	dispatchPointer(interactiveTarget, 'pointerdown', 2);
	const interactiveAxes = interactiveInput.getAxes();
	assert.equal(keyboardIntents.length, beforeInteractive, 'focused interactive DOM controls must suppress keyboard/pointer melee intents');
	assert.deepEqual(
		{ forward: interactiveAxes.forward, strafe: interactiveAxes.strafe, running: interactiveAxes.running, jumpRequested: interactiveAxes.jumpRequested, lockOnRequested: interactiveAxes.lockOnRequested, guarding: interactiveAxes.guarding },
		{ forward: 0, strafe: 0, running: false, jumpRequested: false, lockOnRequested: false, guarding: false },
		'interactive DOM controls must not leak movement/jump/lock/guard state into gameplay',
	);
	...
	for (const code of ['KeyW', 'KeyC', 'Space', 'Tab']) dispatchKey(worldTarget, 'keydown', code, true);
	...
	assert.deepEqual(
		{ forward: pausedAxes.forward, strafe: pausedAxes.strafe, running: pausedAxes.running, jumpRequested: pausedAxes.jumpRequested, lockOnRequested: pausedAxes.lockOnRequested, guarding: pausedAxes.guarding, lookX: pausedAxes.lookX, lookY: pausedAxes.lookY, cameraZoom: pausedAxes.cameraZoom },
		{ forward: 0, strafe: 0, running: false, jumpRequested: false, lockOnRequested: false, guarding: false, lookX: 0, lookY: 0, cameraZoom: 0 },
		'pause must neutralize queued gameplay axes and action edges',
	);
	pauseBlocked = false;
	dispatchKey(pausedTarget, 'keydown', 'KeyC');
	...
	touchInput = new TouchJoystick(lifecycleDocument.body, { isInputBlocked: () => touchBlocked });
...
console.log(JSON.stringify({
	...
	attack: cfg,
	windupSteering: { turnMultiplier: 0.68, preActiveOnly: true, activeHoming: false },
	inputs: {
		keyboard: ['KeyC', 'KeyR'],
		interactionReserved: 'KeyE',
		mouse: 'button0-light',
		gamepad: ['A/button0-jump', 'X/button2-light', 'Y/button3-heavy'],
		touch: ['light', 'heavy', 'jump', 'guard', 'lock-on'],
		interactiveDomGameplaySuppressed: ['movement', 'jump', 'guard', 'lock-on', 'light', 'heavy', 'pointer-light', 'pointer-guard'],
		pauseGameplayInputSuppressed: true,
		touchPauseGameplayInputSuppressed: true,
		lifecycleIsolation: ['document-visibility', 'window-pagehide', 'held-key-repeat', 'held-movement-repeat', 'touch-pointer-reset'],
	},
	assetPolicy: {
		newModel: false,
		fabricatedAttackClip: false,
		canonicalAnimationConfig: 'src/3d/gameplay/playerConfig.js',
		sharedMaterialCoreUnchanged: true,
	},
