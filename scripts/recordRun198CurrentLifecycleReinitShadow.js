...
- Session/concurrency: started strictly from Run197 merge main \`0128166a…\` after re-reading GOVERNANCE, Run197/ADR-0217, recent commits and QUESTIONS_FOR_OWNER; no Run198/ADR-0218 occupancy existed. Remote main is rechecked before publication and again immediately before checkpoint publication.
...
- Lifecycle proof: first generation reaches canonical-active real-tick freeze, then a capture-phase preflight owner disposes the canonical transaction at the real \`pagehide\` boundary immediately before the unchanged runtime's own once-only teardown executes. The same document then calls the real exported \`initGame3D()\` again, receives distinct scene/renderer/chunk/player identities, proves F4 toggles exactly once per keypress, re-enters canonical mode, and repeats pagehide teardown. ${summary}
- Leak/identity proof: lifecycle counters are ${lifecycle}. Both teardown generations end with zero tracked RAF and interval handles; EventTarget listener/EventBus/3D DOM baselines are equal generation-to-generation; renderer/chunk/input/player/camera/world-event/HUD and every NPC/animal/dragon dispose path fires exactly once per generation. Second init does not reuse first scene/player/renderer/chunk-manager identity.
...
append('DECISIONS.md', `## ADR-0218 — Canonical ownership must survive real pagehide teardown and a clean second init before any startup source-of-truth switch (run 198)

**Risk Seviyesi:** LOW

**Karar:** Run197 tick-ownership transaction bir sonraki migration kapısında default runtime'a bağlanmaz. Önce developer-only browser preflight, canonical aktifken gerçek \`pagehide\` lifecycle sınırına girer. Capture-phase preflight listener canonical gate'i rollback+dispose eder; ardından mevcut \`game3d.js\`'in değişmemiş once-only pagehide teardown zinciri çalışır. Aynı document içinde gerçek exported \`initGame3D()\` ikinci kez çağrılır ve tamamen yeni current-runtime nesneleriyle temiz boot kanıtlanır. İkinci generation da canonical active→pagehide yolunu tekrarlar.

**Neden:** Run197 gerçek RAF tick altında simulation freeze/resume kanıtladı fakat navigation/lifecycle sınırı ayrı bir failure sınıfıdır: RAF veya input listener sızıntısı, EventBus subscription birikmesi, iki kez F4 toggle, eski scene/resource identity reuse veya canonical gate'in disposed current resources üzerinde rollback denemesi ancak teardown+re-init ile görünür. Default startup seçimini bu kapıdan önce değiştirmek, geri dönüşü zor bir lifecycle hatasını oyuncu yoluna taşıyabilirdi.

**Ölçüm:** ${summary} Lifecycle sayaçları ${lifecycle}. Her generation'da canonical tick pause hedeflerinin tamamı gerçek RAF tarafından çağrılıp bloklanır; pagehide başlangıcında mode \`canonical\`dır. Canonical owner capture phase'de dispose olur, sonra unchanged runtime teardown renderer/chunk/input/player/controls/free-camera/world-event/HUD ve tüm entity dispose yollarını tam bir kez çağırır. Her teardown sonunda tracked RAF=0 ve interval=0. İkinci boot first-generation scene/player/renderer/chunk-manager nesnelerini reuse etmez. F4 ilk keydown'da true, ikinci keydown'da false olur; duplicate key listener yoktur. İki generation'ın active ve teardown EventTarget/EventBus/DOM imzaları birbirine eşittir; console/page error 0.

**Alternatifler:** (1) Gerçek navigation ile yeni document açmak tek başına reddedildi; browser document'i GC ettiği için same-document duplicate-listener/re-init problemlerini maskeleyebilir. (2) Yalnız unit-level \`dispose()\` çağrıları reddedildi; gerçek once-only pagehide listener sırasını doğrulamaz. (3) Default \`game3d.html\` importlarını şimdi değiştirmek reddedildi; lifecycle kanıtından önce ürün yolunu etkilerdi. (4) Canonical gate'i runtime teardown'dan sonra dispose etmek reddedildi; zaten dispose edilmiş borrowed current scene üzerinde rollback yapma riski taşır. Bu yüzden test-only capture-phase owner sıralaması seçildi.
...
append('STABLE_TAGS.md', `- ${stableTag} — run 198 canonical-active pagehide + same-document clean re-init preflight (ADR-0218); two lifecycle generations + exact single-dispose paths + zero RAF/interval leak + listener/EventBus/DOM baseline equality + distinct runtime identities + single F4 toggle + zero console error PASS; live runtime delta 0.`);
