...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run238-editor-structural-scene-atomicity-v2');
...
async function main() {
  ...
  fs.writeFileSync(validScene, JSON.stringify({
    ...
    objects: [{
      id: 'run238-baseline-castle', name: 'Run238 Baseline Castle', asset: 'marker-castle',
  ...
  try {
    ...
    await page.waitForFunction(() => (
      ...
      window.__WESTEROS_WORLD_EDITOR__.editableObjects[0]?.userData?.editorId === 'run238-baseline-castle'
    ), null, { timeout: 30000 });
    await page.locator('#we-hierarchy .we-hierarchy-item').first().click();
    await page.waitForTimeout(150);
    const before = await capture();
    assert(before.objects.length === 1 && before.objects[0].id === 'run238-baseline-castle', `Deterministic valid baseline failed: ${JSON.stringify(before)}`);
    ...
    assert(before.selection.startsWith('Seçim: ') && !before.selection.includes('yok'), `Baseline selection missing: ${before.selection}`);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '01-valid-loaded-baseline.png'), fullPage: true });
    ...
    console.log(`[checkRun238EditorStructuralSceneAtomicityV2] PASS ${JSON.stringify({ objectCount: before.objects.length, handledErrors: handled.length, unexpectedErrors: unexpected.length, screenshots: 3 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}
main().catch((error) => { console.error(`[checkRun238EditorStructuralSceneAtomicityV2] FAIL: ${error.stack || error}`); process.exitCode = 1; });
