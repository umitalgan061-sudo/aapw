...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run237-editor-malformed-scene-atomicity');
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => (
      ...
      document.querySelectorAll('#we-hierarchy .we-hierarchy-item').length >= 2
    ), null, { timeout: 120000 });
    ...
    assert(before.objects.length >= 2, `Expected populated baseline editor scene: ${JSON.stringify(before)}`);
    assert(before.selection.startsWith('Seçim: ') && !before.selection.includes('yok'), `Expected a selected baseline object: ${before.selection}`);
    ...
    assert(handledLoadErrors.length === 1, `Malformed JSON should produce exactly one handled load error: ${JSON.stringify(handledLoadErrors)}`);
    ...
    console.log(`[checkRun237EditorMalformedSceneAtomicity] PASS ${JSON.stringify({ objectCount: before.objects.length, instanceGroupCount: before.instanceGroupCount, selection: before.selection, handledLoadErrors: handledLoadErrors.length, unexpectedErrors: unexpectedErrors.length, screenshots: 3 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun237EditorMalformedSceneAtomicity] FAIL: ${error.stack || error}`); process.exitCode = 1; });
