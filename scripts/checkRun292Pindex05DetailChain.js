...
// Run277-282 each prepended their install-listener block above the previous ones, so offline
// registration was reverse chronological (04→03→02→01) while canonical-dev execution stayed
// chronological. Run292 continues that same prepend convention: Pindex-05's block now sits above
// all of them, so its offline index is the smallest of the five while its boot-time activation
// still runs last (after Pindex-01..04), matching `checkRun284PindexDetailChain.js`'s original
// Pindex-01..04-only contract.
...
console.log('[checkRun292Pindex05DetailChain] PASS: chronological runtime + reverse-prepend offline Pindex-01→05 contracts locked');
