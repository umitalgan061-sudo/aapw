...
import { createPlayerMotionPresentationContract } from '../src/3d/gameplay/playerMotionPresentationContract.js';
...
for(let i=0;i<inputs.length;i+=1){const state=buildPlayerMotionPresentationState(previous,inputs[i]);state.traversalContract=createPlayerMotionPresentationContract(state);timeline.push(buildTraversalTimelineEntry(previous?.traversal??null,inputs[i]));const health=evaluatePlayerPresentationHealth(state,previous,timeline);assert.equal(typeof health.healthy,'boolean');assert.ok(health.score>=0&&health.score<=1);previous=state;}
