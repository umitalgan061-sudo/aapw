...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run264-editor-micro-scale');
...
async function main() {
  ...
  try {
    ...
    const teardown = await page.evaluate(() => {
      const micro = window.__WESTEROS_EDITOR_MICRO_SCALE_RUN264__;
      const guard = window.__WESTEROS_EDITOR_MICRO_SCALE_BOUNDS_GUARD_RUN264__;
      micro?.dispose?.();
      guard?.dispose?.();
      return {
        microPresent: Boolean(window.__WESTEROS_EDITOR_MICRO_SCALE_RUN264__),
        guardPresent: Boolean(window.__WESTEROS_EDITOR_MICRO_SCALE_BOUNDS_GUARD_RUN264__)
      };
    });
    assert(!teardown.microPresent && !teardown.guardPresent, `Run264 teardown globals leaked: ${JSON.stringify(teardown)}`);
    assert(errors.length === 0, `Browser errors: ${errors.join(' | ')}`);
    console.log(`[checkRun264EditorMicroScaleBrowser] PASS ${JSON.stringify({ scaleMeta, directScale: { scale: directScale.scale, inputText: directScale.inputText, persisted: directScale.persisted }, afterQuickShrink, teardown, screenshots: 3 })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun264EditorMicroScaleBrowser] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
