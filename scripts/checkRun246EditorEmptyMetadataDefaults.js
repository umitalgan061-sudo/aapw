...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run246-editor-empty-metadata-defaults');
...
async function main() {
  ...
  const fixturePath = path.join(ARTIFACT_DIR, 'empty-editor-metadata.scene.json');
  const expected = {
    id: 'run246-tree-0001',
    name: 'Run246 Empty Metadata Tree',
    asset: 'marker-tree',
    position: [-11, 0, 17],
    rotation: [0.625, -0.875, 1.5],
  ...
  try {
    ...
    assert(before.snapEnabled === false && before.snapSize === 4.25, `Prepared snap state drifted: ${JSON.stringify(before)}`);
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '01-before-empty-metadata-load.png'), fullPage: true });
    ...
    assert(after.snapEnabled === true && after.snapSize === 1, `Empty editor metadata did not compose snap defaults: ${JSON.stringify(after)}`);
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '02-after-empty-metadata-load.png'), fullPage: true });

    console.log(`[checkRun246EditorEmptyMetadataDefaults] PASS ${JSON.stringify({ object: after.object, snapEnabled: after.snapEnabled, snapSize: after.snapSize, gridVisible: after.gridVisible, unexpectedErrors: errors.length, screenshots: 2 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun246EditorEmptyMetadataDefaults] FAIL: ${error.stack || error}`); process.exitCode = 1; });
