...
// Execute the exact shipped lock-on core without importing the rest of gameLoopHelpers.js.
// The runtime module legitimately imports Three.js for camera/movement helpers, while this Node
// contract only needs the DOM-free lock-on section. Browser acceptance below the CI workflow still
// loads the complete shipped module through game3d.html.
const loopSource = fs.readFileSync(new URL('../src/3d/gameLoopHelpers.js', import.meta.url), 'utf8');
const lockStart = loopSource.indexOf('export const PLAYER_LOCK_ON_CONFIG');
const lockEnd = loopSource.indexOf('const _forward = new THREE.Vector3()');
assert.ok(lockStart >= 0 && lockEnd > lockStart, 'unable to isolate shipped Player lock-on core');
const lockCoreSource = loopSource.slice(lockStart, lockEnd);
for (const forbidden of ["from 'three'", 'new THREE.', 'worldToChunkCoord', 'CHUNK_CONFIG']) {
	assert.ok(!lockCoreSource.includes(forbidden), `lock-on core must stay pure: ${forbidden}`);
}
const lockModuleSource = `${lockCoreSource}\n//# sourceURL=aapw-player-lock-on-core.js`;
const lockModuleUrl = `data:text/javascript;base64,${Buffer.from(lockModuleSource).toString('base64')}`;
const {
...
const hidden = entity('hidden-center', 0, 6); hidden.object3D.visible = false;
assert.equal(evaluatePlayerLockTarget({ playerPosition: player, forward, entity: hidden }).reason, 'unavailable');
const disabled = entity('disabled-center', 0, 5); disabled.object3D.userData.lockOnDisabled = true;
assert.equal(evaluatePlayerLockTarget({ playerPosition: player, forward, entity: disabled }).reason, 'unavailable');

const candidates = [hidden, disabled, entity('near-edge', 8, 8), entity('center-farther', 0, 12), entity('behind', 0, -4)];
assert.equal(selectPlayerLockTarget({ playerPosition: player, forward, candidates }).id, 'center-farther');
...
assert.ok(facingPlayer.rotation.y > 0 && facingPlayer.rotation.y <= 0.55 + 1e-9);
for (let i = 0; i < 20; i += 1) applyPlayerLockFacing(facingPlayer, { x: 10, z: 0 }, 0.05);
assert.ok(Math.abs(facingPlayer.rotation.y - Math.PI / 2) < 0.01);
...
try {
	...
	assert.equal(snapshot.locked, false); assert.equal(events.at(-1)?.detail.reason, 'toggle-release');
	snapshot = controller.update({ playerPosition: player, forward, candidates: [target], toggleRequested: true });
	assert.equal(snapshot.locked, true); target.object3D.position.z = PLAYER_LOCK_ON_CONFIG.BREAK_DISTANCE_METERS + 1;
	snapshot = controller.update({ playerPosition: player, forward, candidates: [target] });
	assert.equal(snapshot.locked, false); assert.equal(events.at(-1)?.detail.reason, 'range-break');
	target.object3D.position.z = 10; controller.update({ playerPosition: player, forward, candidates: [target], toggleRequested: true });
	snapshot = controller.update({ playerPosition: player, forward, candidates: [] });
	assert.equal(snapshot.locked, false); assert.equal(events.at(-1)?.detail.reason, 'target-removed');
	controller.update({ playerPosition: player, forward, candidates: [target], toggleRequested: true }); target.object3D.visible = false;
	snapshot = controller.update({ playerPosition: player, forward, candidates: [target] });
	assert.equal(snapshot.locked, false); assert.equal(events.at(-1)?.detail.reason, 'target-unavailable'); target.object3D.visible = true;
	controller.update({ playerPosition: player, forward, candidates: [target], toggleRequested: true }); target.object3D.userData.lockOnDisabled = true;
	snapshot = controller.update({ playerPosition: player, forward, candidates: [target] });
	assert.equal(snapshot.locked, false); assert.equal(events.at(-1)?.detail.reason, 'target-unavailable'); delete target.object3D.userData.lockOnDisabled;
	controller.update({ playerPosition: player, forward, candidates: [target], toggleRequested: true });
	snapshot = controller.update({ playerPosition: player, forward: { x: 0, z: -1 }, candidates: [target] });
	assert.equal(snapshot.locked, false); assert.equal(events.at(-1)?.detail.reason, 'view-break');
} finally {
...
const previousNavigator = Object.getOwnPropertyDescriptor(globalThis, 'navigator'); let pads = [];
