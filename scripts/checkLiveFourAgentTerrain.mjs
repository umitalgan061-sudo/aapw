...
const policyId = 'live-four-agent-owner-map-terrain-2026-08-13-v2';
const centers = [
...
try {
  ...
  const editorProof = await editor.evaluate(async ({ id, expected, cellCenters }) => {
    ...
    const samples = cellCenters.map(([cell, x, y]) => {
      ...
      return { cell, sampledCell: sample?.cell, layer: sample?.layer, height: sample?.liveHeightMeters, weight: sample?.weight };
    });
    return {
