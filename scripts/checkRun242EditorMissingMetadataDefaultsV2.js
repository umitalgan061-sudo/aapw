...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run242-editor-missing-metadata-defaults-v2');
...
async function main() {
  ...
  try {
    ...
    assert(before.objectCount === 0, `Expected clean editor baseline: ${JSON.stringify(before)}`);
    assert(before.gridVisible === false && before.snapEnabled === false && before.snapSize === 2.5, `Pre-load controls were not prepared: ${JSON.stringify(before)}`);
    ...
    assert(after.gridVisible === true, `Missing editor.gridVisible did not default true after completed load: ${JSON.stringify(after)}`);
    assert(after.snapEnabled === true, `Missing editor.snapEnabled did not default true after completed load: ${JSON.stringify(after)}`);
    assert(after.snapSize === 1, `Missing editor.snapSize did not default to 1 after completed load: ${JSON.stringify(after)}`);
    ...
    console.log(`[checkRun242EditorMissingMetadataDefaultsV2] PASS ${JSON.stringify({ object: after.object, gridVisible: after.gridVisible, snapEnabled: after.snapEnabled, snapSize: after.snapSize, unexpectedErrors: errors.length, screenshots: 2 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun242EditorMissingMetadataDefaultsV2] FAIL: ${error.stack || error}`); process.exitCode = 1; });
