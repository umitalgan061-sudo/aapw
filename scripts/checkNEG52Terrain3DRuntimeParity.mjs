...
import { sampleG52NearDetail } from '../godot/terrain-authoring/geocells/ne/g52_near_detail.mjs';
import {
  G52_TERRAIN3D_RUNTIME_PARITY,
...
const SOURCE_PATH = path.join(PROOF_DIR, 'g52-runtime-source.json');
const BAKE_PATH = path.join(PROOF_DIR, 'g52-runtime-bake.json');
...
function fail(message) {
  console.error(`[checkNEG52Terrain3DRuntimeParity] FAIL: ${message}`);
  process.exit(1);
}
...
function buildSource() {
  const bounds = G52_TERRAIN3D_RUNTIME_PARITY.normalizedBounds;
  const payload = {
    schema: 'westeros-g52-terrain3d-source-v1',
    policyId: G52_TERRAIN3D_RUNTIME_PARITY.id,
    sourceMapSha256: EXPECTED_SOURCE_SHA,
    terrain3dVersion: G52_TERRAIN3D_RUNTIME_PARITY.terrain3dVersion,
    width: SOURCE_SIZE,
    height: SOURCE_SIZE,
  ...
  for (let y = 0; y < SOURCE_SIZE; y += 1) {
    ...
    for (let x = 0; x < SOURCE_SIZE; x += 1) {
      ...
      requireCondition(fields.every(Number.isFinite), `non-finite G52 source at ${x},${y}`);
      ...
      if (sample.waterConfidence >= 0.999999) {
        ...
        maxCanonicalWaterTintDelta = Math.max(
          ...
          Math.abs(sample.tintR - 1),
          Math.abs(sample.tintG - 1),
          Math.abs(sample.tintB - 1),
        );
  ...
  requireCondition(waterDominantSamples > 0 && landDominantSamples > 0, 'G52 mixed coastline collapsed to one semantic class');
  requireCondition(canonicalWaterSamples > 0, 'G52 source has no canonical-water proof samples');
...
function writeSource() {
  ...
  console.log(`G52_RUNTIME_SOURCE_METRICS=${JSON.stringify({
  ...
  console.log('NE_G52_RUNTIME_PARITY_SOURCE_OK');
}
...
function compareBakeToSource(source, bake) {
  requireCondition(bake.schema === 'westeros-g52-terrain3d-bake-v1', `unexpected bake schema ${bake.schema}`);
  ...
  requireCondition(Number.isFinite(bake.boundaryProbeMaxHeightError) && bake.boundaryProbeMaxHeightError <= HEIGHT_TOLERANCE,
    `Terrain3D 255/256 boundary probe error ${bake.boundaryProbeMaxHeightError}`);
  ...
  requireCondition(maxSnowError <= UNIT_TOLERANCE, `Terrain3D snow-weight parity error ${maxSnowError}`);
...
async function verifyBrowserAdapter(bake) {
  ...
  try {
    ...
    const result = await page.evaluate(async (payload) => {
      const { G52_TERRAIN3D_RUNTIME_PARITY, createG52Terrain3DWorldSampler } = await import('/src/3d/world/worldReferenceTerrainAdapter.js');
      ...
      const sampler = createG52Terrain3DWorldSampler(payload, { mapBounds: WORLD_SCALE.MAP_BOUNDS, metersPerMapUnit: WORLD_SCALE.METERS_PER_MAP_UNIT });
      const b = G52_TERRAIN3D_RUNTIME_PARITY.normalizedBounds;
      let maxNodeError = 0;
      let finiteSamples = 0;
...
if (process.argv.includes('--verify')) {
  ...
  console.log(`G52_RUNTIME_PARITY_METRICS=${JSON.stringify({ ...terrain3d, ...browser, bakeChecksum: bake.bakeChecksum })}`);
  console.log('NE_G52_TERRAIN3D_RUNTIME_PARITY_OK');
} else {
  writeSource();
}
