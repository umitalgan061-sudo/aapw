...
// V2 deliberately feathers the pre-Doom transition with deterministic refugia rather than drawing a
// perfect binary ring. Hard-barren points must always be rejected; zero-pressure outer world must
// always survive; the transition should contain both survivors and rejected pockets so the volcanic
// province has no visible circular vegetation contour.
let hardBarren = 0;
