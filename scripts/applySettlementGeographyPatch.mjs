...
if (!updated.includes("./settlementGeographyPolicy.js")) {
	...
	console.log('[applySettlementGeographyPatch] applied deterministic geography integration + material provenance + shoreline probe + footprint relief');
}

let patchedPolicy = policySource;
function policyReplaceOnce(before, after, label) {
	const count = patchedPolicy.split(before).length - 1;
	if (count !== 1) throw new Error(`[applySettlementGeographyPatch] policy ${label}: expected 1 match, found ${count}`);
	patchedPolicy = patchedPolicy.replace(before, after);
}
if (!patchedPolicy.includes('shorelineDistanceMeters')) {
	...
	policyReplaceOnce(
		...
		'coast evidence output',
	);
}
console.log('[applySettlementGeographyPatch] coastal + footprint policy staged');