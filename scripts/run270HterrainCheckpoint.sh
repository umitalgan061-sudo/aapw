...
PROGRESS_MARKER='## Run 270 — finalize Run264 HTerrain authoring checkpoint'
if ! grep -Fq "$PROGRESS_MARKER" 3D_GAME_PROGRESS.md; then
  cat >> 3D_GAME_PROGRESS.md <<'EOF'

## Run 270 — finalize Run264 HTerrain authoring checkpoint
...
- Visual/normal proof PASS with real Mesa llvmpipe OpenGL: two 1280x720 terrain camera angles render a deterministic hill with four texture regions, and a 17x17 same-tile height edit queues one normal tile through the compatibility bridge and changes the sampled normal from approximately (0.498,0.498,1.0) to (0.051,0.502,0.722).
...
ADR_MARKER='## ADR Run270-HTerrain — Isolated Godot HTerrain authoring workspace'
if ! grep -Fq "$ADR_MARKER" DECISIONS.md; then
  cat >> DECISIONS.md <<'EOF'

## ADR Run270-HTerrain — Isolated Godot HTerrain authoring workspace
...
**Etkilenen sistemler:** `godot/terrain-authoring/`, Run264/Run270 CI ve authoring dokümantasyonu. Mevcut HTML/JS/Three.js/PWA runtime davranışı etkilenmez.
...
if ! grep -Fq 'run270,2026-08-11,run264-hterrain-authoring-checkpoint' perf_log.csv; then
  printf '%s\n' 'run270,2026-08-11,run264-hterrain-authoring-checkpoint,,,,,,,"Godot 4.6.3 HTerrain authoring checkpoint; two-angle real-render evidence + normal-baker proof + full PWA/browser chain PASS; mobile drawCalls 35 triangles 195929 geometries 30 textures 22"' >> perf_log.csv
fi

if ! grep -Fq 'Run270 Run264 HTerrain authoring checkpoint' STABLE_TAGS.md; then
  printf '\n- `%s` — Run270 Run264 HTerrain authoring checkpoint; Godot 4.6.3 Terrain Data/texture/detail tools, two-angle visual evidence, real-render normal-baker proof, PWA/perf/full smoke PASS.\n' "$TAG" >> STABLE_TAGS.md
fi
