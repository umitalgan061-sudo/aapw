...
try {
	...
	const result = await page.evaluate(async () => {
		...
		const { buildExpeditionJourneyText, evaluateExpeditionJourney } = await import('/src/3d/gameplay/interactionFieldReadiness.js');
		...
		const plan = evaluateExpeditionJourney(restored.inventory, [
		...
		const text = buildExpeditionJourneyText(plan);
		...
		const renderedRoute = rendered.includes('Toplam: 55 km · 2 yol azığı')
			&& rendered.includes('dragonstone-watch-road → dragonstone-harbor-road')
			&& rendered.includes('Rota hazır · kalan yol azığı: 0');
		const sequential = plan.complete === true
			&& plan.legs[0].requiredTravelPacks === 0
			&& plan.legs[1].requiredTravelPacks === 1
			&& plan.legs[2].remainingTravelPacksAfter === 0;
		dialogueBox.dispose();
		host.remove();
		return { renderedRoute, sequential, preserved, rendered };
	});

	if (pageErrors.length || consoleErrors.length) {
		throw new Error(`Journey-planning browser proof emitted errors: ${JSON.stringify({ pageErrors, consoleErrors })}`);
	}
	for (const key of ['renderedRoute', 'sequential', 'preserved']) {
		if (!result[key]) throw new Error(`Journey-planning browser assertion failed: ${key} ${JSON.stringify(result)}`);
	}
	console.log(`[RPG Chromium] PASS sequential journey planning UX ${JSON.stringify(result)}`);
} finally {
	await browser.close();
