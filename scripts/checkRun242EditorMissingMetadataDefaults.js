...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run242-editor-missing-metadata-defaults');
...
async function main() {
  ...
  try {
    ...
    assert(after.gridVisible === true, `Missing editor.gridVisible did not default true: ${JSON.stringify(after)}`);
    assert(after.snapEnabled === true, `Missing editor.snapEnabled did not default true: ${JSON.stringify(after)}`);
    assert(after.snapSize === 1, `Missing editor.snapSize did not default to 1: ${JSON.stringify(after)}`);
    ...
    console.log(`[checkRun242EditorMissingMetadataDefaults] PASS ${JSON.stringify({ object: after.object, gridVisible: after.gridVisible, snapEnabled: after.snapEnabled, snapSize: after.snapSize, unexpectedErrors: errors.length, screenshots: 2 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun242EditorMissingMetadataDefaults] FAIL: ${error.stack || error}`); process.exitCode = 1; });
