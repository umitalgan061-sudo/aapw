...
for (const token of [
	...
	"auroraCurtainMorphology = 'broken-asymmetric-ray-sheets-v10-rising-phosphor'",
...
assert(v4.includes('float edgeCore = exp(-abs(aboveEdge) / 0.024) * 0.72;'), 'V4 narrow edge phosphor contract missing.');
assert(v4.includes('float risingCore = step(0.0, aboveEdge) * exp(-max(aboveEdge, 0.0) / 0.24) * 0.95;'), 'V4 rising phosphor reach contract missing.');
assert(v4.includes('finalColor += oxygenGreen * phosphorCore * 0.86;'), 'V4 calibrated rising phosphor output missing.');
...
console.log('[checkRun221AuroraFinalContract] PASS: final V4 consumes camera-relative atmosphere with broken ray sheets and rising phosphor cores; V5 required-token night calibration, world-direction anchoring and offline PWA dependencies are locked.');
