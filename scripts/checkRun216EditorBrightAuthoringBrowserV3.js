...
const OUT = path.join(ROOT, 'artifacts', 'run216-editor-bright-authoring-v3');
...
async function main() {
  ...
  try {
    ...
    const first = await page.evaluate(() => ({
      ...
      fbxStatus: document.querySelector('#we-fbx-status')?.textContent || ''
    }));
    ...
    assert(!first.fbxStatus.includes('Sunucu klasör listelemesini açmıyor'), `FBX boot scan still reports unavailable directory listing: ${first.fbxStatus}`);
    ...
    console.log(`[checkRun216EditorBrightAuthoringBrowserV3] PASS: ${JSON.stringify({ first: first.snapshot, second, fbxStatus: first.fbxStatus })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun216EditorBrightAuthoringBrowserV3] FAIL: ${error?.stack || error}`);
  process.exit(1);
});
