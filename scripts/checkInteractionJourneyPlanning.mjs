...
assert.deepEqual(readyPlan.legs.map((leg) => ({
	index: leg.index,
	allowed: leg.allowed,
	required: leg.requiredTravelPacks,
	before: leg.remainingTravelPacksBefore,
	after: leg.remainingTravelPacksAfter,
})), [
...
const undiscovered = evaluateExpeditionJourney(inventoryWith({ travelPacks: 2 }), [
	threeLegs[0],
	{ ...threeLegs[1], discovered: false },
	threeLegs[2],
]);
...
const combatBlocked = evaluateExpeditionJourney(inventoryWith({ travelPacks: 2 }), [
	{ ...threeLegs[0], inCombat: true },
]);
...
const routeBlocked = evaluateExpeditionJourney(inventoryWith({ travelPacks: 2 }), [
	{ ...threeLegs[0], routeOpen: false },
]);
...
assert.equal(noKit.startingTravelPacks, 0, 'travel packs cannot grant route capacity without the field kit');

const malformed = evaluateExpeditionJourney(evaluateFieldReadiness(inventoryWith({ travelPacks: 2 })), [
	{ originId: 'dragonstone', destinationId: '  ', discovered: true, routeOpen: true, distanceKm: -5 },
]);
...
assert.equal(empty.totalDistanceKm, 0);
assert.equal(empty.totalRequiredTravelPacks, 0);
assert.equal(buildExpeditionJourneyText(empty), 'Sefer Rotası\nHenüz rota planlanmadı.');

const capped = evaluateExpeditionJourney(inventoryWith({ travelPacks: 99 }), [
	{ originId: 'dragonstone', destinationId: 'far-road', discovered: true, routeOpen: true, inCombat: false, distanceKm: 85 },
]);
assert.equal(capped.complete, false);
assert.deepEqual(capped.legs[0].reasons, [FAST_TRAVEL_BLOCK_REASON.INSUFFICIENT_PROVISIONS]);

console.log(`[RPG] PASS sequential expedition journey planning ${JSON.stringify({
	ready: { distanceKm: readyPlan.totalDistanceKm, provisions: readyPlan.totalRequiredTravelPacks },
	blockedAt: underProvisioned.blockedAtLegIndex,
	undiscoveredAt: undiscovered.blockedAtLegIndex,
})}`);
