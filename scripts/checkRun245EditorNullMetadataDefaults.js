...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run245-editor-null-metadata-defaults');
...
async function main() {
  ...
  const fixturePath = path.join(ARTIFACT_DIR, 'null-editor-metadata.scene.json');
  const expected = {
    id: 'run245-tree-0001',
    name: 'Run245 Null Metadata Tree',
    asset: 'marker-tree',
    position: [14, 0, -9],
    rotation: [-0.5, 0.75, -1.25],
    scale: [0.007, 1.25, 2]
  };
  ...
  try {
    ...
    await page.locator('#we-snap-size').fill('3.5');
    await page.locator('#we-snap-size').dispatchEvent('change');
    const before = await capture();
    assert(before.snapEnabled === false && before.snapSize === 3.5, `Prepared snap state drifted: ${JSON.stringify(before)}`);
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '01-before-null-metadata-load.png'), fullPage: true });
    ...
    assert(after.snapEnabled === true && after.snapSize === 1, `Null editor metadata did not compose snap defaults: ${JSON.stringify(after)}`);
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '02-after-null-metadata-load.png'), fullPage: true });

    console.log(`[checkRun245EditorNullMetadataDefaults] PASS ${JSON.stringify({ object: after.object, snapEnabled: after.snapEnabled, snapSize: after.snapSize, gridVisible: after.gridVisible, unexpectedErrors: errors.length, screenshots: 2 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun245EditorNullMetadataDefaults] FAIL: ${error.stack || error}`); process.exitCode = 1; });
