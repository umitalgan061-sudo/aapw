...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run230-editor-responsive-resize');
...
async function main() {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'desktop-to-mobile-resize.png'), fullPage: false });
    ...
    console.log(`[checkRun230EditorResponsiveResize] PASS ${JSON.stringify({ desktop, resizedMobile })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun230EditorResponsiveResize] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
