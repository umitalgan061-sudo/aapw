...
const OUT = path.join(ROOT, 'artifacts', 'run264-editor-micro-scale-v3');
const MIN = 0.000001;
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => Boolean(
      ...
      window.__WESTEROS_EDITOR_MICRO_SCALE_BOUNDS_GUARD_RUN264__
    ), null, { timeout: 120000 });
    ...
    await page.waitForFunction(() => ['we-scale-x','we-scale-y','we-scale-z'].every((id) => {
      const input = document.getElementById(id);
      return input.min === '0.000001' && input.step === '0.000001';
    }), null, { timeout: 5000 });
    ...
    const direct = await page.evaluate(async (id) => {
      ...
      const json = serializeEditorScene(window.__WESTEROS_WORLD_EDITOR__.editableObjects, window.__WESTEROS_WORLD_EDITOR__.instanceManager.serialize(), window.__WESTEROS_WORLD_EDITOR__.getEditorState());
      const record = json.objects.find((item) => item.id === id);
      return { scale: object.scale.toArray(), text, persisted: record?.transform?.scale, json: JSON.stringify(json) };
    }, targetId);
    assert(direct.scale.every((value) => Math.abs(value - MIN) < 1e-12), `Runtime lost micro-scale: ${JSON.stringify(direct)}`);
    assert(direct.text.every((value) => value === '0.000001'), `Inspector rounded micro-scale: ${JSON.stringify(direct.text)}`);
    assert(direct.persisted?.every((value) => Math.abs(value - MIN) < 1e-12), `Serializer lost micro-scale: ${JSON.stringify(direct.persisted)}`);
    assert(direct.json.includes('0.000001'), 'Serialized JSON does not contain 0.000001');

    fs.mkdirSync(OUT, { recursive: true });
    await page.screenshot({ path: path.join(OUT, '01-direct-micro-scale.png'), fullPage: true });
    ...
    const afterShrink = await page.evaluate(() => ['we-scale-x','we-scale-y','we-scale-z'].map((id) => document.getElementById(id).value));
    assert(afterShrink.every((value) => value === '0.000001'), `Quick shrink display lost precision: ${JSON.stringify(afterShrink)}`);
    ...
    const teardown = await page.evaluate(() => {
      ...
      window.__WESTEROS_EDITOR_MICRO_SCALE_BOUNDS_GUARD_RUN264__?.dispose?.();
      return [Boolean(window.__WESTEROS_EDITOR_MICRO_SCALE_RUN264__), Boolean(window.__WESTEROS_EDITOR_MICRO_SCALE_BOUNDS_GUARD_RUN264__)];
    });
    assert(teardown.every((value) => value === false), `Run264 globals leaked: ${JSON.stringify(teardown)}`);
    assert(errors.length === 0, `Browser errors: ${errors.join(' | ')}`);
    console.log(`[checkRun264EditorMicroScaleBrowserV2] PASS ${JSON.stringify({ beforeCount: beforeIds.length, targetId, direct: { scale: direct.scale, text: direct.text, persisted: direct.persisted }, afterShrink, screenshots: 2 })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => { console.error(`[checkRun264EditorMicroScaleBrowserV2] FAIL: ${error.stack || error}`); process.exitCode = 1; });
