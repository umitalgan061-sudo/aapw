...
async function main() {
	...
	try {
		...
		const result = await page.evaluate(async () => {
			...
			const stateSnapshot = () => ({
				cameraPosition: arr(state.camera.position), cameraQuaternion: arr(state.camera.quaternion),
				cameraMatrix: state.camera.matrix.elements.map(round), cameraMatrixWorld: state.camera.matrixWorld.elements.map(round),
			...
			const renderOffscreen = () => {
				...
				const pixels = new Uint8Array(720 * 450 * 4);
				state.renderer.readRenderTargetPixels(target, 0, 0, 720, 450, pixels);
				state.renderer.setRenderTarget(previous);
				let rolling = 2166136261 >>> 0;
				for (let index = 0; index < pixels.length; index += 37) rolling = Math.imul(rolling ^ pixels[index], 16777619) >>> 0;
			...
			const profiles = {
				backgroundOnly: new Set(),
				skyOnly: new Set([state.sky]),
				starsOnly: new Set([state.stars]),
				waterOnly: new Set([state.water]),
				terrainLights: new Set([...terrain, ...lights]),
				opaqueWorld,
				allNoStars: new Set(direct.filter((object) => object !== state.stars)),
				allNoSky: new Set(direct.filter((object) => object !== state.sky)),
				allNoWater: new Set(direct.filter((object) => object !== state.water)),
			...
			for (const [name, include] of Object.entries(profiles)) layerStability[name] = measureProfile(include);
			for (const [object, visible] of originalVisibility) object.visible = visible;

			const beforeState = stateSnapshot();
			const before = renderOffscreen();
		...
		console.log('[checkCurrentRuntimeRenderRestoreDiagnostics] PASS: rollback state exact; layer stability map captured for render-oracle qualification.');
	} finally {
