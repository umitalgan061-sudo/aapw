#!/usr/bin/env node
/** Run196 focused real-createScene render/state restore diagnostic with wind-grass isolation. */
const fs = require('fs');
...
async function main() {
	...
	try {
		...
		const result = await page.evaluate(async () => {
			...
			if (!state.grass || state.grass.parent !== state.scene) throw new Error('Run180 grass direct scene root missing');
			if (!integration.inventory.roots.includes(state.grass)) throw new Error('Run196 inventory omitted Run180 grass');
			...
			const stateSnapshot = () => ({
				...
				grass: { uuid: state.grass.uuid, parentUuid: state.grass.parent?.uuid || null, placedCount: state.grassStats?.placedCount ?? null, centerCell: state.grassStats?.centerCell ? { ...state.grassStats.centerCell } : null },
			});
			...
			const profiles = {
				grassOnly: new Set([state.grass]),
				allExceptGrass: new Set(direct.filter((object) => object !== state.grass)),
			...
			const grassBefore = state.grass;
			const beforeState = stateSnapshot();
			const staticBeforeVisibility = state.grass.visible; state.grass.visible = false; const staticBefore = renderOffscreen(); state.grass.visible = staticBeforeVisibility;
			const fullBefore = renderOffscreen();
			...
			if (state.grass.parent !== null) throw new Error('Run180 grass remained attached during canonical mode');
			...
			if (state.grass !== grassBefore || state.grass.parent !== state.scene) throw new Error('Run180 grass identity/parent not restored');
			const restoredState = stateSnapshot();
			const staticAfterVisibility = state.grass.visible; state.grass.visible = false; const staticAfter = renderOffscreen(); state.grass.visible = staticAfterVisibility;
			const fullAfter = renderOffscreen();
			state.controls.update(); state.camera.updateMatrixWorld(true); const afterControlsState = stateSnapshot();

			const stateEqual = JSON.stringify(beforeState) === JSON.stringify(restoredState);
			const controlsStateEqual = JSON.stringify(beforeState) === JSON.stringify(afterControlsState);
			const staticRenderEqual = JSON.stringify(staticBefore) === JSON.stringify(staticAfter);
			const fullSubmissionEqual = fullBefore.calls === fullAfter.calls && fullBefore.triangles === fullAfter.triangles;
			const grassDeltaCalls = layerStability.fullScene.frames[2].calls - layerStability.allExceptGrass.frames[2].calls;
			const grassDeltaTriangles = layerStability.fullScene.frames[2].triangles - layerStability.allExceptGrass.frames[2].triangles;
			...
			return { layerStability, staticBefore, staticAfter, fullBefore, fullAfter, canonical, stateEqual, controlsStateEqual, staticRenderEqual, fullSubmissionEqual, grassDeltaCalls, grassDeltaTriangles, inventoryRoots: integration.inventory.roots.length, directSceneRootCount: integration.inventory.directSceneRootCount, infrastructureCount: integration.inventory.infrastructureCount };
		});
		...
		console.log(`[checkCurrentRuntimeRenderRestoreDiagnostics] STATE: ${JSON.stringify({ stateEqual: result.stateEqual, controlsStateEqual: result.controlsStateEqual, staticRenderEqual: result.staticRenderEqual, fullSubmissionEqual: result.fullSubmissionEqual, grassDeltaCalls: result.grassDeltaCalls, grassDeltaTriangles: result.grassDeltaTriangles, inventoryRoots: result.inventoryRoots, directSceneRootCount: result.directSceneRootCount, infrastructureCount: result.infrastructureCount })}`);
		if (!result.stateEqual || !result.controlsStateEqual) throw new Error('rollback JS/Three state equality failed');
		if (!result.layerStability.allExceptGrass.stable12 || !result.layerStability.allExceptGrass.stable23) throw new Error('all-except-grass static visual oracle is not byte-stable');
		if (!result.staticRenderEqual) throw new Error('pre/post all-except-grass render differs');
		if (!result.fullSubmissionEqual) throw new Error('full current render submission counts changed');
		if (result.grassDeltaCalls !== 1 || result.grassDeltaTriangles !== 24000) throw new Error(`Run180 grass render delta changed: ${result.grassDeltaCalls}/${result.grassDeltaTriangles}`);
		console.log('[checkCurrentRuntimeRenderRestoreDiagnostics] PASS: Run180 grass is the qualified time-varying layer (+1 call/+24000 tris); all-except-grass render is byte-stable and exact across rollback; grass ownership/identity restored.');
