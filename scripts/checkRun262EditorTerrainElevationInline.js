...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run262-editor-terrain-elevation-inline');
...
async function main() {
  ...
  async function findVisibleStroke() {
    ...
    const stroke = await page.evaluate(({ left, top, width, height }) => {
      ...
      const candidates = [[0.50,0.68],[0.40,0.68],[0.60,0.68],[0.50,0.58],[0.35,0.58],[0.65,0.58],[0.50,0.78],[0.30,0.72],[0.70,0.72],[0.25,0.82],[0.75,0.82],[0.20,0.62],[0.80,0.62],[0.35,0.82],[0.65,0.82],[0.50,0.88]];
      for (const [nx, ny] of candidates) {
        const clientX = left + width * nx;
        const clientY = top + height * ny;
        const point = live.surfacePointFromClient(clientX, clientY);
        if (point && [point.x, point.y, point.z].every(Number.isFinite)) return { clientX, clientY, point: point.toArray(), normalized: [nx, ny] };
      }
      return null;
  ...
  async function stroke(strokePoint) {
    await page.mouse.move(strokePoint.clientX, strokePoint.clientY);
  ...
  async function capture(assetId) {
    return page.evaluate(async (wantedAssetId) => {
      ...
      return {
        object: { asset: object.userData.editorAssetId, scale: object.scale.toArray(), signedElevation: object.userData.editorTerrainElevationMeters },
        offset, colliderY, baseY, meshRayY: hit?.point?.y ?? null,
  ...
  async function runCase({ mode, assetId, direction, screenshot }) {
    await waitReady();
    assert(errors.length === 0, `Boot browser errors: ${errors.join(' | ')}`);
    const point = await findVisibleStroke();
    ...
    assert(state.object, `${mode} did not create an elevation stamp: ${JSON.stringify(state)}`);
    assert(state.object.scale[1] === 2, `${mode} scale strength drifted: ${JSON.stringify(state.object)}`);
    assert(state.object.signedElevation === direction * 2, `${mode} signed metadata drifted: ${JSON.stringify(state.object)}`);
    assert(direction < 0 ? state.offset < -1.99 : state.offset > 1.99, `${mode} elevation direction drifted: ${JSON.stringify(state)}`);
    assert(direction < 0 ? state.colliderY < state.baseY - 1.99 : state.colliderY > state.baseY + 1.99, `${mode} collider drifted: ${JSON.stringify(state)}`);
  ...
  try {
    ...
    const water = await runCase({ mode: 'water-add', assetId: 'editor-water-cell', direction: -1, screenshot: '02-water-lowers-terrain.png' });
    const land = await runCase({ mode: 'land-add', assetId: 'editor-land-cell', direction: 1, screenshot: '03-land-raises-terrain.png' });
    assert(Math.abs(water.state.baseY - water.state.colliderY - 2) < 0.01, `Sea brush did not lower by 2m: ${JSON.stringify(water.state)}`);
    assert(Math.abs(land.state.colliderY - land.state.baseY - 2) < 0.01, `Land brush did not raise by 2m: ${JSON.stringify(land.state)}`);
    console.log(`[checkRun262EditorTerrainElevationInline] PASS ${JSON.stringify({ waterOffset: water.state.offset, landOffset: land.state.offset, strengthMeters: 2, screenshots: 3, unexpectedErrors: errors.length })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun262EditorTerrainElevationInline] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
