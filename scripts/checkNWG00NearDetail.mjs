...
import {
  buildG00NearDetailProbe,
  measureG00NearDetailSurface,
  measureG00RuntimeVegetation,
} from '../godot/terrain-authoring/geocells/nw/g00_near_detail.mjs';
import {
...
function reportFatal(error) {
  ...
  console.error(`::error title=NW G00 Near Detail Validation::${githubEscape(detail)}`);
}
...
const roadPath = measureG00RoadPath(runtime);
const crossings = findG00CrossingEdges(runtime);
...
if (surface.sourceSamples !== 66049) throw new Error(`expected 257x257 near-detail source field, got ${surface.sourceSamples}`);
if (surface.landDetailSamples < 64) throw new Error(`G00 land micro-detail collapsed: ${surface.landDetailSamples}`);
if (surface.minTint < 0.90 || surface.maxTint > 1.000001) throw new Error(`G00 tint range escaped policy: ${surface.minTint}/${surface.maxTint}`);
if (surface.minRoughness < 0.72 || surface.maxRoughness > 0.940001) throw new Error(`G00 roughness range escaped policy: ${surface.minRoughness}/${surface.maxRoughness}`);
if (surface.maxAdjacentTintDelta > 0.08 || surface.maxAdjacentRoughnessDelta > 0.14) throw new Error(`G00 near-detail adjacent discontinuity: ${surface.maxAdjacentTintDelta}/${surface.maxAdjacentRoughnessDelta}`);
if (surface.maxGuardBandTintDelta > 0.05 || surface.maxGuardBandRoughnessDelta > 0.08) throw new Error(`G00 near-detail guard-band seam: ${surface.maxGuardBandTintDelta}/${surface.maxGuardBandRoughnessDelta}`);
if (surface.maxHeightDeltaMeters > 1e-8 || surface.maxRockWeightDelta > 1e-8 || surface.maxSnowWeightDelta > 1e-8
  || surface.maxRoadCoverageDelta > 1e-8 || surface.maxPathCoverageDelta > 1e-8) {
  throw new Error(`Near Detail modified a qualified source: ${JSON.stringify({
    height: surface.maxHeightDeltaMeters,
    rock: surface.maxRockWeightDelta,
    snow: surface.maxSnowWeightDelta,
    road: surface.maxRoadCoverageDelta,
    path: surface.maxPathCoverageDelta,
  })}`);
}
if (surface.maxCanonicalWaterTintDelta > 1e-8 || surface.maxCanonicalWaterRoughnessDelta > 1e-8) {
  throw new Error(`Near Detail leaked onto canonical G00 water: ${surface.maxCanonicalWaterTintDelta}/${surface.maxCanonicalWaterRoughnessDelta}`);
}
if (roadPath.maxCanonicalWaterCoverageOutsideSettlement > 0.000001) {
  throw new Error(`qualified G00 Road/Path water safety drifted: ${roadPath.maxCanonicalWaterCoverageOutsideSettlement}`);
}
if (roadPath.activeRoadSamples > 0 && !crossings.some((edge) => edge.tier === 'road')) throw new Error('active G00 road exists without a live road crossing');
if (roadPath.activePathSamples > 0 && !crossings.some((edge) => edge.tier === 'path')) throw new Error('active G00 path exists without a live footpath crossing');
if (crossings.some((edge) => edge.tier === 'road') && roadPath.activeRoadSamples === 0) throw new Error('live G00 road crossing vanished');
if (crossings.some((edge) => edge.tier === 'path') && roadPath.activePathSamples === 0) throw new Error('live G00 path crossing vanished');

if (vegetation.coreInstanceCount > 0) {
  if (vegetation.minRoadDistanceMeters < 9.999) throw new Error(`G00 vegetation violates runtime road exclusion: ${vegetation.minRoadDistanceMeters}`);
  if (vegetation.minSeatDistanceMeters < 89.999) throw new Error(`G00 vegetation violates runtime seat exclusion: ${vegetation.minSeatDistanceMeters}`);
  if (vegetation.minHeightAboveWaterMeters < 1.499) throw new Error(`G00 vegetation violates runtime shore margin: ${vegetation.minHeightAboveWaterMeters}`);
  if (vegetation.minCanonicalLandFactor < 0.55) throw new Error(`G00 imported vegetation escaped canonical land: ${vegetation.minCanonicalLandFactor}`);
}
if (vegetation.guardOnlyCount > Math.max(32, vegetation.coreInstanceCount * 3 + 16)) {
  throw new Error(`G00 vegetation guard-band density spike: ${vegetation.guardOnlyCount}/${vegetation.coreInstanceCount}`);
}
...
if (emitArg) {
  ...
  fs.writeFileSync(output, `${JSON.stringify(buildG00NearDetailProbe(runtime))}\n`);
}
console.log(`NW_G00_NEAR_DETAIL_METRICS=${JSON.stringify(metrics)}`);
console.log('NW_G00_NEAR_DETAIL_VALIDATION_OK');
