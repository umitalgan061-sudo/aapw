...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run224-editor-duplicate-history');
...
async function main() {
  ...
  try {
    ...
    console.log(`[checkRun224EditorDuplicateHistory] PASS ${JSON.stringify({ afterButtonDuplicate, afterKeyboardDuplicate, afterRedo })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun224EditorDuplicateHistory] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
