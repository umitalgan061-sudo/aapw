  ...
  await page.evaluate(async ({ amount: value, phase: expectedPhase }) => {
    ...
    const onMotion = (event) => {
      ...
      const isTarget = expectedPhase === 'iframe'
        ? frame.dodgeInvulnerable === true
        : frame.dodgeInvulnerable === false && frame.dodgeElapsed > 0.24;
      if (!isTarget) return;
      ...
      window.__dodgeDamageProbe = { armed: true, triggered: true, phase: expectedPhase, before, after, payload: structuredClone(payload), frame: structuredClone(frame) };
    };
...
try {
  ...
  need(iframeProof.frame.dodgeInvulnerable === true, `expected i-frame telemetry ${JSON.stringify(iframeProof.frame)}`);
  need(iframeProof.frame.dodgeElapsed >= 0.05 && iframeProof.frame.dodgeElapsed <= 0.24, `i-frame outside contract ${iframeProof.frame.dodgeElapsed}`);
  ...
  need(recoveryProof.frame.dodgeInvulnerable === false && recoveryProof.frame.dodgeElapsed > 0.24, `expected punishable recovery ${JSON.stringify(recoveryProof.frame)}`);
