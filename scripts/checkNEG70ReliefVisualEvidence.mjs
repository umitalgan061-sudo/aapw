...
const OUT = path.resolve(ROOT, arg ? arg.slice('--out-dir='.length) : 'artifacts/ne-g70-relief-visual');
const MAP_SHA = '20702972e8f45f0fbdc4da5fa68e890a82e4e822e1d58e2f369d8bc5b9c571a1';
const { loadPlaywright, startStaticServer } = devServerHelper;
const relief = buildG70Terrain3DReliefSource();
const biome = buildG70Terrain3DBiomeSource();
const metrics = measureG70Terrain3DRelief();
...
requireOk(metrics.canonicalSea === 96 && metrics.canonicalLand === 0, 'G70 must remain 96/96 canonical sea');
requireOk(metrics.heightSpan === 0 && metrics.addedReliefAbsMax === 0, 'G70 visual proof detected invented submarine relief');
requireOk(metrics.maxGuardHeightDelta === 0 && metrics.maxGuardNormalDelta === 0, 'G70 visual guard continuity changed');
requireOk(relief.heights.length === biome.heights.length, 'merged Biome/Relief source dimensions diverged');
for (let i = 0; i < relief.heights.length; i += 1) {
  requireOk(relief.heights[i] === biome.heights[i], `Relief changed merged Biome height at sample ${i}`);
}

const playwright = loadPlaywright();
requireOk(Boolean(playwright), 'Playwright is required for relief visual evidence');
...
try {
  ...
  const perspective = await page.evaluate(async ({ reliefSource, biomeSource }) => {
    ...
    for (let y = 0; y < size; y += 1) {
      for (let x = 0; x < size; x += 1) {
        ...
        colors.push(biomeSource.colorR[i] * shade, biomeSource.colorG[i] * shade, biomeSource.colorB[i] * shade);
      }
    }
    ...
    window.__g70Relief.render('near');
    return { vertices: positions.length / 3, triangles: indices.length / 3, minHeight, maxHeight, heightSpan: maxHeight - minHeight };
  }, { reliefSource: relief, biomeSource: biome });

  requireOk(perspective.vertices === 65 * 65, 'perspective source grid changed');
  requireOk(perspective.heightSpan === 0, 'perspective mesh invented G70 bathymetry');
  for (const kind of ['near', 'far']) {
    await page.evaluate((value) => window.__g70Relief.render(value), kind);
    const png = await page.locator('#proof').screenshot();
    requireOk(png.length > 1024, `${kind} relief PNG unexpectedly small`);
    fs.writeFileSync(path.join(OUT, `g70-relief-${kind}.png`), png);
    hashes[kind] = sha256(png);
  }
  await page.evaluate(() => window.__g70Relief.dispose());
  ...
  const topdown = await page.evaluate(async () => {
    ...
    return { outputResolution:[1200,800], contextResolution:[600,400], transitionEdges, reliefPatchAlpha:0, gridOverlay:false };
  });

  const worldPng = await page.locator('#world').screenshot();
  requireOk(worldPng.length > 4096, 'full-world relief PNG unexpectedly small');
  requireOk(topdown.transitionEdges > 100, 'full-world geographic transitions disappeared');
  requireOk(topdown.reliefPatchAlpha === 0, 'G70 Relief must not paint an artificial cell patch');
  requireOk(topdown.gridOverlay === false, 'grid overlay must remain disabled');
  fs.writeFileSync(path.join(OUT, 'g70-relief-full-world.png'), worldPng);
  hashes.fullWorld = sha256(worldPng);
  requireOk(pageErrors.length === 0, `browser page errors: ${pageErrors.join(' | ')}`);

  const output = { mapSha256: MAP_SHA, reliefMetrics: metrics, perspective, topdown, sha256: hashes };
  fs.writeFileSync(path.join(OUT, 'g70-relief-visual-metrics.json'), `${JSON.stringify(output, null, 2)}\n`);
  console.log(`G70_RELIEF_VISUAL_METRICS=${JSON.stringify(output)}`);
  console.log('NE_G70_RELIEF_VISUAL_EVIDENCE_OK');
} finally {
  await browser.close();
