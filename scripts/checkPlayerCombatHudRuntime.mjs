...
try {
	...
	await page.evaluate(() => globalThis.dispatchEvent(new CustomEvent('aapw:player-lock-on', { detail: { locked: true, targetId: 'runtime-guard', distanceMeters: 1.2, reason: 'tracking' } })));
	await page.waitForFunction(() => document.querySelector('.g3d-combat-status')?.dataset.state === 'locked', null, { timeout: 1000 });
	...
	const metrics = { ok: true, baseline, active, lockProjection, outOfRange, inRange, parryProjection, afterParry, guardProjection, afterGuard, noTarget, noTargetReset, browserErrors: errors };
