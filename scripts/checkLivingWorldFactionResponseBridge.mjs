...
const actors = [
  ...
  { id: 'suspicious-1', factionId: 'merchants', suspicious: true, unidentified: true },
  { id: 'surrendered-1', factionId: 'raiders', surrendered: true },
];
const services = {
  factions: { getFactionIdForActor: (actor) => actor.factionId },
  reputation: { getReputation: (_actor, target) => target.id === 'wanted-1' || target.id === 'surrendered-1' ? -80 : target.id === 'suspicious-1' ? 0 : 60 },
...
const input = { tick: 4, actors, observations: [
  ...
  { actorId: 'guard-1', targetId: 'suspicious-1', confidence: 0.8 },
...
assert.equal(a.counts.observations, 4);
assert.equal(a.decisions.length, 4);
assert.equal(a.decisions[0].action, 'assist');
assert.equal(a.decisions[1].action, 'engage');
assert.equal(a.decisions[2].action, 'investigate');
assert.equal(a.decisions[3].action, 'pursue');
assert.equal(a.events.length, 3);
assert.deepEqual(a.events.map((event) => event.action), ['engage', 'investigate', 'pursue']);
...
assert.equal(applied.emitted, 3);
assert.deepEqual(calls, ['decision:assist', 'decision:engage', 'decision:investigate', 'decision:pursue', 'event:engage', 'event:investigate', 'event:pursue']);
