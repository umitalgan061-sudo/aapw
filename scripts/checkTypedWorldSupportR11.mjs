...
for(const [typedPath] of owners) if(!ledger.includes(typedPath)) failures.push(`R11 ledger missing ${typedPath}`);
if(failures.length){console.error(`R11 typed world-support check failed with ${failures.length} issue(s):`);for(const failure of failures)console.error(`- ${failure}`);process.exit(1);}
console.log('R11 typed world-support check passed.');
