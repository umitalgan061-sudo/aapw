...
import { G17, G17_RELIEF_POLICY, sampleG17Relief } from '../godot/terrain-authoring/geocells/sw/g17_relief.mjs';
...
function fail(message) { throw new Error(`[checkSWG17ReliefVisualEvidence] FAIL: ${message}`); }
...
if (contextRun.status !== 0) fail(`qualified hydrology SDF context failed: ${contextRun.stderr || contextRun.stdout}`);
...
try {
  ...
  const renderMetrics = await page.evaluate(async ({ samples, visualSize, bounds, heightScale }) => {
    ...
    let minY = Infinity; let maxY = -Infinity; let roughnessSum = 0;
    ...
    scene.add(new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ vertexColors: true, roughness: roughnessSum / samples.length, metalness: 0, side: THREE.DoubleSide })));
    const water = new THREE.Mesh(new THREE.PlaneGeometry(spanX * 2.2, spanZ * 2.2), new THREE.MeshPhysicalMaterial({ color: 0x397e9a, transparent: true, opacity: 0.25, roughness: 0.18, metalness: 0, depthWrite: false, side: THREE.DoubleSide }));
    water.rotation.x = -Math.PI / 2; scene.add(water);
    scene.add(new THREE.HemisphereLight(0xcdeeff, 0x10232c, 1.6));
    const sun = new THREE.DirectionalLight(0xffefd2, 2.4); sun.position.set(-spanX * 0.35, span * 0.6, -spanZ * 0.3); scene.add(sun);
    ...
    window.__g17ReliefProof = { renderer, scene, camera, spanX, spanZ, span, minY, maxY, render(kind) {
      ...
      camera.lookAt(0, (minY + maxY) * 0.5, 0); renderer.render(scene, camera);
    }};
    window.__g17ReliefProof.render('near');
    return { vertices: positions.length / 3, triangles: indices.length / 3, averageRoughness: roughnessSum / samples.length, diagnosticHeightScale: heightScale, diagnosticVerticalSpan: maxY - minY, worldSpanMeters: { x: spanX, z: spanZ } };
  ...
  async function capture(kind, name) {
    ...
    fs.writeFileSync(path.join(OUT_DIR, name), png); return sha256(png);
  }
  ...
  const metrics = {
    schema: 'westeros-g17-relief-visual-evidence-v2',
    ...
    fullWorld: {
      ...
      contextPolicy: 'qualified G17 SDF/no-overlay owner-map silhouette; Relief ownership is proven by near/far plus numeric G07 parity and real Terrain3D import seams',
    },
  ...
  console.log(`SW_G17_RELIEF_VISUAL_ERROR=${error?.stack || error}`); process.exitCode = 1;
} finally {
