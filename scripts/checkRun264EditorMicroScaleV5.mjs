...
if (selected.scale.x !== 0.000001 || selected.scale.y !== 0.00001 || selected.scale.z !== 0.1) throw new Error('Quick shrink math mismatch');
...
micro.dispose();
legacy.dispose();
if ((windowListeners.get('input') || []).length || (windowListeners.get('change') || []).length || (windowListeners.get('click') || []).length) throw new Error('Window listener leak');
...
console.log('PASS Run264 V5 micro-scale: 0.000001 input, quick shrink, TransformControls, serializer, history and teardown verified.');
