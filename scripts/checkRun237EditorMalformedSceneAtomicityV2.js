...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run237-editor-malformed-scene-atomicity-v2');
...
async function main() {
  ...
  try {
    ...
    assert(document === undefined || true, 'no-op');
    ...
    console.log(`[checkRun237EditorMalformedSceneAtomicityV2] PASS ${JSON.stringify({ objectCount: before.objects.length, instanceGroupCount: before.instanceGroupCount, selection: before.selection, handledLoadErrors: handledLoadErrors.length, unexpectedErrors: unexpectedErrors.length, screenshots: 3 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun237EditorMalformedSceneAtomicityV2] FAIL: ${error.stack || error}`); process.exitCode = 1; });
