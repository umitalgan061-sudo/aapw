...
// Run301 activates the already-proven Run290 Pindex-05 candidate only on the opt-in canonical dev surface.
if (selector.getActiveSource() === 'canonical' && selector.activation?.windowState?.terrainGroup) {
  const { applyPindex05DetailToTerrainGroup } = await import('../experiments/run290/worldReferencePindex05Detail.js');
  const run301Detail = applyPindex05DetailToTerrainGroup(selector.activation.windowState.terrainGroup);
  ...
  status.textContent += ' | pindex05-detail=' + run301Detail.touchedVertices + 'v';
  state.renderer.render(state.scene, state.camera);
}

// The experimental module is outside src/3d by design, so warm it into the canonical-dev cache explicitly.
async function warmRun301Pindex05OfflineSurface() {
...
warmRun301Pindex05OfflineSurface().catch((error) => {
  document.body.dataset.run301Pindex05CacheReady = 'false';
  console.error('[run301Pindex05] cache warm failed', error);
});
