...
for (const marker of requiredMarkers) assert.ok(source.includes(marker), `missing quest/dialogue boundary: ${marker}`);
...
for (const [index, status] of statusOrder.entries()) {
  assert.ok(statusBody.indexOf(status) >= 0, `missing status ${status}`);
  if (index > 0) assert.ok(statusBody.indexOf(status) > statusBody.indexOf(statusOrder[index - 1]), `status order drift before ${status}`);
}

assert.match(source, /if \\(current\\.status === QUEST_STATUS\\.LOCKED \\|\\| current\\.status === QUEST_STATUS\\.COMPLETED\\) continue;/, 'locked/completed quests must not consume dialogue triggers');
assert.match(source, /if \\(!current\\.rewardGranted\\) \\{ current\\.rewardGranted = true; onReward\\(quest\\.reward, quest\\); \\}/, 'quest rewards must remain idempotent');
assert.match(source, /progression\\.snapshot\\(\\)\\.level >= 2 && reputation\\.get\\(INTERACTION_FACTIONS\\.DRAGONSTONE\\) >= 10/, 'mercy resolution must keep level + reputation gate');
