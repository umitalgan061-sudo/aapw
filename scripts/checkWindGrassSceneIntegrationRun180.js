...
async function main() {
	...
	const context = await browser.newContext({
		...
		isMobile: true,
		hasTouch: true,
		userAgent: 'WesterosPWA-Run180GrassIntegration/3.0',
	});
	...
	try {
		await page.goto(`http://127.0.0.1:${port}/scripts/fixtures/run180-scene-harness.html`, { waitUntil: 'domcontentloaded', timeout: 15000 });
		const result = await page.evaluate(async () => {
			...
			fail(mesh.userData.run180FirstFrameSafe === true, 'first-frame marker missing');
			fail(state.grassStats === grass.userData.run180WindGrass, 'grassStats does not reference live metadata');
			...
			const spawn = mapToWorldXZ(PLAYER_CONFIG.SPAWN_MAP_X, PLAYER_CONFIG.SPAWN_MAP_Y, WORLD_SCALE.MAP_BOUNDS, WORLD_SCALE.METERS_PER_MAP_UNIT);
			...
			const expectedCell = { x: Math.round(state.camera.position.x / 120), z: Math.round(state.camera.position.z / 120) };
			...
			fail(recenteredCell.x === expectedCell.x && recenteredCell.z === expectedCell.z, `first render did not recenter grass: ${JSON.stringify({ initialCell, expectedCell, recenteredCell })}`);
			fail(mesh.count > 0 && mesh.count <= 1200, `mobile grass count ${mesh.count} outside 1..1200`);
			grass.visible = false;
			state.renderer.render(state.scene, state.camera);
			const withoutGrass = { calls: state.renderer.info.render.calls, triangles: state.renderer.info.render.triangles };
			grass.visible = true;
			state.renderer.render(state.scene, state.camera);
			const withGrass = { calls: state.renderer.info.render.calls, triangles: state.renderer.info.render.triangles };
			...
			return { initialCell, recenteredCell, patches: mesh.count, trianglesPerPatch, drawCallDelta, triangleDelta };
		});
		assert(consoleErrors.length === 0 && pageErrors.length === 0, `console/page errors: ${JSON.stringify({ consoleErrors, pageErrors })}`);
		console.log(`[checkWindGrassSceneIntegrationRun180] PASS: bootstrap cell (${result.initialCell.x},${result.initialCell.z}) -> player-camera cell (${result.recenteredCell.x},${result.recenteredCell.z}) on first render; ${result.patches} mobile patches are really submitted as +${result.drawCallDelta} draw call / +${result.triangleDelta.toLocaleString('en-US')} triangles (${result.trianglesPerPatch}/patch), teardown PASS.`);
	} finally {
		await context.close();
		await browser.close();
