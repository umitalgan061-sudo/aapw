...
assert.equal(tundra.permanentIce, 0,
  'tundra width fixture must stay outside the always-winter permanent-ice halo');
...
// Begin at the authored always-winter core center. Sampling north of the center and then moving
// south would first approach the core, so a strictly shrinking width would be the wrong contract.
let previous = profileAt(0.145, 0.115);
let maxTopStep = 0;
let maxFullStep = 0;
for (let normalizedY = 0.125; normalizedY <= 0.345; normalizedY += 0.01) {
  ...
  assert(current.topMeters <= previous.topMeters + 1e-9,
    `coastal apron top must not widen while moving south from the always-winter core near normalizedY=${normalizedY.toFixed(3)}`);
  assert(current.fullMeters <= previous.fullMeters + 1e-9,
    `coastal apron full-strength height must not widen while moving south from the always-winter core near normalizedY=${normalizedY.toFixed(3)}`);
  assert(current.fullMeters < current.topMeters,
    `coastal apron must preserve a valid fade interval near normalizedY=${normalizedY.toFixed(3)}`);
  previous = current;
}

assert(maxTopStep < 0.35,
  `coastal apron top must remain continuous between adjacent map-aligned climate samples; max step=${maxTopStep}`);
