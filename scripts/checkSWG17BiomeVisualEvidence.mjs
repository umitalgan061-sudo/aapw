...
const OUT_DIR = path.resolve(OUT_ARG ? OUT_ARG.slice('--out-dir='.length) : 'artifacts/sw-g17-biome-visual');
const CONTEXT_DIR = path.join(OUT_DIR, 'hydrology-context');

function fail(message) {
  throw new Error(`[checkSWG17BiomeVisualEvidence] FAIL: ${message}`);
}
...
// Reuse the already-qualified G17 SDF/no-overlay world context rather than drawing a biome rectangle.
...
fs.writeFileSync(path.join(OUT_DIR, 'g17-biome-full-world-topdown.png'), contextTopdown);

const visualSize = 129;
const samples = [];
let minRoughness = Infinity;
let maxRoughness = -Infinity;
let minR = Infinity; let minG = Infinity; let minB = Infinity;
let maxR = -Infinity; let maxG = -Infinity; let maxB = -Infinity;
for (let y = 0; y < visualSize; y += 1) {
  ...
  for (let x = 0; x < visualSize; x += 1) {
    ...
    requireCondition(sample.marineWeight >= 1 - 1e-12 && sample.landWeight <= 1e-12, 'visual probe leaked terrestrial biome');
    requireCondition(Math.abs(sample.height + 4) <= 1e-12, 'visual probe changed merged hydrology height');
    minRoughness = Math.min(minRoughness, sample.roughness);
    maxRoughness = Math.max(maxRoughness, sample.roughness);
    minR = Math.min(minR, sample.color[0]); minG = Math.min(minG, sample.color[1]); minB = Math.min(minB, sample.color[2]);
    maxR = Math.max(maxR, sample.color[0]); maxG = Math.max(maxG, sample.color[1]); maxB = Math.max(maxB, sample.color[2]);
    samples.push([sample.height, ...sample.color, sample.roughness]);
  }
}
requireCondition(Math.hypot(maxR - minR, maxG - minG, maxB - minB) > 0.01, 'macro marine color field is visually flat');
...
try {
  ...
  const renderMetrics = await page.evaluate(async ({ samples, visualSize, bounds }) => {
    ...
    for (let row = 0; row < visualSize; row += 1) {
      ...
      for (let col = 0; col < visualSize; col += 1) {
        ...
        positions.push(world.x - centerWorld.x, source[0] * 16, world.z - centerWorld.z);
        colors.push(source[1], source[2], source[3]);
    ...
    const water = new THREE.Mesh(
      ...
      new THREE.MeshPhysicalMaterial({ color: 0x397e9a, transparent: true, opacity: 0.34, roughness: 0.2, metalness: 0, depthWrite: false, side: THREE.DoubleSide }),
    );
    water.rotation.x = -Math.PI / 2;
    scene.add(water);
    scene.add(new THREE.HemisphereLight(0xcdeeff, 0x10232c, 1.8));
    const sun = new THREE.DirectionalLight(0xffefd2, 2.15);
    sun.position.set(-spanX * 0.35, span * 0.65, -spanZ * 0.25);
    scene.add(sun);
    scene.fog = new THREE.Fog(0x071722, span * 0.9, span * 2.4);
    const camera = new THREE.PerspectiveCamera(48, 960 / 640, 1, span * 6);
    window.__g17BiomeProof = { renderer, scene, camera, spanX, spanZ, span, render(kind) {
      if (kind === 'near') camera.position.set(-spanX * 0.22, span * 0.13, spanZ * 0.24);
      else camera.position.set(0, span * 0.52, spanZ * 0.5);
      camera.lookAt(0, -55, 0);
      renderer.render(scene, camera);
    }};
    window.__g17BiomeProof.render('near');
    return { vertices: positions.length / 3, triangles: indices.length / 3, averageRoughness: roughnessSum / samples.length, worldSpanMeters: { x: spanX, z: spanZ } };
  }, { samples, visualSize, bounds: G17.normalizedBounds });

  async function capture(kind, name) {
    await page.evaluate((value) => window.__g17BiomeProof.render(value), kind);
  ...
  const nearSha256 = await capture('near', 'g17-biome-near.png');
  const farSha256 = await capture('far', 'g17-biome-far.png');
  requireCondition(pageErrors.length === 0, `browser page errors: ${pageErrors.join(' | ')}`);

  const fullWorldSha256 = sha256(contextTopdown);
  const metrics = {
    schema: 'westeros-g17-biome-visual-evidence-v1',
    sourceMapSha256: '20702972e8f45f0fbdc4da5fa68e890a82e4e822e1d58e2f369d8bc5b9c571a1',
    visualSize,
    colorRange: { min: [minR, minG, minB], max: [maxR, maxG, maxB] },
    ...
    fullWorld: {
      ...
      contextPolicy: 'qualified G17 SDF/no-overlay owner-map diagnostic; biome ownership is proven by near/far and numeric seams',
    },
  ...
  fs.writeFileSync(path.join(OUT_DIR, 'g17-biome-visual-metrics.json'), `${JSON.stringify(metrics, null, 2)}\n`);
  console.log(`SW_G17_BIOME_VISUAL_METRICS=${JSON.stringify(metrics)}`);
  console.log('SW_G17_BIOME_VISUAL_OK');
} catch (error) {
  console.log(`SW_G17_BIOME_VISUAL_ERROR=${error?.stack || error}`);
  process.exitCode = 1;
} finally {
  await browser.close();
