...
for (const token of requiredBootTokens) {
  if (!boot.includes(token)) throw new Error(`Run302 canonical activation missing: ${token}`);
}
...
if (p4 < 0 || p5 <= p4) throw new Error('Run302 must apply Pindex-05 after the established Pindex-04 layer');
...
for (const token of [
  ...
  if (!candidate.includes(token)) throw new Error(`Run302 proven candidate contract drifted: ${token}`);
}
if (candidate.includes('Math.random(')) throw new Error('Run302 candidate must remain deterministic');
if (game3d.includes('worldReferencePindex05Detail') || game3d.includes('run302Pindex05')) {
  throw new Error('Run302 must remain opt-in canonical-dev only; default game3d runtime changed');
}
if (boot.includes("../src/3d/world/worldReferencePindex05Detail.js")) {
  throw new Error('Run302 must not prematurely promote the experimental candidate into src/3d');
}

console.log('[checkRun302Pindex05CanonicalActivation] PASS: proven Pindex-05 candidate activates after Pindex-04 only on canonical-dev and receives explicit offline warming');
