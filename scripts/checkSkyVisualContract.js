	...
	try {
		...
		const result = await page.evaluate(async () => {
			...
			for (let i = 0; i < positions.count; i++) {
				...
				fail(u >= -1e-6 && u <= 1 + 1e-6 && v >= -1e-6 && v <= 1 + 1e-6, `sky uv drift at ${i}`);
			...
			return { vertexCount: positions.count, indexCount: index.count, maxRadiusError, geometryDisposeCount, materialDisposeCount };
		});
		assert(result.vertexCount === 561 && result.indexCount === 2880, 'sky topology mismatch escaped browser contract');
		console.log(`[checkSkyVisualContract] PASS: ${result.vertexCount} vertices, ${result.indexCount} indices, 1900m camera-follow BackSide sphere, aurora/day-night shader contract PASS, disposal ${result.geometryDisposeCount}/${result.materialDisposeCount}.`);
	} finally {
		await browser.close();
