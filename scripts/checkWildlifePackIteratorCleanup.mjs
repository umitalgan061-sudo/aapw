...
let naturalCloses = 0;
const naturalSource = { [Symbol.iterator]() { let index = 0; return { next() { index += 1; return index === 1 ? { done: false, value: { x: 0, z: -2 } } : { done: true }; }, return() { naturalCloses += 1; return { done: true }; } }; } };
assert.doesNotThrow(() => wolf.update(3, undefined, naturalSource));
assert.equal(naturalCloses, 0, 'naturally exhausted pack iterators must not be closed redundantly');
