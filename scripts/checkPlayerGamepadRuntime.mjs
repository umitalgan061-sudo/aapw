...
try{
	...
	await setPads([{index:1}]); await resetMotionHistory(); await waitHistory('motion',(m)=>m.state==='idle'&&m.isGrounded,'post-light idle',5000);
	...
	const dpadRun=await moveThenStop({index:1,buttons:{12:true}},'D-pad forward walk'); const dpadDir=dpadRun.direction; need(dpadDir?.distance>0.5,`D-pad displacement ${dpadDir?.distance ?? 0}`); const dpadDot=beforeDir.x*dpadDir.x+beforeDir.z*dpadDir.z; need(dpadDot>0.97,`D-pad must share camera-relative forward direction; dot=${dpadDot}`); need(Math.abs(dpadRun.moving.speedMps-full.speedMps)<0.25,`D-pad digital walk speed ${dpadRun.moving.speedMps} vs ${full.speedMps}`);
	...
	const metrics={ok:true,baseline:{state:baseline.state,stamina:baseline.stamina},light:{input:lightInput,serial:lightStart.serial,haptic:lightHaptic.options},analog:{partialSpeedMps:partial.speedMps,fullSpeedMps:full.speedMps,ratio:Number(ratio.toFixed(3)),fullDisplacementMeters:Number(beforeDir.distance.toFixed(3))},dpad:{speedMps:dpadRun.moving.speedMps,directionDot:Number(dpadDot.toFixed(3)),displacementMeters:Number(dpadDir.distance.toFixed(3))},camera:{directionDotAfterRightStick:Number(dot.toFixed(3)),postOrbitDisplacementMeters:Number(afterDir.distance.toFixed(3)),triggerZoomExercised:0.35},dodge:{state:dodge.state,speedMps:dodge.speedMps,staminaBefore:dodgeBase.stamina,staminaAfter:dodge.stamina},sprint:{speedMps:sprint.speedMps,stamina:sprint.stamina,state:sprint.state},fallback:{device:fallback,guarding:guard.guarding},heavy:{input:heavyInput,serial:heavyStart.serial,damageScale:heavyStart.damageScale,haptic:heavyHaptic.options},browserErrors:errors};
	fs.writeFileSync(path.join(outDir,'gamepad-runtime.json'),`${JSON.stringify(metrics,null,2)}\n`); console.log(`PLAYER_GAMEPAD_RUNTIME_OK ${JSON.stringify({analogRatio:metrics.analog.ratio,dpadDot:metrics.dpad.directionDot,cameraDot:metrics.camera.directionDotAfterRightStick,dodgeSpeedMps:dodge.speedMps,sprintSpeedMps:sprint.speedMps,errors:errors.length})}`);
