...
const directionBetween=(start,end)=>{const dx=end.position.x-start.position.x,dz=end.position.z-start.position.z,length=Math.hypot(dx,dz);return length>0.01?{x:dx/length,z:dz/length,distance:length}:null;};
...
try{
	...
	await resetMotionHistory(); await setPads([{index:1,axes:[0,-1,0,0],buttons:{10:true}}]); const sprint=await waitHistory('motion',(m)=>m.state==='sprint'&&m.runIntent&&m.speedMps>6&&m.stamina<baseline.stamina,'sprint');
