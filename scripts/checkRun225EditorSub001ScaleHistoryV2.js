...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run225-editor-sub001-scale-history-v2');
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction((expected) => Math.abs(Number(document.getElementById('we-scale-x').value) - expected) < 1e-9, PRECISE_SCALE, { timeout: 30000 });
    const inspectorScaleAfterReselect = await page.evaluate(() => [
    ...
    assert(Math.abs(inspectorScaleAfterReselect[0] - PRECISE_SCALE) < 1e-9, `Inspector lost redone sub-0.01 scale after explicit re-selection: ${JSON.stringify(inspectorScaleAfterReselect)}`);
    assert(inspectorScaleAfterReselect[1] === 1 && inspectorScaleAfterReselect[2] === 1, `Inspector unedited axes drifted after explicit re-selection: ${JSON.stringify(inspectorScaleAfterReselect)}`);

    await page.screenshot({ path: path.join(ARTIFACT_DIR, '02-redo-reselected-sub001-scale.png'), fullPage: true });
    ...
    console.log(`[checkRun225EditorSub001ScaleHistoryV2] PASS ${JSON.stringify({ original, scaled, undoHistory, redoHistory, selectionAfterRestore, inspectorScaleAfterReselect, screenshots: 3 })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun225EditorSub001ScaleHistoryV2] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
