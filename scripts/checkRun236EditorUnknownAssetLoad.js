...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run236-editor-unknown-asset-load');
...
async function main() {
  ...
  try {
    ...
    console.log(`[checkRun236EditorUnknownAssetLoad] PASS ${JSON.stringify({ objectCount: state.objects.length, hierarchyCount: state.hierarchy.length, selection: state.selection, settings: { gridVisible: state.gridVisible, snapEnabled: state.snapEnabled, snapSize: state.snapSize }, screenshots: 2 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun236EditorUnknownAssetLoad] FAIL: ${error.stack || error}`); process.exitCode = 1; });
