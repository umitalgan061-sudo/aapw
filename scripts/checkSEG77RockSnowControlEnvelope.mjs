...
for (let y = 0; y < sampleSize; y += 1) {
  ...
  for (let x = 0; x < sampleSize; x += 1) {
    ...
    checksum = fnv(checksum, Math.round(s.snowWeight * 255));
    if (x > 0) {
      const p = sampleG77RockSnow(lerp(b.xMin, b.xMax, (x - 1) / (sampleSize - 1)), ny);
      maxAdjacentRockStep = Math.max(maxAdjacentRockStep, Math.abs(s.rockBlend - p.rockBlend));
      maxAdjacentSnowStep = Math.max(maxAdjacentSnowStep, Math.abs(s.snowWeight - p.snowWeight));
    }
    if (y > 0) {
      const p = sampleG77RockSnow(nx, lerp(b.yMin, b.yMax, (y - 1) / (sampleSize - 1)));
      maxAdjacentRockStep = Math.max(maxAdjacentRockStep, Math.abs(s.rockBlend - p.rockBlend));
      maxAdjacentSnowStep = Math.max(maxAdjacentSnowStep, Math.abs(s.snowWeight - p.snowWeight));
    }
  }
...
function downsample(source, size) {
  ...
  for (let y = 0; y < nextSize; y += 1) for (let x = 0; x < nextSize; x += 1) out[y * nextSize + x] = source[(y * 2) * size + x * 2];
  return [out, nextSize];
}
...
const metrics = { sampleSize, samples: sampleSize * sampleSize, fractionalRockSamples, fractionalSnowSamples,
  waterLeakSamples, minRock: Number(minRock.toFixed(8)), maxRock: Number(maxRock.toFixed(8)),
  maxAdjacentRockStep: Number(maxAdjacentRockStep.toFixed(8)), maxAdjacentSnowStep: Number(maxAdjacentSnowStep.toFixed(8)),
  maxWestGuardRockDelta: Number(maxWestGuardRockDelta.toFixed(8)), maxNorthGuardRockDelta: Number(maxNorthGuardRockDelta.toFixed(8)),
  maxWestGuardSnowDelta: Number(maxWestGuardSnowDelta.toFixed(8)), maxNorthGuardSnowDelta: Number(maxNorthGuardSnowDelta.toFixed(8)),
  lodSizes, checksum };
