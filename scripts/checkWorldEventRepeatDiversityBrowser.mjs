...
try {
  ...
  const proof = await page.evaluate(async ({ maxIntervalSeconds, eventCount, resumeSpikeSeconds }) => {
    ...
    const runSequence = (seed, nightFactor, { injectResumeSpike = false } = {}) => {
      ...
      const eventName = `repeat-diversity-${seed}-${nightFactor ?? 'legacy'}-${injectResumeSpike ? 'resume' : 'steady'}`;
      const ids = [];
      ...
      while (ids.length < eventCount) {
        ...
        for (let second = 0; second <= maxIntervalSeconds && ids.length === before; second += 1) {
          if (injectResumeSpike && !spikeInjected && ids.length >= Math.floor(eventCount / 3)) {
    ...
    const legacyWithResumeSpike = runSequence(148, undefined, { injectResumeSpike: true });
    const legacyWithResumeSpikeRepeat = runSequence(148, undefined, { injectResumeSpike: true });
