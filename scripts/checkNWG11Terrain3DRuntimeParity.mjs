...
async function emit() {
  ...
  try {
    ...
    fs.mkdirSync('godot/terrain-authoring/.terrain3d-proof', { recursive: true }); const out = { schema: 'westeros-g11-runtime-source-v1', width: W, height: H, normalizedBounds: bounds, ...payload }; fs.writeFileSync(probePath, JSON.stringify(out)); console.log(`NW_G11_PARITY_SOURCE_OK checksum=${checksum(out.heights)}`);
  } finally { await browser.close(); await new Promise(r => server.close(r)); }
}
function verify() {
  const source = JSON.parse(fs.readFileSync(probePath, 'utf8')); const bake = JSON.parse(fs.readFileSync(bakePath, 'utf8'));
  if (bake.schema !== 'westeros-g11-terrain3d-bake-v1') throw new Error('unexpected bake schema'); if (source.width !== bake.width || source.height !== bake.height) throw new Error('dimension mismatch');
  ...
  const mean = sum / source.heights.length; if (max > 0.01 || mean > 0.0025) throw new Error(`parity error max=${max} mean=${mean}`);
