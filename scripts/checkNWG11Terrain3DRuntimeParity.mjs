...
function checksum(values) {
	...
	for (const v of values) { const b = Buffer.allocUnsafe(4); b.writeFloatLE(v); h.update(b); }
	return h.digest('hex');
}
...
async function emit() {
	...
	try {
		...
		const payload = await page.evaluate(async ({ W, H, bounds }) => {
			...
			const sample = createHeightSampler(WORLD_DEFAULTS.WORLD_SEED, undefined, pads);
			const heights = [];
			for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
				const nx = bounds.minX + (bounds.maxX - bounds.minX) * x / (W - 1);
				const ny = bounds.minY + (bounds.maxY - bounds.minY) * y / (H - 1);
				const p = normalizedReferenceToWorldXZ(nx, ny, WORLD_SCALE.MAP_BOUNDS, WORLD_SCALE.METERS_PER_MAP_UNIT);
				heights.push(sample(p.x, p.z));
			}
			return { heights, seed: WORLD_DEFAULTS.WORLD_SEED };
		}, { W, H, bounds });
		fs.mkdirSync('godot/terrain-authoring/.terrain3d-proof', { recursive: true });
		const out = { schema: 'westeros-g11-runtime-source-v1', width: W, height: H, normalizedBounds: bounds, ...payload };
		fs.writeFileSync(probePath, JSON.stringify(out));
		console.log(`NW_G11_PARITY_SOURCE_OK checksum=${checksum(out.heights)}`);
	} finally { await browser.close(); await new Promise(r => server.close(r)); }
}

function verify() {
	const source = JSON.parse(fs.readFileSync(probePath, 'utf8'));
	...
	let max = 0, sum = 0;
	for (let i = 0; i < source.heights.length; i++) { const d = Math.abs(source.heights[i] - bake.heights[i]); max = Math.max(max, d); sum += d; }
	const mean = sum / source.heights.length;
	if (max > 0.01 || mean > 0.0025) throw new Error(`parity error max=${max} mean=${mean}`);
	console.log(`NW_G11_TERRAIN3D_RUNTIME_PARITY_OK max=${max.toFixed(6)} mean=${mean.toFixed(6)} source=${checksum(source.heights)} bake=${checksum(bake.heights)}`);
}
