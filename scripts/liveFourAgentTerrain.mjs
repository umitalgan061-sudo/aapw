/**
 * Browser adoption layer for the latest terrain authored by the four corner agents.
 * Existing owner-map-space authoring/runtime sources stay authoritative; this module only projects
 * them into the live Three.js world and World Editor. GeoCell bounds select available sources,
 * while a two-source-pixel feather avoids a hard square edge against unfinished neighbours.
 */
...
import { sampleG65Terrain3dRuntimeGridNormalized } from './runtime/g65Terrain3dRuntimeGrid.mjs';
...
export const LIVE_FOUR_AGENT_TERRAIN_POLICY = Object.freeze({
  id: 'live-four-agent-owner-map-terrain-2026-08-13-v3',
...
const CELLS = Object.freeze([
  Object.freeze({ id: 'G00', agent: 'Buzul Muhafızı', layer: 'Relief', x0: 0, x1: 1 / 8, y0: 0, y1: 1 / 8, sample: (x, y) => sampleG00ReliefHeight(x, y) }),
  Object.freeze({ id: 'G52', agent: 'Şafak Kartalı', layer: 'Rock/Snow', x0: 5 / 8, x1: 6 / 8, y0: 2 / 8, y1: 3 / 8, sample: (x, y) => sampleG52RockSnow(x, y).heightMeters }),
  Object.freeze({ id: 'G70', agent: 'Şafak Kartalı', layer: 'Hydrology', x0: 7 / 8, x1: 1, y0: 0, y1: 1 / 8, sample: (x, y) => sampleG70Hydrology(x, y).heightMeters }),
  Object.freeze({ id: 'G07', agent: 'Günbatımı Ustası', layer: 'Terrain3D runtime bake', x0: 0, x1: 1 / 8, y0: 7 / 8, y1: 1, sample: (x, y) => sampleG07Terrain3dBakeNormalized(x, y)?.height ?? null }),
  Object.freeze({ id: 'G17', agent: 'Günbatımı Ustası', layer: 'Relief', x0: 1 / 8, x1: 2 / 8, y0: 7 / 8, y1: 1, sample: (x, y) => sampleG17SeabedHeight(x, y).height }),
  Object.freeze({ id: 'G65', agent: 'Kızıl Ufuk', layer: 'Terrain3D runtime grid', x0: 6 / 8, x1: 7 / 8, y0: 5 / 8, y1: 6 / 8, sample: (x, y) => sampleG65Terrain3dRuntimeGridNormalized(x, y)?.height ?? null }),
  Object.freeze({ id: 'G75', agent: 'Kızıl Ufuk', layer: 'Relief', x0: 7 / 8, x1: 1, y0: 5 / 8, y1: 6 / 8, sample: (x, y) => sampleG75ReliefHeight(x, y) }),
]);
...
export function ownerMapNormalizedToLiveWorld(normalizedX, normalizedY) {
  ...
  return Object.freeze({
    x: (normalizedX * MAP_WIDTH - center.x) * WORLD_SCALE.METERS_PER_MAP_UNIT,
    z: (normalizedY * MAP_HEIGHT - center.y) * WORLD_SCALE.METERS_PER_MAP_UNIT,
  });
}
...
export function sampleLiveFourAgentHeight(worldX, worldZ, fallbackHeightMeters) {
  ...
  return Object.freeze({
    heightMeters: fallbackHeightMeters + (authored.liveHeightMeters - fallbackHeightMeters) * authored.weight,
    authored,
  });
}
...
const GAME_INSTALL = Symbol.for('westeros.live-four-agent-terrain.game.v3');
...
export function installLiveFourAgentEditorTerrain(api = window.__WESTEROS_WORLD_EDITOR__) {
  ...
  const mapCenter = ownerMapNormalizedToLiveWorld(0.5, 0.5);
  ...
  let touchedVertices = 0;
  for (let index = 0; index < position.count; index += 1) {
    const worldX = mapCenter.x + position.getX(index);
    const worldZ = mapCenter.z + position.getZ(index);
