...
assert.deepEqual({ forward: idle.forward, strafe: idle.strafe, lookX: idle.lookX, lookY: idle.lookY, cameraZoom: idle.cameraZoom, running: idle.running, guarding: idle.guarding, dodgePressed: idle.dodgePressed }, { forward: 0, strafe: 0, lookX: 0, lookY: 0, cameraZoom: 0, running: false, guarding: false, dodgePressed: false });
...
const first = samplePlayerGamepad(makePad({ buttons: { 0: true, 1: true, 2: true, 3: true } }), { jump: false, dodge: false, light: false, heavy: false }); assert.equal(first.jumpPressed, true); assert.equal(first.dodgePressed, true); assert.equal(first.lightPressed, true); assert.equal(first.heavyPressed, true);
const held = samplePlayerGamepad(makePad({ buttons: { 0: true, 1: true, 2: true, 3: true } }), first.buttons); assert.equal(held.jumpPressed, false); assert.equal(held.dodgePressed, false); assert.equal(held.lightPressed, false); assert.equal(held.heavyPressed, false);
const released = samplePlayerGamepad(makePad(), held.buttons); const dodgeAgain = samplePlayerGamepad(makePad({ axes: [0, -1], buttons: { 1: true } }), released.buttons); assert.equal(dodgeAgain.dodgePressed, true); assert.ok(dodgeAgain.magnitude > 0.99);
...
const unmapped = samplePlayerGamepad(makePad({ mapping: '', axes: [1, -1, 1, -1], buttons: { 0: true, 1: true, 2: true, 3: true, 4: true, 10: true }, values: { 6: 1, 7: 1 } })); assert.equal(unmapped.forward, 0); assert.equal(unmapped.lookX, 0); assert.equal(unmapped.running, false); assert.equal(unmapped.dodgePressed, false); assert.equal(unmapped.lightPressed, false);
...
for (const contract of ["JUMP: 0", "DODGE: 1", "LIGHT: 2", "HEAVY: 3", "GUARD: 4", "ZOOM_OUT: 6", "ZOOM_IN: 7", "SPRINT: 10", "GAMEPAD_CAMERA_MAX_FRAME_SECONDS = 0.3", "Math.min(GAMEPAD_CAMERA_MAX_FRAME_SECONDS, nowSeconds - this._lastPollSeconds)", "dodgePressed: buttons.dodge && !previousButtons.dodge", "gamepad.dodgePressed && gamepad.magnitude > 0", "gamepad.axes?.[2]", "gamepad.axes?.[3]", "pad.mapping === 'standard'", "pulsePlayerGamepadMelee", "dual-rumble", "visibilitychange", "visibility-hidden"]) assert.ok(source.includes(contract), `missing ${contract}`);
