...
for (const inheritedAction of ['constructor', 'toString', '__proto__']) assert.equal(pulsePlayerGamepadAction(hapticPad, inheritedAction), false, `inherited action ${inheritedAction} must fail closed`);
assert.equal(pulsePlayerGamepadAction(hapticPad, new String('light')), false, 'boxed action kind must not coerce into a haptic profile');
assert.equal(calls.length, 4, 'invalid haptic action keys must not reach the actuator');
