...
async function main() {
  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      ...
      return {
        patrolIntent: patrol.intent === 'patrol', patrolMoved,
        observeIntent: observe.intent === 'observe', chaseIntent: chase.intent === 'chase',
        chaseClosedDistance: combatDistance < chaseStartDistance,
        combatIntent: combat.intent === 'combat', combatBlendRaised: combat.combatBlend > 0.5,
        investigateIntent: investigate?.intent === 'investigate',
        investigateObservedWithinBound: investigateSchedulerFrames <= 30, investigateMoved,
        returnedToPatrol: returned, returnIntent: returnSnapshot?.intent === 'patrol',
        groupAlertReleased, boundedTickBudget, finitePosition,
      };
    });
