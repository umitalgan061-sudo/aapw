...
assert(policy.normalContrastMin <= 0.12, 'low-signal zones must strongly suppress the visible 22m normal repeat');
assert(policy.normalContrastMax <= 0.65, 'high-signal zones must not restore the full periodic micro-normal contrast');
...
material.dispose();
console.log('[checkTerrainAperiodicSurface] PASS: shipped fog bootstrap applies stronger aperiodic albedo/roughness breakup and caps periodic micro-normal contrast while preserving geometry/collider authority and the offline shell.');
