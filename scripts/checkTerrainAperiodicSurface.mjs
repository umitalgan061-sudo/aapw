...
assert.equal(policy.offlineShellIntegrated, true);
...
assert(policy.fadeStartMeters >= 600);
assert(policy.maxDistanceMeters > policy.fadeStartMeters);
assert(policy.albedoAmplitude >= 0.15 && policy.albedoAmplitude <= 0.2, 'macro breakup must remain visible in terrain-level shipped captures');
assert(policy.roughnessAmplitude >= 0.08 && policy.roughnessAmplitude <= 0.15);
assert(policy.periodicRoughnessRetention <= 0.25, 'legacy 22m roughness atlas must not dominate shipped near-field shading');
assert(policy.normalContrastMin <= 0.05, 'low-signal zones must nearly suppress the visible 22m normal repeat');
assert(policy.normalContrastMax <= 0.35, 'high-signal zones must not restore the periodic micro-normal grid');
...
assert(shader.fragmentShader.includes('aapwAperiodicRoughness'));
assert(shader.fragmentShader.includes(`mix(aapwAperiodicRoughness, roughnessFactor, ${policy.periodicRoughnessRetention.toFixed(2)})`));
...
assert.equal(installTerrainAperiodicSurface(), false, 'fog bootstrap side effect must install only once');
...
material.dispose();
console.log('[checkTerrainAperiodicSurface] PASS: shipped fog bootstrap applies aperiodic albedo/roughness breakup while strongly suppressing the legacy 22m normal+roughness grid and preserving geometry/collider authority.');
