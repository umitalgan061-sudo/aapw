...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run247-editor-partial-metadata-defaults');
...
async function main() {
  ...
  const fixturePath = path.join(ARTIFACT_DIR, 'partial-editor-metadata.scene.json');
  const expected = {
    id: 'run247-castle-0001',
    name: 'Run247 Partial Metadata Castle',
    asset: 'marker-castle',
    position: [13, 0, -21],
    rotation: [-0.5, 1.125, -1.75],
    scale: [1.25, 0.007, 2.5]
  };
  ...
  try {
    ...
    await page.locator('#we-snap-size').fill('3.75');
    await page.locator('#we-snap-size').dispatchEvent('change');
    const before = await capture();
    assert(before.snapEnabled === false && before.snapSize === 3.75, `Prepared snap state drifted: ${JSON.stringify(before)}`);
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '01-before-partial-metadata-load.png'), fullPage: true });
    ...
    await page.waitForFunction(() => (
      ...
      Number(document.getElementById('we-snap-size')?.value) === 1
    ), null, { timeout: 30000 });
    ...
    assert(after.snapEnabled === true && after.snapSize === 1, `Missing snap fields did not compose defaults: ${JSON.stringify(after)}`);
    assert(after.gridChecked === false && after.gridDisabled === true && after.gridVisible === false, `Explicit gridVisible=false or owner grid policy drifted: ${JSON.stringify(after)}`);
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '02-after-partial-metadata-load.png'), fullPage: true });

    console.log(`[checkRun247EditorPartialMetadataDefaults] PASS ${JSON.stringify({ object: after.object, snapEnabled: after.snapEnabled, snapSize: after.snapSize, gridVisible: after.gridVisible, unexpectedErrors: errors.length, screenshots: 2 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun247EditorPartialMetadataDefaults] FAIL: ${error.stack || error}`); process.exitCode = 1; });
