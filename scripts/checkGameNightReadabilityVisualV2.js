...
const OUT = path.join(ROOT, 'artifacts', 'run216-game-night-readability-v2');
...
async function main() {
  ...
  if (!playwright) {
    console.error('[checkGameNightReadabilityVisualV2] SKIP: Playwright is not available.');
    process.exit(2);
  }
  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      ...
      const readability = lights.hemisphere.children.find((child) => (
        child.userData?.gameNightReadability === true
      ));
      if (!readability?.isHemisphereLight) {
        throw new Error('Gameplay night readability fill light was not created.');
      }
      ...
      const midnightSunIntensity = lights.sun.intensity;
      const midnightHemisphereIntensity = lights.hemisphere.intensity;
      ...
      const noonSunIntensity = lights.sun.intensity;
      const noonHemisphereIntensity = lights.hemisphere.intensity;
      const noonFillIntensity = readability.intensity;

      const snapshot = {
        midnightNightFactor: midnight.nightFactor,
        midnightSunIntensity,
        midnightHemisphereIntensity,
        midnightFillIntensity,
        noonNightFactor: noon.nightFactor,
        noonSunIntensity,
        noonHemisphereIntensity,
        noonFillIntensity,
        readableLuma,
        legacyDarkLuma,
    ...
    assert(Math.abs(result.midnightSunIntensity - 0.05) < 1e-9, `Canonical midnight sun drifted: ${result.midnightSunIntensity}`);
    assert(Math.abs(result.midnightHemisphereIntensity - 0.25) < 1e-9, `Canonical midnight hemisphere drifted: ${result.midnightHemisphereIntensity}`);
    ...
    assert(Math.abs(result.noonSunIntensity - 1.4) < 1e-9, `Canonical noon sun drifted: ${result.noonSunIntensity}`);
    assert(Math.abs(result.noonHemisphereIntensity - 1.1) < 1e-9, `Canonical noon hemisphere drifted: ${result.noonHemisphereIntensity}`);
    ...
    fs.writeFileSync(
      path.join(OUT, 'gameplay-midnight-readability.png'),
      Buffer.from(png, 'base64')
    );

    console.log(
      `[checkGameNightReadabilityVisualV2] PASS: midnight fill ${result.midnightFillIntensity.toFixed(2)}, ` +
      `noon fill ${result.noonFillIntensity.toFixed(2)}, synthetic gameplay luminance ` +
      `${result.legacyDarkLuma.toFixed(2)} -> ${result.readableLuma.toFixed(2)} ` +
      `(${result.readabilityGain.toFixed(2)}x), canonical day/night factors and key lights unchanged.`
    );
  } finally {
    await page.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkGameNightReadabilityVisualV2] FAIL: ${error?.stack || error}`);
  process.exit(1);
});
