...
requirePattern(/vSwellSlope\s*\*\s*swellShadingFade\s*\*\s*6\.5\s*\+\s*rippleSlope\(\s*vWorldPosition\.xz\s*,\s*uTime\s*\)\s*\*\s*rippleFade/, 'water normal must retain the depth-safe long-swell geometry while amplifying only its reflection response');
