...
requirePattern(/const\s+farMaterial\s*=\s*material\.clone\(\)\s*;\s*farMaterial\.depthWrite\s*=\s*false\s*;/, 'far-water layer must not write depth beneath the dense near mesh');
requirePattern(/farWater\.renderOrder\s*=\s*-1\s*;/, 'far-water layer must render before the dense near mesh');
