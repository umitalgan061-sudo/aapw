 ...
 * Validates the existing desktop InstancedMesh structure, deterministic transforms/materials and
 * teardown without changing runtime behavior.
 */
...
async function main() {
	...
	try {
		...
		const result = await page.evaluate(async () => {
			...
			for (let meshIndex = 0; meshIndex < 4; meshIndex++) {
				...
				for (let instanceIndex = 0; instanceIndex < mesh.count; instanceIndex++) {
					...
					for (let elementIndex = 0; elementIndex < 16; elementIndex++) {
						...
						fail(Number.isFinite(a) && close(a, b, 1e-8), `child ${meshIndex} deterministic matrix drift`);
					}
				}
			...
			fail(first.group.children[0].count === first.group.children[1].count, 'pine trunk/foliage count mismatch');
			fail(first.group.children[2].count === first.group.children[3].count, 'round trunk/foliage count mismatch');
		...
		console.log(`[checkVegetationVisualContract] PASS: ${result.placedCount} deterministic trees, mesh counts ${result.perMeshCounts.join('/')}, 4 InstancedMeshes, disposal ${result.geometryDisposeCount}/${result.materialDisposeCount}.`);
	} finally {
		await browser.close();
