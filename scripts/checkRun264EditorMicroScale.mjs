...
if (Math.abs(selected.scale.x - 0.000001) > 1e-12) throw new Error(`1e-6 input was not preserved: ${selected.scale.x}`);
if (scaleX.value !== '0.000001') throw new Error(`Inspector rounded 1e-6: ${scaleX.value}`);

if (!dispatchWindow('change', scaleY, '0')) throw new Error('Run264 change capture did not own scale event');
if (Math.abs(selected.scale.y - 0.000001) > 1e-12) throw new Error(`zero was not clamped to 1e-6: ${selected.scale.y}`);
if (scaleY.value !== '0.000001') throw new Error(`clamped display lost precision: ${scaleY.value}`);
...
if (selected.scale.z !== previousZ) throw new Error('invalid change mutated object');
if (scaleZ.value !== previousZ.toFixed(6)) throw new Error(`invalid change did not restore precision: ${scaleZ.value}`);
...
if (Math.abs(selected.scale.x - 0.000001) > 1e-12) throw new Error(`quick shrink x floor mismatch: ${selected.scale.x}`);
if (Math.abs(selected.scale.y - 0.00001) > 1e-12) throw new Error(`quick shrink y mismatch: ${selected.scale.y}`);
if (Math.abs(selected.scale.z - 0.1) > 1e-12) throw new Error(`quick shrink z mismatch: ${selected.scale.z}`);
...
if (scaleZ.value !== '0.000001') throw new Error(`TransformControls display rounded 1e-6: ${scaleZ.value}`);
...
if (Math.abs(persistedScale[0] - 0.000001) > 1e-12 || Math.abs(persistedScale[2] - 0.000001) > 1e-12) {
...
if (hierarchyRefreshes < 3) throw new Error(`expected hierarchy refreshes after valid scale mutations: ${hierarchyRefreshes}`);
if (historyCaptures < 3) throw new Error(`expected history captures after valid scale mutations: ${historyCaptures}`);

micro.dispose();
boundsGuard?.dispose?.();
legacy.dispose();
if ((windowListeners.get('input') || []).length !== 0) throw new Error('input listener leaked after dispose');
if ((windowListeners.get('change') || []).length !== 0) throw new Error('change listener leaked after dispose');
if ((windowListeners.get('click') || []).length !== 0) throw new Error('click listener leaked after dispose');
if ((transformListeners.get('objectChange') || []).length !== 0) throw new Error('TransformControls listener leaked after dispose');
