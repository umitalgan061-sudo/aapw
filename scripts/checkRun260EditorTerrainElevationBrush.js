...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run260-editor-terrain-elevation-brush');
...
async function main() {
  ...
  async function dispatchStroke(stroke) {
    await page.mouse.move(stroke.clientX, stroke.clientY);
  ...
  try {
    ...
    console.log(`[checkRun260EditorTerrainElevationBrush] PASS ${JSON.stringify({ waterOffset: water.state.offset, landOffset: land.state.offset, strengthMeters: 2, screenshots: 3, unexpectedErrors: errors.length })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun260EditorTerrainElevationBrush] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
