/**
 * Kızıl Ufuk / SE GeoCell G77 hydrology authoring refinement.
 *
 ...
 * mask-cell centre.
 *
 * This first SE unit is deliberately authoring/QA-only. It does not live under
 * src/3d and therefore cannot become an accidental runtime/PWA dependency before
 * a later isolated visual-integration pass proves it safe.
 */
...
export function measureG77Hydrology() {
  ...
  for (let y = 0; y < cellsY; y += 1) {
    for (let x = 0; x < cellsX; x += 1) {
      ...
      if (sampleG77WaterConfidence(nx, ny) !== water) centreMismatches += 1;
    }
  }
