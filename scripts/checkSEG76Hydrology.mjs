...
assert.equal(metrics.waterCells, 14, 'G76 canonical water-centre inventory changed');
assert.equal(metrics.landCells, 82, 'G76 canonical land-centre inventory changed');
assert.equal(metrics.boundaryEdges, 12, 'G76 canonical coastline topology changed');
...
assert.equal(metrics.fractionalSamples, 180, 'G76 fractional coastline fingerprint changed');
assert.equal(metrics.hardCellMaxStep, 1);
assert.equal(metrics.maxAdjacentStep, 0.25, 'G76 refined coastline step changed');
assert.equal(metrics.confidenceChecksum, 2873220698, 'G76 confidence checksum changed');
