...
try {
	...
	// First Shift tap begins sprint. The second tap inside the controller's bounded window becomes dodge.
	...
	const duringDodge = await page.evaluate(() => ({
		latest: window.__playerMotionFrames.at(-1),
		frames: window.__playerMotionFrames.slice(),
	}));
	...
	// Keep Shift held after the dodge. Once the dodge window closes, the same real input resumes sprint.
	...
	const report = {
		schema: 'aapw-player-sprint-dodge-runtime-v1',
		baseline,
		dodge: dodgeFrame,
		sprintStart,
		sprintEnd,
