...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run226-editor-signed-rotation-history');
const PRECISE_DEGREES = -179.9;
const PRECISE_RADIANS = PRECISE_DEGREES * Math.PI / 180;
...
async function main() {
  ...
  try {
    ...
    }, PRECISE_RADIANS, { timeout: 30000 });
    await page.waitForTimeout(220);
    await page.evaluate(() => window.__WESTEROS_EDITOR_HISTORY__.captureNow());

    const rotated = await readFirstObject();
    assert(Math.abs(rotated.rotation[1] - PRECISE_RADIANS) < 1e-12, `Signed near-boundary rotation was not applied exactly: ${JSON.stringify(rotated)}`);
    ...
    ), PRECISE_RADIANS, { timeout: 120000 });
    ...
    assert(JSON.stringify(afterRedo) === JSON.stringify(rotated), `Redo did not restore signed near-boundary rotation exactly: ${JSON.stringify({ rotated, afterRedo })}`);
    ...
    console.log(`[checkRun226EditorSignedRotationHistory] PASS ${JSON.stringify({ original, rotated, undoHistory, redoHistory, selectionAfterRestore, inspectorAfterReselect, screenshots: 3 })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun226EditorSignedRotationHistory] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
