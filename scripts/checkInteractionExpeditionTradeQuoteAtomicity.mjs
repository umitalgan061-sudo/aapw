...
let failedServiceNestedPurchase;
let failedNestedGrantCalls = 0;
const failedService = smithingEconomy.purchase(whetstoneOffer, () => {
  ...
  failedServiceNestedPurchase = smithingEconomy.purchase(rationOffer, () => { failedNestedGrantCalls += 1; return true; });
...
assert.equal(failedServiceNestedPurchase.reason, 'purchase-in-progress', 'failed service callbacks must not stage a nested purchase against pre-commit stock');
assert.equal(failedNestedGrantCalls, 0, 'failed nested purchases must reject before invoking their inventory grant callback');
...
let successCredit;
let successRestore;
let successNestedPurchase;
let successNestedGrantCalls = 0;
const guardedSuccess = guardedSuccessEconomy.purchase(whetstoneOffer, () => {
  ...
  successNestedPurchase = guardedSuccessEconomy.purchase(rationOffer, () => { successNestedGrantCalls += 1; return true; });
  return true;
});
...
assert.equal(successNestedPurchase.reason, 'purchase-in-progress', 'successful service callbacks must not consume another offer before the outer purchase commits');
assert.equal(successNestedGrantCalls, 0, 'successful nested purchases must reject before invoking their inventory grant callback');
