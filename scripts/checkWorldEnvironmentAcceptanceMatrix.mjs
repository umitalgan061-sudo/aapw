...
try {
  ...
  const boot = await page.evaluate(async ({ width, height }) => {
    ...
    const normalTextures = terrainMeshes.map((mesh) => mesh.material?.normalMap).filter(Boolean);
    const roughnessTextures = terrainMeshes.map((mesh) => mesh.material?.roughnessMap).filter(Boolean);
    const world = {
    ...
    return {
      ...
      terrainMicroPolicy: TERRAIN_MICRO_SURFACE_POLICY.id,
      terrainMicroRepeatMeters: TERRAIN_MICRO_SURFACE_POLICY.detailRepeatMeters,
      ...
      microSurfaceViolations: terrainMeshes.filter((mesh) => (
        mesh.userData.currentTerrainMicroSurface?.policyId !== TERRAIN_MICRO_SURFACE_POLICY.id
        || mesh.userData.currentTerrainMicroSurface?.uvChannel !== 1
        || !mesh.geometry?.getAttribute?.('uv1')
        || mesh.material?.normalMap?.channel !== 1
        || mesh.material?.roughnessMap?.channel !== 1
        || mesh.material?.normalMap?.userData?.terrainMicroSurfacePolicy !== TERRAIN_MICRO_SURFACE_POLICY.id
        || mesh.material?.roughnessMap?.userData?.terrainMicroSurfacePolicy !== TERRAIN_MICRO_SURFACE_POLICY.id
      )).length,
      ...
      sharedNormalTextureCount: new Set(normalTextures.map((texture) => texture.uuid)).size,
      sharedRoughnessTextureCount: new Set(roughnessTextures.map((texture) => texture.uuid)).size,
      world,
    };
  ...
  assert(boot.microSurfaceViolations === 0, `world-space terrain micro-surface violations: ${boot.microSurfaceViolations}`);
