...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run244-editor-explicit-snap-enabled-load');
...
async function main() {
  ...
  const fixturePath = path.join(ARTIFACT_DIR, 'explicit-snap-enabled.scene.json');
  const expected = {
    id: 'run244-tree-0001',
    name: 'Run244 Snap Enabled Tree',
    asset: 'marker-tree',
    position: [13.125, 0, -21.875],
    rotation: [0.375, -1.25, 2.625],
    scale: [0.007, 1.375, 2.25]
  };
  fs.writeFileSync(fixturePath, JSON.stringify({
    schemaVersion: 1,
    world: { name: 'Westeros', coordinateSystem: 'threejs-y-up', units: 'meters' },
    editor: { gridVisible: true, snapEnabled: true, snapSize: 2.5 },
    objects: [{
      id: expected.id,
      name: expected.name,
  ...
  try {
    ...
    await page.locator('#we-snap-size').fill('0.25');
    await page.locator('#we-snap-size').dispatchEvent('change');
    const before = await capture();
    assert(before.snapEnabled === false && before.snapSize === 0.25, `Prepared snap state drifted: ${JSON.stringify(before)}`);
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '01-before-explicit-snap-enabled-load.png'), fullPage: true });
    ...
    assert(after.snapEnabled === true && after.snapSize === 2.5, `Explicit snap-enabled metadata drifted: ${JSON.stringify(after)}`);
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '02-after-explicit-snap-enabled-load.png'), fullPage: true });

    console.log(`[checkRun244EditorExplicitSnapEnabledLoad] PASS ${JSON.stringify({ object: after.object, snapEnabled: after.snapEnabled, snapSize: after.snapSize, gridVisible: after.gridVisible, unexpectedErrors: errors.length, screenshots: 2 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun244EditorExplicitSnapEnabledLoad] FAIL: ${error.stack || error}`); process.exitCode = 1; });
