...
try {
	pads = [makePad({ index: 0, mapping: '', axes: [1, -1, 1, -1], buttons: { 0: true, 1: true, 2: true, 3: true, 4: true, 5: true, 10: true } })];
	...
	pads = [makePad({ index: 4, mapping: '', axes: [1, -1], buttons: { 1: true, 2: true, 3: true, 5: true } })]; const lost = input.getAxes(); assert.equal(lost.forward, 0); assert.equal(lost.strafe, 0); assert.equal(deviceEvents().at(-1)?.detail.device, 'keyboard-pointer');
	...
	pads = [makePad({ index: 0, axes: [0, -0.71], buttons: { 10: true } })]; const hysteresisHold = input.getAxes(); assert.equal(hysteresisHold.running, true, 'active sprint must remain stable below fresh-start threshold');
	pads = [makePad({ index: 0, axes: [0, -0.60], buttons: { 10: true } })]; const hysteresisRelease = input.getAxes(); assert.equal(hysteresisRelease.running, false, 'sprint must release below hysteresis floor');
	pads = [makePad({ index: 0, axes: [0, -0.71], buttons: { 10: true } })]; const noRestart = input.getAxes(); assert.equal(noRestart.running, false, 'mid-band stick must not restart sprint after release');
	pads = [makePad({ index: 0, axes: [0, -0.90], buttons: { 10: true } })]; assert.equal(input.getAxes().running, true, 'strong re-input must restart sprint'); pads = [makePad({ index: 0, axes: [0, -0.90] })]; assert.equal(input.getAxes().running, false, 'releasing L3 must end sprint immediately');
