...
try {
	...
	assert.equal(combatEvents().length, 0); assert.equal(deviceEvents().length, 0);
	pads = [makePad({ index: 1, axes: [0.4, -0.8], buttons: { 2: true, 4: true, 10: true } })];
	const first = input.getAxes(); assert.ok(first.forward > 0.6 && first.strafe > 0.25); assert.equal(first.running, true); assert.equal(first.guarding, true); assert.equal(combatEvents().length, 0);
	pads = [makePad({ index: 1, axes: [0.4, -0.8] })]; input.getAxes();
	pads = [makePad({ index: 1, axes: [0.4, -0.8], buttons: { 2: true } })]; input.getAxes(); assert.equal(combatEvents('light').length, 1); input.getAxes(); assert.equal(combatEvents('light').length, 1);
	pads = [makePad({ index: 0, axes: [-1, 0], buttons: { 3: true } }), makePad({ index: 1, axes: [0.4, -0.8], buttons: { 2: true } })];
	const sticky = input.getAxes(); assert.ok(sticky.forward > 0.6 && sticky.strafe > 0); assert.equal(combatEvents('heavy').length, 0); assert.equal(deviceEvents().filter((e) => e.detail?.gamepadIndex === 0).length, 0);
	pads = [makePad({ index: 0, axes: [-1, 0], buttons: { 3: true, 4: true } }), makePad({ index: 1, connected: false })];
	const handoff = input.getAxes(); assert.ok(handoff.strafe < -0.95); assert.equal(handoff.guarding, true); assert.equal(combatEvents('heavy').length, 0); assert.equal(deviceEvents().at(-1)?.detail.gamepadIndex, 0);
