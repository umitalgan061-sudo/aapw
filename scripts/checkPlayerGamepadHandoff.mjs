...
try {
	pads = [makePad({ index: 0, mapping: '', axes: [1, -1, 1, -1], buttons: { 0: true, 1: true, 2: true, 3: true, 4: true, 10: true } })];
	...
	pads = [makePad({ index: 4, mapping: '', axes: [1, -1], buttons: { 1: true, 2: true, 3: true } })]; const lost = input.getAxes(); assert.equal(lost.forward, 0); assert.equal(lost.strafe, 0); assert.equal(deviceEvents().at(-1)?.detail.device, 'keyboard-pointer');
	pads = [makePad({ index: 0, axes: [0, -1], buttons: { 0: true, 1: true, 2: true, 3: true } })]; const reconnectHeld = input.getAxes(); assert.equal(reconnectHeld.jumpRequested, false); assert.equal(reconnectHeld.running, false, 'held B during reconnect must be seeded rather than synthesize dodge'); assert.equal(combatEvents('light').length, 1); assert.equal(combatEvents('heavy').length, 1);
