...
assert.equal(classifyFaunaLod(1800).level, 'far');
...
const habitatEval = evaluateFaunaHabitat(
  habitat(),
  'deer',
  { hour: 12, day: 3, season: 'summer', weather: 'clear' },
  { species: 'deer', habitatId: 'forest-01', count: 2, capacity: 8 },
);
...
const blockedHabitat = evaluateFaunaHabitat(
  habitat({ canonicalBiome: 'ocean', biome: 'ocean', score: 1, waterDepth: 2 }),
  'deer',
  { hour: 12 },
);
assert.equal(blockedHabitat.eligible, false);

const steepHabitat = evaluateFaunaHabitat(
  habitat({ slope: 0.9 }),
  'horse',
  { hour: 12 },
);
...
const badNavHabitat = evaluateFaunaHabitat(
  habitat({ navReachable: false }),
  'deer',
  { hour: 12 },
);
assert.equal(badNavHabitat.eligible, false);

const badGroundHabitat = evaluateFaunaHabitat(
  habitat({ groundValid: false }),
  'deer',
  { hour: 12 },
);
assert.equal(badGroundHabitat.eligible, false);

const badWaterHabitat = evaluateFaunaHabitat(
  habitat({ waterValid: false }),
  'deer',
  { hour: 12 },
);
...
assert.ok(stormWolf.score >= 0);
assert.ok(stormWolf.score <= 1);

const basePlanInput = {
  worldSeed: 'aapw-fauna-ecology',
  now: 3600 * 12,
  clock: { hour: 12, day: 3, season: 'summer', weather: 'clear' },
  habitats: [habitat()],
...
assert.ok(Array.isArray(plan.spawn));
assert.ok(Array.isArray(plan.updates));
assert.ok(Array.isArray(plan.commands));
assert.ok(Array.isArray(plan.events));
assert.ok(Array.isArray(plan.groups));
assert.ok(Array.isArray(plan.schedule));
assert.ok(Array.isArray(plan.despawn));
...
assert.ok(typeof plan.fingerprint === 'string');
...
for (const directive of plan.spawn) {
  ...
  assertFrozen(assetAudit, 'asset audit');
...
const staleThreatPlan = planFaunaEcologyTick({
  ...basePlanInput,
  threats: [threat({ ageSeconds: LIVING_WORLD_FAUNA_ECOLOGY_POLICY.activeThreatSeconds + 10 })],
});
assert.equal(staleThreatPlan.updates.some((item) => item.state === 'flee'), false);

const audibleThreatPlan = planFaunaEcologyTick({
  ...basePlanInput,
  threats: [threat({ visible: false, heard: true, distanceMeters: 25 })],
});
...
const farThreatPlan = planFaunaEcologyTick({
  ...basePlanInput,
  threats: [threat({ distanceMeters: 250, visible: true })],
});
assert.equal(farThreatPlan.updates[0].state, 'roam');

const invisibleThreatPlan = planFaunaEcologyTick({
  ...basePlanInput,
  threats: [threat({ visible: false, heard: false, distanceMeters: 5 })],
});
assert.equal(invisibleThreatPlan.updates[0].state, 'roam');

const lowConfidenceThreatPlan = planFaunaEcologyTick({
  ...basePlanInput,
  threats: [threat({ confidence: 0.1, distanceMeters: 10 })],
});
assert.ok(['flee', 'roam'].includes(lowConfidenceThreatPlan.updates[0].state));

const group = planFaunaGroupCommand({
  groupId: 'pack-01',
  members: [candidate({ id: 'wolf-02' }), candidate({ id: 'wolf-01' }), candidate({ id: 'wolf-03' })],
  state: 'flee',
  target: { x: 55, z: 144 },
});
...
const placementManifest = buildFaunaPlacementManifest({
  species: 'horse',
  habitat: habitat({ biome: 'meadow', position: { x: 55, z: 88 } }),
});
...
const despawnDead = planFaunaDespawnSweep({
  now: 500,
  candidates: [candidate({ id: 'dead-wolf', health: 0, distanceMeters: 50 })],
});
assert.equal(despawnDead.despawn.length, 1);
assert.equal(despawnDead.despawn[0].reason, 'dead-owner-requested');

const despawnFar = planFaunaDespawnSweep({
  now: 500,
  candidates: [candidate({ id: 'far-wolf', health: 1, distanceMeters: 5000, spawnTime: 1 })],
});
assert.equal(despawnFar.despawn.length, 1);
assert.equal(despawnFar.despawn[0].reason, 'outside-world-interest');

const protectedFar = planFaunaDespawnSweep({
  now: 500,
  candidates: [candidate({ id: 'protected-wolf', protected: true, distanceMeters: 5000, spawnTime: 1 })],
});
assert.equal(protectedFar.despawn.length, 0);

const activeButFresh = planFaunaDespawnSweep({
  now: 20,
  candidates: [candidate({ id: 'fresh-wolf', health: 1, distanceMeters: 5000, spawnTime: 10 })],
});
assert.equal(activeButFresh.despawn.length, 0);

const notActive = planFaunaDespawnSweep({
  now: 500,
  candidates: [candidate({ id: 'inactive-wolf', active: false, distanceMeters: 5000, spawnTime: 1 })],
});
...
const malformedPlan = planFaunaEcologyTick({
  worldSeed: null,
  now: Infinity,
  clock: { hour: NaN, day: Infinity, season: null, weather: null },
  habitats: [
    habitat({ id: null, position: { x: NaN, z: Infinity } }),
    habitat({ id: 'bad-score', score: NaN, occupancy: Infinity, position: null }),
  ],
  candidates: [
    candidate({ id: null, position: { x: NaN, z: NaN }, distanceMeters: NaN }),
  ],
  threats: [
    threat({ id: null, distanceMeters: Infinity, ageSeconds: NaN, position: { x: NaN, z: NaN } }),
  ],
  population: [
    { species: null, habitatId: null, capacity: NaN, count: Infinity, juveniles: NaN },
  ],
});
...
const replayTape = [
  { ...basePlanInput, now: 0 },
  { ...basePlanInput, now: 5 },
  { ...basePlanInput, now: 10 },
  { ...basePlanInput, now: 15 },
  { ...basePlanInput, now: 20 },
];
...
const reorderedInput = {
  ...
  population: [
    { species: 'wolf', habitatId: 'forest-02', count: 1, juveniles: 0, adults: 1 },
    { species: 'wolf', habitatId: 'forest-01', count: 2, juveniles: 0, adults: 2 },
  ],
};
const reorderedA = planFaunaEcologyTick(reorderedInput);
const reorderedB = planFaunaEcologyTick({
  ...reorderedInput,
  habitats: [...reorderedInput.habitats].reverse(),
  candidates: [...reorderedInput.candidates].reverse(),
  threats: [...reorderedInput.threats].reverse(),
  population: [...reorderedInput.population].reverse(),
});
assert.deepEqual(reorderedA, reorderedB);

const refresh = planPopulationRefresh({
  worldSeed: 'population-test',
  now: 900,
  clock: { hour: 7, day: 4, season: 'spring', weather: 'rain' },
  habitats: [
    habitat(),
    habitat({ id: 'taiga-01', biome: 'taiga', position: { x: 600, z: 700 }, waterDepth: 0.1, slope: 0.2 }),
    habitat({ id: 'meadow-01', biome: 'meadow', position: { x: 800, z: 300 }, waterDepth: 0.05, slope: 0.1 }),
    habitat({ id: 'mountain-01', biome: 'mountain', position: { x: 900, z: 1200 }, waterDepth: 0.1, slope: 0.4 }),
  ],
  population: [
    { species: 'deer', habitatId: 'forest-01', count: 2, juveniles: 1, adults: 1 },
    { species: 'wolf', habitatId: 'taiga-01', count: 2, juveniles: 0, adults: 2 },
  ],
});
assertFrozen(refresh, 'population refresh');
assert.equal(refresh.kind, 'population-refresh');
assert.ok(refresh.rows.length > 0);
assert.ok(refresh.rows.length <= LIVING_WORLD_FAUNA_ECOLOGY_POLICY.maxPopulationRows);
...
assert.ok(springRows[0].juveniles >= 0);
assert.ok(springRows[0].target >= 1);

const customProfile = getFaunaSpeciesProfile('otter', {
  otter: {
    habitat: ['river', 'lake'],
    active: ['dawn', 'day'],
    rest: ['night'],
    ...
    groupMax: 4,
    assetFamily: 'animals',
    sourceCandidates: ['assets/models/animals/otter.glb'],
  },
});
...
const customPlan = planFaunaEcologyTick({
  ...
  candidates: [candidate({ id: 'otter-01', species: 'otter' })],
  threats: [],
  speciesTable: {
    otter: {
      habitat: ['meadow'],
...
const applied = applyFaunaEcologyTick(plan, {
  dispatchCommand: (command) => {
    appliedCalls.push(`command:${command.id}`);
    return command.id;
  },
  updateActor: (payload) => {
    appliedCalls.push(`update:${payload.actorId || payload.id || 'anonymous'}`);
    return payload.id || payload.actorId;
  },
  spawnGroup: (directive) => {
    appliedCalls.push(`spawn:${directive.id}`);
    return directive.id;
  },
  despawn: (payload) => {
    appliedCalls.push(`despawn:${payload.id}`);
    return payload.id;
  },
}, 120);
...
const failingApply = applyFaunaEcologyTick(plan, {
  dispatchCommand: () => { throw new Error('dispatch-failure'); },
  updateActor: () => { throw new Error('update-failure'); },
  spawnGroup: () => { throw new Error('spawn-failure'); },
}, 121);
assert.equal(failingApply.ok, false);
assert.ok(failingApply.errors.length >= 1);
assertFrozen(failingApply.errors, 'apply errors');
...
const emptyGroup = planFaunaGroupCommand({ groupId: 'empty', members: [] });
assert.equal(emptyGroup.members.length, 0);
assert.equal(emptyGroup.command.type, 'roam');

const emptyPlacement = buildFaunaPlacementManifest({ species: 'unknown' });
assert.equal(emptyPlacement.ground, true);
assert.equal(emptyPlacement.navigation, true);
assert.equal(emptyPlacement.position, null);
assert.equal(emptyPlacement.asset.materialContract, 'MaterialAssignmentCore');
...
assert.equal(new Set(scenarioOutputs.map((item) => item.fingerprint)).size, scenarioOutputs.length);
...
const invalidPlan = {
  ...plan,
  spawn: [{ id: 'bad', placement: { groundRequired: false, navRequired: false }, materialContract: 'wrong', placementContract: 'wrong' }],
  governance: { ...plan.governance, materialOwner: 'wrong', placementOwner: 'wrong', editorRuntimeImport: true },
};
...
const candidateStateMatrix = [
  candidate({ id: 'near-roam', species: 'deer', lod: 'near', distanceMeters: 40 }),
  candidate({ id: 'distant-roam', species: 'deer', lod: 'distant', distanceMeters: 250 }),
  candidate({ id: 'far-roam', species: 'deer', lod: 'far', distanceMeters: 1800 }),
  candidate({ id: 'culled-roam', species: 'deer', lod: 'culled', distanceMeters: 5000 }),
  candidate({ id: 'near-wolf', species: 'wolf', lod: 'near', distanceMeters: 30 }),
  candidate({ id: 'distant-wolf', species: 'wolf', lod: 'distant', distanceMeters: 300 }),
  candidate({ id: 'far-wolf', species: 'wolf', lod: 'far', distanceMeters: 2000 }),
  candidate({ id: 'near-horse', species: 'horse', lod: 'near', distanceMeters: 50 }),
  candidate({ id: 'distant-horse', species: 'horse', lod: 'distant', distanceMeters: 400 }),
  candidate({ id: 'far-horse', species: 'horse', lod: 'far', distanceMeters: 1700 }),
  candidate({ id: 'near-dragon', species: 'dragon', lod: 'near', distanceMeters: 70 }),
  candidate({ id: 'distant-dragon', species: 'dragon', lod: 'distant', distanceMeters: 350 }),
  candidate({ id: 'far-dragon', species: 'dragon', lod: 'far', distanceMeters: 2100 }),
];

for (const row of candidateStateMatrix) {
  const state = planFaunaEcologyTick({
    ...basePlanInput,
    candidates: [row],
    threats: row.species === 'dragon' ? [threat()] : [],
  });
...
const hourMatrix = [
  ...
  { hour: 1, expected: 'night' },
  { hour: 4.99, expected: 'night' },
  { hour: 5, expected: 'dawn' },
  { hour: 5.5, expected: 'dawn' },
  { hour: 7.99, expected: 'dawn' },
  { hour: 8, expected: 'day' },
  { hour: 12, expected: 'day' },
  { hour: 17.99, expected: 'day' },
  { hour: 18, expected: 'dusk' },
  { hour: 19, expected: 'dusk' },
  { hour: 20.99, expected: 'dusk' },
  { hour: 21, expected: 'night' },
  { hour: 22.5, expected: 'night' },
...
const weatherMatrix = ['clear', 'rain', 'snow', 'fog', 'storm'];
for (const weather of weatherMatrix) {
  ...
  assert.ok(weatherPlan.fingerprint.length === 8);
}

const seasonMatrix = ['spring', 'summer', 'autumn', 'winter'];
for (const season of seasonMatrix) {
...
const biomeMatrix = [
  habitat({ id: 'forest-a', biome: 'forest' }),
  habitat({ id: 'meadow-a', biome: 'meadow' }),
  habitat({ id: 'hills-a', biome: 'hills', slope: 0.32 }),
  habitat({ id: 'taiga-a', biome: 'taiga', waterDepth: 0.1 }),
  habitat({ id: 'mountain-a', biome: 'mountain', slope: 0.55 }),
  habitat({ id: 'volcanic-a', biome: 'volcanic', waterDepth: 0.2, slope: 0.6 }),
  habitat({ id: 'ruins-a', biome: 'ruins', waterDepth: 0.1, slope: 0.3 }),
  habitat({ id: 'roadside-a', biome: 'roadside', waterDepth: 0.05, slope: 0.12 }),
  habitat({ id: 'settlement-edge-a', biome: 'settlement-edge', waterDepth: 0.04, slope: 0.1 }),
  habitat({ id: 'lake-a', biome: 'lake', waterDepth: 0.2, canonicalBiome: 'lake' }),
  habitat({ id: 'river-a', biome: 'river', waterDepth: 0.1, canonicalBiome: 'river' }),
  habitat({ id: 'ocean-a', biome: 'ocean', waterDepth: 2, canonicalBiome: 'ocean' }),
  habitat({ id: 'cliff-a', biome: 'cliff', waterDepth: 0.1, slope: 0.9, canonicalBiome: 'cliff' }),
];

for (const entry of biomeMatrix) {
  const habitatPlan = planFaunaEcologyTick({
    ...basePlanInput,
    habitats: [entry],
    candidates: [],
    threats: [],
...
const populationMatrix = Array.from({ length: 18 }, (_, index) => ({
  species: ['deer', 'wolf', 'horse', 'dragon'][index % 4],
  habitatId: `habitat-${index % 6}`,
  count: index % 9,
  juveniles: index % 3,
  adults: index % 7,
  stressed: index % 4,
...
const matrixRefresh = planPopulationRefresh({
  worldSeed: 'matrix',
  now: 1200,
  clock: { hour: 14, day: 4, season: 'autumn', weather: 'clear' },
  habitats: biomeMatrix.slice(0, 6).map((item) => ({ ...item, canonicalBiome: item.biome })),
  population: populationMatrix,
});
assert.ok(matrixRefresh.rows.length <= LIVING_WORLD_FAUNA_ECOLOGY_POLICY.maxPopulationRows);
assert.ok(matrixRefresh.rows.every((row) => row.capacity >= 0));
assert.ok(matrixRefresh.rows.every((row) => row.count >= 0));
assert.ok(matrixRefresh.rows.every((row) => row.target >= 0));
assert.ok(matrixRefresh.rows.every((row) => row.suitability >= 0 && row.suitability <= 1));

const largeCandidateSet = Array.from({ length: 90 }, (_, index) => candidate({
  id: `candidate-${index.toString().padStart(3, '0')}`,
  species: ['deer', 'wolf', 'horse', 'dragon'][index % 4],
  lod: ['near', 'distant', 'far'][index % 3],
  distanceMeters: 50 + index * 60,
}));
const largeThreatSet = Array.from({ length: 70 }, (_, index) => threat({
  id: `threat-${index.toString().padStart(3, '0')}`,
  kind: ['player', 'wolf', 'dragon', 'guard'][index % 4],
  distanceMeters: 15 + index * 8,
}));
const boundedPlan = planFaunaEcologyTick({
  ...basePlanInput,
  candidates: largeCandidateSet,
  threats: largeThreatSet,
});
assert.ok(boundedPlan.updates.length <= LIVING_WORLD_FAUNA_ECOLOGY_POLICY.maxCandidates);
assert.ok(boundedPlan.commands.length <= LIVING_WORLD_FAUNA_ECOLOGY_POLICY.maxCommands);
assert.ok(boundedPlan.events.length <= LIVING_WORLD_FAUNA_ECOLOGY_POLICY.maxEvents);
assert.ok(boundedPlan.groups.length <= LIVING_WORLD_FAUNA_ECOLOGY_POLICY.maxCandidates);
assert.equal(validateFaunaEcologyPlan(boundedPlan).ok, true);

const largeHabitats = Array.from({ length: 100 }, (_, index) => habitat({
  id: `habitat-${index.toString().padStart(3, '0')}`,
  biome: index % 5 === 0 ? 'taiga' : index % 4 === 0 ? 'meadow' : 'forest',
  position: { x: index * 11, z: index * 17 },
}));
const boundedHabitats = planPopulationRefresh({
  worldSeed: 'large-habitats',
  now: 5000,
  clock: { hour: 10, day: 8, season: 'summer', weather: 'clear' },
  habitats: largeHabitats,
});
...
const distanceSweepCandidates = [
  candidate({ id: 'safe-01', distanceMeters: 100, health: 1, spawnTime: 1 }),
  candidate({ id: 'edge-01', distanceMeters: LIVING_WORLD_FAUNA_ECOLOGY_POLICY.cullDistanceMeters + 1, health: 1, spawnTime: 1 }),
  candidate({ id: 'dead-01', distanceMeters: 50, health: 0, spawnTime: 1 }),
  candidate({ id: 'protected-01', distanceMeters: 9999, health: 0, protected: true, spawnTime: 1 }),
];
const sweep = planFaunaDespawnSweep({ now: 1000, candidates: distanceSweepCandidates });
assert.equal(sweep.despawn.length, 2);
assert.ok(sweep.despawn.some((item) => item.id === 'edge-01'));
assert.ok(sweep.despawn.some((item) => item.id === 'dead-01'));
assert.equal(sweep.despawn.some((item) => item.id === 'protected-01'), false);

const staleReplay = runFaunaEcologyReplay([
  { ...basePlanInput, now: 0, threats: [threat({ ageSeconds: 999 })] },
  { ...basePlanInput, now: 10, threats: [threat({ ageSeconds: 0 })] },
  { ...basePlanInput, now: 20, threats: [] },
]);
...
const comparisonMismatch = compareFaunaEcologyRuns(
  replayA.outputs,
  replayA.outputs.map((item, index) => index === 0 ? { ...item, fingerprint: 'deadbeef' } : item),
);
...
const frozenNestedChecks = [
  ['plan.clock', plan.clock],
  ['plan.governance', plan.governance],
  ['plan.navigation', plan.navigation],
  ['plan.settlement', plan.settlement],
  ['plan.spawn', plan.spawn],
  ['plan.updates', plan.updates],
  ['plan.events', plan.events],
  ['plan.commands', plan.commands],
];
for (const [label, value] of frozenNestedChecks) assertFrozen(value, label);
...
for (const event of plan.events) {
  ...
  assert.ok(typeof event.tick === 'number');
}

for (const group of plan.groups) {
  assertFrozen(group, 'group');
  assertFrozen(group.formation, 'formation');
  assert.ok(group.count >= 1);
  assert.ok(group.count <= LIVING_WORLD_FAUNA_ECOLOGY_POLICY.maxGroupMembers);
}

for (const command of plan.commands) {
  assertFrozen(command, 'command');
  assertFrozen(command.payload, 'command payload');
  assert.ok(command.priority >= 1);
  assert.ok(command.expiresAfterSeconds > 0);
}

const pressurePlan = planFaunaEcologyTick({
  ...basePlanInput,
  candidates: [candidate({ id: 'pressure-wolf', species: 'wolf', lod: 'near', distanceMeters: 30 })],
  threats: [
    threat({ id: 'dragon-close', kind: 'dragon', distanceMeters: 20 }),
    threat({ id: 'player-mid', kind: 'player', distanceMeters: 30 }),
    threat({ id: 'guard-far', kind: 'guard', distanceMeters: 90 }),
  ],
});
...
const predatorPlan = planFaunaEcologyTick({
  ...basePlanInput,
  candidates: [candidate({ id: 'wolf-hunt', species: 'wolf', lod: 'near', distanceMeters: 50 })],
  threats: [threat({ kind: 'deer', id: 'prey-01', distanceMeters: 30 })],
});
assert.equal(predatorPlan.updates[0].state, 'stalk');
assert.equal(predatorPlan.updates[0].targetId, 'prey-01');

const dragonPlan = planFaunaEcologyTick({
  ...basePlanInput,
  habitats: [habitat({ biome: 'mountain', id: 'mountain-dragon', slope: 0.4, waterDepth: 0.3, position: { x: 1000, z: 1100 } })],
  candidates: [candidate({ id: 'dragon-01', species: 'dragon', groupId: 'solo-dragon', distanceMeters: 100 })],
  threats: [threat({ id: 'player-dragon', kind: 'player', distanceMeters: 20 })],
});
assert.ok(dragonPlan.updates.some((item) => item.species === 'dragon'));
assert.ok(dragonPlan.updates.some((item) => item.state === 'flee'));

const horsePlan = planFaunaEcologyTick({
  ...basePlanInput,
  clock: { hour: 13, season: 'summer', weather: 'clear' },
  habitats: [habitat({ biome: 'meadow', id: 'horse-meadow', position: { x: 400, z: 400 }, slope: 0.1 })],
  candidates: [candidate({ id: 'horse-01', species: 'horse', groupId: 'horse-herd', distanceMeters: 100 })],
  threats: [threat({ kind: 'wolf', id: 'wolf-threat', distanceMeters: 20 })],
});
...
const nightDeerPlan = planFaunaEcologyTick({
  ...basePlanInput,
  clock: { hour: 23, season: 'winter', weather: 'snow' },
  candidates: [candidate({ id: 'deer-night', species: 'deer', distanceMeters: 120 })],
  threats: [],
});
assert.equal(nightDeerPlan.clock.phase, 'night');
assert.equal(nightDeerPlan.updates[0].schedule.rest, true);

const cullPlan = planFaunaEcologyTick({
  ...basePlanInput,
  now: 1000,
  candidates: [candidate({ id: 'cull-me', distanceMeters: 6000, spawnTime: 0 })],
  threats: [],
});
...
const unsupportedSpeciesPlan = planFaunaEcologyTick({
  ...basePlanInput,
  candidates: [candidate({ id: 'strange-01', species: 'unknown-beast' })],
  threats: [threat()],
});
