...
assert.match(source, /if \(isDodgeInvulnerable\(\)\) \{[\s\S]*payload\.amount = 0; payload\.mitigation = 'dodge';/);
...
console.log(`PLAYER_DODGE_IFRAMES_OK start=${start.toFixed(2)} end=${end.toFixed(2)} duration=${duration.toFixed(2)}`);
