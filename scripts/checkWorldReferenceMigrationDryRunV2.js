#!/usr/bin/env node
/**
 * Run 184 full-reference migration dry-run, syntax-safe V2.
 *
 * Exercises the future 9000x7000 / 137.5 km² world scale through real browser-resolved modules
 * without switching live runtime constants. It proves 14/14 settlement positions/pads, canonical
 * hydrology, MST topology and all 13 slope-aware road routes before a later runtime migration.
 */
...
async function main() {
	...
	if (!playwright) {
		console.error('[checkWorldReferenceMigrationDryRunV2] SKIP: Playwright unavailable.');
		process.exit(2);
	}
	...
	for (const file of runtimeFiles) {
		assert(!fs.readFileSync(path.join(ROOT, file), 'utf8').includes('worldReferenceMigrationPlan.js'), `${file}: run184 must stay dry-run only`);
	}

	console.log(`[checkWorldReferenceMigrationDryRunV2] PASS: full-map ${data.plan.areaKm2.toFixed(1)} km² ${data.plan.gridColumns}x${data.plan.gridRows}; 14/14 seats round-trip+flat+hydrology safe; min seat edge margin ${minSeatEdgeMargin.toFixed(1)}m; MST 13/13 invariant; target roads ${totalRoadKm.toFixed(2)}km maxGrade ${maxRoadGrade.toFixed(1)}° min edge margin ${minRoadEdgeMargin.toFixed(1)}m; max seat migration ${maxShift.toFixed(1)}m; runtime remains unwired.`);
}

main().catch((error) => {
	console.error('[checkWorldReferenceMigrationDryRunV2] FAIL:', error);
	process.exit(1);
});