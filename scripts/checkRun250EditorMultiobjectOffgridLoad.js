...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run250-editor-multiobject-offgrid-load');
...
async function main() {
  ...
  const fixturePath = path.join(ARTIFACT_DIR, 'multiobject-offgrid.scene.json');
  const expected = [
    {
      id: 'run250-tree-0001', name: 'Run250 Offgrid Tree North', asset: 'marker-tree',
      position: [-17.375, 0.125, 24.625], rotation: [0.625, -1.25, 2.125], scale: [0.007, 1.75, 0.5]
    },
    {
      id: 'run250-tree-0002', name: 'Run250 Offgrid Tree East', asset: 'marker-tree',
      position: [9.125, 2.375, -11.875], rotation: [-0.375, 0.875, -2.375], scale: [1.25, 0.333, 2.5]
    },
    {
      id: 'run250-tree-0003', name: 'Run250 Offgrid Tree West', asset: 'marker-tree',
      position: [-3.625, 1.625, 6.875], rotation: [2.75, -0.625, 0.125], scale: [0.125, 2.25, 0.875]
    }
  ];
  ...
  try {
    ...
    assert(before.objects.length === 0, `Editor should start empty: ${JSON.stringify(before.objects)}`);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '01-before-multiobject-load.png'), fullPage: true });

    await page.locator('#we-load-file').setInputFiles(fixturePath);
    await page.waitForFunction(() => (
      window.__WESTEROS_WORLD_EDITOR__?.editableObjects?.length === 3 &&
    ...
    assert(after.objects.length === sortedExpected.length, `Loaded object count drifted: ${JSON.stringify(after.objects)}`);
    assert(JSON.stringify(after.objects) === JSON.stringify(sortedExpected), `One or more off-grid transforms were quantized, reordered by identity, or otherwise drifted: ${JSON.stringify({ expected: sortedExpected, actual: after.objects })}`);
    assert(new Set(after.objects.map((object) => object.id)).size === sortedExpected.length, `Object identities collided during multi-object import: ${JSON.stringify(after.objects)}`);
    ...
    assert(after.history?.observersStarted === true && after.history?.restoring === false, `History controller unstable after multi-object scene load: ${JSON.stringify(after.history)}`);
    assert(after.toast === 'Scene JSON yüklendi.', `Load completion signal drifted: ${after.toast}`);
    assert(errors.length === 0, `Browser errors: ${errors.join(' | ')}`);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, '02-after-multiobject-load.png'), fullPage: true });

    console.log(`[checkRun250EditorMultiobjectOffgridLoad] PASS ${JSON.stringify({ objectCount: after.objects.length, objects: after.objects, snapEnabled: after.snapEnabled, snapSize: after.snapSize, unexpectedErrors: errors.length, screenshots: 2 })}`);
  } finally {
    await context.close(); await browser.close(); await new Promise((resolve) => server.close(resolve));
  }
}

main().catch((error) => { console.error(`[checkRun250EditorMultiobjectOffgridLoad] FAIL: ${error.stack || error}`); process.exitCode = 1; });
