#!/usr/bin/env node
/** Regression guard for Run 145's three FAZ 8 world-event additions. */
const fs = require('fs');

const source = fs.readFileSync('src/3d/gameplay/worldEvents.js', 'utf8');
const expected = [
  { id: 'mummer_troupe', timeOfDay: null, weight: 'UNCOMMON' },
  { id: 'shepherd_flock', timeOfDay: 'day', weight: 'COMMON' },
  { id: 'stargazing_maester', timeOfDay: 'night', weight: 'RARE' },
];
...
if (!source.includes('// Run 145 live count: 15 of 49 entries are time-gated.')) {
  failures.push('Run 145 live-count comment missing');
}

if (failures.length) {
  console.error(`[checkRun145WorldEvents] FAIL: ${failures.length} issue(s)`);
  for (const failure of failures) console.error(`  - ${failure}`);
  process.exit(1);
}

console.log('[checkRun145WorldEvents] PASS: 3 new events present; night/day/ungated contracts preserved.');
