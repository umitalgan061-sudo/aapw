...
const replay = buildG77ReliefProbe();
...
if (probe.rows.some((row) => row.some((height) => !Number.isFinite(height)))) throw new Error('G77 relief probe contains non-finite height');
if (JSON.stringify(probe) !== JSON.stringify(replay)) throw new Error('G77 relief probe is not deterministic within one process');
if (probe.reliefChecksum !== replay.reliefChecksum) throw new Error('G77 relief checksum changed across deterministic replay');

const metrics = Object.freeze({
  policyId: probe.policyId, sourceMapSha256: probe.sourceMapSha256, geoCell: probe.geoCell, layer: probe.layer,
  sourceGridSize: probe.sourceGridSize, terrain3dRegionSize: probe.terrain3dRegionSize, terrain3dImportSize: probe.terrain3dImportSize,
  ...
  reliefChecksum: probe.reliefChecksum, deterministicReplayChecksum: replay.reliefChecksum,
});
