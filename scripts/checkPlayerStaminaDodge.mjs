...
function simulateSprint(seconds, dt = 1 / 60) {
  let stamina = cfg.maxStamina;
  let exhausted = false;
  let travelled = 0;
  for (let elapsed = 0; elapsed < seconds; elapsed += dt) {
    const step = Math.min(dt, cfg.maxFrameDelta);
    const sprinting = !exhausted && stamina > 0;
    travelled += (sprinting ? cfg.sprintSpeed : 0) * step;
    if (sprinting) {
      stamina = Math.max(0, stamina - cfg.sprintDrain * step);
...
function simulateRecovery(initial, seconds, dt = 1 / 60) {
  let stamina = initial;
  let delay = cfg.regenDelay;
  for (let elapsed = 0; elapsed < seconds; elapsed += dt) {
    const step = Math.min(dt, cfg.maxFrameDelta);
    delay = Math.max(0, delay - step);
    if (delay <= 0) stamina = Math.min(cfg.maxStamina, stamina + cfg.regen * step);
  }
  return stamina;
}
...
function dodgeTravel(delta) {
  const dt = Math.min(delta, cfg.maxFrameDelta);
  return cfg.dodgeSpeed * dt;
}

assert.ok(dodgeTravel(5) <= cfg.dodgeSpeed * cfg.maxFrameDelta + 1e-9, 'large frame must be capped');
const substepsAtMaxFrame = Math.ceil(dodgeTravel(cfg.maxFrameDelta) / cfg.collisionStep);
assert.ok(substepsAtMaxFrame >= 2, 'max-frame dodge must resolve collision in multiple substeps');
...
console.log(JSON.stringify({
  ok: true,
  contract: 'player-stamina-sprint-dodge',
  maxStamina: cfg.maxStamina,
  sprintSpeedMps: cfg.sprintSpeed,
  dodgeSpeedMps: cfg.dodgeSpeed,
  dodgeCost: cfg.dodgeCost,
  collisionStepMeters: cfg.collisionStep,
  maxFrameDeltaSeconds: cfg.maxFrameDelta,
  sprintAfter1_5s: sprint,
  availableDodges,
}, null, 2));
