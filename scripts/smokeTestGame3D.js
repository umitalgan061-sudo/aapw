 ...
 * - `game3dSmokeChecksDebugTools.js` — F4 debug camera and F2 debug/profiling panel.
 * - `game3dSmokeChecksWorldEvents.js` — bounded-frame WorldEventSystem/toast and day/night gating
 *   browser acceptance; ordinary 1-second foreground frames intentionally replace legacy huge-delta
 *   shortcuts so browser smoke matches the shipped resume-safety contract.
 ...
 * (`game3dSmokeChecksScene.js` hit 573/600, its F4/F2/world-event checks moved into
 * `game3dSmokeChecksDebugTools.js`). The bounded-resume world-event acceptance later moved into its
 * own focused module so debug-tool ownership and world-event frame semantics stay independent.
 * See each file's own header comment for why. Every module is under this project's 600-line cap.
 *
