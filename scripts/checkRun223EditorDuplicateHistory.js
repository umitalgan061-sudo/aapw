...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'run223-editor-duplicate-history');
...
async function main() {
  ...
  try {
    ...
    assert(await duplicateButton.isDisabled(), 'Duplicate button should start disabled without a copyable selection');
    ...
    assert(!(await duplicateButton.isDisabled()), 'Duplicate button stayed disabled after selecting an editable object');
    ...
    const afterButtonDuplicate = await page.evaluate(() => ({
    ...
    assert(await pasteButton.isDisabled(), 'Duplicate action should not alter clipboard/paste availability');
    ...
    const afterKeyboardDuplicate = await page.evaluate(() => ({
      objects: window.__WESTEROS_WORLD_EDITOR__.editableObjects.map((object) => ({
        assetId: object.userData?.editorAssetId,
        editorId: object.userData?.editorId,
        x: object.position.x,
    ...
    console.log(`[checkRun223EditorDuplicateHistory] PASS ${JSON.stringify({ afterButtonDuplicate, afterKeyboardDuplicate, afterRedo })}`);
  } finally {
    await context.close();
    await browser.close();
...
main().catch((error) => {
  console.error(`[checkRun223EditorDuplicateHistory] FAIL: ${error.stack || error}`);
  process.exitCode = 1;
});
