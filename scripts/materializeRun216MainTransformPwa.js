...
const MARKER = '// Run216 main-based World Editor TransformControls offline shell extension.';
...
function verifyContent(content) {
  ...
  if (!text.startsWith(BLOCK)) fail('Run216 PWA block is not the exact file prefix');
...
function materialize(targetPath = TARGET_PATH) {
  ...
  if (beforeText.includes(MARKER)) fail('Run216 PWA marker exists away from the exact prefix');
  for (const entry of REQUIRED_PATHS) {
...
function runSelfTest() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'run216-main-sw-'));
  ...
  try {
    ...
    if (!first.created) fail('first materialization did not create prefix');
    ...
    if (second.created) fail('second materialization was not idempotent');
    if (!fs.readFileSync(target).equals(firstBytes)) fail('idempotent bytes drifted');
    console.log('PASS Run216 main TransformControls PWA materializer: prepend-only and idempotent');
  } finally {
...
function main() {
  ...
  console.log(`PASS Run216 main TransformControls PWA: ${result.created ? 'prefix added' : 'already exact'}`);
}

try { main(); }
catch (error) {
  console.error(`FAIL Run216 main TransformControls PWA: ${error.stack || error}`);
  process.exitCode = 1;
}
