...
- [x] Yıldızlı gece (`stars.js`) — 1200 tohumlu (seeded) noktadan oluşan üst-yarımküre yıldız
  ...
  ADR-0012. **Run 87 güncellemesi:** her yıldız artık kendi tohumlu faz/frekansıyla yavaşça
  titreşiyor (parlaklık tam sıfıra hiç inmiyor) — `ShaderMaterial`'e geçildi, ilk kalıcı regresyon
  testi eklendi (26. smoke check). Detaylar: DECISIONS.md ADR-0112.
...
- **~~`stars.js`'s starfield is a fixed, non-twinkling pattern~~ — twinkle landed run 87** (per-star
  seeded phase/frequency, `ShaderMaterial` replacing the old flat-opacity `PointsMaterial`, see
  DECISIONS.md ADR-0112). Same seed still always produces the same star positions AND the same
  twinkle pattern (determinism rule unchanged — the phase/frequency stream is deterministic, not
  `Math.random()`). **Still open:** no relation to real astronomical positions/constellations — this
  was never in scope, still a uniformly-scattered procedural field, just no longer flat-brightness.
...
**Sub-task 2 — decision and work (DECISIONS.md ADR-0067):** budget/time still available after
sub-task 1's commit+push, so chained straight into the next priority-order item. `dialogueChoices.js`
had 467/600 lines of headroom post-split, so grew `CHOICES_BY_NPC_ID` from 10 to 12 of 14 NPCs,
adding `berk-guard-1` and `olena-guard-1` (both House Tyrell seats, sharing `ziya-guard-1`'s flavor
family — the pilot's remaining not-yet-covered *houses* are effectively exhausted, so this round adds
Tyrell voice depth rather than new-house coverage; `twin-guard-1` is now the one genuinely uncovered
NPC left, `jon-guard-1` still deliberately excluded). Config-only; zero changes to
`interaction.js`/`dialogueBox.js`/`game3d.js`.
...
specifically:** a one-off Playwright script booted the live `game3d.html` page (zero console/page
errors), then instantiated the real `DialogueBox`/`InteractionPrompt`/`createInteractionController`
with the actual `INTERACTION_CONFIG` for both new ids, reading state off the instance's own scoped
DOM refs (not a global `document.querySelector`, which would hit the real running game's own hidden
dialogue box — the same pitfall run 49 flagged). Both NPCs' real greeting, both numbered choice
labels, and (after a simulated `Digit1` press) that choice's own response text with the hint reverted
to "E / Esc - Kapat" all matched exactly. Screenshot of `berk-guard-1`'s response confirms it renders
over the real scene (castle, player, night sky). Zero console/page errors throughout.

**Memory-leak checklist:** N/A — config-only content addition to a frozen object literal; the one-off
verification script's own controller/UI instances were scratch/throwaway on a disposable DOM node.

**Files changed this sub-task:** `src/3d/gameplay/dialogueChoices.js`, `src/3d/gameplay/
gameplayConfig.js` (JSDoc count-only), `DECISIONS.md` (new ADR-0067), `3D_GAME_PROGRESS.md` (this
file). 4 files, ~35 new/changed lines. One commit, direct push to `main`.

**World Coverage (unchanged this sub-task): 96.2% (132.25 km² / 137.5 km²) desktop; 4.5%
(6.25 km² / 137.5 km²) mobile — a dialogue-content-only addition touches no terrain/streaming/chunk
logic.**

**Run totals (2 chained sub-tasks, run 50):** 5 files touched across both sub-tasks
(`src/3d/gameplay/gameplayConfig.js`, `src/3d/gameplay/dialogueChoices.js`, `DECISIONS.md`,
`3D_GAME_PROGRESS.md` counted once each despite being touched in both sub-tasks) and ~300
new/moved/changed lines total (well under the 1200-line/25-file budget). 2 commits, each
regression-guarded (12/12 smoke suite; sub-task 1 verified via unchanged behavior + existing
...
## This Run (2026-08-05, run 74 — scheduled autonomous routine)

**Session Snapshot:** `GOVERNANCE.md` (already existed — created run ~67, this run's earlier "create
it" instruction was a stale/repeated copy of an already-completed one-time step, confirmed by its
presence + content before doing anything else; `CREDITS.md` likewise already existed), `3D_GAME_PROGRESS.md`
(this file), `git log -10`, `DECISIONS.md` last 3 ADRs (0094/0095/0096), `QUESTIONS_FOR_OWNER.md` (7
entries, all still open/unresolved, none this run could close), git detached-HEAD state found at
session start (`HEAD` matched `origin/main` exactly) — reattached to a real local `main` tracking
branch before doing anything else. `git fetch origin main`: up to date, no concurrent-session drift
(GOVERNANCE.md §8.14).

**Baseline health re-verification (before picking a sub-task):** `node --check` clean on every file
under `src/` and `scripts/`. Full smoke suite: **22/22 PASS**. All 7 standing guards clean:
`checkAssetsManifest` (41), `checkCreatureSpeciesConfig` (15, 9 awaiting-model/6 partial-existing),
`checkDialogueChoicesShape` (13/14), `checkPwaInstallability` (OK), `checkServiceWorkerCache` (47 JS
files), `checkSmokeCheckRegistry` (56 files, only the pre-existing `gameplayConfig.js` 597/600 WARN),
`terrainSeatSafetyCheck`/`roadNetworkSafetyCheck` (14/14 seats, 13/13 edges). Confirms GOVERNANCE.md
§18 priority items 1-11 all still genuinely healthy — no fresh work surfaced there.

### Sub-task 1: Grow the world-event flavor pool from 16 to 18 entries (DECISIONS.md ADR-0097)

**Re-confirmed items 12-13 still blocked, not assumed:** re-scanned `assets/models/characters/`
against every import in `creatureSpeciesConfig.js`/`gameplayConfig.js` — the same 5 orphaned `.glb`
files from ADR-0096's audit remain, none newly added, none maps cleanly to a named archetype without
guessing. Also confirmed `dialogueChoices.js`'s "13/14" is a deliberate, ADR-0058-documented exclusion
(`jon-guard-1`), not a real gap — closing it would contradict a standing decision. That leaves item 14
("Yeni özellik"). `gameplay/worldEvents.js`'s flavor pool (FAZ 8) is this project's own established
low-risk growth track for that bucket (grown 4→8→12→14→16 across five prior rounds, ADR-0061/0063/
0065/0068) — continued the exact same pattern.

Added `white_raven` (Citadel white raven announcing a season change — distinct from the existing
generic `raven`/record-keeping `maester_raven`) and `iron_bank` (rumor of an Iron Bank debt-collector
— the pool's first political/economic-tension flavor). Both original text, no HBO material referenced.
Full ADR detail (context/decision/alternatives/verification): ADR-0097.

**DoD status:** `node --check` clean on the one changed file (`worldEvents.js`, 99→101/600 lines).
Smoke suite **22/22 PASS before and after**, byte-identical on every other check (`checkWorldEvents`
asserts the mechanism generically, not fixed ids/count). All 7 standing guards re-run clean and
unaffected. Performance: `perf_log.csv`'s `run74` row bit-identical to `run73` on every GPU-submission
metric (46 draw calls, 393,231 triangles, 44 geometries, 17 textures) — expected, config-only content
with zero new scene objects. **Görsel Doğrulama (§8.5):** a one-off Playwright script (same technique
ADR-0068 established) booted the real `game3d.html`, drove the real `createWorldEventSystem` (seed 7)
until both new ids were actually observed from the real pool (27 `update()` calls, real payloads
matched source exactly), then rendered `white_raven`'s real payload through a real `WorldEventToast`
— screenshot confirms the toast's icon/title/desc rendered over the live scene (castle silhouette,
player model, starlit sky), zero console/page errors throughout. Memory-leak checklist: no new
listener/timer/DOM node in production code (the toast/EventBus instances created in the one-off proof
script are not committed — dev-only, same as ADR-0068's own proof script). Tech debt counter: **0**
(unchanged). No `QUESTIONS_FOR_OWNER.md` entry needed — flavor text carries no tunable numeric value.
This file updated (this entry). ADR written (ADR-0097). Console clean.

**AI Self-Review 2. Geçiş (§8.3):** re-checked both new ids against all 16 existing ones for
collision/thematic overlap before committing — none found; confirmed the two descriptions keep this
pool's established open-ended/ambiguous tone rather than stating a hard game-state fact; confirmed
`worldEvents.js`'s header comment needed no update (it doesn't track a live count inline, unlike
`dialogueChoices.js`'s header, which does and was correctly left alone since this sub-task didn't
touch that file).
...
suite re-ran byte-identical. No "6 months from now" ambiguity: ADR-0097 records the exact reasoning
for both picks and the one alternative (`wildling_rumor`) considered and deliberately not used.

**World Evolution Report:**

| Metric | Before | After | Delta |
|---|---|---|---|
| `WORLD_EVENTS` pool size | 16 | **18** | +2 (`white_raven`, `iron_bank`) |
| `worldEvents.js` line count | 99/600 | **101/600** | +2, comfortable headroom |
| Smoke suite | 22/22 | 22/22 | unchanged (mechanism-generic check) |
| ADRs | 96 | **97** | +1 (ADR-0097) |
| perf_log.csv rows | 16 | **17** | +1 (`run74`) |
| World Coverage (desktop / mobile) | 96.2% / 4.5% | 96.2% / 4.5% | unchanged (deliberate constant) |
| Tech debt count | 0 | **0** | unchanged |
| Draw calls / triangles | 46 / 393,231 | 46 / 393,231 | unchanged (config-only, no scene objects) |

**Oyuncu fark eder mi:** evet, ama küçük bir dozda — dünya artık ara sıra iki yeni olay daha
gösterebilir (bir Citadel kuzgunu mevsim değişimini duyuruyor, ya da Demir Banka'dan bir tahsildarın
geldiği fısıldanıyor), oyuncunun periyodik olarak gördüğü ortam-zenginleştirme kartlarına iki yeni
çeşit ekliyor; hiçbir mekanik/davranış değişmedi.

**Not chaining a second sub-task this run.** Per §8.7's run-level time ceiling and §19's "well-scoped,
not rushed" discipline: every other priority item was re-confirmed blocked or intentionally excluded at
this run's own start (see baseline re-verification above) — a second same-run addition to the same
small flavor-text list would start drifting from "well-scoped increment" toward "padding," which
GOVERNANCE.md's own precedent (every prior world-event growth ADR) treats as a reason to stop, not
push further.

**Next step for the next run:** re-scan the priority order fresh, as always — nothing about this run's
own findings changes GOVERNANCE.md §18's order. `gameplayConfig.js`'s 597/600 remains the one
line-count watch-item in the repo (unaffected by this run, since `worldEvents.js` is a separate file)
— the next change that touches it should split it first, same precedent as ever. Confirmed still
blocked, unchanged from run 73: the remaining 6 castle seats and all FAZ 6 animals needing real rigged
models, dragon attack/fire-breath (owner decision pending), `erkek-insan`/`kadin-insan`/`koylu`'s
optional differentiation (no design reason to invent one yet). No blocking bugs, syntax errors, or
regressions found this run.
...
## This Run (2026-08-05, run 78 — scheduled autonomous routine)

**Session Snapshot done first, per GOVERNANCE.md §20:** read `GOVERNANCE.md` in full. This run's
incoming instruction opened with "first create GOVERNANCE.md" as a one-time critical bootstrap step —
found it already fully created (301 lines) and current, matching the instruction's own rule list
line-for-line (run 76 already did this in an earlier firing of this same recurring instruction, before
this session existed), so that step was a no-op confirmation, not a fresh write. Same for the
instruction's `CREDITS.md`/`assets_manifest.json` `dateAdded`-coverage asks — both already complete
(`CREDITS.md` 102 lines, 41 assets across 4 sources; every manifest entry already carries `dateAdded`,
confirmed via a small Python walk of the JSON, zero missing). Also read `3D_GAME_PROGRESS.md`'s last
"This Run" entry (run 77), `DECISIONS.md`'s last 3 ADRs (0098/0099/0100), `QUESTIONS_FOR_OWNER.md` in
full (7 entries, all still open, none resolvable unattended this run), `STABLE_TAGS.md`/`perf_log.csv`/
`CATCH_UP.md`/`RULES_CHANGELOG.md` tails. Session started on a detached `HEAD` at `1950c52` (run 77's
final commit); `git checkout main && git pull` then `git fetch origin main` confirmed local `main`
...
8 standing guards clean, zero WARN (run 77's split resolved the only standing watch-item).
`perf_log.csv` `run78` baseline row sampled: 46 draw calls / 393,231 triangles / 44 geometries / 17
textures — bit-identical to run75-77, as expected with no scene change yet.

**Priority re-scan (GOVERNANCE.md §18):** items 1-4 (terrain macro relief, road network, ground color,
castle texturing) confirmed already done in prior runs. Items 5-6 (syntax/blocking bugs): clean, none
found. Items 7-11 (performance/memory/tech-debt/smoke-test/coverage): all healthy, unchanged from run
77. Item 12 (dragon attack, FAZ 5-6 animals) and item 13 (FAZ 11 species) remain blocked exactly as
run 77 recorded — owner decision pending in `QUESTIONS_FOR_OWNER.md` for the former, real rigged
models not on disk for the latter (re-confirmed: no new files under `assets/models/` since run 74's
audit). No line-count watch-item exists (run 77 resolved the only one). That leaves item 14 ("Yeni
özellik"). `gameplay/worldEvents.js`'s flavor pool remains this project's established, low-risk,
repeatedly re-used growth track for exactly this bucket — picked as this run's sub-task, same as runs
74/75 picked it for the identical reason (no other item on the list was actionable without an owner
decision or missing assets).

### Sub-task 1: Grow `gameplay/worldEvents.js`'s flavor pool from 20 to 22 entries (DECISIONS.md ADR-0101)

Added `red_comet` (a kızıl kuyruklu yıldız lingering in the sky for days, read as a dynastic omen —
distinct from the existing `falling_star`'s single fleeting streak) and `hunting_party` (a hunting
party returning to a castle gate with the day's game) — the pool's first entirely low-stakes,
tension-free slice-of-life entry. Both original text, no HBO material. Full reasoning, alternatives
considered, and color-collision check in ADR-0101.

**DoD status:** `node --check` clean on `worldEvents.js` (105/600 lines, was 103). Smoke suite
...
`g3d-loading-hidden`), drove the real `createWorldEventSystem` (seed 7) until both new ids came out of
the real 22-entry pool (27 `update()` calls), confirmed both payloads match source exactly, then
...
(unchanged). ADR-0101 written.

**AI Self-Review 2. Geçiş (§8.3):** re-checked both new ids don't collide with any of the 20 existing
ones or their colors; confirmed `red_comet`/`hunting_party`'s framing stays generic fantasy vocabulary,
not HBO-specific; confirmed `worldEvents.js`'s JSDoc header needed no update (doesn't track a live
count).
...
against every existing one for tone/theme/color collision before being written, and proven with a real
render, not just a config-shape assertion. No "6 months from now" ambiguity: ADR-0101 records the same
reasoning template ADR-0097/0098 already established, so a future run finding 22 entries won't need to
guess why any of the last four were added.

**World Evolution Report:**

| Metric | Before | After | Delta |
|---|---|---|---|
| `WORLD_EVENTS` entries | 20 | **22** | +2 (`red_comet`, `hunting_party`) |
| `worldEvents.js` lines | 103/600 | **105/600** | +2, comfortable headroom |
| ADR headers in `DECISIONS.md` | 100 | **101** | +1 (ADR-0101) |
| `perf_log.csv` rows | 21 | **22** | +1 (`run78`) |
...
yeni: bazen gökyüzünde günlerce asılı kalan kızıl bir kuyruklu yıldız haberi, bazen de kaleye dönen bir
av birliği haberi görecek. Dünyanın kendisi (arazi/yol/kale/NPC/hayvan) değişmedi.

**Next step for the next run:** re-scan the priority order fresh, as always. No line-count watch-item
remains. Still blocked, unchanged from run 77: the remaining 6 castle seats and all FAZ 6 animals
needing real rigged models, dragon attack/fire-breath (owner decision pending in
`QUESTIONS_FOR_OWNER.md`), `erkek-insan`/`kadin-insan`/`koylu` differentiation (no design reason to
invent one). `WORLD_EVENTS` now sits at 22 entries — a genuinely new, unrepresented theme will need to
be invented again for any further growth (this run's two, `red_comet`/`hunting_party`, were themselves
invented fresh, not drawn from any pre-existing "considered but not chosen" list). No blocking bugs,
syntax errors, or regressions found this run.
...
## This Run (2026-08-06, run 87)

**Fresh Session Snapshot at container boot:** `GOVERNANCE.md`/`CREDITS.md` already existed and were
already current against this run's incoming instruction (which asked for their creation as a first
sub-task) — confirmed via a diff-read, not recreated; both were built by run 86. `git fetch origin
main` confirmed local `main` already matched `origin/main` (`20001b9`) — no concurrent-session
conflict (§8.14). Read `3D_GAME_PROGRESS.md`'s run 86 entry, `DECISIONS.md`'s last 3 ADRs
(0109-0111), `QUESTIONS_FOR_OWNER.md` in full (9 entries, all still open), `STABLE_TAGS.md`/
`perf_log.csv`/`CATCH_UP.md`/`RULES_CHANGELOG.md` tails. Ran the full smoke suite before picking any
work: **25/25 PASS**, all 6 standing static guards clean, no regression at session start.

### Sub-task 1: starfield twinkle + its first regression test (DECISIONS.md ADR-0112)

Priority re-scan (GOVERNANCE.md §18): items 1-11 re-confirmed healthy/already-resolved (not
re-litigated from scratch — cross-checked against `3D_GAME_PROGRESS.md`'s own prior "already
resolved" notes). Items 12-13 still blocked on missing rigged models / the owner's pending
health-system decision. That left item 14 ("Yeni özellik") — but rather than a fourth straight run
touching `worldEvents.js` (runs 84/85/86 all did), this run picked a different, long-standing Known
Issue with genuine zero test coverage: `stars.js`'s starfield, flagged "fixed, non-twinkling" since
FAZ 2 (run 7).

**What shipped:** each of the 1200 stars now brightens/dims independently over real time via a
per-star seeded phase/frequency baked into the geometry at creation (deterministic — same seed,
same twinkle pattern, never `Math.random()`). `PointsMaterial` replaced by a hand-written
`ShaderMaterial` (same pattern `world/water.js` already established). Full reasoning, alternatives,
and threshold rationale in ADR-0112.

**A real bug caught before commit, not shipped:** the first draft of the vertex shader's inline `//`
comment referenced `` `sizeAttenuation: false` `` in backticks — since the shader source is itself a
JS template literal, those backticks prematurely closed it, corrupting the shader string.
`node --check` did not catch this (the corrupted split was still syntactically valid JS, just
semantically wrong); the **full Playwright smoke suite did** — `game3d.html` failed to boot entirely
(`pageerror: Unexpected identifier 'sizeAttenuation'`). Fixed by rewording the comment before commit.
Recorded in ADR-0112 as a root-cause note (this project's own `/* glsl */ \`...\`` convention means
literal backticks can never appear anywhere inside a shader string, comments included) rather than
silently re-tried — exactly the failure mode the smoke-test gate exists to catch pre-commit.

**DoD status:** `node --check` clean (`stars.js` 169/600, `game3d.js` 545/600 unchanged). Full smoke
suite **26/26 PASS** after the fix (was 25 — `checkStarfieldTwinkle` is the starfield's first
regression check of any kind, deliberately placed in `game3dSmokeChecks.js` rather than the
thematically-closer but already-573/600 `game3dSmokeChecksScene.js` — budget-driven placement,
documented in that file's header). All 6 standing static guards clean, no new line-count WARNs.
**Real proof beyond the committed check:** a dev-only (not committed) Playwright script rendered an
isolated starfield to a real `WebGLRenderTarget` at two different `elapsedSeconds` values with
everything else fixed and read back actual framebuffer pixels — **1054 of 1070 real star pixels
changed brightness** between the two captures (nightFactor=1 both times), and **zero bright pixels**
at nightFactor=0 (day still fully hides every star). **Real visual proof, 2 angles, zero
console/page errors in both:** (1) the live `game3d.html` scene driven to a real nighttime state
(ADR-0111's `performance.now()`-offset technique, reused not rediscovered) — dark night sky, black
castle silhouette, visible star points, a real world-event toast fired in-frame, proving the whole
scene composites correctly; (2) the isolated pixel-proof script's own zenith-look render, a close-up
angle the live scene's default orbit view never shows. Memory-leak checklist: `disposeStarfield`
still disposes geometry+material (now with 2 more attributes on the same geometry, no new
allocation category) — proven not to throw in the committed check. Tech debt counter: **0**
(unchanged). `perf_log.csv` `run87` row bit-identical to run76-86 (pure shader/attribute change, no
new draw call/geometry/texture). `QUESTIONS_FOR_OWNER.md` gained one entry for the twinkle
speed/amplitude feel constants (same uncalibrated-tuning-value pattern as ADR-0089/0096/0111).

**AI Self-Review 2. Geçiş (§8.3):** confirmed `updateStarfield`'s new parameter was updated at its
one and only call site (searched every reference across `src`/`scripts`); confirmed the twinkle
floor can only ever produce equal-or-dimmer brightness than the old flat opacity at the same
`nightFactor`, never brighter (no regression at full night); confirmed the fixed `gl_PointSize`
reproduces the old `sizeAttenuation: false` behavior exactly, just computed without three.js's
built-in attenuation term; confirmed no `TEMP`/`HACK`/`FIXME`/`WORKAROUND`.

**Session Quality Gate (§8.6) after 1 sub-task:** confidence **5/5** — low-risk, fully reversible,
a real coverage gap closed (not just a visual polish item), a real bug caught and fixed by the
project's own regression guard before it ever reached a commit (the guard doing exactly its job),
proven with real rendered-pixel numeric evidence AND real matching-scene screenshots, zero
terrain/scale/height touch. **Stopping here after 1 sub-task:** same reasoning as runs 84-86 —
items 1-13 remain healthy/blocked, item 14 is the only lever, and this run already used it once;
a second "Yeni özellik" pass this run risks the same filler pattern ADR-0110's Alternatives section
warned against. No "6 months from now" ambiguity: ADR-0112 records the full shader design, the
caught-bug root cause, and every rejected alternative.

**World Evolution Report:**

| Metric | Before | After | Delta |
|---|---|---|---|
| Starfield animation | flat, uniform opacity per scene (no per-star variation) | **per-star seeded twinkle** (independent phase/frequency, 1200 stars) | mechanism change |
| Starfield test coverage | 0 checks | **1 check, 6 assertions** (`checkStarfieldTwinkle`) | new, closes a real gap |
| Star pixels changing brightness over time (measured) | n/a (no animation existed) | **1054/1070 (98.5%) in an 8000ms-simulated window** | new invariant, verified |
| Smoke suite | 25/25 | **26/26** | +1 |
| `stars.js` lines | 99/600 | **169/600** | +70 |
| `game3dSmokeChecks.js` lines | 346/600 | **444/600** | +98 (new check) |
| ADR headers in `DECISIONS.md` | 111 | **112** | +1 (ADR-0112) |
| `QUESTIONS_FOR_OWNER.md` open entries | 9 | **10** | +1 (twinkle feel tuning) |
| `perf_log.csv` rows | 30 | **31** | +1 (`run87`) |
| World Coverage (desktop / mobile) | 96.2% / 4.5% | 96.2% / 4.5% | unchanged (no world change) |
| Tech debt count | 0 | **0** | unchanged |
| Draw calls / triangles | 46 / 393,231 | 46 / 393,231 | unchanged (no scene object added) |

**Oyuncu fark eder mi:** evet, ince ama gerçek bir fark — gece gökyüzündeki yıldızlar artık sabit
parlaklıkta durmuyor, her biri kendi ritminde hafifçe parlayıp sönüyor (hiçbiri tamamen kaybolmadan).
Oyunun geri kalanı (dünya ölçeği, yollar, kaleler, NPC'ler, ejderha) değişmedi.

**Next step for the next run:** re-scan the priority order fresh, as always. Blocked items unchanged
since run 80: 6 remaining castle seats + all FAZ 6 animals need real rigged models, dragon attack/
fire-breath needs the owner's pending health-system decision. `CATCH_UP.md`'s next 10-run digest due
at run 88 (1 run away). `RULES_CHANGELOG.md`'s next consolidation pass due ~run 96 (unchanged).
Periodic platform check due ~run 90-100 (unchanged). No blocking bugs, syntax errors, or regressions
found this run — the one bug found (the shader-comment backtick issue) was caught and fixed within
this same sub-task, before any commit.
...
## This Run (2026-08-06, run 95 — scheduled autonomous routine)

**Session Snapshot:** the incoming scheduled prompt's "first create GOVERNANCE.md" instruction is
stale boilerplate again (this is run 95, not run 1) — `GOVERNANCE.md`/`CREDITS.md`/`CATCH_UP.md`/
...
detached 51 commits ahead of local `main`'s stale cached ref; `git fetch origin main` showed
`origin/main` already at that same commit (`b12e425`) — run 94's own push had genuinely succeeded.
Fast-forwarded local `main` to `origin/main` before starting any work — clean tree, no divergence,
nothing lost.
...
7/14) still blocked on real models for the remaining 6 seats — unchanged. Items 5-11 (syntax/blocking
bugs/perf/mem leak/tech debt/smoke test/world coverage) all healthy, re-confirmed by this run's own
pre-work baseline (`node --check` clean across `src/3d`, `checkDialogueChoicesShape.js` OK — 13/14
pilot coverage, `checkSmokeCheckRegistry.js` OK — 28 checks/8 modules, 66 files under the 600-line cap
with the same 2 known WARNs; full `smokeTestGame3D.js` suite **28/28 PASS**, 0 FAIL). Items 12-13
(FAZ 7 dragon follow-ups / FAZ 11 species) remain blocked on models. That leaves item 14 ("yeni
özellik") as the actionable lever — run 94's own "Next step" named the 7 remaining 2-choice NPCs as
open candidates without pre-vetting one specifically, so this run picked `doran-guard-1` (Dorne) for
house-coverage reasons (see ADR-0121's Context/Alternatives).

### Sub-task 1: `doran-guard-1`'s 3rd dialogue choice (DECISIONS.md ADR-0121)

Full reasoning, alternatives considered, and decision detail are in ADR-0121 — not duplicated here.

**DoD durumu:** `node --check` clean (`dialogueChoices.js`, 218/600, was 211). `checkDialogueChoicesShape.js`
OK: 13/13 entries resolve, all choices within the 3-slot limit, pilot coverage unchanged at 13/14.
`checkSmokeCheckRegistry.js` re-run post-change: unchanged (28 checks/8 modules, same 2 known WARNs).
Full smoke suite re-run: **28/28 PASS**, 0 FAIL (before-baseline also 28/28 PASS, 0 FAIL — regresyon
yok).
...
**Real headless-Chromium proof + real visual proof, 2 moments, zero console/page errors in both**
(dev-only, uncommitted Playwright script, same methodology ADR-0115/ADR-0117/ADR-0119/ADR-0120 used,
deleted before commit): the real `game3d.html` was booted, then the same live `CHOICES_BY_NPC_ID`/
`DialogueBox`/`InteractionPrompt`/`createInteractionController` modules `game3d.js` itself imports
were dynamically imported from inside the page and driven against a synthetic `doran-guard-1` NPC.
(1) All 3 real choice labels render in order, hint reads exactly `'1/2/3 - Seç, Esc - Kapat'`, the
real world-event toast ("Ejderha Görüldü!") and health bar (`60/100` — an earlier incidental dragon
bite during this run's own boot-and-settle wait, unrelated to this change) both visible in the
background. (2) Pressing `Digit3` shows the exact new response text with `{name}` replaced by the
synthetic NPC's display name, hint reverts to `'E / Esc - Kapat'`, choice list cleared, health bar
unchanged at `60/100`. Zero console/page errors throughout.

**Perf sampling note:** `perf_log.csv`'s `run95` row (46/393,231/44/17) is bit-identical to the
run76-94 baseline — expected, zero geometry/texture code touched.

**AI Self-Review 2. Geçiş (§8.3):** confirmed the new 3rd choice's angle (personal cost-of-isolation
confession) is distinct from `doran-guard-1`'s existing 2 choices (political tension + secret-garden
lore) and from every other 3-choice NPC's own 3rd choice theme so far — no near-duplicate. Header-
comment NPC-list prose updated consistently. No `TEMP`/`HACK`/`FIXME`/`WORKAROUND`. Memory leak
...
hâlâ net mi" tereddüdü yok: ADR-0121 records the exact angle picked and why, same as its 6
predecessors.
...
| Metric | Before | After | Delta |
|---|---|---|---|
| NPCs with a 3rd dialogue choice | 6/13 | **7/13** | +1 (`doran-guard-1`) |
| Dialogue choice pilot coverage | 13/14 | 13/14 | değişmedi (mevcut bir NPC büyüdü, yeni NPC eklenmedi) |
| Smoke suite | 28/28 | **28/28** | aynı 28 kontrol, yeniden doğrulandı |
| ADR headers in `DECISIONS.md` | 120 | **121** | +1 (ADR-0121) |
| `perf_log.csv` rows | 37 | **38** | +1 (`run95`) |
| World Coverage (desktop / mobile) | 96.2% / 4.5% | 96.2% / 4.5% | değişmedi |
| Tech debt count | 0 | **0** | değişmedi |
| Draw calls / triangles | 46 / 393,231 | 46 / 393,231 | değişmedi (saf config-data değişikliği) |

**Oyuncu fark eder mi:** evet, ama küçük ölçekte — `doran-guard-1` ile konuşan bir oyuncu artık 3.
bir soru sorabilir ve muhafızın kişisel bir bedel/pişmanlık cevabı alır; dünyanın geri kalanı
değişmedi.

**Next step for the next run:** öncelik sırası baştan taranacak. Bloklu kalan her şey değişmedi: 6
kale hâlâ dokusuz, FAZ 6 hayvanları model bekliyor. Item 14 tekrar seçilirse: kalan 6 iki-seçenekli
NPC'den biri (`xaro-guard-1`'in bu run'da açık bırakılan bir sonraki aday — bkz. ADR-0121'in
"Alternatives considered" / `stannis-guard-2`/`balon-guard-1`/`robin-guard-1`/`ziya-guard-1`/
`berk-guard-1`) veya `worldEvents.js`'nin flavor pool'u. `RULES_CHANGELOG.md`'nin sıradaki
konsolidasyonu ~run 96'da (değişmedi). `CATCH_UP.md`'nin sıradaki özeti run 98'de (değişmedi).
...
## This Run (2026-08-06, run 105 — scheduled autonomous routine)

**Concurrency/snapshot:** this container's local `origin/main` tracking ref was stale (a shallow `--depth 50` fetch returned a commit — `b091711`, run 72/ADR-0095 — far behind the repo's real current state), while the local object store still held run 104's tip (`3694918`, merge of PR #4) from a prior firing of this same routine, unreachable from any branch after `git checkout main` moved HEAD to the stale ref. Verified the real current state via the GitHub API directly (`list_branches`: `main` = `3694918`, matching the local dangling commit exactly; `list_pull_requests`/`list_commits` confirm PRs #2-4 really did merge) rather than trusting the local git fetch, then `git reset --hard 3694918` to realign local `main` before branching `work` from it. **No data was lost** — this was a stale local tracking ref, not a real revert on GitHub — but it's worth flagging: a future run trusting `git fetch`/`origin/main` at face value in this container could silently work from run-72-era code. GOVERNANCE.md §8.14 already requires a `git fetch` before/after each sub-task; this run additionally cross-checked against the GitHub API when the two disagreed. Read `GOVERNANCE.md` (already fully populated from an earlier run — no recreation needed), `3D_GAME_PROGRESS.md`, `DECISIONS.md`'s last 3 ADRs, and `QUESTIONS_FOR_OWNER.md`.

**Priority re-scan (§18):** item 1 (macro relief) done since ADR-0075, item 2 (road network) done since ADR-0076, item 3 (grass/rock terrain color) already implemented in `terrain.js`'s vertex-color blend. Item 4 (texture the remaining 6 procedural-only kingdom seats) is blocked on new castle-shaped CC0/CC-BY model assets not present in this environment — the 8 seats in `CASTLE_MODEL_ASSIGNMENTS` already have real, textured models; sourcing 6 more binary assets isn't something this routine should do unattended. Items 5-8 (syntax/blocking bugs/perf/memory) checked clean via the full `node --check` sweep + 30/30 real-browser smoke suite below. That left item 9 (teknik borç): run 104's own ADR-0131 had already flagged `game3d.js` at 590/600 and said it "must not receive another feature wiring increment before an approved extraction/refactor" — the most concrete, already-identified debt item available.

### Sub-task: extract `gameLoopHelpers.js` from `game3d.js` (DECISIONS.md ADR-0132)

Moved `computeCameraRelativeMove`, `combineAxes`, `collectCameraCollidables`, `streamAroundOrbitTarget`, and `bindResize` — five pure, parameter-only functions with no closure over `initGame3D`'s locals — into a new `gameLoopHelpers.js` module. `game3d.js`: 590→471 lines (also dropped its now-unused `THREE`/`CHUNK_CONFIG`/`worldToChunkCoord` imports). Updated stale `game3d.js`-attributes-these-functions doc comments in `sceneManager.js`, `camera.js`, `input.js`, `ui/README.md`, and `ARCHITECTURE.md` (left `3D_GAME_PROGRESS.md`/`DECISIONS.md`'s own historical run-log entries untouched, as point-in-time records).

**DoD:** full repo `node --check` sweep clean. `checkSmokeCheckRegistry.js`: 70 files, 0 near/over the 600-line cap for `game3d.js` (WARN cleared; `dragonController.js`'s pre-existing unrelated 579/600 WARN remains), 30 checks/10 modules unchanged. Full real-browser `smokeTestGame3D.js`: **30/30 PASS**, zero console/page errors — this run's environment did not hit the readiness timeout run 104 saw, so this is a genuine full-suite pass, not the partial one run 104 had to accept. `collectPerfSnapshot.js` `run105` row: 46 draw calls / 393,231 triangles / 44 geometries / 17 textures / 307MB heap — bit-identical draw-call/triangle/geometry/texture counts to `run103`'s baseline (fps reads 3, consistent with this environment's known software-rendering ceiling, not a regression). No before/after screenshots: the change has no visual delta to show (identical render output per the perf snapshot), so the smoke suite's real WebGL boot plus the bit-identical counts are the applicable evidence per ADR-0132's own reasoning. Confidence **5/5**; technical debt: **0** (the 600-line-cap risk this sub-task existed to address is now resolved with headroom, not merely deferred).

**Memory/performance:** pure relocation — no new listeners/timers/GPU resources, no disposal-flow changes; the moved functions' own module-local reused arrays/vectors (`_forward`/`_right`/`_move`/`_cameraCollidables`) moved with them unchanged.
...
## This Run (2026-08-06, run 107 — scheduled autonomous routine)

**Concurrency/snapshot:** read `GOVERNANCE.md` in full (already populated, not recreated), `3D_GAME_PROGRESS.md`'s run 106 entry, `DECISIONS.md`'s last 3 ADRs (0131-0133), and `QUESTIONS_FOR_OWNER.md` in full. Cross-verified local git state against the real GitHub state per §8.14: `mcp__github__list_branches` confirmed `main` = `9ad8ae8` (run 106's PR #6 merge), matching the local detached HEAD exactly — no stale-ref surprise this time. `mcp__github__list_pull_requests` confirmed PRs #1-6 all closed+merged, `merged_by`/`merged_at` populated, confirming this same automated routine both opens and merges its own PRs (consistent with runs 2-106). `git checkout main && git reset --hard 9ad8ae8 && git checkout -B work` before any edit.

**Baseline regression guard:** full `node --check` sweep (`src/`+`scripts/`+`service-worker.js`) clean. `checkSmokeCheckRegistry.js`: 31 checks/11 modules, only the pre-existing `dragonController.js` 579/600 WARN (unchanged across several runs, not yet urgent). Full `scripts/smokeTestGame3D.js`: **31/31 PASS**, 0 FAIL, zero console/page errors — this run's environment did not hit the readiness timeout runs 104/106 saw. `collectPerfSnapshot.js` baseline sample (discarded before commit, not kept as a CSV row): 46 draw calls / 393,231 triangles / 44 geometries / 17 textures — matches the established run76-105 baseline exactly.

**Priority re-scan (§18):** items 1-3 (macro relief/road network/terrain color) remain closed per their own standing safety-check guards. Item 4 (texture the remaining 6 kingdom seats) stays asset-blocked, unchanged. Items 5-11 (syntax/bugs/perf/memory/tech-debt/smoke/coverage) all clean per the baseline sweep above — `dragonController.js`'s 579/600 WARN has held steady (not growing) across multiple runs, so a forced split still isn't the most valuable next lever. Items 12-13 (FAZ 7 dragon follow-ups, FAZ 11 species) remain model/owner-decision-blocked. That left item 14 (new feature): the world's real day/night cycle had no on-screen readout anywhere — the same discoverability gap `settlementCompass.js`/`controlsHelp.js` (runs 104/106) already closed for direction and controls, and a genuine, non-duplicate gap (verified by reading `ui/README.md`'s full widget list before picking it, not assumed).

### Sub-task: day/night clock, plus a root-cause fix for a real mobile HUD collision it exposed (DECISIONS.md ADR-0134)

Added `DayNightClock` (`ui/dayNightClock.js`): a 24-hour HH:MM readout plus a day/twilight/night icon, sourced from `lighting.js`'s own `updateDayNightLighting()` return value, throttled to repaint only when the displayed game-minute changes. While capturing this run's own required mobile visual proof (§8.5), the world-event toast was found genuinely overlapping **both** the health bar and the settlement compass on a 390×844 viewport — not merely the new clock. Root cause: `game3d.css` never declared `box-sizing` (unlike the 2D page's `style.css`, which already resets to `border-box`), so every `min()`/`max()`/`calc()` width and safe-area position in the file was silently rendering larger than its own authors' comments describe. Fixed at the source with one global `box-sizing: border-box` reset, plus moving the mobile event-toast's `top` from ADR-0129's back-link-only `64px` to `184px`, clearing the full second HUD row (compass/clock). Full reasoning, alternatives, and verification detail are in ADR-0134 — not duplicated here.

**DoD durumu:** `node --check` clean across every changed `.js` file (CSS has no equivalent; verified via real-browser proof instead). `checkSmokeCheckRegistry.js` re-run post-change: 32 checks/12 modules (the new `game3dSmokeChecksDayNightClock.js`), same pre-existing `dragonController.js` WARN, unrelated. `checkServiceWorkerCache.js`/`checkPwaInstallability.js` both pass with the new precache entry (`SHELL_CACHE` bumped v8→v9). Full smoke suite re-run: **32/32 PASS**, 0 FAIL (before-baseline was 31/31 PASS — regresyon yok, +1 new check). **Konsol Temizliği:** the new check's own headless boot recorded zero console/page errors; the dev-only visual-proof script (never committed) also recorded zero across both viewports.

**Real headless-Chromium proof + real visual proof, 2 viewports, zero console/page errors in both** (dev-only proof script written directly to the session scratchpad, never inside the repo tree — no delete-before-commit step needed — run over a local static server via `devServerHelper.js`): the real `game3d.html` was booted to `GAME_READY`. (1) Desktop 1280×720: clock renders top-left below the back-link with a real, live HH:MM value and an icon matching the visible dawn-lit sky (`WORLD_DEFAULTS.START_TIME_OF_DAY_RATIO = 0.3`), zero measured overlap with any other widget. (2) Mobile 390×844: before the CSS fix, a real triggered world-event toast measurably overlapped both the health bar and compass bounding boxes (`overlap: true` against both); after the fix, the same triggered toast plus all four corner widgets (back-link, health bar, compass, clock) render with zero measured overlaps, confirmed both by bounding-box math and by eye in the captured screenshots.

**Perf sampling:** `perf_log.csv`'s `run107` row (`46/393231/44/17`) sampled fresh via `scripts/collectPerfSnapshot.js`, bit-identical to the run76-105 baseline — expected, this is a DOM/CSS-only change with zero Three.js geometry/texture/renderer impact.

**AI Self-Review 2. Geçiş (§8.3):** confirmed `dayNightClock.js`'s local threshold duplication (matching `worldEvents.js`'s `NIGHT_THRESHOLD`/`DAY_THRESHOLD`) is a deliberate, documented choice respecting `ui/README.md`'s "no camera/gameplay imports" blast-radius rule, not an oversight — same precedent `mulberry32`'s cross-file duplication already establishes. Confirmed the `box-sizing: border-box` reset changes no *intended* rendered size (every affected calc() was already written assuming it). Extended the existing `checkWorldEvents` toast-lifecycle smoke check to instantiate real `HealthBar`/`SettlementCompass`/`DayNightClock` and assert mobile clearance against all three (not only the back-link, ADR-0129's original scope) — closing the actual detection gap that let this collision go unnoticed across runs 102-106, not just fixing today's instance of it. Also found and fixed a pre-existing structural bug in `ui/README.md` (a bullet's own continuation prose had drifted to sit after the *next* bullet instead of its own, from an earlier run's insertion) and added the previously-undocumented `healthBar.js` entry to that same file while already touching it. No `TEMP`/`HACK`/`FIXME`/`WORKAROUND`. Memory leak checklist: `DayNightClock` owns no listeners/timers/GPU resources; `dispose()` removes its one DOM subtree.

**Session Quality Gate (§8.6):** confidence **5/5** — the new widget is a low-risk, well-proven pattern (third of its kind); the mobile collision it exposed was root-caused rather than papered over, fixed at its actual source (a missing global CSS reset, not a one-off width tweak), and the regression check was strengthened so the same bug class can't silently recur a third time. "6 ay sonra hâlâ net mi" tereddüdü yok.

**World Evolution Report:**

| Metric | Before (run start) | After (run 107 end) | Delta |
|---|---|---|---|
| Discoverability HUD widgets | 3 (controls-help, compass, health bar) | **4** | +1 (`dayNightClock.js`) |
| Mobile HUD collision bugs (toast vs. health-bar/compass) | 1 latent, undetected | **0** | fixed at root cause (`box-sizing`) + regression-guarded |
| Smoke suite | 31/31 | **32/32** | +1 new check |
| Smoke check modules | 11 | **12** | +1 (`game3dSmokeChecksDayNightClock.js`) |
| ADR headers in `DECISIONS.md` | 133 | **134** | +1 (ADR-0134) |
| `perf_log.csv` rows | 47 | **48** | +1 (`run107`) |
| Draw calls / triangles | 46 / 393,231 | 46 / 393,231 | değişmedi (DOM/CSS-only change) |
| World Coverage (desktop / mobile) | 96.2% / 4.5% | 96.2% / 4.5% | değişmedi |
| Tech debt count | 0 | **0** | değişmedi |

**Oyuncu fark eder mi:** evet, iki şekilde — (1) artık oyun içi saati (gündüz/alacakaranlık/gece ikonuyla birlikte) her cihazda görebilir; (2) mobilde bir dünya olayı bildirimi göründüğünde artık can barı veya pusulayla asla çakışmıyor (önceden sessizce çakışıyordu, fark edilmemişti).
...
## This Run (2026-08-07, run 114 — scheduled autonomous routine)
...
`origin/main`'s real tip (`d90aab4`, run 113's stable-tag/§8.11 commit). This run's own fired-prompt
text still says "create GOVERNANCE.md first" — already satisfied by an earlier run (re-confirmed:
`GOVERNANCE.md` exists, complete, all 22 sections present; `CREDITS.md`/`CATCH_UP.md`/
`RULES_CHANGELOG.md`/`STABLE_TAGS.md` all likewise already exist, none recreated). Read
`3D_GAME_PROGRESS.md`'s run 113 entry, `DECISIONS.md`'s ADR-0138/0139/0140, and
`QUESTIONS_FOR_OWNER.md` in full (no unresolved item forcing this run's hand; run-63 leaked-key
entry stays open pending owner action, unchanged). No `git` drift found before committing below.
...
all clean per the baseline sweep. Item 9 (teknik borç): zero, no fresh candidate (`game3d.js`
482/600 has real headroom). Items 12-13 remain owner-decision/asset-model-blocked, unchanged.
§8.12/§13/§15 maintenance windows checked — none due this run (consolidation ~run 116, `CATCH_UP.md`
~run 118, next platform check ~run 132-142). Run 113's own ADR-0140 named "grounding more seats'
terrain neighborhoods" as its follow-up, but that needs its own measured-not-estimated perf
sub-task (per `debug/README.md`'s standing note on how `PHASE1_PREVIEW_RADIUS_CHUNKS` has always
been grown historically) — too large to responsibly finish this run, deferred rather than
half-measured. With 1-13 exhausted, item 14 taken, following run 102/103/129/130's own low-risk
precedent: grow `worldEvents.js`'s flavor pool by one entry.

### Sub-task: `sellsword_arrival` — a new UNCOMMON, ungated world event (DECISIONS.md ADR-0141)

Added one new `Object.freeze`-array entry to `gameplay/worldEvents.js`'s `WORLD_EVENTS`: a wandering
sellsword arriving at a castle gate ("Kiralık Kılıç Gelişi"), UNCOMMON rarity, no `timeOfDay` gate
(deliberately ambiguous text works at any hour). Fixed the file's own two relative-count header
comments in place (6 of 28→29 gated, 22 of 28→23 of 29 ungated) so neither goes stale, matching
ADR-0130's own precedent for the same file. Full reasoning, thematic-overlap check against all 28
prior entries, risk analysis (LOW — additive data only, no logic change), and alternatives
considered (COMMON tier, night gate, a full hireable-NPC feature, growing the preview radius
instead) are in `DECISIONS.md` ADR-0141.
...
(13/13 edges), both byte-identical to run 113's recorded values.

**DoD durumu:**
- [x] `node --check` clean on the one changed file (`gameplay/worldEvents.js`) plus a full repo
      sweep (86 JS files, unchanged — no new file this run)
- [x] Smoke test — baseline **33/33 PASS** before, **33/33 PASS** after (same check count, the
      existing id-based gating assertions needed no change — 0 FAIL, zero console/page errors both
      times)
- [x] Görsel kanıt — real headless-Chromium proof (dev-only proof script, direct real-module
      ...
      viewports (desktop 1280x720, mobile 390x844): searched seeds 1-5000 for one whose first draw
      is the new event, confirmed the real toast DOM's title/desc `textContent` match the picked
      payload exactly (Turkish characters included, no encoding issue), confirmed the mobile toast's
      bounding rect clears the back-link control. Zero console/page errors at both viewports.
- [x] Performans bütçesi — `collectPerfSnapshot.js run114` sample: drawCalls/triangles/geometries/
      textures all bit-identical to run113 (50/608296/48/17) — expected, config-data-only change
- [x] Teknik borç sayacı — **0** (unchanged; `worldEvents.js` grew to ~186/600, still well under cap)
- [x] `3D_GAME_PROGRESS.md` güncellendi (this entry)
- [x] ADR yazıldı — `DECISIONS.md` ADR-0141 (Risk Seviyesi: LOW, Alternatives Considered, Geri alma
      planı, Gelecek Faz Etkisi all present)
- [x] Commit atıldı (below)
- [x] Konsol Temizliği — zero console/page errors across the full smoke suite re-run and the proof
      script's own two viewports

**Incidental fix (§8.3-adjacent, not a separate sub-task):** `GOVERNANCE.md` §16's deferred-rules
table still described the "30-commit performans trend grafiği" item as merely eligible, even though
run 110's ADR-0137 already implemented it (`scripts/analyzePerfTrend.js`). Corrected the table row
to "✅ Ele alındı (run 110, ADR-0137)" with a one-line summary — a documentation-debt find noticed
while re-reading `GOVERNANCE.md` per the Session Snapshot procedure, same class of fix ADR-0130 set
precedent for. No code/behavior change.

**Yeni soru:** none — a discrete COMMON/UNCOMMON/RARE tier choice under this project's own
already-established rubric (ADR-0110), not a continuous tunable constant needing a real playtest,
same reasoning every prior flavor-only `worldEvents.js` addition (run 83's `traveling_singer`,
run 95's `shackled_prisoner`, etc.) used to skip a `QUESTIONS_FOR_OWNER.md` entry too.

**AI Self-Review 2. Geçiş (§8.3):** independently re-verified — confirmed no existing entry's theme
genuinely overlaps (re-read all 28 prior `desc` strings before writing the ADR's Decision, not
assumed); confirmed the comment-count arithmetic by hand (6+22=28 before, 6+23=29 after — both
comments now internally consistent with each other and with `WORLD_EVENTS.length`); confirmed the
existing gating smoke check's assertions key off fixed id strings, not array length/index, so an
unrelated 29th entry can't spuriously affect them; confirmed the proof script's own seed-search loop
disposes every probe system/bus across up to 5000 iterations (no accumulating listeners) before
creating the real, kept instances. No `TEMP`/`HACK`/`FIXME`/`WORKAROUND`. Memory leak checklist: n/a
(config-data-only in `worldEvents.js`; the proof script is never committed, its own instances are
explicitly disposed).

**Session Quality Gate (§8.6):** confidence **5/5** — low-risk, well-proven pattern (same shape as
ADR-0102/0110/0111/0129/0130), a genuinely distinct new entry verified (not assumed) against every
prior entry's text, zero regression in the full smoke suite, real 2-viewport proof with exact-text
DOM assertions, zero open design ambiguity worth escalating. "6 ay sonra hâlâ net mi" tereddüdü yok
— the ADR spells out exactly which prior entries this was checked against and why each is distinct.

**World Evolution Report:**

| Metric | Before (run start) | After (run 114 end) | Delta |
|---|---|---|---|
| `worldEvents.js` flavor-pool entries | 28 | **29** | +1 (`sellsword_arrival`) |
| Draw calls / triangles / geometries / textures | 50 / 608,296 / 48 / 17 | **unchanged** | 0 (config-data-only) |
| Smoke suite | 33/33 | **33/33** | unchanged (existing check needed no modification) |
| `checkSmokeCheckRegistry.js` | 33 checks/13 modules | **33 checks/13 modules** | unchanged |
| ADR headers in `DECISIONS.md` | 140 | **141** | +1 (ADR-0141) |
| `perf_log.csv` rows | 55 | **56** | +1 (`run114`) |
| Open questions in `QUESTIONS_FOR_OWNER.md` | (unchanged from run 113) | **unchanged** | none added (discrete tier choice, not a calibration constant) |
| World Coverage (desktop / mobile) | 96.2% / 4.5% | 96.2% / 4.5% | değişmedi |
| Tech debt count | 0 | **0** | değişmedi |

**Oyuncu fark eder mi:** evet, ama küçük — dünya olayları bildirim havuzuna (24 saat boyunca
periyodik olarak tetiklenen "kuzgun geldi"/"nöbet değişimi" tarzı kısa bildirimler) bir tane daha
eklendi; oyuncu yeterince uzun oynarsa er ya da geç kale kapısına yaklaşan bir kiralık kılıç
bildirimini görebilir. Dünyanın görsel/3D tarafında hiçbir değişiklik yok.

**Next step for the next run:** fresh `origin/main` fetch and priority re-scan first, per §8.14.
Bloklu kalan her şey değişmedi: 6 kale hâlâ dokusuz, FAZ 6 hayvanları model bekliyor. No file near
the 600-line cap. `RULES_CHANGELOG.md`'nin next consolidation ~run 116 (yaklaşıyor, sıradaki 1-2
çalıştırmada ele alınabilir), `CATCH_UP.md`'nin next summary ~run 118 (last done run 108). A
natural, larger item-14 follow-up (not required, just available): ADR-0140's own deferred
follow-up, growing `PHASE1_PREVIEW_RADIUS_CHUNKS` (or force-grounding specific seats) so Xaro/Night
King qualify for vegetation clustering too — needs its own dedicated perf-measurement sub-task per
`debug/README.md`'s standing note, not attempted half-measured this run.
