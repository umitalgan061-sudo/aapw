...
## ADR-0123: `stannis-guard-2`'s 3rd dialogue choice — the pilot's 9th NPC to use the 3rd slot, 2nd Baratheon seat

**Date:** 2026-08-06 (run 97, scheduled autonomous routine).
...
**Context:** Session Snapshot (GOVERNANCE.md §20) confirmed `GOVERNANCE.md`/`CREDITS.md`/
`CATCH_UP.md`/`RULES_CHANGELOG.md`/`STABLE_TAGS.md`/`QUESTIONS_FOR_OWNER.md` already exist from prior
runs (the incoming scheduled prompt's "first create GOVERNANCE.md" instruction is stale boilerplate
again, run 97 not run 1) — all read in full rather than recreated. Repo state check found local
`main` a stale ref 1 commit behind `origin/main` with local `HEAD` itself already detached at the
correct, current `origin/main` commit (`1147cf0`) — a shallow-clone local-ref quirk, same pattern run
96 diagnosed, not real divergence. `git fetch origin main` + `git checkout -B main origin/main`
resolved it, clean tree, no lost work. Pre-work baseline: `node --check` clean across all `src/`+
`scripts/` files, `checkDialogueChoicesShape.js` OK (13/14 pilot coverage), `checkSmokeCheckRegistry.js`
OK (28 checks/8 modules; 2 known line-count WARNs, `game3d.js` 585/600 and `dragonController.js`
579/600 — both grew slightly since run 89's last-flagged 545/600 for `game3d.js`, still under the cap,
not yet urgent, noted for a future split), full `smokeTestGame3D.js` suite **28/28 PASS**, 0 FAIL.
...
debt/smoke test/world coverage) all healthy per the baseline above — 0 tech debt. Items 12-13
(FAZ 7 dragon follow-ups / FAZ 11 species) remain blocked on models or the resolved-but-uncalibrated
health-system constants (already logged in `QUESTIONS_FOR_OWNER.md`). §8.12 rule consolidation was
just done last run (96), not due again until ~run 116. `CATCH_UP.md`'s next digest due at run 98
(next run). Periodic platform check due ~run 111-121 (not due). That leaves item 14 as the only
actionable lever — ADR-0122's own "Alternatives considered" named `stannis-guard-2` first among the
5 remaining 2-choice NPCs (`stannis-guard-2`/`balon-guard-1`/`robin-guard-1`/`ziya-guard-1`/
`berk-guard-1`).
...
**Decision:** `stannis-guard-2` (the hilltop watchman, whose existing 2 choices cover what he watches
for and his working relationship with `stannis-guard-1`) gets a 3rd choice — "Burada tek başına nöbet
tutmak seni hiç yalnızlaştırıyor mu?" ("Does keeping watch alone up here ever make you feel
isolated?"). Response ("Yalnızlaştırıyor elbette, yabancı. Ama tepeden bakınca kaleyi de,
aşağıdakileri de görürüm — hiç yalnız değilmişim gibi hissettiren de bu.") stays in-tone with the
existing 2 choices while adding a personal-solitude angle genuinely distinct from every other
3-choice NPC's own 3rd-choice theme so far (fear-of-being-caught-off-guard for `umit-guard-1`,
duty/fatigue for `berkalp-guard-1`, suspicion for `twin-guard-1`, regret-over-wit for
`olena-guard-1`, private-conviction for `stannis-guard-1` — its own house-mate's 3rd choice, kept
distinct by asking about isolation rather than doctrinal doubt, fear-under-a-ruler for
`cersei-guard-1`, cost-of-isolation-as-a-region for `doran-guard-1` — Dorne's political isolation,
not a person's loneliness, kept distinct on that basis, restlessness/what-if-I-left for
`xaro-guard-1`). Implemented as one new `Object.freeze({label, response})` entry appended to
`CHOICES_BY_NPC_ID['stannis-guard-2']` in `gameplay/dialogueChoices.js`, plus the file's header
comment updated to record the 9th 3-choice NPC and this ADR. No other file touched —
`interactionConfig.js`'s existing `GREETINGS_BY_NPC_ID` entry, `npcConfig.js`'s spawn entry,
`interaction.js`'s `DIALOGUE_CHOICE_KEY_CODES` handling, and `game3d.js`'s keydown wiring are all
unchanged and already generic across every NPC's choice count.

**Real headless-Chromium proof + real visual proof, 2 moments, zero console/page errors in both**
(dev-only, uncommitted Playwright script, same methodology ADR-0115/117/119/120/121/122 used, deleted
...
were dynamically imported from inside the page and driven against a synthetic `stannis-guard-2` NPC.
(1) All 3 real choice labels render in order ("1) Tepede tam olarak ne arıyorsun?" / "2) Birinci
nöbetçiyle aranız nasıl?" / "3) Burada tek başına nöbet tutmak seni hiç yalnızlaştırıyor mu?"), hint
reads exactly `'1/2/3 - Seç, Esc - Kapat'`. (2) Pressing `Digit3` shows the exact new response text
with `{name}` replaced by the synthetic NPC's display name, hint reverts to `'E / Esc - Kapat'`,
choice list cleared (0 `.g3d-dialogue-box-choice` elements remaining). Both moments' screenshots
additionally caught the real live scene running behind the dialogue box (a dragon mid-attack, health
bar dropping 100→40 between the two moments) — purely incidental to this change (a background
simulation, not something this sub-task touched) but a useful confirmation the rest of the booted
scene keeps running normally alongside the dialogue UI. Zero console/page errors throughout
...
**Alternatives considered:**
- *`worldEvents.js`'s flavor pool instead (27 -> 29 entries).* Rejected, same reasoning ADR-0115/
  117/119/120/121/122 used — still a legitimate future lever, but a 9th NPC reaching the 3rd dialogue
  slot continues the more informative, already-proven-safe proof point, and `stannis-guard-2` was
  already the specifically-named next candidate from ADR-0122.
- *A 3rd fact about the watch/signal-fire system instead.* Rejected — the existing 2 choices already
  cover what he watches for and his relationship to the other watchman; a 3rd near-duplicate would
  read as padding, same reasoning the prior 7 ADRs used to prefer a personal-question 3rd choice.
- *One of the other 4 remaining 2-choice NPCs instead (`balon-guard-1`/`robin-guard-1`/
  `ziya-guard-1`/`berk-guard-1`).* Rejected in favor of `stannis-guard-2` specifically because it was
  ADR-0122's own named next candidate and gives Baratheon its 2nd 3-choice seat (matching the 2-per-
  house pattern already reached by Lannister via `cersei-guard-1`/`twin-guard-1`) — logged here as
  the still-open candidates for a future run.
- *Do nothing this sub-task, stop after the baseline check.* Rejected — Session Quality Gate
  confidence stayed high (existing, well-established pattern, 8 prior ADRs to follow, zero open
  design ambiguity) and the run budget/time cap were nowhere close, so GOVERNANCE.md §19's chaining
  rule applies.

**Consequences:** `stannis-guard-2` becomes the pilot's 9th NPC (of 13 with any dialogue choices, 14
total real NPCs) to use the 3rd dialogue slot, and Baratheon's 2nd seat to do so. Only 4 NPCs remain
at 2 choices (`balon-guard-1`/`robin-guard-1`/`ziya-guard-1`/`berk-guard-1`).
`checkDialogueChoicesShape.js`'s pilot-coverage line is unaffected (still 13/14 — no new NPC gained
an entry, an existing one grew). No lasting performance cost (config-data-only, zero geometry/
listener/timer touch in the committed diff). `dialogueChoices.js` grows to 230 lines, still well
inside the 600-line cap.

**AI Self-Review 2. Geçiş (§8.3):** confirmed the new 3rd choice's angle (personal solitude/isolation)
is genuinely distinct from `stannis-guard-2`'s existing 2 choices (watch duties + relationship to
`stannis-guard-1`) and from every other 3-choice NPC's own 3rd choice theme, including its own
house-mate `stannis-guard-1`'s (private doctrinal conviction, not personal loneliness) — no
near-duplicate. Confirmed the header-comment update's NPC-list prose stays internally consistent
(removed the standalone `stannis-guard-2` line from the "still 2-choice" list, added its own
new-choice sentence in the same run-by-run chronological style the prior 8 entries use, updated the
inline 3-choice-NPC enumeration). No `TEMP`/`HACK`/`FIXME`/`WORKAROUND` anywhere in the diff. Memory
leak checklist: n/a, config-data-only (no new listener/timer/DOM/geometry-material in the committed
diff; the proof script's own `DialogueBox`/`InteractionPrompt` instances lived only inside its own
disposable headless browser context, never committed).

**Session Quality Gate (§8.6):** confidence **5/5** — 9th-generation repeat of an already-proven,
low-risk pattern (config-data-only, fully reversible, real headless-Chromium + real visual proof both
confirm the exact rendered text end-to-end), zero open design ambiguity (ADR-0122 had already
pre-vetted this exact candidate), zero regression in the full smoke suite (28/28 PASS before and
after). "6 ay sonra hâlâ net mi" tereddüdü yok: this ADR records the exact angle picked, why, and the
house-coverage reasoning behind picking this NPC over the other 4 equally-ready candidates.
...
## ADR-0125: `robin-guard-1`'s 3rd dialogue choice — the pilot's 11th NPC to use the 3rd slot, 1st Vale seat
...
**Context:** Session Snapshot (GOVERNANCE.md §20) confirmed `GOVERNANCE.md`/`CREDITS.md`/`CATCH_UP.md`/
`RULES_CHANGELOG.md`/`STABLE_TAGS.md`/`QUESTIONS_FOR_OWNER.md` all already exist from prior runs (the
incoming scheduled prompt's "first create GOVERNANCE.md" instruction is stale boilerplate again, this
is run 99 not run 1) — all read in full rather than recreated. Repo state check: local `main` branch
ref was a stale ref (behind `origin/main`), `HEAD` was detached at `origin/main`'s current commit
(`493bbef`) after `git fetch origin main` — same shallow-clone local-ref quirk runs 96-98 diagnosed,
not real divergence; `main` re-pointed at `origin/main` before any work started. Pre-work baseline:
`node --check` clean across all `src/`+`scripts/` files, `checkDialogueChoicesShape.js` OK (13/14
pilot coverage), `checkSmokeCheckRegistry.js` OK (28 checks/8 modules; same 2 known line-count WARNs
as run 98 — `game3d.js` 585/600 and `dragonController.js` 579/600, unchanged, not yet urgent), full
`smokeTestGame3D.js` suite **28/28 PASS**, 0 FAIL.
...
dragon follow-ups / FAZ 11 species) remain blocked on models or already-logged owner decisions. §8.12
rule consolidation last done run 96, not due until ~run 116. §15 periodic platform check last done
run 91, not due until ~run 111-121. That leaves item 14 as the only actionable lever — ADR-0124's own
"Alternatives considered" named the 3 remaining 2-choice NPCs as `robin-guard-1`/`ziya-guard-1`/
`berk-guard-1` (listed in that order, matching `dialogueChoices.js`'s own file order), so
`robin-guard-1` is picked as the specific next candidate.

**Gelecek Faz Etkisi:** none — still no further branching/state/persistence/stat hook, same "pilot on
a growing subset" scope ADR-0058 established; a future real dialogue-tree/quest system would replace
this mechanism wholesale rather than be constrained by one more leaf choice.

**Decision:** `robin-guard-1` (the Vale's Eyrie seat, whose existing 2 choices cover why the castle
sits so high and whether its eagles really see everything) gets a 3rd choice — "Bu kadar yükseklikte
nöbet tutmak seni hiç ürkütmüyor mu?" ("Doesn't keeping watch this high up ever unsettle you?").
Response ("Ürkütmüyor değil, yabancı. Ay Kapısı'nın altında neyin beklediğini herkes bilir burada —
ama benim asıl korkum düşmek değil, bir gün aşağıyı özlemek.") leans on the Eyrie's own Moon Door
lore while adding a personal-doubt angle genuinely distinct from every other 3-choice NPC's own 3rd
choice theme so far (fear-of-being-caught-off-guard for `umit-guard-1`, duty/fatigue for
`berkalp-guard-1`, suspicion for `twin-guard-1`, regret-over-wit for `olena-guard-1`,
private-conviction for `stannis-guard-1`, fear-under-a-ruler for `cersei-guard-1`,
cost-of-isolation-as-a-region for `doran-guard-1`, restlessness/what-if-I-left for `xaro-guard-1`,
personal-solitude for `stannis-guard-2`, cost-of-a-culture's-creed for `balon-guard-1` — kept distinct
by making this NPC's own doubt a quiet homesickness for lower ground, not fear of falling itself and
not loneliness). Implemented as one new `Object.freeze({label, response})` entry appended to
`CHOICES_BY_NPC_ID['robin-guard-1']` in `gameplay/dialogueChoices.js`, plus the file's header comment
updated to record the 11th 3-choice NPC and this ADR. No other file touched —
`interactionConfig.js`'s existing `GREETINGS_BY_NPC_ID` entry, `npcConfig.js`'s spawn entry,
`interaction.js`'s `DIALOGUE_CHOICE_KEY_CODES` handling, and `game3d.js`'s keydown wiring are all
unchanged and already generic across every NPC's choice count.

**Real headless-Chromium proof + real visual proof, 2 moments, zero console/page errors in both**
(dev-only, uncommitted Playwright script, same methodology ADR-0115/117/119/120/121/122/123/124 used,
run over a local static server rather than `file://`; the real `createInteractionController`/
`DialogueBox`/`InteractionPrompt`/`CHOICES_BY_NPC_ID` modules `game3d.js` itself imports were
dynamically imported from inside the real booted `game3d.html` page and driven through the actual
`update()`/`handleKeyDown()` API against a synthetic `robin-guard-1` NPC; deleted before commit): (1)
all 3 real choice labels render in order ("1) Neden bu kadar yükseğe yerleştiniz?" / "2) Kartallarınız
gerçekten her şeyi mi görür?" / "3) Bu kadar yükseklikte nöbet tutmak seni hiç ürkütmüyor mu?"), hint
reads exactly `'1/2/3 - Seç, Esc - Kapat'`. (2) Pressing `Digit3` shows the exact new response text
with `{name}` replaced by the synthetic NPC's display name ("Robin Muhafız"), hint reverts to
`'E / Esc - Kapat'`, choice list cleared (0 `.g3d-dialogue-box-choice` elements remaining). Zero
console/page errors throughout (`consoleErrors.length === 0`).

**Perf sampling:** `perf_log.csv`'s `run99` row (`46/393231/44/17`) sampled fresh via
`scripts/collectPerfSnapshot.js`, bit-identical to the run76-98 baseline on draw calls/triangles/
geometries/textures — expected, this change touches zero geometry/texture code.

**Alternatives considered:**
- *`ziya-guard-1` or `berk-guard-1` instead.* Rejected in favor of `robin-guard-1` specifically
  because it was ADR-0124's own first-listed remaining candidate and gives the pilot its first Vale
  3-choice seat — a concrete coverage reason over picking arbitrarily among the rest; logged here as
  the still-open candidates for a future run (`ziya-guard-1`/`berk-guard-1`, both Reach seats).
- *A 3rd fact about the Eyrie's eagles/sky cells instead.* Rejected — the existing 2 choices already
  cover the height-as-defense angle and the eagles' watch; a 3rd near-duplicate would read as
  padding, same reasoning the prior 10 ADRs used to prefer a personal-question 3rd choice.
- *`worldEvents.js`'s flavor pool instead (27 -> 29 entries).* Rejected, same reasoning the prior 10
  ADRs used — still a legitimate future lever, but an 11th NPC reaching the 3rd dialogue slot
  continues the more informative, already-proven-safe proof point.
- *Do nothing this sub-task, stop after the baseline check.* Rejected — Session Quality Gate
  confidence stayed high (existing, well-established pattern, 10 prior ADRs to follow, zero open
  design ambiguity) and the run budget/time cap were nowhere close, so GOVERNANCE.md §19's chaining
  rule applies.

**Consequences:** `robin-guard-1` becomes the pilot's 11th NPC (of 13 with any dialogue choices, 14
total real NPCs) to use the 3rd dialogue slot, and the Vale's 1st seat to do so.
`checkDialogueChoicesShape.js`'s pilot-coverage line is unaffected (still 13/14 — no new NPC gained
an entry, an existing one grew). No lasting performance cost (config-data-only, zero geometry/
listener/timer touch in the committed diff). `dialogueChoices.js` grows to 245 lines, still well
inside the 600-line cap. Only 2 NPCs remain at 2 choices (`ziya-guard-1`/`berk-guard-1`), both Reach
seats.

**AI Self-Review 2. Geçiş (§8.3):** confirmed the new 3rd choice's angle (a quiet homesickness for
lower ground) is genuinely distinct from `robin-guard-1`'s existing 2 choices (why the castle sits so
high + whether the eagles see everything) and from every other 3-choice NPC's own 3rd choice theme —
no near-duplicate, and deliberately not a duplicate of "fear of falling" either (the response text
explicitly separates the two: "benim asıl korkum düşmek değil, bir gün aşağıyı özlemek"). Confirmed
the header-comment update's NPC-list prose stays internally consistent (added `robin-guard-1`'s own
new-choice sentence in the same run-by-run chronological style the prior 10 entries use, updated the
inline 3-choice-NPC enumeration). No `TEMP`/`HACK`/`FIXME`/`WORKAROUND` anywhere in the diff. Memory
leak checklist: n/a, config-data-only (no new listener/timer/DOM/geometry-material in the committed
diff; the proof script's own `DialogueBox`/`InteractionPrompt` instances lived only inside its own
disposable headless browser context, never committed).
...
low-risk pattern (config-data-only, fully reversible, real headless-Chromium + real visual proof both
confirm the exact rendered text end-to-end), zero open design ambiguity (ADR-0124 had already
pre-vetted this exact candidate), zero regression in the full smoke suite (28/28 PASS before and
after). "6 ay sonra hâlâ net mi" tereddüdü yok: this ADR records the exact angle picked, why, and the
house-coverage reasoning behind picking this NPC over the other 2 equally-ready candidates.
...
## ADR-0130: `market_day` — the pilot's first UNCOMMON+day-gated world event

**Status:** Accepted (run 103).

**Risk Seviyesi:** LOW — one additive flavor-data entry plus additive regression assertions extending the existing ADR-0111 gating check; no world/save/render-state mutation, no shared module touched beyond the check file that already tests this exact mechanism.

**Context:** Fresh session/context (new scheduled firing). `GOVERNANCE.md`, `3D_GAME_PROGRESS.md`'s run 102 entry, `DECISIONS.md`'s last 3 ADRs (0127-0129), and `QUESTIONS_FOR_OWNER.md` all read first per the Session Snapshot procedure; `GOVERNANCE.md`/`CREDITS.md`/`CATCH_UP.md`/`RULES_CHANGELOG.md`/`STABLE_TAGS.md`/`QUESTIONS_FOR_OWNER.md` all confirmed already present, not recreated. `ARCHITECTURE.md` not re-read (last touched 2026-08-05, under the 7-day threshold). `git fetch origin main` found the repo already at `origin/main`'s tip (`1a361b4`, run 102's ADR-0129 merge) with a clean working tree — no realignment needed before starting.

**Baseline regression guard:** full `node --check` sweep (`src/`+`scripts/`) — clean. Full `scripts/smokeTestGame3D.js` — **29/29 PASS**, 0 FAIL, before any new code. `checkSmokeCheckRegistry.js` OK (29 checks/9 modules; same 2 known line-count WARNs as run 102 — `game3d.js` 587/600 and `dragonController.js` 579/600, unchanged, still under the cap, not yet urgent).

**Priority re-scan (GOVERNANCE.md §18):** items 1-3 (terrain macro relief/road network/ground color) remain DONE per their own standing safety-check guards. Item 4 (castle texturing, 7/14) still blocked on real models for the remaining 6 seats, unchanged since run 96. Items 5-11 all healthy (0 tech debt, smoke suite green, coverage unchanged). Item 12 (dialogue 3rd-choice pilot) is exhausted per run 101/ADR-0128 (13/13 choice-enabled NPCs, `jon-guard-1` deliberately excluded per ADR-0058). Items 12-13 (dragon follow-ups / FAZ 11 species) remain blocked on models or already-logged owner decisions in `QUESTIONS_FOR_OWNER.md`. That left `worldEvents.js`'s flavor pool (item 14, following run 102's own precedent) as the only actionable, non-duplicate lever this run.

**Gelecek Faz Etkisi:** none — purely additive flavor content and its own regression coverage, no new system, no branching/state/persistence hook. A future FAZ 8 expansion (e.g. a per-season event calendar) would read this same `WORLD_EVENTS` array shape.

**Decision:** Add `market_day` — an UNCOMMON-rarity, `timeOfDay: 'day'`-gated event ("Pazar Günü" / "Kale meydanında pazar kuruldu; tüccarlar mallarını gün ışığında sergiliyor, pazarlık sesleri yankılanıyor."). Before this run, the pool's 5 gated entries covered COMMON+night (`wolf_howl`), UNCOMMON+night (`falling_star`), RARE+night (`northern_lights`), RARE+day (`eclipse`), and COMMON+day (`harvest_wagons`, ADR-0129) — leaving UNCOMMON+day as the one gated rarity/time-of-day combination with zero coverage. A bustling market is a genuinely daytime-only phenomenon by real-world convention (nobody trades goods in a pitch-dark castle square) and a specific, notable occurrence rather than routine ambiance, matching this project's own UNCOMMON definition (ADR-0110) — distinct from `harvest_wagons` (grain delivery, COMMON, routine), `trade_caravan` (an approaching, ungated caravan, not yet arrived), and `iron_bank` (a debt-collector visit, ungated, ominous-adjacent rather than festive). Implemented as one new `Object.freeze`-array entry appended to `WORLD_EVENTS` in `gameplay/worldEvents.js`. Also fixed two comments in the same file that had gone stale after run 102's `harvest_wagons` addition without being updated then (a documentation-debt find, not a behavior change): the header comment's hardcoded "4 entries" (now expressed as "currently 6 of 28", explicitly noted as relative so a future addition doesn't repeat the same staleness) and the `pickWeightedEvent` fallback comment's "22 of 26" (now "22 of 28" — the ungated count itself, 22, was unchanged by either run 102's or this run's addition, only the gated/total counts moved).

**Real headless-Chromium proof + real visual proof, 2 moments, zero console/page errors in both** (dev-only, uncommitted proof script written to the session scratchpad — never inside the repo tree, so no delete-before-commit step was needed — run over a local static server via `devServerHelper.js`): the real `game3d.html` was booted, the live `createWorldEventSystem`/`EventBus`/`WorldEventToast` modules dynamically imported and driven through their real APIs. (1) Desktop 1280×720: searched seeds until one produced `market_day` as the first draw at forced solid noon (`nightFactor=0`), re-emitted it through the real bus, and confirmed the real toast DOM contains the exact title "Pazar Günü" and the exact description text; a fresh 1000-draw forced-midnight (`nightFactor=1`) run on a separate seed never once produced `market_day`. (2) Mobile 390×844: the same deterministic seed's first noon draw rendered the same real toast; its bounding rect sits at `top:64px` (below the persistent top HUD row, matching ADR-0129's mobile-safe placement rule) with no overlap against the "← 2D Haritaya Dön" control. Zero console/page errors throughout both moments (`consoleErrors.length === 0`).

**Verification:** full JavaScript syntax sweep (`worldEvents.js`, `game3dSmokeChecksDebugTools.js`) clean. Full smoke suite re-run: **29/29 PASS**, 0 FAIL (before-baseline also 29/29 PASS — regresyon yok); the existing `world-event day/night gating (ADR-0111)` check was extended in place (not duplicated) with `noonFiresMarketDay`/`midnightNeverFiresMarketDay` assertions alongside its existing `eclipse`/`harvest_wagons` ones, so the smoke registry stays at 29 checks/9 modules — no new check module needed since this is the same mechanism ADR-0111/ADR-0129 already exercise. `checkPwaInstallability.js` and `checkServiceWorkerCache.js` both pass unchanged (no new asset, no new file).

**Perf sampling:** `perf_log.csv`'s `run103` row (`46/393231/44/17`) sampled fresh via `scripts/collectPerfSnapshot.js`, bit-identical to the run76-102 baseline on draw calls/triangles/geometries/textures — expected, this change touches zero geometry/texture/renderer code.

**Alternatives considered:**
- *A second RARE+night entry instead.* Rejected — `northern_lights` already covers RARE+night; a second entry in an already-covered slot would read as padding rather than filling a genuinely uncovered role, unlike UNCOMMON+day.
- *Leaving `market_day` ungated (always-eligible, like `trade_caravan`).* Rejected — the event's own text ("gün ışığında", "in daylight") is as unambiguous about timing as `harvest_wagons`'s was in ADR-0129; leaving it ungated would let a market bustle fire at midnight, undermining the same day/night immersion ADR-0111 introduced this whole mechanism to protect.
- *A COMMON market/trade event instead of UNCOMMON.* Rejected — a specific market day is a notable occurrence a player would remark on, not routine background ambiance like `guard_change`/`blacksmith_hammer`; UNCOMMON matches the project's own tier definition (ADR-0110) more honestly, and also happens to fill the one empty gated-rarity slot.
- *Leaving the two stale comments alone (out of scope).* Rejected — both were directly about the exact array this ADR modifies and would have become more wrong (not less) if left unfixed while adding a 6th gated entry; fixing them cost one line each and prevents a future run from re-discovering the same staleness `berk-guard-1`'s header-comment fix (ADR-0128) already set a precedent for doing inline.

**Consequences:** the flavor pool grows to 28 entries (27→28); the gated-rarity coverage matrix now has all 6 (rarity × time-of-day) combinations that make sense represented at least once (COMMON+night, COMMON+day, UNCOMMON+night, UNCOMMON+day, RARE+night, RARE+day) — a natural stopping point for this specific "fill the coverage gap" lever, distinct from simply padding the pool further. No lasting performance cost (config-data-only, confirmed by the bit-identical perf row). `worldEvents.js` grows to ~183 lines, `game3dSmokeChecksDebugTools.js` grows by 3 lines — both nowhere near the 600-line cap.

**AI Self-Review 2. Geçiş (§8.3):** confirmed `market_day`'s theme (a festive, daylight-bound commercial gathering) is genuinely distinct from every other entry's theme, including the two other trade-adjacent ones (`trade_caravan` — en route, not yet arrived, ungated; `iron_bank` — a debt collector, ominous rather than festive, ungated) and the other COMMON+day entry (`harvest_wagons` — grain delivery to castle stores, not a public market). Confirmed the coverage-gap reasoning is real (verified by re-reading all 5 pre-existing gated entries' rarity/timeOfDay pairs before writing the Decision, not assumed). No `TEMP`/`HACK`/`FIXME`/`WORKAROUND`. Memory leak checklist: n/a, config-data-only in `worldEvents.js`; the proof script's own `WorldEventToast`/`EventBus` instances were either explicitly disposed or left to the page-close teardown (documented inline, no repo-tracked leak since the script itself is never committed).
...
## ADR-0132: Extract `gameLoopHelpers.js` from `game3d.js` (tech-debt/line-cap refactor, no behavior change)

**Status:** Accepted (run 105).

**Risk Seviyesi:** LOW — pure code relocation, no gameplay/world/save/UI behavior change; each moved function already took every input as a parameter, so no closure semantics changed.

**Context:** ADR-0131 (run 104) left `game3d.js` at 590/600 lines and flagged that it "must not receive another feature wiring increment before an approved extraction/refactor" — GOVERNANCE.md's Golden Rule #7 (600-line file cap) and §12's Dinamik Öncelik Ayarı (tech debt rising across runs triggers a priority bump) both point at the same file. Priority re-scan this run found items 1-3 of GOVERNANCE.md §18 already complete (macro relief ADR-0075, road network ADR-0076, terrain vertex-color grass/rock blend already in `terrain.js`) and item 4 (texturing the remaining 6 procedural-only kingdom seats) blocked on new castle-shaped CC0/CC-BY model assets not present in this environment (`CASTLE_MODEL_ASSIGNMENTS` in `world/settlements.js` already covers the 8 seats that do have a real model, each already textured via `createStoneMaterial`) — not something a scheduled routine should attempt to source new binary assets for without owner review. That left item 9 (teknik borç) as the next genuinely actionable item, and `game3d.js`'s near-cap size was this run's most concrete, already-flagged instance of it.

**Decision:** move `computeCameraRelativeMove`, `combineAxes`, `collectCameraCollidables`, `streamAroundOrbitTarget`, and `bindResize` — all five are pure functions that take every dependency as a parameter, none close over `initGame3D`'s local scope — into a new `gameLoopHelpers.js` module, and have `game3d.js` import them. Also dropped `game3d.js`'s now-unused `THREE`, `CHUNK_CONFIG`, and `worldToChunkCoord` imports (each was only referenced by the relocated functions). `game3d.js` drops from 590 to 471 lines, `gameLoopHelpers.js` is a new 142-line file — both comfortably under the cap with real headroom for the next feature. Doc-comment cross-references in `sceneManager.js`, `camera.js`, `input.js`, and `ui/README.md` that pointed at these functions "in `game3d.js`" were updated to point at `gameLoopHelpers.js`; `ARCHITECTURE.md`'s per-module entries were updated the same way (its historical run-log-style entries in `3D_GAME_PROGRESS.md`/`DECISIONS.md` were deliberately left as-is — those are point-in-time records, not living documentation).

**Alternatives considered:** extracting the tick loop's body (or the `pagehide` disposal block) instead was rejected — both close over a large number of `initGame3D`'s locals (`state`, `handleInteractKeyDown`, `frameId`, `unsubscribePlayerDied`, `spawnWorld`, …), so pulling either out would mean either a large parameter-passing surface or turning `initGame3D` itself into a stateful object, a much higher-risk change than a mechanical function move for the same line-count benefit. Leaving `game3d.js` at 590/600 and simply not adding to it further was rejected because the file already needed one more constant-listener wiring line for any future controls/UI addition per ADR-0131's own note, and 10 lines of margin was not a durable buffer.

**Verification:** full repo `node --check` sweep clean (including both touched and new files). `checkSmokeCheckRegistry.js`: 70 files all within the 600-line cap (previously 69 files/2 near-cap; `game3d.js`'s WARN is gone, `dragonController.js`'s pre-existing 579/600 WARN is unrelated and untouched), 30 checks/10 modules unchanged. Full real-browser `smokeTestGame3D.js` suite: **30/30 PASS**, zero console/page errors, including the `game3d.html` GAME_READY/phase1-scene boot check and every gameplay-behavior check that exercises the tick loop this refactor's functions run inside. `collectPerfSnapshot.js` `run105` row: `46` draw calls / `393231` triangles / `44` geometries / `17` textures — bit-identical to `run103`'s baseline, confirming the relocation changed no rendered output. No separate before/after screenshots taken: this is an invisible internal refactor (identical function bodies, identical call sites, identical render output per the perf snapshot above), so the smoke suite's real WebGL boot + the bit-identical triangle/draw-call counts are the applicable evidence, not a visual diff — GOVERNANCE.md §8.5's "kanıt yetersizse DONE sayılmaz" standard is about visual changes lacking visual proof, not about manufacturing screenshots for a change with no visual delta to show.

**Etkilenen sistemler:** `game3d.js`, new `gameLoopHelpers.js`; doc-only touches to `sceneManager.js`, `camera.js`, `input.js`, `ui/README.md`, `ARCHITECTURE.md`. No gameplay, world-generation, save, or asset systems touched.

**Gelecek Faz Etkisi:** `game3d.js` now has ~130 lines of headroom before the next required split — the next feature wired into its bootstrap/tick loop (e.g. a FAZ 6 animal species, FAZ 7 combat follow-up) can land without an immediate forced refactor. `gameLoopHelpers.js` itself has no import from `game3d.js`, so it's safe for any future second render-loop entry point to reuse without risk of a cycle.
...
## ADR-0134: Day/night clock for FAZ 8 discoverability, plus a root-cause fix for a real mobile HUD collision it exposed

**Status:** Accepted (run 107).

**Risk Seviyesi:** LOW (new widget) / MEDIUM for the CSS `box-sizing` change (touches every fixed-position HUD widget's rendered box on `game3d.html`) — no gameplay/world/save/renderer behavior changes either way; fully covered by the existing + extended real-browser smoke suite plus fresh visual proof at both viewports.

**Context:** the world has run a real, seeded day/night cycle since early on, and `gameplay/worldEvents.js` has gated flavor events by it since ADR-0111, but no on-screen element ever told the player what time it actually was — the same discoverability gap `settlementCompass.js`/`controlsHelp.js` (runs 104/106) already closed for direction and controls. Priority re-scan this run found items 1-11 of GOVERNANCE.md §18 clean (full `node --check` sweep clean; 31/31 real-browser smoke suite green before any change — this run's environment did not hit the readiness timeout runs 104/106 saw), item 4 (remaining 6 kingdom-seat textures) still asset-blocked, and items 12-13 (FAZ 7 dragon follow-ups, FAZ 11 species) still model-blocked — leaving a fresh FAZ 8 discoverability lever as the best-scoped, non-duplicate next sub-task, following the exact run-104/106 precedent.

While writing this run's own required visual proof (`GOVERNANCE.md` §8.5) for the new clock's placement on a 390×844 mobile viewport, a real, pre-existing defect surfaced: `gameplay/worldEvents.js`'s toast, while shown, overlapped **both** the health bar (top-right) and the settlement compass (also top-right, second row) — not merely the new clock this run was adding. Root cause: `game3d.css` never declared `box-sizing` (unlike `style.css`, the 2D page's own stylesheet, which already resets to `border-box`), so every `width: min(Npx, calc(100vw - Mpx))` / safe-area position in the file — all written assuming a border-box model — was actually computed against content-box width, then silently grew past its intended viewport-safe bound by each element's own padding+border. On the toast specifically this inflated its real rendered width from an intended 360px to ~392px on a 390px-wide viewport. Separately (not fully explained by the box-sizing bug alone), ADR-0129's own mobile toast-placement fix (run 102) only ever checked clearance against the back-link — it never accounted for the health bar (already existing at that time) or the settlement compass (added later, run 106) sharing the same top-right band, so a two-line toast description could still reach down far enough to clip the second HUD row.

**Decision:** two changes, one commit, because the second was only discovered by properly testing the first:
1. Add `DayNightClock` (`ui/dayNightClock.js`) — a disposable, pointer-inert widget mirroring `SettlementCompass`'s shape: renders a 24-hour HH:MM readout plus a day/twilight/night icon from `lighting.js`'s own `updateDayNightLighting()` return value (`timeRatio`/`nightFactor`), throttling DOM writes to "the displayed game-minute changed" (same reasoning `SettlementCompass`'s 10m distance bucket already uses). Placed top-left (mirroring the compass's top-right slot), below the always-present back-link. The day/night icon thresholds are a small **local duplicate** of `worldEvents.js`'s own `NIGHT_THRESHOLD`/`DAY_THRESHOLD` (0.6/0.15) rather than an import — `ui/README.md`'s "no camera/gameplay imports" blast-radius convention forbids a `ui/` module importing from `gameplay/`, and `mulberry32`'s own cross-file duplication (in `worldEvents.js`, `animals.js`, `npc.js`) already establishes "duplicate a tiny constant across this exact folder boundary" as this project's precedent over violating it.
2. Add `*, *::before, *::after { box-sizing: border-box; }` to the top of `game3d.css` — the single fix that makes every existing `min()`/`max()`/`calc()` width and position in the file behave as its own authors' comments already describe, not a widget-by-widget patch. Also moved the mobile (`<=600px`) event-toast's `top` from ADR-0129's `64px` (back-link-only clearance) to `184px`, clearing the taller of the two second-row widgets (the compass, ~48px tall, at `top:122px`) with a 14px margin — found and verified via real measured bounding boxes, not assumed.

**Alternatives considered:** patching only the toast's own width calc (e.g. `calc(100vw - 56px)`) instead of fixing `box-sizing` globally was rejected — it would have papered over this one instance while leaving every other widget's `min()`/`max()` calc silently over-budget by its own padding, the same latent-bug shape that just produced this collision, for the next widget added at a narrow viewport. Leaving the toast at `64px` and instead moving the compass/clock further down was rejected — the toast is transient (~6s) while the compass/clock are always-visible; better to give the always-visible pair the higher screen-space priority (matching ADR-0129's own precedent of moving the *toast*, not the always-on back-link, to fix the first version of this collision class).

**Verification:** full JS syntax sweep clean (CSS has no `node --check` equivalent; verified via the real-browser proof below instead). `checkSmokeCheckRegistry.js`: 32 checks/12 modules (added `game3dSmokeChecksDayNightClock.js`), `dragonController.js`'s pre-existing 579/600 WARN unchanged, unrelated. `checkServiceWorkerCache.js`/`checkPwaInstallability.js` both pass with the new `ui/dayNightClock.js` precache entry (`SHELL_CACHE` bumped v8→v9). Full real-browser `smokeTestGame3D.js`: **32/32 PASS**, 0 FAIL, zero console/page errors — including a new dedicated `dayNightClock.js` check (noon/midnight/twilight icon selection, an arbitrary HH:MM resolution, sub-minute-change throttling, out-of-range-ratio normalization, mobile-viewport fit, dispose) and the extended `checkWorldEvents` toast-lifecycle check, which now also instantiates real `HealthBar`/`SettlementCompass`/`DayNightClock` instances and asserts the mobile toast clears all three (not only the back-link, ADR-0129's original scope) — closing the exact gap that let this collision go undetected across runs 102-106. Dev-only, uncommitted real-browser visual proof (uncommitted script, deleted before commit — real `game3d.html` booted to `GAME_READY`, both 1280×720 and 390×844 viewports): before the fix, the toast visibly overlapped the health bar and compass at 390×844 (measured bounding-box overlap `true` against both); after, all four widgets (back-link, health bar, compass, clock) plus a real triggered toast render with zero measured overlaps and zero console/page errors on both viewports; the clock itself showed a real, live HH:MM value with the icon matching the visible dawn-lit sky state (`WORLD_DEFAULTS.START_TIME_OF_DAY_RATIO = 0.3`).

**AI Self-Review 2. Geçiş (§8.3):** confirmed the local threshold duplication in `dayNightClock.js` is deliberate (blast-radius rule, not an oversight) and documented as such in both files. Confirmed the `box-sizing` reset doesn't change any *intended* rendered width — every affected `min()`/`max()`/`calc()` expression's own comments already describe a border-box-shaped safe-area budget; this fix makes the code finally do what its comments already claimed. No `TEMP`/`HACK`/`FIXME`/`WORKAROUND`. Memory leak checklist: `DayNightClock` owns no listeners/timers/GPU resources, `dispose()` removes its one DOM subtree — same shape as `SettlementCompass`.

**Gelecek Faz Etkisi:** future HUD widgets on `game3d.html` now render at their actually-intended box size by default, closing an entire latent-bug class rather than one instance of it. A future quest log or minimap can reuse the same top-left/top-right corner convention this run's placement follows.

**Session Quality Gate (§8.6):** confidence **5/5** — the new widget is a low-risk, well-proven pattern (third of its kind after `settlementCompass.js`/`controlsHelp.js`); the collision it exposed was root-caused (not papered over), fixed at its actual source, and covered by a strengthened regression check so the same class of bug cannot silently recur a third time. "6 ay sonra hâlâ net mi" tereddüdü yok — both the widget's own reasoning and the box-sizing root cause are spelled out above.

**Geri alma planı:** `git revert` the single commit — removes `ui/dayNightClock.js`, its wiring/styles/cache entry/smoke check, the `box-sizing: border-box` reset, and the mobile toast `top` offset change (reverting to ADR-0129's `64px`, which would restore the collision this ADR fixes — a deliberate note for whoever reverts, not a silent regression).

## ADR-0135: Close `gameplay/README.md` and `world/README.md` documentation drift (13 undocumented files)

**Status:** Accepted (run 108).

**Risk Seviyesi:** LOW — documentation-only change; zero `.js` files touched, zero behavior/render/API change.

**Context:** priority re-scan this run (§18) found items 1-11 clean (full `node --check` sweep clean; 32/32 real-browser smoke suite green, no readiness timeout this run) and items 4/12/13 still asset/model-blocked, unchanged. Before picking a fresh item-14 feature (the run-104/106/107 discoverability-widget precedent), this run's Session Snapshot step read `gameplay/README.md` and `world/README.md` in full — the same "read the widget list before picking, don't assume" discipline ADR-0134 itself established — and found both had drifted well behind the actual `src/3d/gameplay/`/`src/3d/world/` directory contents. `gameplay/README.md`'s "Files" section documented only 6 of the folder's 17 `.js` files (missing `playerConfig.js`, `npcConfig.js`, `animalConfig.js`, `dragonConfig.js`, `interactionConfig.js`, `dialogueChoices.js`, `health.js`, `dragons.js`, `dragonController.js`, `dragonFlightMath.js`, `dragonSpawns.js`, `creatureSpeciesConfig.js` — an entire FAZ 7 dragon subsystem, this project's first health system, and the FAZ 11 species registry, all undocumented). `world/README.md` was missing `roads.js` and `roadPathfinder.js` — the FAZ priority-item-2 road network. This is exactly the kind of gap the project's own multi-run history keeps warning about (e.g. ADR-0107's "13/14" NPC-dialogue false-gap that several runs re-investigated before run 83 closed it): every future run's Session Snapshot reads these READMEs to orient itself before picking a sub-task, so an incomplete file list risks a future run re-deriving already-written reasoning from scratch, or worse, missing that a subsystem (like the dragon attack/health system) already exists at all.

**Decision:** add one README entry per undocumented file, in both folders, each summarizing the file's real exported API/purpose and its governing ADR — written from actually reading each file's own header/exports (not guessed), matching every existing entry's own level of detail. Also fixed one already-present, now-confirmed-stale line while in the same file: `interaction.js`'s README entry said `choicesByNpcId` covered "2 of 14 NPCs" (a run-44-era number, `dialogueChoices.js`'s own header shows the pilot has grown NPC-by-NPC across runs 44-99); the real, verified-live count (`node -e "import(...)"` against `dialogueChoices.js`'s actual `CHOICES_BY_NPC_ID` export) is 13 of 14 — corrected with a pointer to why the 14th (`jon-guard-1`) is deliberately excluded (ADR-0058/GOVERNANCE.md §17), not left as an apparent gap. No `.js` file, config value, or runtime behavior changed.

**Alternatives considered:** picking a new item-14 feature instead (continuing the discoverability-widget streak) was rejected for this run specifically because the doc-drift gap was found *while* doing that feature's own required "read the existing widget list first" step — fixing a real, freshly-discovered gap in the same session it was found beats deferring it to a hypothetical future run that might not re-discover it the same way (the exact failure mode `ADR-0107` already illustrates for a different kind of gap). A partial fix (only the folder with the larger gap, `gameplay/`) was rejected — `world/README.md`'s missing road-network entry is smaller in file count but arguably higher-value to have documented, since roads are GOVERNANCE.md §18 item 2, a completed-but-easy-to-re-investigate priority item.

**Verification:** `node scripts/checkSmokeCheckRegistry.js` re-run post-change: unchanged 32 checks/12 modules, unchanged file-cap scan (`dragonController.js`'s pre-existing 579/600 WARN, no other file near the cap) — confirms the README edits didn't touch anything the registry scans. Full real-browser `smokeTestGame3D.js`: **32/32 PASS**, 0 FAIL, zero console/page errors (baseline re-confirmed both before and after this run's own change, per GOVERNANCE.md §8.14's concurrency-guard pattern applied to a regression guard instead). `collectPerfSnapshot.js` `run108` row: 46 draw calls / 393,231 triangles / 44 geometries / 17 textures — bit-identical to the run76-107 baseline, as expected for a documentation-only change. No visual proof taken — this change has no rendered-UI surface at all (unlike ADR-0132's code-only refactor, which at least changed what module owns running code; this changes only prose), so `git diff --stat` (2 files, `+103/-9` lines, both `.md`) is the applicable evidence, not a screenshot.

**AI Self-Review 2. Geçiş (§8.3):** confirmed every new README entry's claimed export names/option shapes were copied from the real file (not the old, possibly-stale doc comments elsewhere) — cross-checked `dragonSpawns.js`/`roads.js`/`roadPathfinder.js`'s exports via `grep -n "^export"` rather than assuming names from prose. Confirmed the corrected "13 of 14" NPC count against the live `CHOICES_BY_NPC_ID` object, not against any other doc's own possibly-stale claim. No `TEMP`/`HACK`/`FIXME`/`WORKAROUND`. Memory leak checklist: n/a — no runtime code touched.

**Etkilenen sistemler:** `src/3d/gameplay/README.md`, `src/3d/world/README.md` only. No `.js`, `.css`, config, or asset-manifest changes.

**Gelecek Faz Etkisi:** every future run's Session Snapshot step now sees a complete, accurate file list for both folders — reduces the risk of a future run re-deriving already-solved reasoning (e.g. why `dragonFlightMath.js`'s arithmetic order is load-bearing) from the code alone, or overlooking that a subsystem (health, dragon attack, species registry, roads) already exists.
