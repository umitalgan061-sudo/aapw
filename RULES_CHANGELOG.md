...
- **Run 297** — Observed that the `checkRun2xx*Detail.js` contract scripts in the pindex series assert only that certain strings appear in a source file; they never execute the module. Run297's `checkRun297BaseFieldHD.mjs` is the first in the series to be a behavioural test (18 executable numeric checks, pinned determinism checksum). Recommend future detail/terrain work follow the behavioural pattern rather than the source-grep one. No GOVERNANCE.md clause changed.
