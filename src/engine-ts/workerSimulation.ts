...
type ResponseResolver = (response: SimulationWorkerResponse) => void;
...
export class SimulationWorkerHost implements Disposable {
  private readonly wasm:WasmHotPath;
  private readonly queue:SimulationWorkerCommand[]=[];
  private readonly waiters=new Map<number,ResponseResolver>();
  private readonly maxQueue:number;
  private readonly maxBatchSamples:number;
  ...
  public dispatch(command:SimulationWorkerCommand):Promise<SimulationWorkerResponse>{
    if(this.disposed)return Promise.resolve({type:'error',requestId:command.requestId,code:'WORKER_DISPOSED',message:'worker host disposed'});
    if(this.queue.length>=this.maxQueue){this.failed+=1;return Promise.resolve({type:'error',requestId:command.requestId,code:'QUEUE_OVERFLOW',message:'simulation worker queue is full'});}
    if(this.waiters.has(command.requestId))return Promise.resolve({type:'error',requestId:command.requestId,code:'DUPLICATE_REQUEST_ID',message:`request ${command.requestId} is already pending`});
    this.received+=1;this.queue.push(command);this.peakQueue=Math.max(this.peakQueue,this.queue.length);
    const promise=new Promise<SimulationWorkerResponse>(resolve=>this.waiters.set(command.requestId,resolve));
    void this.pump();
    return promise;
  }
  ...
  public dispose():void{
    ...
    this.disposed=true;
    this.queue.length=0;
    for(const [requestId,resolve] of this.waiters){resolve({type:'error',requestId,code:'WORKER_DISPOSED',message:'worker host disposed'});}
    this.waiters.clear();
    this.wasm.dispose();
  }
  ...
  private async pump():Promise<void>{
    ...
    try{
      while(this.queue.length>0&&!this.disposed){
        ...
        const resolve=this.waiters.get(command.requestId);
        if(resolve){this.waiters.delete(command.requestId);resolve(response);}
      }
    }finally{this.processing=false;}
  }

  private async handle(command:SimulationWorkerCommand):Promise<SimulationWorkerResponse>{
    try{
      switch(command.type){
        case'init':{const capabilities=await this.wasm.initialize();this.completed+=1;return{type:'ready',requestId:command.requestId,capabilities};}
        case'sample-height':{const coordinates=new Float32Array(command.coordinates);if(coordinates.length/2>this.maxBatchSamples)throw new Error(`batch exceeds ${this.maxBatchSamples} samples`);const result=this.wasm.sampleHeightBatch({coordinates,seed:command.seed} satisfies TerrainBatch);this.completed+=1;return{type:'sample-height-result',requestId:command.requestId,heights:result.heights.buffer.slice(0),backend:result.backend};}
        case'transform-points':{const points=new Float32Array(command.points);if(points.length/2>this.maxBatchSamples)throw new Error(`batch exceeds ${this.maxBatchSamples} points`);const result=this.wasm.transformPoints({points,originX:command.originX,originZ:command.originZ,scale:command.scale} satisfies PointCloudBatch);this.completed+=1;return{type:'transform-points-result',requestId:command.requestId,points:result.points.buffer.slice(0),backend:result.backend};}
        case'health':this.completed+=1;return{type:'health-result',requestId:command.requestId,capabilities:this.wasm.capabilities(),queues:this.stats()};
        case'dispose':this.completed+=1;this.dispose();return{type:'disposed',requestId:command.requestId};
        default:return assertNever(command);
      }
    }catch(error){this.failed+=1;return{type:'error',requestId:command.requestId,code:'COMMAND_FAILED',message:error instanceof Error?error.message:'simulation worker command failed'};}
  }
}
...
export class TypedSimulationWorkerClient implements SimulationWorkerClient {
  ...
  private sequence=1;
  private waiting=new Map<number,(response:SimulationWorkerResponse)=>void>();
  private disposed=false;
  private _ready=false;
  private _backend:'wasm'|'typescript'='typescript';
  ...
  public get backend():'wasm'|'typescript'{return this._backend;}
  ...
  public dispose():void{if(this.disposed)return;this.disposed=true;for(const [requestId,resolve] of this.waiting){resolve({type:'error',requestId,code:'CLIENT_DISPOSED',message:'client disposed'});}this.waiting.clear();this.transport.removeEventListener?.('message',this.onMessage);this.transport.removeEventListener?.('error',this.onError);}
  ...
  private readonly onError=():void=>{for(const [requestId,resolve]of this.waiting){resolve({type:'error',requestId,code:'WORKER_ERROR',message:'worker transport error'});}this.waiting.clear();this._ready=false;};
}
