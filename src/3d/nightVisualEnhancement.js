...
const NIGHT_CINEMATIC_DEEP_SKY = new THREE.Color(0x607b9f);
const NIGHT_CINEMATIC_TWILIGHT_GROUND = new THREE.Color(0x4d5146);
const NIGHT_CINEMATIC_DEEP_GROUND = new THREE.Color(0x29322f);
...
export function installNightVisualEnhancement(hemisphere) {
	...
	const fill = new THREE.HemisphereLight(
		NIGHT_CINEMATIC_TWILIGHT_SKY,
		NIGHT_CINEMATIC_TWILIGHT_GROUND,
...
export function updateNightVisualEnhancement(hemisphere, nightFactor) {
	...
	// Preserve gameplay readability without the previous flat blue-grey wash: intensity rises with
	// darkness, but deep night loses a little ambient energy as the directional moon becomes the key.
	const readability = THREE.MathUtils.lerp(
		NIGHT_CINEMATIC_DAY_INTENSITY,
		NIGHT_CINEMATIC_FULL_INTENSITY,
		night,
	);
	fill.intensity = readability * THREE.MathUtils.lerp(1, 0.88, deepNight);
