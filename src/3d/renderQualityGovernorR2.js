...
export const RENDER_QUALITY_POLICY = freeze({
  id: 'render-quality-governor-r2-2026-09-16',
  targetFrameMs: 16.67,
  softFrameMs: 20,
  hardFrameMs: 28,
  recoveryFrameMs: 14.5,
  stepDownHoldFrames: 8,
  stepUpHoldFrames: 45,
  maxHistory: 120,
  minPixelRatio: 0.7,
  maxPixelRatio: 2,
  thermalCritical: 0.92,
  thermalHot: 0.78,
  batteryLow: 0.18,
  batteryCritical: 0.08,
  hiddenCooldownFrames: 12,
});
...
const tierConfig = freeze({
  ultra: freeze({ pixelRatio: 1, shadows: 4096, foliage: 1, water: 96, postFx: 1, temporal: 1, animation: 1 }),
  high: freeze({ pixelRatio: 0.92, shadows: 2048, foliage: 0.84, water: 72, postFx: 1, temporal: 1, animation: 1 }),
  balanced: freeze({ pixelRatio: 0.82, shadows: 1024, foliage: 0.68, water: 48, postFx: 0.65, temporal: 1, animation: 0.9 }),
  performance: freeze({ pixelRatio: 0.72, shadows: 512, foliage: 0.5, water: 32, postFx: 0, temporal: 0, animation: 0.78 }),
  compatibility: freeze({ pixelRatio: 0.7, shadows: 256, foliage: 0.32, water: 20, postFx: 0, temporal: 0, animation: 0.62 }),
});

export function normalizeRenderTelemetry(raw = {}) {
  return freeze({
    frameMs: Math.max(0, finite(raw.frameMs, RENDER_QUALITY_POLICY.targetFrameMs)),
    gpuMs: Math.max(0, finite(raw.gpuMs, 0)),
    cpuMs: Math.max(0, finite(raw.cpuMs, 0)),
    drawCalls: integer(raw.drawCalls, 0),
    triangles: integer(raw.triangles, 0),
    memoryPressure: clamp(raw.memoryPressure),
    thermalPressure: clamp(raw.thermalPressure),
    batteryLevel: clamp(raw.batteryLevel, 0, 1),
    batteryCharging: Boolean(raw.batteryCharging),
    hidden: Boolean(raw.hidden),
    visible: raw.visible !== false,
    inputActive: Boolean(raw.inputActive),
    qualityBias: clamp(raw.qualityBias, -1, 1),
    timestamp: Math.max(0, finite(raw.timestamp, 0)),
  });
}
...
export function normalizeRenderGovernorContext(raw = {}) {
  ...
  return freeze({
    backend: backendRank[backend] == null ? 'webgl2' : backend,
    level: tierRank[level] == null ? 'balanced' : level,
    frameIndex: integer(raw.frameIndex, 0),
    seed: text(raw.seed, 'render-quality'),
    userCeiling: tierRank[level] == null ? 'balanced' : level,
    thermalLocked: Boolean(raw.thermalLocked),
    batteryLocked: Boolean(raw.batteryLocked),
  });
}
...
export function computeRenderPressure(telemetry) {
  ...
  const memory = telemetry.memoryPressure;
  const thermal = telemetry.thermalPressure;
  const batteryPenalty = telemetry.batteryCharging ? 0 : clamp((0.35 - telemetry.batteryLevel) / 0.35);
  return round(frame * 0.34 + gpu * 0.2 + cpu * 0.16 + memory * 0.12 + thermal * 0.12 + batteryPenalty * 0.06);
}
...
export function selectQualityTarget(context, telemetry) {
  ...
  let target = current;
  if (context.thermalLocked || context.batteryLocked || telemetry.thermalPressure >= RENDER_QUALITY_POLICY.thermalCritical || telemetry.batteryLevel <= RENDER_QUALITY_POLICY.batteryCritical) {
    target = Math.min(tierRank.compatibility, current + 2);
  } else if (pressure >= 0.82 || telemetry.frameMs >= RENDER_QUALITY_POLICY.hardFrameMs) {
    target = Math.min(tierRank.compatibility, current + 1);
  } else if (pressure <= 0.3 && telemetry.frameMs <= RENDER_QUALITY_POLICY.recoveryFrameMs && telemetry.thermalPressure < RENDER_QUALITY_POLICY.thermalHot) {
...
export function updateQualityState(state, telemetry, context) {
  ...
  const currentRank = tierRank[currentLevel] ?? tierRank.balanced;
  const overloaded = targetRank > currentRank;
  const recovering = targetRank < currentRank;
  const streak = overloaded ? integer(safe.stepDownStreak) + 1 : recovering ? integer(safe.stepUpStreak) + 1 : 0;
  const recoveryStreak = recovering ? 0 : integer(safe.stepUpStreak) + 1;
  const downStreak = overloaded ? streak : 0;
  let nextLevel = currentLevel;
  if (overloaded && downStreak >= RENDER_QUALITY_POLICY.stepDownHoldFrames) nextLevel = target;
  if (recovering && recoveryStreak >= RENDER_QUALITY_POLICY.stepUpHoldFrames) nextLevel = target;
  if (telemetry.hidden && !telemetry.inputActive) nextLevel = 'compatibility';
  return freeze({
    level: nextLevel,
    target,
    stepDownStreak: downStreak,
    stepUpStreak: nextLevel === currentLevel ? recoveryStreak : 0,
    framesInLevel: integer(safe.framesInLevel) + 1,
    pressure: computeRenderPressure(telemetry),
  });
}
...
export function buildQualityProfile(context, telemetry, state) {
  ...
  return freeze({
    ...
    pixelRatio: ratio,
    shadows: Math.max(128, Math.round(cfg.shadows * scale)),
    foliageMultiplier: round(clamp(cfg.foliage * scale, 0.15, 1), 3),
    waterSegments: Math.max(12, Math.round(cfg.water * scale)),
    postFxQuality: round(clamp(cfg.postFx * scale), 3),
    temporalHistory: Boolean(cfg.temporal && context.backend === 'webgpu' && telemetry.memoryPressure < 0.86),
    animationRate: round(clamp(cfg.animation * (telemetry.frameMs > RENDER_QUALITY_POLICY.softFrameMs ? 0.9 : 1), 0.5, 1), 3),
    reducedMotion: Boolean(telemetry.hidden || telemetry.thermalPressure >= RENDER_QUALITY_POLICY.thermalCritical),
  });
}
...
export function pushTelemetry(history, rawTelemetry, maxHistory = RENDER_QUALITY_POLICY.maxHistory) {
  ...
  next.push(normalizeRenderTelemetry(rawTelemetry));
...
export function summarizeRenderTelemetry(history = []) {
  ...
  const frames = safe.map((s) => finite(s.frameMs)).sort((a, b) => a - b);
  const average = (key) => safe.reduce((sum, s) => sum + finite(s[key]), 0) / safe.length;
...
export function createRenderQualityGovernor(options = {}) {
  let disposed = false;
  let state = { level: text(options.level, 'balanced'), stepDownStreak: 0, stepUpStreak: 0, framesInLevel: 0 };
  let history = [];
  const evaluate = (rawTelemetry = {}, rawContext = {}) => {
    ...
    const telemetry = normalizeRenderTelemetry(rawTelemetry);
    const context = normalizeRenderGovernorContext({ ...rawContext, level: state.level });
    state = updateQualityState(state, telemetry, context);
    history = pushTelemetry(history, telemetry);
...
export function renderQualityDigest(result) {
  ...
  const raw = [s.level, s.target, p.backend, p.pixelRatio, p.shadows, p.foliageMultiplier, p.waterSegments, p.postFxQuality, p.temporalHistory, p.animationRate, result?.summary?.p95FrameMs].join('|');
  let h = 2166136261;
  for (const c of raw) { h ^= c.charCodeAt(0); h = Math.imul(h, 16777619) >>> 0; }
  return (h ^ (h >>> 16)).toString(16).padStart(8, '0');
}

export function validateRenderQualityResult(result) {
  const errors = [];
  if (result?.disposed) return freeze({ valid: true, errors: freeze([]) });
...
export function replayRenderQuality(cases = [], initial = {}) {
  const governor = createRenderQualityGovernor(initial);
  return freeze((Array.isArray(cases) ? cases : []).map((item) => governor.evaluate(item.telemetry, item.context)));
}
