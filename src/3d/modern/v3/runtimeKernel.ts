...
const EMPTY_CAMERA: V3CameraState = Object.freeze({ position: Object.freeze({ x: 0, y: 80, z: 120 }), target: Object.freeze({ x: 0, y: 0, z: 0 }), fov: 60, near: 0.1, far: 30_000, dpr: 1 });
...
export interface V3RuntimeMetrics { frameMs: number; simulationMs: number; presentationMs: number; memoryBytes: number; memoryBudgetBytes: number; droppedFrames: number; tickDriftMs: number; legacyCalls: number; }
export interface V3RuntimeKernelOptions { readonly config: V3RuntimeConfig; readonly dependencies?: V3RuntimeDependencies; readonly budget?: V3FrameBudget; }
...
class InternalClock implements V3Clock {
  ...
  advance(nowMs: number, step: (deltaMs: number, tick: Tick) => void): ReturnType<V3Clock['snapshot']> { const result = this.#clock.advance(nowMs, step); this.#snapshot = { nowMs: result.nowMs, tick: result.tick, deltaMs: result.deltaMs, accumulatorMs: result.accumulatorMs, alpha: clamp(result.accumulatorMs / V3_FIXED_STEP_MS, 0, 1) }; return this.#snapshot; }
  snapshot(): ReturnType<V3Clock['snapshot']> { return this.#snapshot; }
}
...
export class V3RuntimeKernel implements V3RuntimeController {
  ...
  async start(): Promise<boolean> {
    ...
    if (this.#legacy && this.#config.enableLegacyAdapter) {
      try {
        const started = await this.#legacy.invoke('load', async () => { await this.#legacy?.load(); });
        if (!started) { this.enterRecovery('legacy-load-failed'); return false; }
      } catch (error) {
        this.enterRecovery(error instanceof Error ? error.message : 'legacy-load-failed');
        return false;
      }
    }
    const now = this.#now();
    ...
    this.#lastPacket = Object.freeze({ frame: 0, backend: this.#backend, quality: this.#quality, viewport: this.#viewport, camera: this.#camera, visibleIds: [], shadowIds: [], estimatedGpuMs: 0, uploadBytes: 0 });
    this.#running = true;
    this.#paused = false;
  ...
  pause(reason = 'manual'): void { this.assertAlive(); if (!this.#running) return; this.#paused = true; this.emit('runtime/paused', { reason }); }
  resume(reason = 'manual'): void { this.assertAlive(); if (!this.#running) return; this.#paused = false; this.emit('runtime/resumed', { reason }); }

  async stop(reason = 'manual'): Promise<void> {
    if (this.#disposed) return;
    this.#running = false;
    this.#paused = false;
    if (this.#legacy?.isLoaded()) await this.#legacy.invoke('unload', async () => { await this.#legacy?.unload(); });
  ...
  tick(nowMs = this.#now()): V3RuntimeSnapshot {
    ...
    let steps = 0;
    const clock = this.#clock.advance(nowMs, (deltaMs, tick) => { steps += 1; this.#frame += 1; this.#simulationTimeMs += deltaMs; this.emit('runtime/tick', { frame: this.#frame, tick, deltaMs }); this.#applyActions(); });
    ...
    this.#health = { ...this.#health, frameMs, simulationMs, presentationMs: Math.max(0, frameMs - simulationMs), tickDriftMs: Math.max(0, clock.accumulatorMs - this.#budget.totalMs), droppedFrames: this.#health.droppedFrames + (clock.deltaMs > this.#budget.totalMs * 2 ? 1 : 0) };
    this.#lastPacket = this.buildPacket();
    this.emit('render/frame', this.#lastPacket);
    return this.snapshot();
  }

  dispatch(action: V3Action): boolean { this.assertAlive(); if (!action.action.trim()) return false; this.#actions.set(action.action, action); this.emit('input/action', action); return true; }

  snapshot(): V3RuntimeSnapshot {
    return Object.freeze({ schema: 'aapw.runtime.v3', schemaVersion: 1, frame: this.#frame, tick: makeTick(Number(this.#clock.snapshot().tick)), simulationTimeMs: this.#simulationTimeMs, host: this.#config.host, backend: this.#backend, quality: this.#quality, running: this.#running, paused: this.#paused, recoveryStage: this.#recoveryStage, camera: this.#camera, viewport: this.#viewport, health: this.health(), digest: this.digest() });
  }
  ...
  async dispose(): Promise<void> { if (this.#disposed) return; await this.stop('dispose'); this.#disposed = true; this.#listeners.clear(); this.#actions.clear(); }
  setQuality(quality: QualityTier, reason = 'manual'): void { this.assertAlive(); const previous = this.#quality; this.#quality = quality; if (previous !== quality) this.emit('runtime/quality', { previous, next: quality, reason }); }
  setBackend(backend: 'webgpu' | 'webgl2'): void { this.assertAlive(); this.#backend = backend; }

  private buildPacket(): V3RenderPacket {
    const visibleIds = Object.freeze([...this.#actions.keys()].filter(id => id.length <= 128).sort());
  ...
  private applyActions(): void { if (this.#actions.size === 0) return; const staleBefore = this.#frame - 12; for (const [key, action] of this.#actions) if (action.frame < staleBefore && action.phase === 'released') this.#actions.delete(key); }
  private enterRecovery(reason: string): void { const from = this.#recoveryStage; this.#recoveryStage = from === 'recovering' ? 'failed' : 'recovering'; this.emit('runtime/recovery', { from, to: this.#recoveryStage, reason }); this.emit('runtime/error', { code: 'V3_RECOVERY', message: reason, recoverable: this.#recoveryStage !== 'failed' }); }
  private digest(): string { const source = `${this.#worldId}:${this.#frame}:${Number(this.#clock.snapshot().tick)}:${this.#backend}:${this.#quality}:${Math.round(this.#simulationTimeMs)}`; let hash = 2166136261; for (let i = 0; i < source.length; i += 1) { hash ^= source.charCodeAt(i); hash = Math.imul(hash, 16777619); } return (hash >>> 0).toString(16).padStart(8, '0'); }
