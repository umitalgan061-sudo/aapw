...
export interface RuntimeFacadeAdapterOptions {
...
function vector(value: MutableVector3Like | undefined, fallback: MutableVector3Like): { x: number; y: number; z: number } {
...
export class RuntimeFacadeAdapter implements RuntimeSceneAdapter {
  ...
  constructor(options: RuntimeFacadeAdapterOptions) {
    this.#read = options.read;
    this.#viewport = options.viewport ?? (() => ({ width: typeof innerWidth === 'number' ? innerWidth : 1280, height: typeof innerHeight === 'number' ? innerHeight : 720, dpr: typeof devicePixelRatio === 'number' ? devicePixelRatio : 1 }));
    this.#writeQuality = options.writeQuality;
  }
  ...
  captureCamera(): CameraFrameState {
    ...
    return Object.freeze({ position: Object.freeze(vector(camera?.position, { x: 0, y: 3, z: 6 })), target: Object.freeze(vector(camera?.target, { x: 0, y: 1, z: 0 })), yaw: finite(camera?.rotation?.y, 0) as never, pitch: 0.25, zoom: 6 });
  }
  ...
  getRenderMetrics() {
    const render = this.#read()?.renderer?.info?.render;
    const memory = this.#read()?.renderer?.info?.memory;
    return Object.freeze({ drawCalls: Math.max(0, Math.trunc(finite(render?.calls))), triangles: Math.max(0, Math.trunc(finite(render?.triangles))), visibleObjects: Math.max(0, Math.trunc(finite(render?.calls))), textureBytes: Math.max(0, Math.trunc(finite(memory?.textures) * 1024 * 1024)) });
  }
