import { InputIntent, NetworkSnapshot, RuntimeBudget, RuntimeSnapshot, Tick, asTick, checksumObject, nextTick } from './domain.ts';
...
export class ModernRuntimeV5 {
  ...
  async submitIntent(intent: InputIntent): Promise<boolean> {
    ...
    if (violations.length > 0) { this.telemetry.metric('input.rejected', 1); return false; }
  ...
  async step(deltaSeconds: number, signal?: AbortSignal): Promise<RuntimeStepResult> {
    ...
    while (this.#accumulator >= step && ticks < this.#config.maxCatchUpSteps) {
      ...
      if (Number(this.#tick) % this.#config.saveIntervalTicks === 0 && this.#save.shouldSave(this.#tick)) await this.saveCheckpoint();
    }
    const budget = this.scheduler.getBudget();
    const frameMs = (typeof performance === 'undefined' ? Date.now() : performance.now()) - startMs;
    this.health.record(frameMs, budget, this.#tick);
    const report = this.telemetry.reportHealth();
    return { ticksSimulated: ticks, currentTick: this.#tick, remainingSeconds: this.#accumulator, healthScore: report.score, degraded: report.score < 70 || budget.totalMs > budget.maxTotalMs };
  }

  async saveCheckpoint(): Promise<void> {
    const entities = this.world.snapshot();
    const snapshot: RuntimeSnapshot = { version: 5, tick: this.#tick, digest: { tick: this.#tick, entityCount: entities.length, commandCount: 0, eventCount: 0, checksum: checksumObject(entities) }, entities, metadata: {} };
    const result = await this.#save.save(snapshot);
    if (result.ok) this.#lastSnapshotChecksum = result.checksum;
  }

  reconcile(snapshot: NetworkSnapshot): ReconciliationResult {
    ...
    const replayedInputs = this.network.predictions.after(snapshot.tick).length;
    this.applyNetworkSnapshot(snapshot);
    return { fromTick: local.tick, toTick: snapshot.tick, correctedEntities: snapshot.entities.length, replayedInputs, accepted: snapshot.tick >= local.tick };
  }
