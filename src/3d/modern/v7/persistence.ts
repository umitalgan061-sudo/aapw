...
export class VersionedPersistence implements Disposable {
  ...
  async save<T>(slot: string, payload: T, tick: Tick): Promise<V7Result<SaveEnvelope<T>>> {
    if (this.#disposed) return this.fail('PERSIST_DISPOSED'); if (!this.validSlot(slot)) return this.fail('SLOT_INVALID'); if (this.#revisions.size >= this.maxSlots && !this.#revisions.has(slot)) return this.fail('SLOT_LIMIT');
    ...
    const serialized = safeJson(envelope); if (!serialized.ok) return serialized; if (serialized.value.length > this.maxSerializedBytes) return this.fail('SAVE_SIZE_LIMIT');
  ...
  async load<T = unknown>(slot: string): Promise<V7Result<SaveEnvelope<T>>> {
    if (this.#disposed) return this.fail('PERSIST_DISPOSED'); if (!this.validSlot(slot)) return this.fail('SLOT_INVALID'); this.#reads += 1;
  ...
  async restore<T = unknown>(slot: string): Promise<V7Result<SaveEnvelope<T>>> {
    const loaded = await this.load<T>(slot); if (!loaded.ok) return loaded; const migrated = this.migrate(loaded.value); if (!migrated.ok) return migrated as V7Result<SaveEnvelope<T>>; this.#restores += 1; return { ok: true, value: migrated.value as SaveEnvelope<T> };
  }
  ...
  #backup(slot: string): Promise<void> {
    if (!this.backupCount) return Promise.resolve();
    return this.#adapter.read(slot).then(async (raw) => { if (!raw) return; for (let index = this.backupCount; index >= 1; index -= 1) { const source = index === 1 ? slot : `${slot}.bak${index - 1}`; const target = `${slot}.bak${index}`; const content = await this.#adapter.read(source); if (content) await this.#adapter.write(target, content); } this.#backups += 1; });
  }
  #validSlot = (slot: string): boolean => typeof slot === 'string' && /^[a-zA-Z0-9._-]{1,64}$/.test(slot);
  #migrateEnvelope(envelope: SaveEnvelope): V7Result<SaveEnvelope> {
    let current = envelope; while (current.schema < this.currentSchema) { const migration = this.#migrations.get(current.schema); if (!migration) return this.fail('MIGRATION_MISSING'); const payload = migration.apply(current.payload); const revision = current.revision + 1; current = Object.freeze({ ...current, schema: migration.to, payload, revision, checksum: digest(migration.to, this.build, current.tick, current.slot, revision, payload) }); } return { ok: true, value: current };
  }
  #validEnvelope(envelope: SaveEnvelope): boolean { return Boolean(envelope && typeof envelope.slot === 'string' && Number.isInteger(envelope.schema) && envelope.schema >= 1 && envelope.schema <= this.currentSchema && envelope.checksum === digest(envelope.schema, envelope.build, envelope.tick, envelope.slot, envelope.revision, envelope.payload)); }
  #migrate<T>(envelope: SaveEnvelope<T>): V7Result<SaveEnvelope<T>> { return this.#migrateEnvelope(envelope); }
  #validSlot(slot: string): boolean { return this.#validSlot(slot); }
