...
export function resolveSettlementServiceInteraction(
  ...
  if (!context.isOpen) return Object.freeze({ ...base, allowed: false, reason: 'service-closed' });
  if (!context.hasAccess) return Object.freeze({ ...base, allowed: false, reason: 'access-denied' });
  if (missingQuestIds.length > 0) return Object.freeze({ ...base, allowed: false, reason: 'quest-locked' });
  if (!availableActions.includes(action)) return Object.freeze({ ...base, allowed: false, reason: 'action-unavailable' });
