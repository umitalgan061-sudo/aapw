...
import { DEFAULT_PRODUCTION_CONFIG_P, checksumP, integerP, nonNegativeP, nowP, type AssetManifestItemP, type CameraFrameP, type EventSinkP, type FrameBudgetP, type InputActionStateP, type ProductionEventMapP, type ProductionLifecycle, type ProductionRuntimeConfigP, type RuntimeFrameReportP, type RuntimeHealthP, type WorldCommandP, type WorldSnapshotP } from './contracts.ts';
...
import { MemorySaveStorageP, ProductionPersistencePipeline } from './persistencePipeline.ts';
...
export interface ProductionRuntimeOptionsP {
  ...
  readonly saveStorage?: MemorySaveStorageP;
  readonly assetLoaders?: Readonly<Record<string, AssetLoaderP<unknown>>>;
}

export interface ProductionRuntimeTickInputP {
  readonly deltaSeconds: number;
  readonly camera?: CameraFrameP;
  readonly input?: Partial<InputActionStateP>;
  readonly budget?: Partial<FrameBudgetP>;
  readonly interests?: readonly InterestSourceP[];
  readonly memoryMb?: number;
  readonly networkRttMs?: number;
}

export interface ProductionRuntimeSnapshotP {
  readonly lifecycle: ProductionLifecycle;
  readonly frame: number;
  readonly tick: number;
  readonly revision: number;
  readonly actors: ReturnType<ProductionWorldPipeline['actors']>;
  readonly health: RuntimeHealthP;
  readonly render: ReturnType<ProductionRenderBudget['plan']>;
  readonly digest: number;
}

export interface ProductionRuntimeStatsP {
  readonly frame: number;
  readonly lifecycle: ProductionLifecycle;
  readonly revision: number;
  readonly clock: ReturnType<ProductionClock['state']>;
  readonly world: ReturnType<ProductionWorldPipeline['stats']>;
  readonly network: ReturnType<ProductionNetworkPipeline['stats']>;
  readonly assets: ReturnType<ProductionAssetPipeline['stats']>;
  readonly diagnostics: ReturnType<ProductionDiagnosticsPipeline['stats']>;
  readonly events: ReturnType<ProductionEventHub['stats']>;
  readonly migration: ReturnType<TypeScriptMigrationLedgerP['stats']>;
  readonly digest: number;
}
...
export class ProductionRuntime {
  ...
  #lifecycle: ProductionLifecycle = 'created';
  #frame = 0;
  #revision = 0;
  #startedAt = nowP();
  #disposed = false;
  ...
  constructor(options: ProductionRuntimeOptionsP = {}) {
    this.config = Object.freeze({ ...DEFAULT_PRODUCTION_CONFIG_P, ...options.config, seed: integerP(options.config?.seed ?? DEFAULT_PRODUCTION_CONFIG_P.seed), simulationHz: Math.max(10, Math.min(240, integerP(options.config?.simulationHz ?? DEFAULT_PRODUCTION_CONFIG_P.simulationHz))), maxStepsPerFrame: Math.max(1, Math.min(32, integerP(options.config?.maxStepsPerFrame ?? DEFAULT_PRODUCTION_CONFIG_P.maxStepsPerFrame))) });
    ...
    this.events.on('runtime:error', () => undefined);
  }
  ...
  boot(): void {
    this.#ensureLive();
    if (this.#lifecycle !== 'created' && this.#lifecycle !== 'stopped') return;
    this.#transition('booting');
    this.clock.reset();
    this.network.connect(0);
  ...
  registerMigrationModules(paths: readonly string[]): number {
    let registered = 0;
    for (const path of paths) {
      const module = classifyModuleP(path);
      if (!this.migration.get(module.path)) { this.migration.register(module); registered += 1; }
    }
    return registered;
  }
  ...
  spawn(actor: Parameters<ProductionWorldPipeline['spawn']>[0]): boolean {
    this.#ensureRunning();
    const spawned = this.world.spawn(actor, this.clock.tick);
    if (spawned) this.#revision += 1;
    return spawned;
  }

  despawn(id: number, reason = 'requested'): boolean {
    this.#ensureRunning();
    const removed = this.world.despawn(id, this.clock.tick, reason);
  ...
  queueNetwork<T>(kind: string, payload: T, reliable = true): void { this.#ensureRunning(); this.network.queue(kind, payload, this.clock.tick, reliable); }
  ...
  tick(input: ProductionRuntimeTickInputP): ProductionRuntimeSnapshotP {
    ...
    const begin = (name: string): void => phaseStart.set(name, nowP());
    const end = (name: string): void => { const start = phaseStart.get(name); if (start !== undefined) durations.set(name, Math.max(0, nowP() - start)); };

    begin('input'); end('input');
    begin('simulation');
    const clockResult = this.clock.advance(input.deltaSeconds, (tick, dt, simulationSeconds) => {
      this.#simulateStep(tick, dt, simulationSeconds, sample, input.interests ?? []);
    });
    end('simulation');
    ...
    begin('render');
    const budget = this.#budget(input.budget, durations, frameStarted);
    const renderObservation = this.render.observe(budget);
    end('render');
    begin('telemetry');
    ...
    const report: RuntimeFrameReportP = Object.freeze({ frame: this.#frame, tick: this.clock.tick, lifecycle: this.#lifecycle, steps: clockResult.steps, alpha: clockResult.alpha, render, health, phaseTimings: Object.freeze([...durations.entries()].map(([phase, durationMs]) => Object.freeze({ phase: phase as never, durationMs, budgetMs: phaseBudget(phase) }))), digest: checksumP({ frame: this.#frame, tick: this.clock.tick, revision: this.#revision, input: sample.sequence, renderObservation, health, world: this.world.digest() }) });
    this.events.emit('runtime:frame', report);
    return this.snapshot();
  }

  snapshot(): ProductionRuntimeSnapshotP {
    const health = this.diagnostics.latest();
    return Object.freeze({ lifecycle: this.#lifecycle, frame: this.#frame, tick: this.clock.tick, revision: this.#revision, actors: this.world.actors(), health, render: this.render.plan, digest: checksumP({ lifecycle: this.#lifecycle, frame: this.#frame, tick: this.clock.tick, revision: this.#revision, world: this.world.digest(), network: this.network.stats().checksum, assets: this.assets.digest(), render: this.render.plan }) });
  }

  worldSnapshot(): WorldSnapshotP {
    const actors = this.world.actors().map(actor => ({ id: actor.id, x: actor.transform.position.x, y: actor.transform.position.y, z: actor.transform.position.z, yaw: actor.transform.yaw, health: actor.health, stamina: actor.stamina, flags: actor.transform.flags }));
    return Object.freeze({ tick: this.clock.tick, revision: this.#revision, actors: Object.freeze(actors), checksum: checksumP(actors) });
  }

  restoreWorld(snapshot: WorldSnapshotP): void {
    this.#ensureLive();
    if (checksumP(snapshot.actors) !== snapshot.checksum) throw new Error('invalid world snapshot checksum');
    const current = new Map(this.world.actors().map(actor => [actor.id, actor]));
    for (const actor of snapshot.actors) {
      const existing = current.get(actor.id);
      if (!existing) continue;
      this.world.updateActor(actor.id, { transform: { ...existing.transform, position: { x: actor.x, y: actor.y, z: actor.z }, yaw: actor.yaw, flags: actor.flags }, health: Math.max(0, Math.min(existing.maxHealth, actor.health)), stamina: Math.max(0, Math.min(existing.maxStamina, actor.stamina)) }, snapshot.tick);
    }
    this.clock.reset(snapshot.tick, snapshot.tick / this.clock.config.hz);
    this.#revision = Math.max(0, integerP(snapshot.revision));
  }

  async save(slot: string): Promise<void> {
    this.#ensureLive();
    const snapshot = this.worldSnapshot();
    await this.persistence.save(slot, { runtime: this.snapshot(), world: snapshot }, this.clock.tick);
  }

  async load(slot: string): Promise<boolean> {
    this.#ensureLive();
    const save = await this.persistence.load(slot);
    if (!save) return false;
    const world = save.state.world as WorldSnapshotP | undefined;
    if (world) this.restoreWorld(world);
    return true;
  }
  ...
  dispose(): void {
    ...
    this.#disposed = true;
    if (this.#lifecycle === 'running' || this.#lifecycle === 'paused') this.stop();
    this.assets.dispose();
    this.events.clear();
    this.#transition('disposed');
  }

  #simulateStep(tick: number, dt: number, _simulationSeconds: number, sample: ReturnType<ProductionInputPipeline['process']>, interests: readonly InterestSourceP[]): void {
    this.world.updateInterests(interests, tick);
    const player = this.world.get(1);
    if (player) {
      const speed = sample.sprint ? 7 : 4;
      const nextVelocity = { x: sample.moveX * speed, y: player.transform.velocity.y, z: sample.moveZ * speed };
      this.world.updateActor(1, { transform: { ...player.transform, velocity: nextVelocity, position: { x: player.transform.position.x + nextVelocity.x * dt, y: player.transform.position.y + nextVelocity.y * dt, z: player.transform.position.z + nextVelocity.z * dt }, yaw: Math.atan2(nextVelocity.x, Math.max(0.0001, nextVelocity.z)) || player.transform.yaw } }, tick);
    }
    for (const command of this.world.drainCommands(64)) this.#executeCommand(command, tick);
    this.world.emitEvent(tick, 'simulation:tick', 'production', undefined, { actors: this.world.count(), input: sample.sequence });
  }

  #executeCommand(command: WorldCommandP, tick: number): void {
    if (command.kind === 'despawn') this.world.despawn(command.actorId, tick, 'command');
    if (command.kind === 'damage') {
      const actor = this.world.get(command.actorId);
      const amount = Number(command.payload.amount ?? 0);
      if (actor) this.world.updateActor(actor.id, { health: Math.max(0, actor.health - Math.max(0, Number.isFinite(amount) ? amount : 0)) }, tick);
    }
  }

  #applyInput(input: Partial<InputActionStateP>): void {
    if (input.moveX !== undefined) this.input.setAxis('moveX', Number(input.moveX));
    if (input.moveZ !== undefined) this.input.setAxis('moveZ', Number(input.moveZ));
    if (input.lookX !== undefined) this.input.setAxis('lookX', Number(input.lookX));
    if (input.lookY !== undefined) this.input.setAxis('lookY', Number(input.lookY));
    for (const action of ['jump', 'sprint', 'dodge', 'primary', 'secondary', 'interact', 'pause'] as const) if (input[action] !== undefined) input[action] ? this.input.press(action) : this.input.release(action);
  }

  #budget(input: Partial<FrameBudgetP> | undefined, durations: Map<string, number>, frameStarted: number): FrameBudgetP {
    const fallback = (name: string): number => durations.get(name) ?? 0;
    const targetMs = Math.max(1, input?.targetMs ?? (this.render.plan.tier === 'low' || this.render.plan.tier === 'safe' ? 33.3 : 16.6));
    const totalMs = Math.max(0, input?.totalMs ?? nowP() - frameStarted);
    return Object.freeze({ inputMs: Math.max(0, input?.inputMs ?? fallback('input')), simulationMs: Math.max(0, input?.simulationMs ?? fallback('simulation')), worldMs: Math.max(0, input?.worldMs ?? fallback('world')), networkMs: Math.max(0, input?.networkMs ?? fallback('network')), assetMs: Math.max(0, input?.assetMs ?? fallback('assets')), renderMs: Math.max(0, input?.renderMs ?? fallback('render')), telemetryMs: Math.max(0, input?.telemetryMs ?? fallback('telemetry')), totalMs, targetMs });
  }

  #transition(next: ProductionLifecycle): void {
    const previous = this.#lifecycle;
    this.#lifecycle = next;
    this.events.emit('runtime:lifecycle', { previous, next });
  }
...
function phaseBudget(phase: string): number { return { input: 1, simulation: 4, world: 3, network: 2, assets: 3, render: 8, telemetry: 1 }[phase] ?? 1; }
