...
const frameSeverity = (ms: number, t: DiagnosticThresholdsV4): DiagnosticSeverityV4 => ms >= t.frameMsCritical ? 'critical' : ms >= t.frameMsWarning ? 'warning' : ms >= t.frameMsWarning * 0.8 ? 'notice' : 'info';
const ratioSeverity = (ratio: number, t: DiagnosticThresholdsV4): DiagnosticSeverityV4 => ratio >= t.memoryCriticalRatio ? 'critical' : ratio >= t.memoryWarningRatio ? 'warning' : ratio >= t.memoryWarningRatio * 0.9 ? 'notice' : 'info';
const queueSeverity = (size: number, t: DiagnosticThresholdsV4): DiagnosticSeverityV4 => size >= t.queueCritical ? 'critical' : size >= t.queueWarning ? 'warning' : size >= t.queueWarning * 0.75 ? 'notice' : 'info';
const lossSeverity = (loss: number, t: DiagnosticThresholdsV4): DiagnosticSeverityV4 => loss >= t.networkLossCritical ? 'critical' : loss >= t.networkLossWarning ? 'warning' : loss >= t.networkLossWarning * 0.5 ? 'notice' : 'info';
...
export class RuntimeDiagnosticsV4 {
  ...
  record(tick: number, severity: DiagnosticSeverityV4, domain: DiagnosticDomainV4, code: string, message: string, details: Readonly<Record<string, string | number | boolean>> = {}): DiagnosticEventV4 {
    ...
    const event = Object.freeze({ id: `d-${normalizedTick}-${this.#sequence++}`, tick: normalizedTick, timestamp: finite(this.#now()), severity, domain, code: code.trim() || 'RUNTIME_DIAGNOSTIC', message: message.trim() || 'Runtime diagnostic event', details: Object.freeze({ ...details }) });
  ...
  inspect(tick: number, stats: RuntimeStatsV4, budget: BudgetUsageV4, health: RuntimeHealthV4): readonly DiagnosticEventV4[] {
    ...
    const frame = frameSeverity(finite(stats.frameMs), this.thresholds);
    if (SEVERITY_RANK[frame] > 0) created.push(this.record(tick, frame, 'render', 'FRAME_BUDGET', `Frame budget pressure: ${finite(stats.frameMs).toFixed(2)}ms`, { frameMs: finite(stats.frameMs) }));
    const memoryBudget = Math.max(1, finite(budget.memoryBytes));
    const ratio = Math.min(1, Math.max(0, finite(budget.memoryUsedBytes) / memoryBudget));
    const memory = ratioSeverity(ratio, this.thresholds);
    if (SEVERITY_RANK[memory] > 0) created.push(this.record(tick, memory, 'runtime', 'MEMORY_BUDGET', `Memory budget pressure: ${(ratio * 100).toFixed(1)}%`, { memoryRatio: ratio, memoryUsed: finite(budget.memoryUsedBytes), memoryBudget }));
    const queue = queueSeverity(Math.max(0, Math.trunc(finite(stats.pendingCommands))), this.thresholds);
    if (SEVERITY_RANK[queue] > 0) created.push(this.record(tick, queue, 'scheduler', 'COMMAND_QUEUE', `Command queue depth: ${Math.max(0, Math.trunc(finite(stats.pendingCommands)))}`, { queueSize: Math.max(0, Math.trunc(finite(stats.pendingCommands))) }));
    const loss = Math.max(0, finite(stats.packetLossRatio));
    const network = lossSeverity(loss, this.thresholds);
    if (SEVERITY_RANK[network] > 0) created.push(this.record(tick, network, 'network', 'PACKET_LOSS', `Packet loss: ${(loss * 100).toFixed(2)}%`, { packetLoss: loss }));
  ...
  recent(limit = 32, minimumSeverity: DiagnosticSeverityV4 = 'info'): readonly DiagnosticEventV4[] {
    ...
    return Object.freeze(this.#events.filter((event) => SEVERITY_RANK[event.severity] >= threshold).slice(-Math.max(0, Math.min(limit, this.maxEvents))));
  }
  ...
  #summarize(events: readonly DiagnosticEventV4[]): DiagnosticSummaryV4 {
    const counts = { info: 0, notice: 0, warning: 0, critical: 0 } as Record<DiagnosticSeverityV4, number>;
    ...
    return Object.freeze({ total: events.length, info: counts.info, notice: counts.notice, warning: counts.warning, critical: counts.critical, domains: Object.freeze(domains), latestTick, health: counts.critical > 0 ? 'critical' : counts.warning > 0 ? 'degraded' : 'nominal' });
  }
