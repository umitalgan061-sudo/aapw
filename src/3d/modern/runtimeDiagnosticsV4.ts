...
const SEVERITY_RANK: Record<DiagnosticSeverityV4, number> = {
  info: 0,
  notice: 1,
  warning: 2,
  critical: 3,
...
const DOMAINS: readonly DiagnosticDomainV4[] = Object.freeze([
  'runtime', 'scheduler', 'ecs', 'network', 'asset', 'render', 'input', 'persistence', 'platform',
]);
...
export class RuntimeDiagnosticsV4 {
  ...
  record(
    tick: number,
    severity: DiagnosticSeverityV4,
    domain: DiagnosticDomainV4,
    code: string,
    message: string,
  ...
  inspect(
    ...
    budget: BudgetUsageV4,
    health: RuntimeHealthV4,
  ): readonly DiagnosticEventV4[] {
    ...
    if (SEVERITY_RANK[frameSeverity] > SEVERITY_RANK.info) {
      created.push(this.record(tick, frameSeverity, 'render', 'FRAME_BUDGET', `Frame budget pressure: ${frameMs.toFixed(2)}ms`, { frameMs }));
    }
    ...
    if (SEVERITY_RANK[memorySeverity] > SEVERITY_RANK.info) {
      created.push(this.record(tick, memorySeverity, 'runtime', 'MEMORY_BUDGET', `Memory budget pressure: ${(memoryRatio * 100).toFixed(1)}%`, { memoryRatio, memoryUsed, memoryBudget }));
    }
    ...
    if (SEVERITY_RANK[queueSeverity] > SEVERITY_RANK.info) {
      created.push(this.record(tick, queueSeverity, 'scheduler', 'COMMAND_QUEUE', `Command queue depth: ${queueSize}`, { queueSize }));
    }

    const packetLoss = Math.max(0, finite(stats.packetLossRatio));
    const networkSeverity = severityForLoss(packetLoss, this.thresholds);
    if (SEVERITY_RANK[networkSeverity] > SEVERITY_RANK.info) {
      created.push(this.record(tick, networkSeverity, 'network', 'PACKET_LOSS', `Packet loss: ${(packetLoss * 100).toFixed(2)}%`, { packetLoss }));
    }

    if (!health.healthy) {
      created.push(this.record(tick, 'critical', 'runtime', 'HEALTH_REPORT', 'Runtime health reports a failure state', { healthy: false }));
    }
  ...
  capture(
    capturedAt: number,
    runtime: RuntimeSnapshotV4 | null,
    stats: RuntimeStatsV4 | null,
    budget: BudgetUsageV4 | null,
    health: RuntimeHealthV4 | null,
  ): DiagnosticSnapshotV4 {
    ...
    return Object.freeze({
      capturedAt: finite(capturedAt, this.#now()),
      runtime,
      stats,
      budget,
      health,
  ...
  recent(limit = 32, minimumSeverity: DiagnosticSeverityV4 = 'info'): readonly DiagnosticEventV4[] {
    ...
    return Object.freeze(this.#events
      .filter((event) => SEVERITY_RANK[event.severity] >= threshold)
      .slice(-max));
  }

  summary(): DiagnosticSummaryV4 {
  ...
  exportJson(): string {
    return JSON.stringify(this.capture(this.#now(), null, null, null, null));
  }
  ...
  #emptySummary(): DiagnosticSummaryV4 {
    return Object.freeze({
      ...
      warning: 0,
      critical: 0,
      domains: Object.freeze(Object.fromEntries(DOMAINS.map((domain) => [domain, 0])) as Record<DiagnosticDomainV4, number>),
      latestTick: 0,
      health: 'nominal',
    });
  }
