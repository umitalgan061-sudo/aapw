...
export class RuntimeDiagnosticsV4 {
  ...
  record(tick: number, severity: DiagnosticSeverityV4, domain: DiagnosticDomainV4, code: string, message: string, details: Readonly<Record<string, string | number | boolean>> = {}): DiagnosticEventV4 {
    ...
    const event: DiagnosticEventV4 = Object.freeze({ id: `d-${this.#lastTick}-${this.#sequence++}`, tick: normalizedTick, timestamp: finite(this.#now()), severity, domain, code: code.trim() || 'RUNTIME_DIAGNOSTIC', message: message.trim() || 'Runtime diagnostic event', details: cloneRecord(details) });
