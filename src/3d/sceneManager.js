/**
 * Scene bootstrap for the boot-preview world. Setup-time factories live here; per-frame update and
 * disposal remain with game3d.js beside the tick loop. World geography, terrain, settlement and
 * asset placement authorities are never duplicated here.
 * @module sceneManager
 */
...
import { createWater, setWaterDepthField, WATER_PLANE_SEGMENTS_DESKTOP, WATER_PLANE_SEGMENTS_MOBILE } from './world/water.js';
...
import { auditRuntimeSurface } from './materials/RuntimeSurfaceAudit.js';
...
function readManualQualityLevel() {
  try { return window.localStorage.getItem(STORAGE_KEYS.QUALITY_SETTING); } catch { return null; }
}
...
function logSurfaceAudit(label, group) {
  const audit = auditRuntimeSurface(group, { expectedRole: label });
  console.info(
    `[sceneManager] ${label} surface audit: meshes=${audit.meshCount}, materials=${audit.materialCount}, ` +
      `baseColorMapped=${audit.baseColorMapped}, normalMapped=${audit.normalMapped}, roughnessMapped=${audit.roughnessMapped}, ` +
      `normalizedTextureFlags=${audit.normalizedTextureFlags}.`,
  );
  return audit;
}
...
export function createScene(canvas) {
  ...
  const renderQuality = resolveRenderQuality({ coarsePointer: isCoarsePointerDevice(), manualLevel: readManualQualityLevel() });
  ...
  scene.add(stars);
  const water = createWater(WORLD_DEFAULTS.WATER_LEVEL_METERS, isCoarsePointerDevice() ? WATER_PLANE_SEGMENTS_MOBILE : WATER_PLANE_SEGMENTS_DESKTOP);
  scene.add(water);
  ...
  console.info(`[sceneManager] Loaded ${chunkManager.loadedCount} terrain chunks (~${chunkManager.getCoveredAreaKm2().toFixed(2)} km²) in ${generationMs.toFixed(0)}ms (${isMobileClass ? 'mobile-budget radius' : 'desktop full preview radius'}).`);

  const groundCollider = createGroundCollider(WORLD_DEFAULTS.WORLD_SEED, undefined, flattenPads);
  const valyriaEcologyPlacement = createValyriaBarrenEcologyPlacementProbe({ sampleHeightMeters: groundCollider.getGroundHeight, seaLevelMeters: WORLD_DEFAULTS.WATER_LEVEL_METERS });
  ...
  console.info(`[sceneManager] Water depth field baked: ${waterDepthField.resolution}² texels over ${waterDepthField.extentMeters}m in ${waterDepthField.bakeMs.toFixed(0)}ms (${(waterDepthField.deepTexelRatio * 100).toFixed(1)}% deep water, ${(waterDepthField.dryTexelRatio * 100).toFixed(1)}% dry land).`);

  const { points: riverPoints, endReason: riverEndReason } = generateRiverPath({ seed: WORLD_DEFAULTS.WORLD_SEED, sampleHeightMeters: groundCollider.getGroundHeight, seaLevelMeters: WORLD_DEFAULTS.WATER_LEVEL_METERS });
  ...
  if (!isMobileClass) {
    for (const seat of settlementsResult.seats) {
      chunkManager.loadSquare(worldToChunkCoord(seat.x, CHUNK_CONFIG.CHUNK_SIZE_METERS), worldToChunkCoord(seat.z, CHUNK_CONFIG.CHUNK_SIZE_METERS), 1);
    }
  }
  console.info(`[sceneManager] Placed ${settlementsResult.seats.length} kingdom-seat settlements; ${chunkManager.loadedCount} terrain chunks resident.`);
  ...
  console.info(`[sceneManager] Built road network: ${roadsResult.edges.length} segment(s), ${(roadsResult.totalLengthMeters / 1000).toFixed(2)} km total, steepest actual segment grade ${roadsResult.maxGradeDegrees.toFixed(1)}°.`);
  ...
  const naturalGeologyAudit = logSurfaceAudit('natural-geology', naturalGeologyResult.group);
  ...
  void upgradeNaturalGeologyAssets(naturalGeologyResult.group, { signal: naturalGeologyAbortController.signal, isMobileClass }).then((upgrade) => {
    if (upgrade.status === 'active') {
      const hydratedAudit = logSurfaceAudit('natural-geology-hydrated', naturalGeologyResult.group);
      naturalGeologyResult.group.userData.naturalGeologySurfaceAudit = hydratedAudit;
  ...
  const vegetationSurfaceAudit = logSurfaceAudit('vegetation-before-winter-upgrade', vegetationResult.group);
  vegetationResult.group.userData.vegetationSurfaceAudit = vegetationSurfaceAudit;
  console.info(`[sceneManager] Scattered vegetation: ${vegetationResult.placedCount}/${vegetationResult.targetCount} tree(s) before geography gate; ${vegetationDistribution.keptInstances} visible instance(s) after region/habitat distribution, ${vegetationDistribution.rejectedByGeography + vegetationDistribution.rejectedByRegionDensity} filtered.`);
  ...
  void upgradeWinterVegetationAssets(vegetationResult.group, { signal: winterVegetationAbortController.signal }).then((upgrade) => {
    if (upgrade.status === 'active') {
      const hydratedAudit = logSurfaceAudit('winter-vegetation-hydrated', vegetationResult.group);
      vegetationResult.group.userData.winterVegetationSurfaceAudit = hydratedAudit;
    ...
    } else if (upgrade.status === 'procedural-fallback') console.info('[sceneManager] Winter GLB unavailable/pointer-only; procedural snow-pine fallback remains active.');
  ...
  return {
    ...
    naturalGeologySurfaceAudit: naturalGeologyAudit,
    valyriaEcologyPlacement,
    vegetation: vegetationResult.group,
    vegetationDistribution,
    vegetationSurfaceAudit,
