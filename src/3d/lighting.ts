/** TypeScript production source. The legacy .js entry is a compatibility shim. */
/**
...
export interface DayNightLights {
  ...
  readonly moonAssetReady?: Promise<unknown>;
}
...
export interface DayNightState {
  ...
  readonly celestialKey: Record<string, unknown>;
...
function makeCelestialSphere(name: string, color: THREE.ColorRepresentation, scale = CELESTIAL_VISUAL_SCALE): THREE.Mesh<THREE.SphereGeometry, THREE.MeshBasicMaterial> {
...
function fitObjectToDiameter(object: THREE.Object3D, diameter: number): boolean {
...
function setMoonAssetStatus(moonAnchor: THREE.Object3D, status: string, detail: Record<string, unknown> = {}) {
...
function installMoonAssetAsync(moonAnchor: THREE.Object3D): Promise<unknown> {
...
export function createDayNightLighting(scene: THREE.Scene) {
...
export function updateDayNightLighting(lights: DayNightLights, elapsedSeconds: number, dayLengthSeconds: number, startRatio: number): DayNightState {
...
export function disposeDayNightLighting(scene: THREE.Scene, lights: DayNightLights): void {
