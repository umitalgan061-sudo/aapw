/** Deterministic micro-detail layer for canonical owner-map Pindex-03 only. */
...
export const PINDEX03_DETAIL_POLICY = Object.freeze({
  id: 'owner-map-pindex03-detail-2026-08-26-v2-western-marine-shelf',
  pindex: 3,
  westernMarineShelfTone: true,
  amplitudeBySurface: Object.freeze({ sea: 0.01, lake: 0.01, soil: 0.05, rock: 0.06, snow: 0.025 }),
});

function hash01(x, y) {
  const value = Math.sin(x * 27.911 + y * 47.173 + 41.337) * 43758.5453;
...
export function applyPindex03DetailToTerrainMesh(mesh) {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const amplitude = PINDEX03_DETAIL_POLICY.amplitudeBySurface[c.surface] ?? 0;
  ...
  const summary = Object.freeze({ policyId: PINDEX03_DETAIL_POLICY.id, pindex: 3, touchedVertices });
...
export function applyPindex03DetailToTerrainGroup(terrainGroup) {
  ...
  let touchedVertices = 0;
  for (const mesh of terrainGroup.children) touchedVertices += applyPindex03DetailToTerrainMesh(mesh).touchedVertices;
  const summary = Object.freeze({ policyId: PINDEX03_DETAIL_POLICY.id, pindex: 3, touchedVertices, meshCount: terrainGroup.children.length });
