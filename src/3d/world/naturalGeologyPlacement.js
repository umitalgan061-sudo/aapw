...
export const NATURAL_GEOLOGY_PLACEMENT_POLICY = Object.freeze({
  id: 'natural-geology-placement-2026-09-01-v3-r2-morphology-blue-noise',
  supersedes: 'natural-geology-placement-2026-08-31-v2-valyria-morphology-aligned',
  deterministic: true,
  renderOnly: true,
  ...
  // Retained as candidate-count budgets only. Coordinates are generated by R2, never from cells.
  ...
  lowDiscrepancySequence: true,
  cranleyPattersonScramble: true,
...
const PLASTIC_CONSTANT = 1.3247179572447458;
const R2_ALPHA_X = 1 / PLASTIC_CONSTANT;
const R2_ALPHA_Z = 1 / (PLASTIC_CONSTANT * PLASTIC_CONSTANT);
...
export function naturalGeologyCandidateUv(seed, index) {
  ...
  const shiftZ = geologyHash01(seed ^ 0x9e3779b9, 709, 17, 5);
  return Object.freeze({
    u: fract(0.5 + shiftX + (i + 1) * R2_ALPHA_X),
    v: fract(0.5 + shiftZ + (i + 1) * R2_ALPHA_Z),
  });
}

export function naturalGeologyCandidateWorld(seed, index, worldWidthMeters, worldDepthMeters) {
  const width = Math.max(1e-9, finite(worldWidthMeters));
  const depth = Math.max(1e-9, finite(worldDepthMeters));
  const uv = naturalGeologyCandidateUv(seed, index);
  return Object.freeze({
    x: (uv.u - 0.5) * width,
    z: (uv.v - 0.5) * depth,
    u: uv.u,
...
function createCluster(seed, index, width, depth, mobile) {
  const uv = naturalGeologyCandidateUv(seed ^ 0x43f6a888, index + 8192);
  const margin = 0.045;
  const x = ((margin + uv.u * (1 - margin * 2)) - 0.5) * width;
  const z = ((margin + uv.v * (1 - margin * 2)) - 0.5) * depth;
...
function valyriaCandidate(seed, index, trial, mode) {
  ...
  if (mode === 'fault') {
    ...
    const uv = naturalGeologyCandidateUv(seed ^ 0x51a7b33f, index * 11 + trial + 16384);
    const dx = (uv.u * 2 - 1) * P.coreRadius.nx * 1.08;
    const dy = (uv.v * 2 - 1) * P.coreRadius.ny * 1.08;
    nx = P.coreCenter.nx + dx;
    ny = P.coreCenter.ny + dy;
  }
...
function createValyriaCluster(seed, index, width, depth, mobile) {
  ...
  const orientation = mode === 'fault'
    ...
    : mode === 'shoulder'
      ? radialAngle + Math.PI * 0.5 + hashSigned(seed, index, 332, 0) * 0.18
      : radialAngle + hashSigned(seed, index, 333, 0) * 0.38;
  const alongRadiusMeters = (mobile ? 115 : 155)
    + geologyHash01(seed, index, 334, 0) * (mobile ? 170 : 310);
  const acrossRatio = mode === 'fault'
    ? 0.16 + geologyHash01(seed, index, 335, 0) * 0.16
    : mode === 'lava-drainage' || mode === 'erosion-gully'
      ? 0.20 + geologyHash01(seed, index, 336, 0) * 0.24
      : 0.28 + geologyHash01(seed, index, 337, 0) * 0.28;
  const kind = mode === 'fault'
    ? 'bedrock-band'
    : mode === 'shoulder'
      ? 'bedrock-band'
      : mode === 'lava-drainage'
  ...
  return Object.freeze({
    ...
    orientation,
    alongRadiusMeters,
    acrossRadiusMeters: alongRadiusMeters * acrossRatio,
    kind,
    strength: 0.72 + geologyHash01(seed, index, 338, 0) * 0.28,
    morphologyMode: mode,
...
function makePlacement({
  ...
  const faultDominates = morphology.faultActivity >= Math.max(morphologyDrainage, morphology.brokenCalderaShoulder);
  ...
  return Object.freeze({
    ...
    candidateIndex,
    candidateDistribution: P.candidateDistribution,
    x,
