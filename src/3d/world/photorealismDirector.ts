...
const n3=(v:readonly number[])=>{const t=Math.max(1e-9,v.reduce((sum,item)=>sum+item,0));return v.map(item=>item/t)};
...
function buildWater(x:EnvironmentSample,w:Readonly<Record<EnvironmentSurface,number>>){const shore=c(1-x.waterDistanceMeters/24),foam=c(shore*(1-w['deep-water'])*.85);return Object.freeze({depthClass:(w['deep-water']>.42?'deep':w['shallow-water']>.25?'shallow':shore>.16?'wet-edge':'dry') as 'dry'|'wet-edge'|'shallow'|'deep',foam,roughness:c(.18+shore*.22+x.windward*.08,.12,.72),normalScale:c(.22+(1-shore)*.22+x.lee*.05,.18,.52),shorelineFade:shore})}
