...
/** Render-only PBR detail. It never changes canonical height, hydrology, placement or collision authority. */
...
function configureTerrainDataTexture(texture, repeatCount) {
...
function getTerrainSurfaceTextures(chunkSizeMeters) {
	const cacheKey = Number(chunkSizeMeters.toFixed(3));
	if (terrainSurfaceCache.has(cacheKey)) return terrainSurfaceCache.get(cacheKey);
	...
	const surface = Object.freeze({ normalMap, roughnessMap, repeatCount });
	terrainSurfaceCache.set(cacheKey, surface);
	return surface;
}
...
export function applyTerrainMicroSurface(material, { chunkSizeMeters = 500 } = {}) {
	...
	const surface = getTerrainSurfaceTextures(chunkSizeMeters);
