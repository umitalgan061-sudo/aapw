...
export function createHeightSampler(seed, fbmOptions, flattenPads = []) {
	...
	return function sampleHeightMeters(worldX, worldZ, maxHeightMeters = DEFAULT_MAX_HEIGHT_METERS) {
		...
		if (flattenPads.length === 0) return applyG07Terrain3dRuntimeParity(baseHeightMeters, worldX, worldZ);
		...
		if (strongestWeight <= 0) return applyG07Terrain3dRuntimeParity(baseHeightMeters, worldX, worldZ);
		const flattenedHeightMeters = baseHeightMeters + (strongestAnchorMeters - baseHeightMeters) * strongestWeight;
		return applyG07Terrain3dRuntimeParity(flattenedHeightMeters, worldX, worldZ);
	};
}
