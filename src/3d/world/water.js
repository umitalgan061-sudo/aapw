...
export function createWater(waterLevelMeters, segments = WATER_PLANE_SEGMENTS) {
	...
	// plane. Dry owner-world fragments are discarded by the canonical coverage mask. The far layer
	// renders first but must not write depth: at 90-degree/full-world camera distances its 6cm Y offset
	// can quantize to the same depth value as the dense near layer, producing a camera-centered moire
	// field even though both shaders are individually stable.
