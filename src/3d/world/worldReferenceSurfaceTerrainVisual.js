...
export const RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY = Object.freeze({
	...
	aerialMaterialRevision: 'v3-regional-lowland-lithology',
	atlasWidth: 192,
	atlasHeight: 128,
	qualityBlend: 0.30,
	biomeBlendMax: 0.56,
	...
	roughnessBlend: 0.50,
	shaderColorMicroVariation: 0.065,
	shaderRoughnessMicroVariation: 0.06,
	lowlandMacroScaleMeters: 1300,
	lowlandMesoScaleMeters: 430,
	lowlandPatchScaleMeters: 145,
	lowlandMaterialSeparation: true,
	lowlandMoistureDomains: true,
	lowlandStoneLagDomains: true,
	lowlandMicroNormal: true,
	naturalTransitionRevision: 'v2-slope-aspect-shelter-lowland-domain',
...
const PINDEX_QUALITY_V2_MEADOW = new THREE.Color(GEOGRAPHIC_REFERENCE_PALETTE.terrain.meadow);
const PINDEX_QUALITY_V2_DRY_HEATHER = new THREE.Color(GEOGRAPHIC_REFERENCE_PALETTE.terrain.dryHeather);
const PINDEX_QUALITY_V2_EXPOSED_EARTH = new THREE.Color(GEOGRAPHIC_REFERENCE_PALETTE.terrain.exposedEarth);
const PINDEX_QUALITY_V2_WET_EARTH = new THREE.Color(GEOGRAPHIC_REFERENCE_PALETTE.terrain.wetEarth);
...
function installRuntimePindexQualityV2Shader(material) {
	...
	material.onBeforeCompile = function onBeforeCompilePindexQualityV2(shader, renderer) {
		...
		shader.fragmentShader = shader.fragmentShader
			...
			.replace('#include <color_fragment>', `#include <color_fragment>\nvec2 pindexQualityUv=clamp(vPindexQualityWorldPosition.xz*pindexQualityMapTransform.xy+pindexQualityMapTransform.zw,vec2(0.0),vec2(1.0));\nvec4 pindexQualityAtlasColor=texture2D(pindexQualityColorAtlas,pindexQualityUv);\nvec4 pindexQualityAtlasData=texture2D(pindexQualityDataAtlas,pindexQualityUv);\nvec3 pindexQualityColor=pindexQualityAtlasColor.rgb;\nfloat pindexQualityRelief=pindexQualityAtlasData.r;\nfloat pindexQualityDry=pindexQualityAtlasColor.a;\nfloat pindexQualityHeightRock=smoothstep(22.0,82.0,vPindexQualityWorldPosition.y)*pindexQualityDry;\npindexQualityColor=mix(pindexQualityColor,vec3(${PINDEX_QUALITY_V2_ROCK_COLOR.r.toFixed(6)},${PINDEX_QUALITY_V2_ROCK_COLOR.g.toFixed(6)},${PINDEX_QUALITY_V2_ROCK_COLOR.b.toFixed(6)}),pindexQualityRelief*pindexQualityDry*${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.reliefRockBlend.toFixed(3)});\npindexQualityColor=mix(pindexQualityColor,vec3(${PINDEX_QUALITY_V2_ROCK_COLOR.r.toFixed(6)},${PINDEX_QUALITY_V2_ROCK_COLOR.g.toFixed(6)},${PINDEX_QUALITY_V2_ROCK_COLOR.b.toFixed(6)}),pindexQualityHeightRock*${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.elevationRockBlend.toFixed(3)});\nfloat pindexQualityNorthSnow=pindexQualityRelief*(1.0-smoothstep(0.08,0.34,pindexQualityUv.y))*smoothstep(16.0,58.0,vPindexQualityWorldPosition.y);\npindexQualityColor=mix(pindexQualityColor,vec3(${PINDEX_QUALITY_V2_SNOW_COLOR.r.toFixed(6)},${PINDEX_QUALITY_V2_SNOW_COLOR.g.toFixed(6)},${PINDEX_QUALITY_V2_SNOW_COLOR.b.toFixed(6)}),pindexQualityNorthSnow*0.42);\nvec2 pindexQualityDetailUv=vPindexQualityWorldPosition.xz*0.021;\nvec4 pindexQualityNoiseA=texture2D(pindexQualityDetailAtlas,pindexQualityDetailUv);\nvec2 pindexQualityRotatedUv=vec2(pindexQualityDetailUv.x*0.8-pindexQualityDetailUv.y*0.6,pindexQualityDetailUv.x*0.6+pindexQualityDetailUv.y*0.8)*2.73+vec2(0.37,0.19);\nvec4 pindexQualityNoiseB=texture2D(pindexQualityDetailAtlas,pindexQualityRotatedUv);\nfloat pindexQualityGrain=(pindexQualityNoiseA.r*2.0-1.0)*0.64+(pindexQualityNoiseB.g*2.0-1.0)*0.36;\nvec2 pindexLowlandMacroUv=vPindexQualityWorldPosition.xz/${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.lowlandMacroScaleMeters.toFixed(1)}+vec2(4.1,-9.7);\nvec2 pindexLowlandMesoUv=vec2(pindexLowlandMacroUv.x*0.79-pindexLowlandMacroUv.y*0.61,pindexLowlandMacroUv.x*0.61+pindexLowlandMacroUv.y*0.79)*${(RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.lowlandMacroScaleMeters / RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.lowlandMesoScaleMeters).toFixed(4)}+vec2(-12.3,7.4);\nvec2 pindexLowlandPatchUv=vec2(vPindexQualityWorldPosition.x*0.63-vPindexQualityWorldPosition.z*0.78,vPindexQualityWorldPosition.x*0.78+vPindexQualityWorldPosition.z*0.63)/${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.lowlandPatchScaleMeters.toFixed(1)}+vec2(21.7,-14.2);\nfloat pindexLowlandMacro=texture2D(pindexQualityDetailAtlas,pindexLowlandMacroUv).r*0.58+texture2D(pindexQualityDetailAtlas,pindexLowlandMacroUv*1.73+vec2(8.6,3.1)).g*0.42;\nfloat pindexLowlandMeso=texture2D(pindexQualityDetailAtlas,pindexLowlandMesoUv).g*0.56+texture2D(pindexQualityDetailAtlas,pindexLowlandMesoUv*1.91+vec2(-5.8,18.9)).b*0.44;\nfloat pindexLowlandPatch=texture2D(pindexQualityDetailAtlas,pindexLowlandPatchUv).b*0.62+texture2D(pindexQualityDetailAtlas,pindexLowlandPatchUv*2.17+vec2(13.4,-2.9)).r*0.38;\nfloat pindexLowlandMask=pindexQualityDry*(1.0-smoothstep(0.34,0.72,pindexQualityRelief))*(1.0-smoothstep(92.0,210.0,vPindexQualityWorldPosition.y));\nfloat pindexLowlandWet=pindexLowlandMask*smoothstep(0.58,0.82,pindexLowlandMacro*0.58+(1.0-pindexLowlandMeso)*0.42);\nfloat pindexLowlandDry=pindexLowlandMask*smoothstep(0.58,0.84,pindexLowlandMeso*0.55+pindexLowlandPatch*0.45)*(1.0-pindexLowlandWet*0.62);\nfloat pindexLowlandStone=pindexLowlandMask*smoothstep(0.69,0.91,pindexLowlandPatch*0.62+pindexLowlandMeso*0.38)*(1.0-pindexLowlandWet*0.48);\nvec3 pindexLowlandMeadow=vec3(${PINDEX_QUALITY_V2_MEADOW.r.toFixed(6)},${PINDEX_QUALITY_V2_MEADOW.g.toFixed(6)},${PINDEX_QUALITY_V2_MEADOW.b.toFixed(6)});\nvec3 pindexLowlandDryHeather=vec3(${PINDEX_QUALITY_V2_DRY_HEATHER.r.toFixed(6)},${PINDEX_QUALITY_V2_DRY_HEATHER.g.toFixed(6)},${PINDEX_QUALITY_V2_DRY_HEATHER.b.toFixed(6)});\nvec3 pindexLowlandEarth=vec3(${PINDEX_QUALITY_V2_EXPOSED_EARTH.r.toFixed(6)},${PINDEX_QUALITY_V2_EXPOSED_EARTH.g.toFixed(6)},${PINDEX_QUALITY_V2_EXPOSED_EARTH.b.toFixed(6)});\nvec3 pindexLowlandWetEarth=vec3(${PINDEX_QUALITY_V2_WET_EARTH.r.toFixed(6)},${PINDEX_QUALITY_V2_WET_EARTH.g.toFixed(6)},${PINDEX_QUALITY_V2_WET_EARTH.b.toFixed(6)});\nvec3 pindexLowlandStoneDark=vec3(${PINDEX_QUALITY_V2_GRANITE_SHADOW.r.toFixed(6)},${PINDEX_QUALITY_V2_GRANITE_SHADOW.g.toFixed(6)},${PINDEX_QUALITY_V2_GRANITE_SHADOW.b.toFixed(6)});\nvec3 pindexLowlandStoneLight=vec3(${PINDEX_QUALITY_V2_GRANITE_SUNLIT.r.toFixed(6)},${PINDEX_QUALITY_V2_GRANITE_SUNLIT.g.toFixed(6)},${PINDEX_QUALITY_V2_GRANITE_SUNLIT.b.toFixed(6)});\npindexQualityColor=mix(pindexQualityColor,pindexLowlandMeadow,pindexLowlandWet*0.105);\npindexQualityColor=mix(pindexQualityColor,mix(pindexLowlandDryHeather,pindexLowlandEarth,pindexLowlandPatch),pindexLowlandDry*0.115);\npindexQualityColor=mix(pindexQualityColor,mix(pindexLowlandStoneDark,pindexLowlandStoneLight,pindexLowlandMeso),pindexLowlandStone*0.085);\npindexQualityColor=mix(pindexQualityColor,pindexLowlandWetEarth,pindexLowlandWet*smoothstep(0.64,0.91,1.0-pindexLowlandPatch)*0.060);\npindexQualityColor*=1.0+pindexLowlandMask*((pindexLowlandMeso-pindexLowlandMacro)*0.085+(pindexLowlandPatch-0.5)*0.045);\npindexQualityColor*=1.0+pindexQualityGrain*mix(0.025,${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.shaderColorMicroVariation.toFixed(3)},pindexQualityAtlasData.a);\ndiffuseColor.rgb=mix(diffuseColor.rgb,pindexQualityColor,${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.qualityBlend.toFixed(3)});`)
			.replace('#include <normal_fragment_maps>', `#include <normal_fragment_maps>\nvec2 pindexLowlandNormalUv=vPindexQualityWorldPosition.xz/31.0;\nfloat pindexLowlandNormalXp=texture2D(pindexQualityDetailAtlas,pindexLowlandNormalUv+vec2(0.075,0.0)).r;\nfloat pindexLowlandNormalXm=texture2D(pindexQualityDetailAtlas,pindexLowlandNormalUv-vec2(0.075,0.0)).r;\nfloat pindexLowlandNormalZp=texture2D(pindexQualityDetailAtlas,pindexLowlandNormalUv+vec2(0.0,0.075)).g;\nfloat pindexLowlandNormalZm=texture2D(pindexQualityDetailAtlas,pindexLowlandNormalUv-vec2(0.0,0.075)).g;\nvec3 pindexLowlandNormalPerturb=normalize(vec3((pindexLowlandNormalXp-pindexLowlandNormalXm)*0.12,1.0,(pindexLowlandNormalZp-pindexLowlandNormalZm)*0.12));\nnormal=normalize(mix(normal,mat3(viewMatrix)*pindexLowlandNormalPerturb,pindexLowlandMask*0.075));`)
			...
			.replace('#include <roughnessmap_fragment>', `#include <roughnessmap_fragment>\nroughnessFactor=mix(roughnessFactor,pindexQualityAtlasData.g,${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.roughnessBlend.toFixed(3)});\nfloat pindexQualityRoughNoise=texture2D(pindexQualityDetailAtlas,vPindexQualityWorldPosition.xz*0.033+vec2(0.11,0.57)).b*2.0-1.0;\nfloat pindexLowlandRough=(pindexLowlandMeso-0.5)*0.085+(pindexLowlandPatch-0.5)*0.060+pindexLowlandDry*0.035-pindexLowlandWet*0.055;\nroughnessFactor=clamp(roughnessFactor+pindexQualityRoughNoise*${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.shaderRoughnessMicroVariation.toFixed(3)}+pindexLowlandRough*pindexLowlandMask,0.04,1.0);`);
	};
	material.customProgramCacheKey = () => `${previousCacheKey()}|${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.id}|${RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.aerialMaterialRevision}`;
	material.userData.runtimePindexQualityV2Shader = RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.id;
	material.userData.runtimePindexAerialMaterialRevision = RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.aerialMaterialRevision;
...
export function applyRuntimePindexTerrainQualityV2ToMesh(mesh) {
	...
	if (prior?.policyId === RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.id && prior?.aerialMaterialRevision === RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.aerialMaterialRevision) return prior;
	...
	const summary = Object.freeze({
		...
		aerialMaterialRevision: RUNTIME_PINDEX_TERRAIN_QUALITY_V2_POLICY.aerialMaterialRevision,
		...
		shaderDetail: true,
		worldSpaceLowlandMaterialDomains: true,
