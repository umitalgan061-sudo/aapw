/** Deterministic micro-detail layer for canonical owner-map Pindex-07 only. */
...
export const PINDEX07_DETAIL_POLICY = Object.freeze({
  id: 'owner-map-pindex07-detail-2026-08-12-v1',
  pindex: 7,
  amplitudeBySurface: Object.freeze({ sea: 0.007, lake: 0.007, soil: 0.038, rock: 0.048, snow: 0.022 }),
});

function hash01(x, y) {
  const value = Math.sin(x * 63.221 + y * 11.483 + 79.337) * 43758.5453;
...
export function applyPindex07DetailToTerrainMesh(mesh) {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const amplitude = PINDEX07_DETAIL_POLICY.amplitudeBySurface[c.surface] ?? 0;
  ...
  const summary = Object.freeze({ policyId: PINDEX07_DETAIL_POLICY.id, pindex: 7, touchedVertices });
...
export function applyPindex07DetailToTerrainGroup(terrainGroup) {
  ...
  let touchedVertices = 0;
  for (const mesh of terrainGroup.children) touchedVertices += applyPindex07DetailToTerrainMesh(mesh).touchedVertices;
  const summary = Object.freeze({ policyId: PINDEX07_DETAIL_POLICY.id, pindex: 7, touchedVertices, meshCount: terrainGroup.children.length });
