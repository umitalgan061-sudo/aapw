...
// Canonical Pindex-05 composition is sea 453 / soil 187 / rock 0 / snow 0, so this strip is
// coastline plus a proportionally larger soil body than Pindex-04 (187/640 vs 149/576 cells).
// Per-vertex soil/sea amplitudes are therefore held marginally below Run282 so the wider soil
// area does not read as noisier than its neighbour. Rock/snow entries are inert here and exist
// only to keep the per-pindex policy schema identical across Run277..Run306.
export const PINDEX05_DETAIL_POLICY = Object.freeze({
  id: 'owner-map-pindex05-detail-2026-08-12-v1',
  pindex: 5,
...
export function applyPindex05DetailToTerrainMesh(mesh) {
  ...
  mesh.userData.run306Pindex05Detail = summary;
  return summary;
}
...
export function applyPindex05DetailToTerrainGroup(terrainGroup) {
  ...
  terrainGroup.userData.run306Pindex05Detail = summary;
  return summary;
}
