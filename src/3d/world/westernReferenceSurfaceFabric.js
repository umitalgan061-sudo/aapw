...
export const WESTERN_REFERENCE_SURFACE_FABRIC_POLICY = Object.freeze({
  id: 'western-reference-surface-fabric-2026-09-06-v22-depositional-meso-depth',
  renderOnly: true,
  ...
  warpMeters: 184,
  drainageMeters: 760,
  braidedDrainageMeters: 470,
  alluviumMeters: 1420,
  depositionalFanMeters: 1640,
  terraceMeters: 540,
  microTerraceMeters: 190,
  fractureMeters: 276,
  frostWashMeters: 590,
  erosionMeters: 312,
  ...
  wetHollowMeters: 430,
  dryBenchMeters: 1380,
  aerialReliefMeters: 920,
  surfaceCarrierMeters: 254,
  soilShadeAmplitude: 0.266,
  rockShadeAmplitude: 0.202,
...
function sampleFabric(worldX, worldZ) {
  ...
  const braidedDrainage = smoothstep(0.70, 0.935, braidedCarrier)
    * smoothstep(0.32, 0.84, moisture) * (0.38 + (1 - aerialRelief) * 0.36 + surfaceCarrier * 0.26)
  ...
  const terrace = smoothstep(0.59, 0.89, ridge01(valueNoise2D(
    x - z * 0.28 + secondaryWarp * 120,
    z + x * 0.16 - secondaryWarp * 90,
    P.terraceMeters,
    113.9,
  ))) * smoothstep(0.33, 0.80, weathering) * (1 - wetHollow * 0.54)
    * (0.40 + broadRelief * 0.32 + surfaceCarrier * 0.28);
  const microTerrace = smoothstep(0.61, 0.90, ridge01(valueNoise2D(
    x + z * 0.31 + secondaryWarp * 48,
    z - x * 0.18 - warpZ * 36,
    P.microTerraceMeters,
    117.3,
  ))) * smoothstep(0.36, 0.83, weathering) * (1 - wetHollow * 0.62)
...
function soilColor(base, f) {
  const shade = 1
    + (f.broadRelief - 0.5) * 0.235 + (f.continental - 0.5) * 0.070
    + (f.aerialRelief - 0.5) * 0.250 + (f.surfaceCarrier - 0.5) * 0.180
    + (f.macro - 0.5) * 0.154 + (f.meso - 0.5) * 0.150
    + (f.fine - 0.5) * 0.060 + (f.micro - 0.5) * 0.020 + (f.subMicro - 0.5) * 0.012
    - f.drainageThread * 0.145 - f.braidedDrainage * 0.125 - f.wetHollow * 0.120 + f.exposedInterfluve * 0.074
    + f.frostWash * 0.064 + f.colluvium * 0.050 + f.stonyPatch * 0.036
    - f.erosionScour * 0.160 - f.heathMosaic * 0.044 + (f.pastureBreak - 0.5) * 0.040
    + f.dryBench * 0.200 + f.alluvium * 0.038 - f.depositionalFan * 0.095 + f.terrace * 0.150 + f.microTerrace * 0.105;
  shadeColor(base, THREE.MathUtils.clamp(shade, 0.48, 1.52));
  ...
  tint(base, COLORS.soilDrainage, clamp01(f.drainageThread * 0.292 + f.braidedDrainage * 0.242 + f.wetHollow * 0.178));
  tint(base, COLORS.soilAlluvium, clamp01(f.alluvium * 0.170 + f.depositionalFan * 0.228 + f.braidedDrainage * f.alluvium * 0.092));
  tint(base, COLORS.soilOxide, clamp01(f.frostWash * f.exposedInterfluve * 0.216 + f.dryBench * f.mineral * 0.196 + f.terrace * f.mineral * 0.176 + f.microTerrace * f.mineral * 0.116));
  tint(base, COLORS.soilStony, clamp01(f.stonyPatch * 0.188 + f.erosionScour * f.mineral * 0.170 + f.colluvium * 0.112 + f.dryBench * 0.158 + f.terrace * 0.148 + f.microTerrace * 0.104));
  tint(base, COLORS.grassLush, f.pastureBreak * f.moisture * 0.134);
  tint(base, COLORS.grassDry, clamp01(f.pastureBreak * (1 - f.moisture) * 0.116 + f.dryBench * 0.174 + f.terrace * (1 - f.moisture) * 0.152 + f.microTerrace * (1 - f.moisture) * 0.090));
}

function rockColor(base, f) {
  const strata = ridge01((f.streak * 0.74 + f.meso * 0.26) % 1);
  const shade = 1
    + (f.broadRelief - 0.5) * 0.140 + (f.continental - 0.5) * 0.052
    + (f.aerialRelief - 0.5) * 0.160 + (f.surfaceCarrier - 0.5) * 0.106
    ...
    + f.colluvium * 0.044 - f.stonyPatch * 0.030 - f.erosionScour * 0.138 + f.dryBench * 0.118
    + f.terrace * 0.086 + f.microTerrace * 0.064 - f.depositionalFan * 0.038 - f.braidedDrainage * 0.042;
  ...
  tint(base, COLORS.rockExposed, clamp01(f.exposedInterfluve * 0.170 + f.erosionScour * 0.126 + f.dryBench * 0.146 + f.terrace * 0.140 + f.microTerrace * 0.094));
