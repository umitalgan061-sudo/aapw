 ...
 * adapter decorrelates the shipped MeshStandardMaterial in world metres so the 22 m micro-normal
 * period is no longer the only readable ground-scale signal.
 */
export const TERRAIN_APERIODIC_SURFACE_POLICY = Object.freeze({
	id: 'terrain-aperiodic-macro-breakup-2026-08-19-v2',
...
const INSTALL_FLAG = Symbol.for('aapw.terrainAperiodicSurface.install.v2');
const MATERIAL_FLAG = Symbol.for('aapw.terrainAperiodicSurface.material.v2');
...
void main() {`)
	...
	// The shipped terrain already owns a world-space 22 m normal/roughness atlas. Do not replace it:
	// vary only its perceptual normal contrast with the aperiodic field so identical 22 m tiles stop
	// reading at equal strength. At far distance aapwTerrainBreakup -> 0, avoiding new shimmer.
