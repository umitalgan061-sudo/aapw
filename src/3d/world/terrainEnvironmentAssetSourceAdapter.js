...
export const TERRAIN_ENVIRONMENT_ASSET_SOURCE_POLICY = freeze({
  id: 'terrain-environment-asset-source-adapter-2026-09-07-v1',
...
function familyAllowed(profile, family) {
...
export function sourceCandidates(category, { biome = '', season = '', winter = false, assetManifest = TERRAIN_ENVIRONMENT_ASSET_MANIFEST } = {}) {
  ...
  const candidates = assetManifest.filter((entry) => familyAllowed(profile, entry.family))
...
export function sourceCandidateScore(entry, { category = '', biome = '', season = '', winter = false, preferred = true, sample = {} } = {}) {
  ...
  if (!profile || !familyAllowed(profile, entry.family)) return 0;
  ...
  if (season && norm(entry.season) === norm(season)) score += 0.10;
  if (sample) {
...
export function selectSourceAsset(category, options = {}) {
  ...
  const ranked = candidates.map((entry, index) => ({
    entry,
    score: sourceCandidateScore(entry, { category, ...options }),
    index,
...
export function resolveRuntimeEnvironmentSource(category, options = {}) {
  ...
  return freeze({
    ...
    policyId: TERRAIN_ENVIRONMENT_ASSET_SOURCE_POLICY.id,
  });
}
...
export function buildRuntimeSourceManifest(requests = []) {
  const entries = requests.map((request) => {
    ...
    return freeze({
      ...
      resolved,
      acceptance: freeze({
        ok: Boolean(resolved?.ok && resolved?.source?.src),
        placeholderAllowed: false,
        proceduralReplacementAllowed: false,
      }),
    });
...
export function sourceManifestByBiome(biome) {
  const b = norm(biome);
  return freeze(TERRAIN_ENVIRONMENT_ASSET_MANIFEST.filter((entry) => (entry.biomes ?? []).some((value) => norm(value) === b)));
}
...
export function sourceCoverageSummary(categories = ['tree', 'shrub', 'grass', 'snow-patch']) {
  const rows = categories.map((category) => {
    const candidates = sourceCandidates(category, {});
    return freeze({ category, candidateCount: candidates.length, preferredCount: candidates.filter((entry) => entry.preferred).length, authoredCount: candidates.filter((entry) => Boolean(entry.src)).length });
  });
  return freeze({ policyId: TERRAIN_ENVIRONMENT_ASSET_SOURCE_POLICY.id, rows, missing: freeze(rows.filter((row) => row.authoredCount === 0).map((row) => row.category)), ok: rows.every((row) => row.authoredCount > 0) });
}
