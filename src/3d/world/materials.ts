/**
 * TypeScript production source for deterministic world PBR materials.
 * Legacy `materials.js` is retained only as a compatibility shim.
 *
 ...
 */
export function createStoneMaterial({ seed, baseColor, repeat = 12 }: { readonly seed: number; readonly baseColor: THREE.Color; readonly repeat?: number }) {
 ...
 * @returns {THREE.MeshStandardMaterial}
 */
export function createRoofMaterial({ seed, repeat = 6 }: { readonly seed: number; readonly repeat?: number }) {
	const random = mulberry32(seed);
	const rowCount = 10;
 ...
 */
export function disposeCastleMaterial(material: THREE.MeshStandardMaterial): void {
 ...
 */
export function prepareImportedGeometryForTexturing(model: THREE.Object3D, { modelScale, metersPerTile = 4 }: { readonly modelScale: number; readonly metersPerTile?: number }) {
	let meshes = 0;
	let uvsGenerated = 0;
