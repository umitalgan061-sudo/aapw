/** Production TypeScript owner for src/3d/world/settlements.js. Legacy .js remains compatibility-only. */
// @ts-nocheck
/**
...
import { createStoneMaterial, createRoofMaterial, disposeCastleMaterial, prepareImportedGeometryForTexturing } from './materials.ts';
...
export async function spawnRealCastleModels({ assetLoader, seats, seed }) {
	...
	const loadOnce = (file) => {
		if (!loadedByFile.has(file)) {
			loadedByFile.set(file, assetLoader.loadModel(file, { fallbackColor: 0x8a8578, fallbackSize: REAL_CASTLE_FOOTPRINT_METERS }));
		}
...
export interface SettlementSeat { readonly id:string; readonly name:string; readonly x:number; readonly y:number }
