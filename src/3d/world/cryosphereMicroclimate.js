 ...
 *
 * The canonical north/always-winter zones remain the only authority for where cold climate exists.
 * This module adds bounded world-space variation inside those authored zones: wind exposure, sheltered
 * hollows, frost pockets, maritime moderation and snow persistence. It never changes terrain height,
 * shoreline placement, hydrology, collider geometry, settlements or canonical map coverage.
...
function sampleMacroExposure(worldX, worldZ) {
	...
	return clamp01(macro * 0.56 + meso * 0.30 + fine * 0.14);
}
...
function sampleWindExposure(worldX, worldZ) {
	...
	return clamp01(smoothstep(0.34, 0.82, alongField * 0.58 + crossField * 0.42));
}
...
function sampleSnowPersistence({ exposure, shelter, frostPocket, maritimeModeration, coastal }) {
	const P = CRYOSPHERE_MICROCLIMATE_POLICY.microclimate;
	return clamp01(
		0.50
		+ shelter * P.shelterGain
		+ frostPocket * P.frostPocketGain
		+ (1 - exposure) * P.snowPersistenceGain
...
export function cryosphereMicroclimateAtWorldXZ(worldX, worldZ, normalizedX = 0.5) {
	if (!Number.isFinite(worldX) || !Number.isFinite(worldZ)) throw new TypeError('cryosphere microclimate coordinates must be finite');
	...
	return Object.freeze({ worldX, worldZ, normalizedX: safeX, exposure, shelter, frostPocket, macroExposure, maritimeModeration, coastal, snowPersistence, tundraResponse, iceResponse, vegetationSuppression: sampleVegetationSuppression({ permanentIce: 0, tundra: 0, exposure, frostPocket, shelter }) });
}
...
export function modulateCryosphereWeights(base, microclimate) {
	...
	const permanentIce = clamp01(permanentIceBase + permanentIceBase * (iceResponseDelta + exposureDelta * 0.70 + shelterDelta * 0.36 + frostDelta * 0.44) - (1 - permanentIceBase) * maritimeDelta * 0.14);
	const tundra = clamp01(tundraBase + tundraBase * (tundraResponseDelta + exposureDelta * 0.42 + shelterDelta * 0.50 + frostDelta * 0.54) - (1 - tundraBase) * maritimeDelta * 0.08);
	return Object.freeze({ ...base, permanentIce, tundra, microclimateResponse: Object.freeze({ exposure, shelter, frostPocket, persistence: clamp01(microclimate.snowPersistence), maritime, iceResponse, tundraResponse, boundedGain: true }) });
}
...
export function vegetationCryosphereSuppressionAtWorldXZ(worldX, worldZ, climate) {
	...
	const suppression = sampleVegetationSuppression({ permanentIce: clamp01(climate?.permanentIce), tundra: clamp01(climate?.tundra), exposure: micro.exposure, frostPocket: micro.frostPocket, shelter: micro.shelter });
...
export function cryosphereTransitionDiagnostics(samples) {
	...
	let minIce = 1, maxIce = 0, minTundra = 1, maxTundra = 0, minPersistence = 1, maxPersistence = 0;
	for (const sample of samples) {
		...
		minIce = Math.min(minIce, ice); maxIce = Math.max(maxIce, ice);
		minTundra = Math.min(minTundra, tundra); maxTundra = Math.max(maxTundra, tundra);
		minPersistence = Math.min(minPersistence, persistence); maxPersistence = Math.max(maxPersistence, persistence);
	}
	return Object.freeze({ sampleCount: samples.length, iceRange: Object.freeze({ min: minIce, max: maxIce }), tundraRange: Object.freeze({ min: minTundra, max: maxTundra }), snowPersistenceRange: Object.freeze({ min: minPersistence, max: maxPersistence }), finite: samples.every((sample) => Number.isFinite(sample?.permanentIce) && Number.isFinite(sample?.tundra)) });
}

export function assertCryosphereMicroclimateContract() {
	const probes = [[0, 0, 0.12], [700, -420, 0.28], [-1300, 860, 0.44], [2140, 1320, 0.72], [-2870, -1040, 0.91]];
	for (const [x, z, nx] of probes) {
		const sample = cryosphereMicroclimateAtWorldXZ(x, z, nx);
		if (![sample.exposure, sample.shelter, sample.frostPocket, sample.snowPersistence, sample.tundraResponse, sample.iceResponse].every(Number.isFinite)) throw new Error('cryosphere microclimate produced a non-finite value');
	}
	return true;
}

export const __TEST_ONLY__ = Object.freeze({ hashCell, smoothNoiseAt, cryosphereMicroclimateAtWorldXZ, modulateCryosphereWeights, cryosphereTransitionDiagnostics });
