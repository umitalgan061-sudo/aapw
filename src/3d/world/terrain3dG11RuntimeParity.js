 ...
 *
 * The artifact is generated by the pinned Terrain3D authoring pipeline and contains a regular
 * high-resolution height grid in normalized owner-map coordinates. GeoCell coordinates are only
 * the artifact address; interpolation is continuous and no grid edge is introduced as terrain.
 */
...
export function createG11Terrain3DBakeSampler(bake) {
	...
	const sx = (bake.width - 1) / (maxX - minX);
	const sy = (bake.height - 1) / (maxY - minY);
	return function sampleNormalized(nx, ny) {
		...
		const x1 = Math.min(bake.width - 1, x0 + 1), y1 = Math.min(bake.height - 1, y0 + 1);
