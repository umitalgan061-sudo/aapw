...
function valueNoise(x, z, seed) {
  ...
  return lerp(lerp(hash2D(ix, iz, seed), hash2D(ix + 1, iz, seed), ux), lerp(hash2D(ix, iz + 1, seed), hash2D(ix + 1, iz + 1, seed), ux), uz);
}
function fbm(x, z, seed, octaves = 4) {
  let total = 0; let weight = 0; let amplitude = 0.55;
  for (let i = 0; i < octaves; i += 1) { total += valueNoise(x, z, seed + i * 73) * amplitude; weight += amplitude; x = x * 2.03 + 11.2; z = z * 2.03 - 7.9; amplitude *= 0.48; }
  return total / Math.max(1e-9, weight);
}
function ridge(x, z, seed) { return 1 - Math.abs(fbm(x, z, seed) * 2 - 1); }
function rotate(x, z, radians) { const c = Math.cos(radians); const s = Math.sin(radians); return { x: x * c - z * s, z: x * s + z * c }; }
function profileField(worldX, worldZ) { return clamp01(fbm(worldX / 5200, worldZ / 5200, 0x3101, 5) * 0.45 + fbm(worldX / 760, worldZ / 760, 0x3102) * 0.35 + fbm(worldX / 190, worldZ / 190, 0x3103) * 0.20); }
function profileAt(worldX, worldZ) {
  const field = profileField(worldX, worldZ); const index = sedimentProfileIndex(field); const next = (index + 1) % TERRAIN_SEDIMENT_PROFILES.length; const blend = smoothstep(0.35, 0.88, valueNoise(worldX / 240, worldZ / 240, 0x3104)); const left = TERRAIN_SEDIMENT_PROFILES[index]; const right = TERRAIN_SEDIMENT_PROFILES[next];
  return Object.freeze({ index, field, blend, deposit: lerp(left.deposit, right.deposit, blend), wash: lerp(left.wash, right.wash, blend), film: lerp(left.film, right.film, blend), crust: lerp(left.crust, right.crust, blend), aggregate: lerp(left.aggregate, right.aggregate, blend), cool: lerp(left.cool, right.cool, blend) });
}
function basinSignal(worldX, worldZ, heightMeters, slopeDegrees) { const broad = fbm(worldX / 760, worldZ / 760, 0x4210, 5); const meso = fbm(worldX / 190, worldZ / 190, 0x4211); const pocket = ridge(worldX / 96, worldZ / 96, 0x4212); const lowland = 1 - smoothstep(95, 240, heightMeters); const gentle = 1 - smoothstep(1.5, 9.5, slopeDegrees); return clamp01((gentle * 0.48 + lowland * 0.18 + (1 - broad) * 0.18 + pocket * 0.16) * (0.58 + meso * 0.42)); }
function rainwashSignal(worldX, worldZ, slopeDegrees, basin) { const rainAxis = rotate(worldX, worldZ, TERRAIN_SEDIMENT_POLICY.rainBearingRadians); const directional = fbm(rainAxis.x / 31, rainAxis.z / 31, 0x4310); const cross = fbm(rainAxis.z / 53, rainAxis.x / 23, 0x4311, 3); const channel = ridge(worldX / 31, worldZ / 83, 0x4312); const washSlope = smoothstep(4, 31, slopeDegrees); const strong = smoothstep(18, 44, slopeDegrees); const lane = smoothstep(0.55, 0.86, channel * 0.55 + directional * 0.30 + cross * 0.15); return clamp01(washSlope * (0.30 + basin * 0.44 + lane * 0.44) + strong * 0.16); }
function aggregateSignal(worldX, worldZ) { return clamp01(ridge(worldX / 13, worldZ / 21, 0x4410) * 0.68 + fbm(worldX / 37, worldZ / 37, 0x4411, 3) * 0.32); }
function filmSignal(worldX, worldZ, basin, slopeDegrees) { const storm = fbm(worldX / 52, worldZ / 52, 0x4510); const micro = valueNoise(worldX / 28, worldZ / 28, 0x4511); return clamp01(basin * (0.28 + storm * 0.50 + micro * 0.22) * (1 - smoothstep(3, 24, slopeDegrees))); }
function crustSignal(worldX, worldZ, heightMeters, basin, wash) { const drying = fbm(worldX / 410, worldZ / 410, 0x4610); return clamp01((1 - basin * 0.72) * smoothstep(45, 520, heightMeters) * (1 - wash * 0.74) * (0.62 + drying * 0.38)); }
function microPoreSignal(worldX, worldZ) { return clamp01(valueNoise(worldX / 4.2, worldZ / 4.2, 0x4710) * 0.55 + ridge(worldX / 7.5, worldZ / 5.8, 0x4711) * 0.45); }
function resolveStateInternal(worldX, worldZ, heightMeters, slopeDegrees) {
  const basin = basinSignal(worldX, worldZ, heightMeters, slopeDegrees); const broadWet = fbm(worldX / 1200, worldZ / 1200, 0x4810); const catchment = clamp01(basin * 0.72 + (1 - broadWet) * 0.28); const wash = rainwashSignal(worldX, worldZ, slopeDegrees, catchment); const aggregate = aggregateSignal(worldX, worldZ); const film = filmSignal(worldX, worldZ, basin, slopeDegrees); const crust = crustSignal(worldX, worldZ, heightMeters, catchment, wash); const pore = microPoreSignal(worldX, worldZ); const profile = profileAt(worldX, worldZ);
  const sedimentLoad = clamp01(catchment * 0.40 + wash * 0.35 + profile.deposit * 0.15 + aggregate * 0.10); const mineralDeposit = clamp01(catchment * 0.36 + wash * 0.18 + profile.deposit * 0.24 + aggregate * 0.22); const muddyFilm = clamp01(film * 0.74 + catchment * 0.17 + profile.film * 0.09); const washBleach = clamp01(wash * 0.52 + aggregate * 0.20 + profile.wash * 0.28); const dryCrust = clamp01(crust * 0.78 + profile.crust * 0.22); const poreRoughness = clamp01(pore * 0.63 + aggregate * 0.37); const transient = clamp01(muddyFilm * 0.64 + dryCrust * 0.18 + film * 0.18);
...
export function resolveTerrainSedimentMaterialResponse({ state, baseColor, baseRoughness = 0.86 }) {
  const p = state.profile; const deposit = state.mineralDeposit * (0.72 + p.deposit * 0.28); const wet = state.muddyFilm * (0.76 + p.film * 0.24); const wash = state.washBleach * (0.76 + p.wash * 0.24); const crust = state.dryCrust * (0.78 + p.crust * 0.22); const aggregate = state.poreRoughness * (0.78 + p.aggregate * 0.22);
  const color = Object.freeze({ r: clamp01(baseColor.r + deposit * 0.045 + wash * 0.020 + crust * 0.018 - wet * 0.022), g: clamp01(baseColor.g + deposit * 0.034 + wash * 0.012 + crust * 0.014 - wet * 0.030), b: clamp01(baseColor.b + deposit * 0.019 + wet * 0.027 - wash * 0.006) });
  return Object.freeze({ color, roughness: clamp01(baseRoughness + aggregate * 0.065 + crust * 0.032 - wet * 0.085), normalStrength: clamp01(state.wash * 0.048 + aggregate * 0.031 + state.film * 0.016) * (0.76 + p.wash * 0.24) });
}
export function sedimentRegionSignature(worldX, worldZ) { const state = resolveStateInternal(Number(worldX) || 0, Number(worldZ) || 0, 70, 8); return Object.freeze({ profileIndex: state.profile.index, basin: Number(state.basin.toFixed(6)), wash: Number(state.wash.toFixed(6)), film: Number(state.film.toFixed(6)), crust: Number(state.crust.toFixed(6)), aggregate: Number(state.aggregate.toFixed(6)) }); }
