...
function normalizeId(value) {
	return String(value ?? `source-${Date.now?.() ?? 0}`).replace(/[^a-zA-Z0-9_.:-]/g, '_').slice(0, MAX_ID_LENGTH);
}
...
function normalizeVelocity(velocity) {
	return {
		x: finiteOr(velocity?.x, 0),
		y: finiteOr(velocity?.y, 0),
		z: finiteOr(velocity?.z, 0),
	};
}

function sourceScore(source, listenerPosition) {
	const p = source.position;
	const l = listenerPosition ?? { x: 0, y: 0, z: 0 };
	const distance = Math.hypot(p.x - l.x, p.y - l.y, p.z - l.z);
	return {
		distance,
...
export class SpatialAudioRegistry {
	...
	update(id, patch = {}) {
		...
		const next = freeze({
			...
			velocity: patch.velocity ? normalizeVelocity(patch.velocity) : current.velocity,
			priority: patch.priority === undefined ? current.priority : clamp(Math.round(finiteOr(patch.priority, current.priority)), 0, 100),
	...
	setState(id, state) {
		if (!Object.values(SOURCE_STATES).includes(state)) return null;
		return this.update(id, { state });
	}

	unregister(id) {
		return this.sources.delete(normalizeId(id));
	}

	get(id) { return this.sources.get(normalizeId(id)) ?? null; }
	...
	clearVirtualized() {
		for (const source of this.sources.values()) if (source.state === SOURCE_STATES.VIRTUAL) this.sources.set(source.id, freeze({ ...source, state: SOURCE_STATES.REGISTERED, sequence: ++this.sequence }));
	}

	snapshot() {
		return freeze({ version: 1, disposed: this.disposed, sequence: this.sequence, maxSources: this.maxSources, maxPositionalSources: this.maxPositionalSources, sourceCount: this.sources.size, sources: [...this.sources.values()].map((source) => ({ ...source, position: { ...source.position }, velocity: { ...source.velocity } })) });
	}
