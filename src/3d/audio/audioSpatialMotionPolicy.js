 ...
 * Audio consumes authoritative gameplay velocities rather than owning physics.
 */
...
export function evaluateAudioSpatialMotion(input = {}) {
  const pitch = computeDopplerPitch(input);
  const gain = computeMotionGain(input);
  return freeze({ version: 1, pitch, gain, relative: computeRelativeVelocity(input.sourceVelocity, input.listenerVelocity), safe: pitch.pitch >= DEFAULTS.minPitch && pitch.pitch <= DEFAULTS.maxPitch && gain.gain >= 0 && gain.gain <= 1 });
}
