...
const STAMINA_STATE_LABELS = Object.freeze({ idle: 'Hazır', walk: 'Yürüme', sprint: 'Depar', dodge: 'Kaçınma', airborne: 'Havada', exhausted: 'Tükendi' });
...
export class HealthBar {
	constructor({ eventsBus, healthChangedEventName, damageEventName, container = document.body }) {
		...
		this._el.setAttribute('role', 'meter'); this._el.setAttribute('aria-label', 'Can'); this._el.setAttribute('aria-valuemin', '0');
		Object.assign(this._el.style, { width: '220px', display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gridTemplateRows: 'auto auto auto', columnGap: '10px' });
		...
		const staminaTrack = document.createElement('div'); staminaTrack.className = 'g3d-health-bar-track g3d-stamina-bar-track'; staminaTrack.style.gridRow = '2'; this._staminaFillEl = document.createElement('div'); this._staminaFillEl.className = 'g3d-health-bar-fill g3d-stamina-bar-fill'; this._staminaFillEl.style.background = 'linear-gradient(90deg, #8a6b15, #e8c85a)'; staminaTrack.appendChild(this._staminaFillEl); this._staminaEl.appendChild(staminaTrack);
		...
		this._motionEventTarget = typeof globalThis.addEventListener === 'function' ? globalThis : null; this._onPlayerMotion = (event) => this._paintStamina(event?.detail); this._motionEventTarget?.addEventListener('aapw:player-motion', this._onPlayerMotion);
	}
	...
	_paintStamina(motion) { const current = motion?.stamina; const maxStamina = motion?.maxStamina; if (!Number.isFinite(current) || !Number.isFinite(maxStamina) || !(maxStamina > 0)) return; const clamped = Math.max(0, Math.min(maxStamina, current)); const ratio = clamped / maxStamina; const rounded = Math.ceil(clamped); const state = typeof motion?.state === 'string' ? motion.state : 'idle'; const stateLabel = STAMINA_STATE_LABELS[state] ?? state; this._staminaFillEl.style.width = `${(ratio * 100).toFixed(1)}%`; this._staminaTextEl.textContent = `${rounded} / ${maxStamina}`; this._staminaEl.setAttribute('aria-valuemax', String(maxStamina)); this._staminaEl.setAttribute('aria-valuenow', String(rounded)); this._staminaEl.setAttribute('aria-valuetext', `${rounded} / ${maxStamina} · ${stateLabel}`); this._staminaEl.dataset.state = state; this._staminaEl.classList.toggle('g3d-stamina-bar-low', ratio > 0 && ratio <= 0.25); this._staminaEl.classList.toggle('g3d-stamina-bar-exhausted', state === 'exhausted'); this._staminaFillEl.style.background = state === 'exhausted' ? 'linear-gradient(90deg, #5f513a, #8c7651)' : (ratio > 0 && ratio <= 0.25 ? 'linear-gradient(90deg, #7a5a0c, #d99a22)' : 'linear-gradient(90deg, #8a6b15, #e8c85a)'); this._staminaFillEl.style.filter = state === 'dodge' ? 'brightness(1.22)' : ''; }
	...
	dispose() { this._eventsBus.off(this._healthChangedEventName, this._onHealthChanged); this._eventsBus.off(this._damageEventName, this._onDamage); this._motionEventTarget?.removeEventListener('aapw:player-motion', this._onPlayerMotion); if (this._flashTimeoutId !== null) clearTimeout(this._flashTimeoutId); this._el.remove(); }
}
