...
export class HealthBar {
	constructor({ eventsBus, healthChangedEventName, damageEventName, container = document.body }) {
		...
		this._combatLock = null; this._combatAttack = null; this._combatDefense = null; this._combatDodgeInvulnerable = false;
	...
	_paintMotion(motion) {
		...
		if (dodgeInvulnerable !== this._combatDodgeInvulnerable) { this._combatDodgeInvulnerable = dodgeInvulnerable; this._renderCombatStatus(); }
	...
	_renderCombatStatus(transient = null) {
		...
		const defenseText = this._combatDefense
			...
			: null;
		const dodgeText = this._combatDodgeInvulnerable ? 'KAÇINMA · DOKUNULMAZ' : null;
		...
		this._combatTextEl.textContent = text; this._combatEl.dataset.state = this._combatDefense ? `defense-${this._combatDefense.mitigation}` : this._combatAttack ? `attack-${this._combatAttack.phase}` : this._combatDodgeInvulnerable ? 'dodge-invulnerable' : this._combatLock ? 'locked' : transient ? 'no-target' : 'free'; this._combatEl.dataset.range = targetInRange === null ? 'unknown' : targetInRange ? 'in-range' : 'out-of-range'; this._combatEl.classList.toggle('g3d-combat-status-active', Boolean(this._combatAttack || this._combatDefense || this._combatDodgeInvulnerable)); this._combatEl.classList.toggle('g3d-combat-status-locked', Boolean(this._combatLock));
	}
