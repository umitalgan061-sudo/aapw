...
export class HealthBar {
	constructor({ eventsBus, healthChangedEventName, damageEventName, container = document.body }) {
		...
		this._combatLock = null; this._combatAttack = null; this._combatDefense = null; this._combatDodgeInvulnerable = false; this._combatDodgeRecovery = false;
	...
	_paintMotion(motion) {
		...
		const dodgeInvulnerable = state === 'dodge' && motion?.dodgeInvulnerable === true;
		const dodgeRecovery = state === 'dodge' && !dodgeInvulnerable && (this._combatDodgeRecovery || wasDodgeInvulnerable);
		if (dodgeInvulnerable !== this._combatDodgeInvulnerable || dodgeRecovery !== this._combatDodgeRecovery) { this._combatDodgeInvulnerable = dodgeInvulnerable; this._combatDodgeRecovery = dodgeRecovery; this._renderCombatStatus(); }
	...
	_renderCombatStatus(transient = null) {
		...
		const defenseText = this._combatDefense
			...
			: null;
		const dodgeText = this._combatDodgeInvulnerable ? 'KAÇINMA · DOKUNULMAZ' : this._combatDodgeRecovery ? 'KAÇINMA · TOPARLANMA · SAVUNMASIZ' : null;
		const primary = defenseText ?? attackText ?? dodgeText; const text = primary ? `${primary}${lockText ? ` · ${lockText}` : ''}${rangeText ? ` · ${rangeText}` : ''}` : (transient ?? lockText ?? 'Serbest');
		this._combatTextEl.textContent = text; this._combatEl.dataset.state = this._combatDefense ? `defense-${this._combatDefense.mitigation}` : this._combatAttack ? `attack-${this._combatAttack.phase}` : this._combatDodgeInvulnerable ? 'dodge-invulnerable' : this._combatDodgeRecovery ? 'dodge-recovery' : this._combatLock ? 'locked' : transient ? 'no-target' : 'free'; this._combatEl.dataset.range = targetInRange === null ? 'unknown' : targetInRange ? 'in-range' : 'out-of-range'; this._combatEl.classList.toggle('g3d-combat-status-active', Boolean(this._combatAttack || this._combatDefense || this._combatDodgeInvulnerable || this._combatDodgeRecovery)); this._combatEl.classList.toggle('g3d-combat-status-locked', Boolean(this._combatLock));
	}
