...
export class TouchJoystick {
	...
	_handlePointerMove(event) {
		...
		const dx = event.clientX - this._origin.x, dy = event.clientY - this._origin.y; const distance = Math.hypot(dx, dy), clampedDistance = Math.min(distance, this._radiusPx); const scale = distance > 0 ? clampedDistance / distance : 0;
