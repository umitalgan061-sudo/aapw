...
export class TouchJoystick {
	constructor(container = document.body, { isInputBlocked = isPlayerGameplayInputBlocked } = {}) {
		...
		this._syncGuardHeld = () => { this._guardHeld = this._guardPointerIds.size > 0; this._guardButton.setAttribute('aria-pressed', String(this._guardHeld)); };
		this._onGuardDown = (event) => {
			if (readPlayerGameplayInputBlocked(this._isInputBlocked)) { event.preventDefault?.(); return; }
			this._guardPointerIds.add(event.pointerId); this._syncGuardHeld();
		...
		this._onGuardUp = (event) => { this._guardPointerIds.delete(event.pointerId); this._syncGuardHeld(); event.preventDefault?.(); };
		this._onGuardLostPointerCapture = (event) => { this._guardPointerIds.delete(event.pointerId); this._syncGuardHeld(); };
		...
		this._onParry = (event) => { if (readPlayerGameplayInputBlocked(this._isInputBlocked)) { event.preventDefault?.(); return; } if (this._guardHeld) this._parryRearmPending = true; else this._parryRequested = true; event.preventDefault?.(); };
		...
		this._onPageHide = () => this._resetGameplayState(); this._onWindowBlur = () => this._resetGameplayState();
		this._visibilityTarget?.addEventListener('visibilitychange', this._onVisibilityChange); this._pageLifecycleTarget?.addEventListener('pagehide', this._onPageHide); this._pageLifecycleTarget?.addEventListener('blur', this._onWindowBlur);
	}
	_resetMovementState() { this._pointerId = null; this._dragX = 0; this._dragY = 0; this._knob.style.transform = ''; this._base.classList.remove('g3d-joystick-active'); }
	_resetGameplayState() {
		const pointerId = this._pointerId;
		if (pointerId !== null) { try { if (this._base.hasPointerCapture?.(pointerId)) this._base.releasePointerCapture?.(pointerId); } catch { /* input reset must stay fail-closed */ } }
		for (const guardPointerId of this._guardPointerIds) { try { if (this._guardButton.hasPointerCapture?.(guardPointerId)) this._guardButton.releasePointerCapture?.(guardPointerId); } catch { /* input reset must stay fail-closed */ } }
		this._resetMovementState(); this._jumpRequested = false; this._dodgeRequested = false; this._parryRequested = false; this._parryRearmPending = false; this._lockOnRequested = false; this._guardPointerIds.clear(); this._guardHeld = false; this._guardButton.setAttribute('aria-pressed', 'false');
	}
	_handlePointerDown(event) {
		...
		try { this._base.setPointerCapture?.(event.pointerId); } catch { /* capture is advisory; active pointer bookkeeping remains authoritative */ }
		event.preventDefault?.();
	}
	_handlePointerMove(event) { if (readPlayerGameplayInputBlocked(this._isInputBlocked)) { this._resetGameplayState(); event.preventDefault?.(); return; } if (event.pointerId !== this._pointerId) return; const dx = event.clientX - this._origin.x, dy = event.clientY - this._origin.y, distance = Math.hypot(dx, dy), clampedDistance = Math.min(distance, this._radiusPx), scale = distance > 0 ? clampedDistance / distance : 0; this._dragX = dx * scale; this._dragY = dy * scale; this._knob.style.transform = `translate(${this._dragX}px, ${this._dragY}px)`; event.preventDefault(); }
	_handlePointerUp(event) { if (event.pointerId !== this._pointerId) return; this._resetMovementState(); }
	getAxes() {
		...
		if (this._parryRearmPending) { this._parryRearmPending = false; this._parryRequested = true; parryRequested = false; }
	...
	setLockOnActive(active) { const locked = Boolean(active); this._lockOnButton.setAttribute('aria-pressed', String(locked)); this._lockOnButton.classList.toggle('g3d-touch-lock-on-active', locked); this._lockOnButton.textContent = locked ? 'Kilitli' : 'Hedef'; }
	dispose() {
		this._visibilityTarget?.removeEventListener('visibilitychange', this._onVisibilityChange); this._pageLifecycleTarget?.removeEventListener('pagehide', this._onPageHide); this._pageLifecycleTarget?.removeEventListener('blur', this._onWindowBlur);
