...
export class TouchJoystick {
	constructor(container = document.body) {
		...
		this._guardButton = document.createElement('button');
		this._guardButton.type = 'button';
		this._guardButton.className = 'g3d-touch-guard-button';
		this._guardButton.textContent = 'Savun';
		this._guardButton.setAttribute('aria-label', 'Savun');
		this._guardButton.setAttribute('aria-pressed', 'false');
		Object.assign(this._guardButton.style, {
			position: 'fixed', right: '112px', bottom: '96px', zIndex: '30', minWidth: '72px',
			minHeight: '48px', borderRadius: '999px', opacity: '0.86', touchAction: 'none',
		});
		...
		this._guardButton.addEventListener('pointerdown', this._onGuardDown);
		this._guardButton.addEventListener('pointerup', this._onGuardUp);
		this._guardButton.addEventListener('pointercancel', this._onGuardUp);
		this._guardButton.addEventListener('pointerleave', this._onGuardUp);
	...
	_handlePointerMove(event) {
		...
		const dx = event.clientX - this._origin.x, dy = event.clientY - this._origin.y;
		const distance = Math.hypot(dx, dy), clampedDistance = Math.min(distance, this._radiusPx);
		const scale = distance > 0 ? clampedDistance / distance : 0;
		this._dragX = dx * scale; this._dragY = dy * scale;
		this._knob.style.transform = `translate(${this._dragX}px, ${this._dragY}px)`;
		event.preventDefault();
	}
	_handlePointerUp(event) {
		if (event.pointerId !== this._pointerId) return;
		this._pointerId = null; this._dragX = 0; this._dragY = 0;
	...
	dispose() {
		...
		this._guardButton.removeEventListener('pointerdown', this._onGuardDown);
		this._guardButton.removeEventListener('pointerup', this._onGuardUp);
		this._guardButton.removeEventListener('pointercancel', this._onGuardUp);
		this._guardButton.removeEventListener('pointerleave', this._onGuardUp);
		this._jumpRequested = false; this._guardHeld = false;
		this._guardButton.remove(); this._jumpButton.remove(); this._base.remove();
	}
}
