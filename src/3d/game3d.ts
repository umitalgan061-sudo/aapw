/** Production TypeScript 3D application runtime. Legacy game3d.js is a compatibility shim. */
...
import { updateAuroraSky, disposeAuroraSky } from './sky.ts';
...
export interface TouchJoystickRuntime {
  readonly getAxes: () => import('./gameLoopHelpers.ts').MovementAxes;
...
export interface InteractionRuntime {
  ...
  readonly update: (entities: readonly RuntimeEntity[], playerPosition: THREE.Vector3) => void;
}
...
export interface SettlementCompassRuntime extends UIObjectRuntime {
  ...
  readonly update: (position: THREE.Vector3, rotationY: number) => void;
}
...
export interface Game3DState extends ReturnType<typeof createScene> {
  ...
  keyboardInput: import('./input.ts').KeyboardInput;
  ...
  mobileSpawnVegetation: THREE.Object3D | null;
  realCastles: THREE.Object3D;
...
gameEvents.on(EVENTS.ASSET_PROGRESS, (payload: { readonly ratio?: number }) => {
	if (typeof payload.ratio === 'number') gameState.set('loadProgress', payload.ratio);
});
...
gameEvents.on(EVENTS.ASSET_ERROR, (payload: unknown) => {
 ...
 */
export async function initGame3D(): Promise<void> {
	try {
		...
		const state = createScene(canvas) as Game3DState;
		...
		const handleInteractKeyDown = (event: KeyboardEvent) => state.interaction.handleKeyDown(event);
		...
		state.dialogueBox.setChoiceHandler((index: number) => state.interaction.handleChoice(index));
		...
		const unsubscribeWeatherTrigger = gameEvents.on(EVENTS.WORLD_EVENT_TRIGGERED, (payload: { readonly id?: string }) => {
		...
		const tick = () => {
			...
			const touchJumpRequested = Boolean(state.touchJoystick?.consumeJumpRequested?.());
			const effectiveKeyboardAxes = touchJumpRequested ? Object.freeze({ ...keyboardAxes, jumpRequested: true }) : keyboardAxes;
			const axes = combineAxes(effectiveKeyboardAxes, state.touchJoystick?.getAxes() ?? null);
			...
			state.player.update(delta, moveDirection, axes.running, effectiveKeyboardAxes.jumpRequested);
			...
			state.npcs = updateEntitiesSafely({
				...
				scene: state.scene,
				label: 'NPC',
				update: (npc: RuntimeEntity) => npc.update(delta, playerPos),
			});
			...
			state.animals = updateEntitiesSafely({
				...
				scene: state.scene,
				label: 'Animal',
				update: (animal: RuntimeEntity) => animal.update(
					delta,
					playerPos,
					state.animals
			...
			state.creatures = updateEntitiesSafely({
				...
				scene: state.scene,
				label: 'Creature',
				update: (creature: RuntimeEntity) => creature.update(
					delta,
					playerPos,
					state.creatures
			...
			state.carts = updateEntitiesSafely({
				...
				scene: state.scene,
				label: 'Cart',
				update: (cart: RuntimeEntity) => cart.update(delta),
			});
			...
			state.dragons = updateEntitiesSafely({
				...
				scene: state.scene,
				label: 'Dragon',
				update: (dragon: RuntimeEntity) => dragon.update(delta, playerPos),
			});
		...
		gameState.set('error', error instanceof Error ? error.message : String(error));
