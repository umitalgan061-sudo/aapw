...
} from './playerCombatIntentPolicy.js';
...
export function createPlayerCombatActionRouter({ target = globalThis, now = () => Date.now() } = {}) {
  ...
  function enqueue(action, source = 'unknown', timestamp = now()) {
    const event = createCombatIntent(action, source, timestamp, ++sequence);
    if (!event || !validateCombatIntent(event)) return false;
    queue.push(event);
  ...
  function drain({ currentTime = now(), max = maxQueue } = {}) {
    const current = Number.isFinite(currentTime) ? currentTime : now();
    const cutoff = current - ttlMs;
    while (queue.length && queue[0].timestamp < cutoff) queue.shift();
    const limit = Math.max(0, Math.min(maxQueue, Math.floor(Number(max) || maxQueue)));
    return sortCombatIntents(queue.splice(0, limit));
  }
