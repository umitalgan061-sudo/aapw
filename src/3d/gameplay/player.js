...
export async function createPlayer({ assetLoader, groundCollider, playerCollider = null, spawn = { x: 0, z: 0 } }) {
	...
	function playAction(name, timeScale = 1) { const next = actions[name]; if (!next) return; next.setEffectiveTimeScale(timeScale); if (currentActionName === name) return; next.reset().fadeIn(PLAYER_CONFIG.ANIMATION_CROSSFADE_SECONDS).play(); if (currentActionName && actions[currentActionName]) actions[currentActionName].fadeOut(PLAYER_CONFIG.ANIMATION_CROSSFADE_SECONDS); currentActionName = name; }
	playAction('idle');
	function moveBy(directionX, directionZ, speed, delta) { const travelMeters = Math.hypot(directionX, directionZ) * speed * delta; const steps = playerCollider ? Math.max(1, Math.ceil(travelMeters / PLAYER_ACTION_CONFIG.MAX_COLLISION_STEP_METERS)) : 1; const stepDelta = steps > 0 ? delta / steps : 0; for (let step = 0; step < steps; step += 1) { let nextX = model.position.x + directionX * speed * stepDelta; let nextZ = model.position.z + directionZ * speed * stepDelta; if (playerCollider) ({ x: nextX, z: nextZ } = playerCollider.resolveXZ(nextX, nextZ)); model.position.x = nextX; model.position.z = nextZ; } }
	function turnToward(directionX, directionZ, delta) { const targetYaw = Math.atan2(directionX, directionZ); const shortestTarget = model.rotation.y + THREE.MathUtils.euclideanModulo(targetYaw - model.rotation.y + Math.PI, Math.PI * 2) - Math.PI; model.rotation.y = THREE.MathUtils.lerp(model.rotation.y, shortestTarget, Math.min(1, PLAYER_CONFIG.TURN_RATE_RADIANS_PER_SECOND * delta)); }
	...
	return { object3D: model, get stamina() { return stamina; }, get maxStamina() { return PLAYER_ACTION_CONFIG.MAX_STAMINA; }, get movementState() { return movementState; }, get sprintExhausted() { return sprintExhausted; }, get isDodging() { return dodgeRemaining > 0; }, getMotionState: motionSnapshot,
		update(delta, moveDirectionXZ, isRunning, jumpRequested = false) { const dt = clamp(Number.isFinite(delta) ? delta : 0, 0, PLAYER_ACTION_CONFIG.MAX_FRAME_DELTA_SECONDS); const frameStartX = model.position.x, frameStartZ = model.position.z; hasMovementInput = moveDirectionXZ.x !== 0 || moveDirectionXZ.z !== 0; runIntent = Boolean(isRunning); lastRunPressAge += dt; dodgeCooldownRemaining = Math.max(0, dodgeCooldownRemaining - dt); regenDelayRemaining = Math.max(0, regenDelayRemaining - dt); if (sprintExhausted && stamina >= PLAYER_ACTION_CONFIG.SPRINT_RESTART_STAMINA) sprintExhausted = false; const runPressed = runIntent && !wasRunHeld; if (runPressed && lastRunPressAge <= PLAYER_ACTION_CONFIG.DODGE_DOUBLE_TAP_WINDOW_SECONDS && hasMovementInput && isGrounded && dodgeCooldownRemaining <= 0 && stamina >= PLAYER_ACTION_CONFIG.DODGE_COST) { const length = Math.hypot(moveDirectionXZ.x, moveDirectionXZ.z) || 1; dodgeDirectionX = moveDirectionXZ.x / length; dodgeDirectionZ = moveDirectionXZ.z / length; dodgeRemaining = PLAYER_ACTION_CONFIG.DODGE_DURATION_SECONDS; dodgeCooldownRemaining = PLAYER_ACTION_CONFIG.DODGE_COOLDOWN_SECONDS + dodgeRemaining; spendStamina(PLAYER_ACTION_CONFIG.DODGE_COST); lastRunPressAge = Infinity; } else if (runPressed) lastRunPressAge = 0; wasRunHeld = runIntent; if (dodgeRemaining > 0) { dodgeRemaining = Math.max(0, dodgeRemaining - dt); moveBy(dodgeDirectionX, dodgeDirectionZ, PLAYER_ACTION_CONFIG.DODGE_SPEED_MPS, dt); turnToward(dodgeDirectionX, dodgeDirectionZ, dt); movementState = 'dodge'; playAction('running', PLAYER_ACTION_CONFIG.DODGE_RUN_ANIMATION_TIMESCALE); } else if (hasMovementInput) { const sprinting = runIntent && isGrounded && !sprintExhausted && stamina > 0; const speed = sprinting ? PLAYER_ACTION_CONFIG.SPRINT_SPEED_MPS : PLAYER_CONFIG.WALK_SPEED_MPS; moveBy(moveDirectionXZ.x, moveDirectionXZ.z, speed, dt); turnToward(moveDirectionXZ.x, moveDirectionXZ.z, dt); if (sprinting) { spendStamina(PLAYER_ACTION_CONFIG.SPRINT_DRAIN_PER_SECOND * dt); movementState = 'sprint'; playAction('running', 1); } else { movementState = isGrounded && runIntent && sprintExhausted ? 'exhausted' : (isGrounded ? 'walk' : 'airborne'); playAction('walking', 1); } } else { movementState = isGrounded ? 'idle' : 'airborne'; playAction('idle', 1); } if (dodgeRemaining <= 0 && jumpRequested && isGrounded) { velocityY = PLAYER_CONFIG.JUMP_SPEED_MPS; isGrounded = false; } ({ heightAboveGroundMeters: heightAboveGround, velocityYMps: velocityY, isGrounded } = integrateJumpArc(heightAboveGround, velocityY, dt, PLAYER_CONFIG.GRAVITY_MPS2)); model.position.y = groundCollider.getGroundHeight(model.position.x, model.position.z) + heightAboveGround; if (regenDelayRemaining <= 0 && dodgeRemaining <= 0 && !(runIntent && hasMovementInput)) stamina = clamp(stamina + PLAYER_ACTION_CONFIG.STAMINA_REGEN_PER_SECOND * dt, 0, PLAYER_ACTION_CONFIG.MAX_STAMINA); planarSpeedMps = dt > 0 ? Math.hypot(model.position.x - frameStartX, model.position.z - frameStartZ) / dt : 0; mixer.update(dt); publishMotionTelemetry(); },
