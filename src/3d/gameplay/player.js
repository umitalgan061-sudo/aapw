...
export async function createPlayer({ assetLoader, groundCollider, playerCollider = null, spawn = { x: 0, z: 0 } }) {
	...
	return { object3D: model, get stamina() { return stamina; }, get maxStamina() { return PLAYER_ACTION_CONFIG.MAX_STAMINA; }, get movementState() { return movementState; }, get sprintExhausted() { return sprintExhausted; }, get isDodging() { return dodgeRemaining > 0; }, getMotionState: motionSnapshot,
		update(delta, moveDirectionXZ, isRunning, jumpRequested = false) {
			...
			if (canStartDodge() && (runJumpDodgeRequested || (runPressed && lastRunPressAge <= PLAYER_ACTION_CONFIG.DODGE_DOUBLE_TAP_WINDOW_SECONDS))) startDodge(moveDirectionXZ); else if (runPressed) lastRunPressAge = 0; wasRunHeld = runIntent;
			if (dodgeRemaining > 0) { dodgeRemaining = Math.max(0, dodgeRemaining - dt); moveBy(dodgeDirectionX, dodgeDirectionZ, PLAYER_ACTION_CONFIG.DODGE_SPEED_MPS, dt); turnToward(dodgeDirectionX, dodgeDirectionZ, dt); movementState = 'dodge'; playAction('running', PLAYER_ACTION_CONFIG.DODGE_RUN_ANIMATION_TIMESCALE); }
			...
			if (dodgeRemaining <= 0 && jumpRequested && isGrounded) { velocityY = PLAYER_CONFIG.JUMP_SPEED_MPS; isGrounded = false; }
			...
			if (dodgeRemaining <= 0) { if (!isGrounded) movementState = 'airborne'; else if (movementState === 'airborne') movementState = hasMovementInput ? (runIntent && sprintExhausted ? 'exhausted' : 'walk') : 'idle'; }
