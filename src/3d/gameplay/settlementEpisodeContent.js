 ...
 */
import { getSettlementQuestChain,getSettlementQuestChainStep,getSettlementQuestChainReward,listSettlementQuestChains } from './settlementCampaignQuestChains.js';
import { getSettlementService,getSettlementQuestObjective,getSettlementDialogueCondition,getSettlementRecipe,getSettlementRoute,getSettlementItem } from './settlementCampaignContent.js';

export const SETTLEMENT_EPISODE_CONTENT_VERSION = 1;
export const SETTLEMENT_EPISODE_LIMITS = Object.freeze({ episodes: 6, beatsPerEpisode: 8, hintsPerBeat: 3, text: 180, tagsPerBeat: 6 });

const makeBeat = (stepId,title,service,action,prompt,tone='positive') => Object.freeze({
  stepId,title,service,action,prompt,
  hints:Object.freeze(['Önce mevcut hizmet kartını aç ve kullanılabilirliğini doğrula.','Koşul veya kaynak eksikse mevcut runtime feedback bilgisini göster.','Başarıdan sonra yeniden runtime görünümünü okuyup sonraki adıma geç.']),
  tags:Object.freeze(['settlement',action,service]),
  ux:Object.freeze({ successTone:tone,blockedTone:'warning',failureTone:'danger',showObjective:true,showService:true,showHistory:true }),
});
...
const EPISODES=Object.freeze(Object.fromEntries(Object.entries(META).map(([id,meta])=>[id,Object.freeze({...meta,chapter:'Yerleşim Hikâyeleri',entry:Object.freeze({type:'settlement',requires:Object.freeze(['settlement-present','runtime-ready']),placementContract:'shared-material-placement-v1'}),beats:BEAT_LIBRARY[id],completion:Object.freeze({rewardSource:'settlementCampaignQuestChains',summary:'Ödül mevcut quest-chain runtime sonucu başarıya ulaştığında gösterilir.'})})])));
...
const LEGACY_SKILLS=Object.freeze({talk:'dialogue',collect:'survival',deliver:'commerce',craft:'smithing',travel:'travel',trade:'commerce',buy:'commerce',sell:'commerce',equip:'smithing',rest:'survival',train:'travel',save:'persistence',interact:'survival'});
const legacyCondition=(conditionId,step)=>{
  const match=String(conditionId??'').match(/^(flag|reputation|quest|item|skill)_\d+$/); if(!match)return null;
  const type=match[1];
...
export function resolveSettlementEpisodeCondition(conditionId,step){return getSettlementDialogueCondition(conditionId)??legacyCondition(conditionId,step);}
...
export function getSettlementEpisodeForChain(chainId){return getSettlementEpisode(chainId);}
export function validateSettlementEpisodeContent(){
  const errors=[];const warnings=[];
  for(const episodeId of listSettlementEpisodes()){
    const episode=EPISODES[episodeId]; if(!listSettlementQuestChains().includes(episode.chainId)){errors.push(`chain:${episodeId}:${episode.chainId}`);continue;}
    const chain=getSettlementQuestChain(episode.chainId); if(!chain){errors.push(`missing-chain:${episodeId}`);continue;}
    ...
    for(const beat of episode.beats){
      if(seen.has(beat.stepId))errors.push(`duplicate:${episodeId}:${beat.stepId}`); seen.add(beat.stepId);
      const step=getSettlementQuestChainStep(episodeId,beat.stepId); if(!step){errors.push(`step:${episodeId}:${beat.stepId}`);continue;}
      ...
      if(step.target&&/^(item_|recipe_|route_)/.test(step.target)&&!(getSettlementItem(step.target)||getSettlementRecipe(step.target)||getSettlementRoute(step.target)))errors.push(`target:${beat.stepId}:${step.target}`);
...
export function buildSettlementEpisodeManifest(){
  return freeze({version:SETTLEMENT_EPISODE_CONTENT_VERSION,contract:'shared-material-placement-v1',episodes:listSettlementEpisodes().map((id)=>({...clone(EPISODES[id]),chain:getSettlementQuestChain(id),reward:getSettlementQuestChainReward(id)})),validation:validateSettlementEpisodeContent()});
}
export function buildSettlementEpisodeBeatView(episodeId,stepId){
  const beat=getSettlementEpisodeBeat(episodeId,stepId);if(!beat)return{ok:false,reason:'unknown-beat'};const step=getSettlementQuestChainStep(episodeId,stepId);if(!step)return{ok:false,reason:'missing-quest-step'};
  return freeze({ok:true,episodeId,stepId,beat,quest:clone(step),service:getSettlementService(beat.service),objective:getSettlementQuestObjective(step.objective),conditions:(step.conditions??[]).map((id)=>resolveSettlementEpisodeCondition(id,step)).filter(Boolean),recipe:step.recipe?getSettlementRecipe(step.recipe):null,route:step.route?getSettlementRoute(step.route):null,reward:getSettlementQuestChainReward(episodeId)});
}
export function createSettlementEpisodeContentResolver(){
  const validation=validateSettlementEpisodeContent();
  return Object.freeze({version:SETTLEMENT_EPISODE_CONTENT_VERSION,valid:validation.ok,validation:clone(validation),list:listSettlementEpisodes,get:getSettlementEpisode,getBeat:getSettlementEpisodeBeat,resolve:buildSettlementEpisodeBeatView,resolveCondition:resolveSettlementEpisodeCondition,manifest:buildSettlementEpisodeManifest});
}
