...
export function resolveSettlementEpisodeAcceptanceRequirement(episodeId,requirement){const entry=MATRIX[episodeId];if(!entry)return{ok:false,reason:'unknown-episode'};if(!REQUIREMENTS.includes(requirement))return{ok:false,reason:'unknown-requirement'};const value=entry[requirement];if(value==null||(Array.isArray(value)&&value.length===0)||value==='')return{ok:false,reason:'coverage-missing',episodeId,requirement};return freeze({ok:true,episodeId,requirement,value:clone(value),label:text(requirement)});}
export function buildSettlementEpisodeAcceptanceScore(episodeId){const entry=MATRIX[episodeId];if(!entry)return null;const results=REQUIREMENTS.map((requirement)=>resolveSettlementEpisodeAcceptanceRequirement(episodeId,requirement));const passed=results.filter((result)=>result.ok).length;return freeze({episodeId,passed,total:REQUIREMENTS.length,percent:Math.round((passed/REQUIREMENTS.length)*100),results});}
export function buildSettlementEpisodeAcceptanceSummary(){const scores=listSettlementEpisodeAcceptanceEpisodes().map(buildSettlementEpisodeAcceptanceScore);const total=scores.reduce((sum,score)=>sum+score.total,0);const passed=scores.reduce((sum,score)=>sum+score.passed,0);return freeze({version:SETTLEMENT_EPISODE_VERTICAL_SLICE_ACCEPTANCE_VERSION,passed,total,percent:total?Math.round((passed/total)*100):0,episodes:scores});}
