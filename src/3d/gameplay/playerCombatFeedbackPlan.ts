...
const makeCue = (
  ...
  channel,
  intensity: Number(clamp(intensity, 0, 1).toFixed(4)),
  cameraImpulse: Number(clamp(intensity * (channel === 'stagger' ? 0.2 : 0.12), 0, 0.2).toFixed(4)),
  audioKey,
  vfxKey,
  hapticMs: Math.round(clamp(intensity * (channel === 'hit' || channel === 'stagger' ? 90 : 45), 0, 110)),
  priority,
});
...
export function buildPlayerCombatFeedbackPlan(
  ...
  if (rangedRelease) cues.push(makeCue('ranged', 0.55 + Number(attack.damageScaleAtCurrentStamina) * 0.08, 'combat.ranged.release', 'combat.ranged.trail', 2));
  else cues.push(makeCue('attack', 0.38 + Number(attack.damageScaleAtCurrentStamina) * 0.12, kind === 'heavy' ? 'combat.heavy.swing' : 'combat.light.swing', kind === 'heavy' ? 'combat.heavy.arc' : 'combat.light.arc', 2));
  if (defense.guardAvailable) cues.push(makeCue('guard', 0.35 + Number(defense.guardDamageMultiplier) * 0.22, 'combat.guard.raise', 'combat.guard.flash', 1));
  ...
  if (reaction.effectiveImpact > 0) {
    ...
    cues.push(makeCue(stagger ? 'stagger' : 'hit', clamp(Number(reaction.effectiveImpact) / Math.max(1, Number(maxPoise)), 0.12, 1), stagger ? 'combat.stagger.impact' : 'combat.hit.impact', stagger ? 'combat.stagger.break' : 'combat.hit.spark', stagger ? 5 : 3));
  }

  const ordered = cues.sort((a, b) => cuePriority(b) - cuePriority(a));
  const dominantCue = ordered[0] ?? null;
  const replayKey = ordered.map((cue) => `${cue.channel}:${cue.intensity}:${cue.priority}`).join('|') || 'none';
...
export function isPlayerCombatFeedbackPlan(value: unknown): value is PlayerCombatFeedbackPlan {
  try {
    ...
    return candidate.version === 1 && Array.isArray(candidate.cues) &&
      (candidate.dominantCue === null || (typeof candidate.dominantCue === 'object' && typeof candidate.dominantCue.channel === 'string')) &&
      typeof candidate.replayKey === 'string' && Object.isFrozen(candidate) && Object.isFrozen(candidate.cues);
  } catch {
    return false;
  }
}
