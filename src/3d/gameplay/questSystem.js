/**
 * Event-driven quest lifecycle for the 3D RPG.
 *
 ...
 * rewards are published as serializable events for existing/future progression adapters,
...
/**
 * First playable RPG chain. Every trigger reuses dialogue already shipped on the two Stannis guard
 * NPCs; no settlement, NPC, dialogue tree, inventory, or economy content is duplicated here.
 *
 * `law-of-the-watch` teaches the basic accept -> objective -> turn-in loop. Finishing it unlocks
 * `watch-under-pressure`, which proves prerequisite gating and a multi-objective quest while using
 * the guards' remaining dialogue choices. The second quest deliberately turns in through the same
 * loyalty answer used by the first quest: after its own two objectives are ready, returning to the
 * first guard and asking why Stannis is the final confirmation step.
 */
export const QUEST_DEFINITIONS = Object.freeze([
	...
	Object.freeze({
		...
		description: 'İlk nöbetçinin sadakat sözünü sınayıp tepedeki nöbetçinin tanıklığını dinle.',
		prerequisiteQuestIds: Object.freeze(['law-of-the-watch']),
		acceptTrigger: Object.freeze({ npcId: 'stannis-guard-1', choiceIndex: 2 }),
		objectives: Object.freeze([
			Object.freeze({
				id: 'ask-about-watch-partner',
				label: 'Tepedeki nöbetçiden kapı nöbetçisiyle uyumlarını öğren',
				trigger: Object.freeze({ npcId: 'stannis-guard-2', choiceIndex: 1 }),
			}),
			Object.freeze({
				id: 'ask-about-solitude',
				label: 'Tepedeki nöbetçinin yalnızlıkla nasıl baş ettiğini öğren',
				trigger: Object.freeze({ npcId: 'stannis-guard-2', choiceIndex: 2 }),
			}),
		]),
		turnInTrigger: Object.freeze({ npcId: 'stannis-guard-1', choiceIndex: 1 }),
		reward: Object.freeze({
			id: 'stannis-watch-confidence',
...
function getPrerequisiteIds(definition) {
	return Array.isArray(definition.prerequisiteQuestIds) ? definition.prerequisiteQuestIds : [];
}

function makeInitialState(definition) {
	return {
		id: definition.id,
		status: getPrerequisiteIds(definition).length > 0 ? STATUS.LOCKED : STATUS.AVAILABLE,
...
function serializeState(state, definition) {
	return {
		...
		status: state.status,
		prerequisiteQuestIds: [...getPrerequisiteIds(definition)],
...
export function createQuestSystem({ eventsBus, worldEventName = null, definitions = QUEST_DEFINITIONS }) {
	...
	for (const definition of definitions) {
		if (!definition?.id || definitionById.has(definition.id)) {
			throw new TypeError(`Quest definitions require unique non-empty ids; received "${definition?.id ?? ''}".`);
		}
		definitionById.set(definition.id, definition);
	}
	for (const definition of definitions) {
		for (const prerequisiteId of getPrerequisiteIds(definition)) {
			if (prerequisiteId === definition.id || !definitionById.has(prerequisiteId)) {
				throw new TypeError(`Quest "${definition.id}" has invalid prerequisite "${prerequisiteId}".`);
			}
		}
	}
	...
	function prerequisitesComplete(definition) {
		return getPrerequisiteIds(definition).every((questId) => stateById.get(questId)?.status === STATUS.COMPLETED);
	}

	function reconcileUnlocks({ emit = true } = {}) {
		for (const definition of definitions) {
			const state = stateById.get(definition.id);
			if (state.status !== STATUS.LOCKED || !prerequisitesComplete(definition)) continue;
			state.status = STATUS.AVAILABLE;
			if (!emit) continue;
			emitState(QUEST_EVENTS.UNLOCKED, state, definition);
			emitToast({
				id: `quest-unlocked:${definition.id}`,
				icon: '🗝️',
				title: `Yeni görev açıldı: ${definition.title}`,
	...
	return {
		...
		handleDialogueChoice(selection) {
			...
			for (const definition of definitions) {
				const state = stateById.get(definition.id);
				if (state.status === STATUS.LOCKED || state.status === STATUS.COMPLETED) continue;
		 ...
		 * ignored so future content additions remain backward-compatible. A quest added in a newer
		 * build is silently unlocked after restore when all of its prerequisites were completed in an
		 * older snapshot; this prevents content additions from getting permanently stuck as `locked`.
		 */
