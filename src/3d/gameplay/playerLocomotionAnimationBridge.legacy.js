...
import { PLAYER_DIRECTIONAL_DIRECTIONS, resolvePlayerDirectionalFingerprint, resolvePlayerDirectionalFullPresentation } from './playerDirectionalLocomotionPolicy.js';
import { PLAYER_LOCOMOTION_ANTICIPATION_VERSION, resolvePlayerLocomotionAnticipationProfile } from './playerLocomotionAnticipationPolicy.js';

export const PLAYER_LOCOMOTION_ANTICIPATION_BRIDGE_VERSION = '2026-09-15-v1';
export const PLAYER_LOCOMOTION_ANTICIPATION_CHANNELS = Object.freeze(['idle','forward','forwardRight','right','backRight','back','backLeft','left','forwardLeft','start','accelerate','cruise','brake','stop','strafe','pivot','recover']);
...
export function normalizePlayerLocomotionAnimationDirectionalWeights(weights={}){
  ...
  if(sum<=0) result.forward=1; else for(const d of PLAYER_DIRECTIONAL_DIRECTIONS) result[d]=round(result[d]/sum);
...
export function resolvePlayerLocomotionChannelWeights(profile={}){
  ...
  const modeValue={ start:profile.startWeight,accelerate:profile.startWeight,cruise:1,brake:profile.brakeWeight,stop:profile.brakeWeight,strafe:Math.max(profile.startWeight??0,(profile.brakeWeight??0)*0.25),pivot:profile.pivotWeight,recover:recovery }[mode];
...
export function resolvePlayerLocomotionChannelEnvelope(profile={}){
  const confidence=clamp(finite(profile.confidence,1),0,1); const groundRisk=clamp(finite(profile.groundRisk),0,1);
...
export function resolvePlayerLocomotionAnimationRequest(input={},previous=null){
  ...
  const channels=resolvePlayerLocomotionChannelWeights(profile); const envelope=resolvePlayerLocomotionChannelEnvelope(profile);
...
export function validatePlayerLocomotionAnimationRequest(request={}){
  const channels=Object.values(request.channels??{}); const channelsOk=channels.length===PLAYER_LOCOMOTION_ANTICIPATION_CHANNELS.length&&channels.every(Number.isFinite)&&channels.every((v)=>v>=0&&v<=1);
  const phaseOk=Number.isFinite(request.phase)&&request.phase>=0&&request.phase<1; const rateOk=Number.isFinite(request.playbackRate)&&request.playbackRate>=0.72&&request.playbackRate<=1.35; const confidenceOk=Number.isFinite(request.confidence)&&request.confidence>=0&&request.confidence<=1; const directionOk=PLAYER_DIRECTIONAL_DIRECTIONS.includes(request.anticipatedDirection);
  return freeze({ok:channelsOk&&phaseOk&&rateOk&&confidenceOk&&directionOk,channelsOk,phaseOk,rateOk,confidenceOk,directionOk});
}

export function createPlayerLocomotionAnimationRequestController({onRequest=null}={}){let previous=null;let frameCount=0;let lastFingerprint='';return freeze({update(input={}){const request=resolvePlayerLocomotionAnimationRequest(input,previous);frameCount+=1;previous=freeze({planarSpeedMps:input.planarSpeedMps,semanticState:request.semanticState,directionAngleRadians:0,phase:request.phase,turnRateDegreesPerSecond:input.turnRateDegreesPerSecond,surfaceConfidence:input.surfaceConfidence,surfaceSlip:input.surfaceSlip});if(typeof onRequest==='function'&&request.fingerprint!==lastFingerprint)onRequest(request);lastFingerprint=request.fingerprint;return freeze({request,frameCount});},read(){return freeze({frameCount,lastFingerprint});},reset(){previous=null;frameCount=0;lastFingerprint='';}});}
