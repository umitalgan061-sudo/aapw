...
function normalizeLod(value) {
  const lod = String(value ?? 'distant').trim().toLowerCase();
  return LOD_INDEX[lod] == null ? 'distant' : lod;
}

function normalizeKind(value) {
  const kind = String(value ?? 'ambient').trim().toLowerCase();
  return KIND_INDEX[kind] == null ? 'ambient' : kind;
}
...
function normalizeCandidate(raw, index) {
  ...
  const freshness = recentWorkTicks >= FAUNA_ACTIVITY_BUDGET_POLICY.staleAfterTicks
    ? 1
    : clamp(recentWorkTicks / FAUNA_ACTIVITY_BUDGET_POLICY.staleAfterTicks, 0, 1);
  const deficit = Math.max(resourceNeed, reproduction, social, movement, migration);
  return freeze({
    id: idOf(raw?.id, `fauna-${index}`),
    species: idOf(raw?.species, 'unknown').toLowerCase(),
    kind,
    lod,
    tick: normalizedTick,
    lastSelectedTick,
    ...
    fairnessEpoch: positiveInt(raw?.fairnessEpoch, 0),
    explicitPriority: clamp(raw?.priority, 0, 1),
    freshness,
    deficit,
  });
}
...
function normalizedContext(context = {}) {
  ...
  return freeze({
    tick: positiveInt(context.tick, 0),
    seed: idOf(context.seed, 'fauna-budget'),
    ...
    weatherStress: clamp(context.weatherStress, 0, 1),
    daylight: clamp(context.daylight ?? 0.65, 0, 1),
    settlementPressure: clamp(context.settlementPressure, 0, 1),
    globalThreat: clamp(context.globalThreat, 0, 1),
  });
}

function scoreCandidate(candidate, context) {
  const lodWeight = LOD_WEIGHT[candidate.lod];
  const kindWeight = KIND_WEIGHT[candidate.kind];
  ...
  const priority = candidate.explicitPriority;
  ...
  const score = kindWeight * lodWeight * (FAUNA_ACTIVITY_BUDGET_POLICY.urgencyFloor + activity * 0.52 + priority * 0.24)
    ...
    + nearBoost
    + weatherBoost
    + threatSignal * context.globalThreat * 0.1
    - healthPenalty
    - energyGate
    - occlusionPenalty
    - context.settlementPressure * (candidate.kind === 'ambient' ? 0.08 : 0)
    + jitter;
...
function dynamicBudget(candidates, context) {
  const averageThreat = candidates.length
    ? candidates.reduce((sum, candidate) => sum + Math.max(candidate.threat, context.globalThreat), 0) / candidates.length
    : 0;
  const averageDeficit = candidates.length
    ? candidates.reduce((sum, candidate) => sum + candidate.deficit, 0) / candidates.length
    : 0;
...
function allocateSlices(ranked, budget) {
  ...
  for (const entry of ranked) {
    ...
    if (selected.length < budget && (entry.adjustedScore > 0 || selected.length < Math.min(4, budget))) {
      selected.push(entry);
      usedKinds.add(entry.kind);
      usedSpecies.add(entry.species);
    }
  }
  return selected;
}
...
function makeSelection(raw, rank, selected, context) {
  const reason = raw.threat >= 0.7
    ? 'threat'
    : raw.resourceNeed >= 0.7
      ? 'resource-deficit'
      : raw.reproduction >= 0.7
        ? 'reproduction'
        : raw.starvationTicks >= FAUNA_ACTIVITY_BUDGET_POLICY.fairnessWindowTicks
          ? 'fairness'
          : raw.lod === 'near'
            ? 'near-lod'
            : 'baseline';
  return freeze({
    ...
    rank,
    score: raw.score,
    adjustedScore: raw.adjustedScore,
    reason,
    reserved: rank < selected,
    starvationTicks: raw.starvationTicks,
    recentWorkTicks: raw.recentWorkTicks,
...
function summarize(entries, selected, context, budget) {
  ...
  for (const entry of entries) {
    if (entry.kind in byKind) byKind[entry.kind] += 1;
    if (entry.lod in byLod) byLod[entry.lod] += 1;
    threat += entry.threat;
    deficit += entry.deficit;
    starvation += entry.starvationTicks;
  }
  return freeze({
    tick: context.tick,
    inputCount: entries.length,
    eligibleCount: selected.length,
    selectedCount: selected.filter((entry) => entry.reserved).length,
    budget,
    averageThreat: round(entries.length ? threat / entries.length : 0),
    averageDeficit: round(entries.length ? deficit / entries.length : 0),
    averageStarvationTicks: round(entries.length ? starvation / entries.length : 0),
...
export function evaluateFaunaActivityBudget(rawCandidates = [], context = {}) {
  const normalizedContext = normalizedContext(context);
  const candidates = Array.isArray(rawCandidates) ? rawCandidates : [];
  const capped = candidates.slice(0, normalizedContext.candidateCap)
    .map((candidate, index) => normalizeCandidate(candidate, index))
    .filter((candidate) => candidate.active && candidate.eligible);
  const scored = capped.map((candidate) => ({
    ...candidate,
    score: scoreCandidate(candidate, normalizedContext),
  }));
  ...
  return freeze({
    ...
    seed: normalizedContext.seed,
    budget,
    selected: freeze(committed),
    deferred: freeze(deferred),
    summary: summarize(scored, selections, normalizedContext, budget),
  });
}
...
export function createFaunaActivityBudgetLedger({ maxHistory = 12 } = {}) {
  ...
  const evaluate = (candidates, context = {}) => {
    ...
    const nextContext = { ...context, tick: positiveInt(context.tick, state.tick) };
    state.tick = nextContext.tick;
    const result = evaluateFaunaActivityBudget(candidates, nextContext);
  ...
  const snapshot = () => freeze({
    ...
    tick: state.tick,
    history: freeze(state.history.map((entry) => freeze({ ...entry }))),
  });
...
export function replayFaunaActivityBudget(candidates, contexts = []) {
  const inputs = Array.isArray(contexts) ? contexts : [];
  return freeze(inputs.map((context) => evaluateFaunaActivityBudget(candidates, context)));
}
...
export function faunaActivityBudgetDigest(result) {
  ...
  const payload = [
    ...
    result?.seed ?? '',
    result?.budget ?? 0,
    ...selected.map((entry) => `${entry.id}:${entry.kind}:${entry.lod}:${entry.rank}:${entry.score}`),
    ...deferred.slice(0, 32).map((entry) => `d:${entry.id}:${entry.score}`),
  ].join('|');
...
export function validateFaunaActivityBudgetResult(result) {
  ...
  for (const entry of selected) {
    ...
    ids.add(entry.id);
    if (!FAUNA_ACTIVITY_KINDS.includes(entry.kind)) errors.push(`kind:${entry.id}`);
    if (!FAUNA_ACTIVITY_LODS.includes(entry.lod)) errors.push(`lod:${entry.id}`);
    if (!Number.isFinite(entry.score) || entry.score < 0) errors.push(`score:${entry.id}`);
    if (!Number.isInteger(entry.rank) || entry.rank < 0) errors.push(`rank:${entry.id}`);
  }
