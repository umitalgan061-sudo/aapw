/**
 * Deterministic settlement interaction rail — input normalization and gate evaluation.
 *
 * Split out of settlementVerticalSlice.js (GOVERNANCE.md Altın Kural 7, 600-line cap).
 * Pure, side-effect-free normalization/validation helpers with no dependency on the
 * runtime factory in the sibling module. Function bodies are unchanged from the
 * pre-split file — only their home moved.
 */
...
export const CAPABILITY_FOR_ACTION = Object.freeze({
...
export const HANDLER_FOR_ACTION = Object.freeze({
...
export function amountMap(value) {
  const result = {};
...
export function reputationMap(value) {
  const result = {};
  const entries = value instanceof Map
    ? [...value.entries()]
    : value && typeof value === 'object' ? Object.entries(value) : [];
...
export function questMap(value) {
  const result = {};
  if (!value || typeof value !== 'object') return result;
  for (const [key, raw] of Object.entries(value)) {
