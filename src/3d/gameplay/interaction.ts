/** Production TypeScript owner for the interaction/quest/economy runtime. The legacy .js file is compatibility-only. */
// @ts-nocheck
/**
...
export function createInteractionController({ interactionPrompt, dialogueBox, greetingTemplate, greetingsByNpcId = {}, choicesByNpcId = {}, radiusMeters, isPaused = () => false, onQuestChanged = () => {}, onReputationChanged = () => {}, onProgressionChanged = () => {}, onWorldStateChanged = () => {}, onInventoryChanged = () => {}, onEconomyChanged = () => {}, onJourneyChanged = () => {} }) {
	...
	function showExpeditionBoard() {
		...
		const boardChoices = activeExpeditionBoard.entries.map((entry) => `${entry.label} — ${entry.ready ? 'HAZIR' : 'KİLİTLİ'}${entry.completed ? ' · ÖDÜL ALINDI' : ''}`);
		if (activeExpeditionBoard.recovery) boardChoices.push(`${activeExpeditionBoard.recovery.label} — ${activeExpeditionBoard.recovery.ready ? 'HAZIR' : 'KİLİTLİ'}`);
		dialogueBox.show(buildExpeditionBoardText(activeExpeditionBoard), boardChoices);
		return true;
	}
	function useMasteryArmorerService() {
		...
		const quantityOf = (itemId) => snapshot.items.find((item) => item.itemId === itemId)?.quantity ?? 0;
		if (quantityOf('dragonstone-expedition-maintenance-kit') >= 1) { showQuartermaster('Bakım kiti zaten çantanda; zırhçı ikinci bir kiti hazırlamaz.'); return false; }
		if (quantityOf('dragonstone-travel-ration-pack') < 1 || quantityOf('dragonstone-whetstone') < 1) { showQuartermaster('Ustalık zırhçı hizmeti için 1 yol azığı paketi ve 1 Nöbetçi Bileği Taşı gerekir.'); return false; }
		const recipe = QUARTERMASTER_OFFERS.find((offer) => offer.id === 'dragonstone-whetstone')?.fulfillment?.craftUpgrade;
		if (!recipe) { showQuartermaster('Zırhçı bakım reçetesini bulamadı.'); return false; }
		const result = inventory.grant('dragonstone-whetstone', 1, { sourceType: 'settlement-service', sourceId: 'dragonstone-watch-armorer-mastery', craftUpgrade: recipe });
		if (result?.crafted !== true) { showQuartermaster('Zırhçı bakım kitini hazırlayamadı; girdilerin değişmedi.'); return false; }
		onInventoryChanged(inventory.snapshot());
		showQuartermaster('Ustalık zırhçı hizmeti tamamlandı: 1 yol azığı + 1 bileği taşı → 1 Dragonstone Sefer Bakım Kiti. Bakır harcanmadı.');
		return true;
	}
...
export type InteractionFactionId = (typeof INTERACTION_FACTIONS)[keyof typeof INTERACTION_FACTIONS];
export type InteractionQuestStatus = (typeof QUEST_STATUS)[keyof typeof QUEST_STATUS];

/** Minimal controller contract consumed by the typed 3D application runtime. */
...
export interface RuntimeInteractionNpc {
