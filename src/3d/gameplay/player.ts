...
import { AssetLoader, type AnimatedObject3D } from '../assetLoader.ts';
import { integrateJumpArc, type XZCollider, type GroundCollider } from '../physics.ts';
...
export interface PlayerAnimationRegistry { readonly [name: string]: THREE.AnimationAction; }
export interface PlayerMotionSnapshot {
  readonly state: PlayerMovementState;
  readonly stamina: number; readonly maxStamina: number; readonly staminaRatio: number; readonly sprintExhausted: boolean; readonly runIntent: boolean;
  readonly poise: number; readonly maxPoise: number; readonly poiseRatio: number; readonly guardBreakRemaining: number; readonly hitStaggerRemaining: number;
  readonly guarding: boolean; readonly parryWindowRemaining: number; readonly defenseResult: PlayerDefenseState;
  readonly attackKind: PlayerAttackKind; readonly attackPhase: 'none' | 'windup' | 'active' | 'recovery'; readonly attackComboStep: number; readonly attackActive: boolean;
  readonly attackRemaining: number; readonly attackCommitRemaining: number; readonly isGrounded: boolean; readonly canDodge: boolean; readonly isDodgeInvulnerable: boolean;
  readonly dodgeElapsed: number; readonly speedMps: number; readonly dodgeRemaining: number; readonly dodgeCooldownRemaining: number; readonly regenDelayRemaining: number;
  readonly position: { readonly x: number; readonly y: number; readonly z: number };
}
export interface PlayerMoveDirection { readonly x: number; readonly z: number; readonly guarding: boolean; }
export interface PlayerController {
  readonly object3D: THREE.Object3D;
  readonly stamina: number; readonly maxStamina: number; readonly poise: number; readonly maxPoise: number;
  readonly movementState: PlayerMovementState; readonly sprintExhausted: boolean; readonly isDodging: boolean;
  readonly isGuarding: boolean; readonly isGuardBroken: boolean; readonly isAttacking: boolean;
  readonly getMotionState: () => PlayerMotionSnapshot;
  readonly update: (delta: number, moveDirectionXZ: PlayerMoveDirection, isRunning: boolean, jumpRequested?: boolean) => void;
  readonly dispose: () => void;
}
export interface PlayerCreateOptions {
  readonly assetLoader: AssetLoader;
  readonly groundCollider: GroundCollider;
  readonly playerCollider?: XZCollider | null;
...
export async function createPlayer({ assetLoader, groundCollider, playerCollider = null, spawn = { x: 0, z: 0 } }: PlayerCreateOptions): Promise<PlayerController> {
	...
	const actions: PlayerAnimationRegistry = {};
	...
	for (const [name, url] of Object.entries(PLAYER_CONFIG.ANIMATION_URLS)) {
		...
		const animations = (animationObject as AnimatedObject3D).animations ?? [];
	...
	function attackTuning(kind: PlayerAttackKind): { cost: number; duration: number; activeStart: number; activeEnd: number; reach: number; commitMeters: number; damageScale: number } {
		return kind === 'heavy'
	...
	function publishAttackWindow(phase: 'start' | 'active-start' | 'active-end' | 'interrupted' | 'finish'): void {
	...
	function captureCombatFeedbackContext(): { readonly stamina: number; readonly poise: number; readonly state: PlayerMovementState; readonly position: { readonly x: number; readonly y: number; readonly z: number } } {
	...
	function publishCombatFeedback(outcome: PlayerDefenseState, rawAmount: number, appliedAmount: number, blockedAmount: number, context: ReturnType<typeof captureCombatFeedbackContext> | null = null): void {
		if (typeof globalThis.dispatchEvent !== 'function' || typeof globalThis.CustomEvent !== 'function') return;
	...
	function publishCombatFeedbackAfterHealth(outcome: PlayerDefenseState, payload: DamagePayload, rawAmount: number, blockedAmount: number): void {
	...
	function canStartAttack(kind: 'light' | 'heavy'): boolean {
	...
	function startAttack(kind: 'light' | 'heavy', chained = false): boolean {
	...
	function updateAttack(dt: number, moveDirectionXZ: PlayerMoveDirection): void {
		if (attackRemaining <= 0) return;
	...
	function onCombatInput(event: Event): void { const detail = (event as CustomEvent<{ readonly kind?: string }>).detail;
		const kind = detail?.kind; if (kind !== 'light' && kind !== 'heavy') return; if (attackRemaining <= 0 && !canStartAttack(kind)) return; bufferedAttackKind = kind; attackBufferRemaining = PLAYER_ACTION_CONFIG.ATTACK_COMBO_BUFFER_SECONDS; }
	...
	function startDodge(moveDirectionXZ: PlayerMoveDirection): void { const length = Math.hypot(moveDirectionXZ.x, moveDirectionXZ.z) || 1; dodgeDirectionX = moveDirectionXZ.x / length; dodgeDirectionZ = moveDirectionXZ.z / length; dodgeElapsed = 0; dodgeRemaining = PLAYER_ACTION_CONFIG.DODGE_DURATION_SECONDS; dodgeCooldownRemaining = PLAYER_ACTION_CONFIG.DODGE_COOLDOWN_SECONDS + dodgeRemaining; spendStamina(PLAYER_ACTION_CONFIG.DODGE_COST); lastRunPressAge = Infinity; }
	...
	function attackPhase(): 'none' | 'windup' | 'active' | 'recovery' { if (attackRemaining <= 0) return 'none'; const tuning = attackTuning(attackKind); if (attackElapsed < tuning.activeStart) return 'windup'; if (attackElapsed < tuning.activeEnd) return 'active'; return 'recovery'; }
	...
	function onIncomingDamage(payload: DamagePayload): void {
	...
	return {
		...
		update(delta: number, moveDirectionXZ: PlayerMoveDirection, isRunning: boolean, jumpRequested = false): void {
