/** Production TypeScript owner for the NPC runtime. The legacy .js file is compatibility-only. */
// @ts-nocheck
/**
...
/** Structural runtime contract exposed to the typed game loop and world spawners. */
export interface NpcRuntimeEntity {
  readonly object3D: THREE.Object3D;
  readonly displayName?: string;
  update(deltaSeconds: number, player?: unknown): void;
  dispose(): void;
}

/** Deterministic guard-awareness input contract. */
export interface NpcGuardAwarenessInput {
  readonly observer: Readonly<{ x: number; z: number }>;
  readonly target: Readonly<{ x: number; z: number }>;
  readonly yawRadians?: number;
  readonly rangeMeters?: number;
  readonly lineOfSight?: boolean;
}

/** Deterministic guard-awareness result contract. */
export interface NpcGuardAwarenessResult {
