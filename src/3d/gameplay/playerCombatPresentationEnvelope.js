...
const CUES = Object.freeze({
  attack: 'combat-attack', hit: 'combat-hit-confirm', guard: 'combat-guard', parry: 'combat-parry',
  dodge: 'combat-dodge', stagger: 'combat-stagger', defeat: 'combat-defeat',
});
...
const freezeCue = (cue, index) => Object.freeze({
  type: cue.type, sequence: index, intensity: clamp01(cue.intensity),
...
export function createPlayerCombatPresentationEnvelope(initial = {}) {
  ...
  const emit = (next = {}) => {
    ...
    const current = Object.freeze({ state: normalizeState(next.state), defeated: Boolean(next.defeated), confirmedHit: Boolean(next.confirmedHit), hitIntensity: clamp01(next.hitIntensity ?? 1), attackIntensity: clamp01(next.attackIntensity ?? 0.7) });
    const cue = deriveCue(previous, current);
    const output = Object.freeze({ sequence, state: current.state, defeated: current.defeated, cue: cue ? freezeCue(cue, sequence) : null });
    sequence += 1;
    previous = current;
    return output;
  };
  return Object.freeze({ emit, dispose() { disposed = true; }, snapshot() { return Object.freeze({ sequence, previous }); } });
}
