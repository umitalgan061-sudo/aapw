...
export function collectCameraCollidables(state, worldX, worldZ) { _cameraCollidables.length = 0; const chunkSize = CHUNK_CONFIG.CHUNK_SIZE_METERS, centerChunkX = worldToChunkCoord(worldX, chunkSize), centerChunkZ = worldToChunkCoord(worldZ, chunkSize); for (let dz = -1; dz <= 1; dz++) for (let dx = -1; dx <= 1; dx++) { const mesh = state.chunkManager.getLoadedChunkMesh(centerChunkX + dx, centerChunkZ + dz); if (mesh) _cameraCollidables.push(mesh); } for (const part of state.settlements.children) _cameraCollidables.push(part); for (const villagePart of state.villages?.children ?? []) _cameraCollidables.push(villagePart); for (const realCastle of state.realCastles.children) _cameraCollidables.push(realCastle); for (const icePart of state.iceLandmarks?.children ?? []) _cameraCollidables.push(icePart); return _cameraCollidables; }
