...
export function createPlayerLockOnController(config = {}) {
	...
	return { update({ playerPosition, forward, candidates = [], toggleRequested = false } = {}) { const player = planarPosition(playerPosition); if (!player) { clear('invalid-player'); return snapshot(); } if (toggleRequested && lockedEntity) { clear('toggle-release'); return snapshot(); } if (lockedEntity) { if (!candidates.includes(lockedEntity)) { clear('target-removed'); return snapshot(); } if (!entityLockAvailable(lockedEntity)) { clear('target-unavailable'); return snapshot(); } const target = planarPosition(entityObject(lockedEntity)); lastDistanceMeters = target ? Math.hypot(target.x - player.x, target.z - player.z) : Infinity; if (!target || lastDistanceMeters > breakDistanceMeters) { clear('range-break'); return snapshot(); } const tracking = evaluatePlayerLockTarget({ playerPosition: player, forward, entity: lockedEntity, maxDistanceMeters: breakDistanceMeters, halfAngleDegrees: trackHalfAngleDegrees }); if (!tracking.eligible && tracking.reason === 'angle') { clear('view-break'); return snapshot(); } return snapshot(); } if (!toggleRequested) return snapshot(); const selected = selectPlayerLockTarget({ playerPosition: player, forward, candidates, maxDistanceMeters: acquireDistanceMeters, halfAngleDegrees }); if (!selected) { dispatchLockOn({ locked: false, targetId: null, reason: 'no-target' }); return snapshot(); } lockedEntity = selected.entity; lockedId = selected.id; lastDistanceMeters = selected.distanceMeters; dispatchLockOn({ locked: true, targetId: lockedId, reason: 'acquired', distanceMeters: Number(lastDistanceMeters.toFixed(3)), angleDegrees: Number(selected.angleDegrees.toFixed(2)), targetPosition: Object.freeze({ ...selected.position }) }); return snapshot(); }, clear, getSnapshot: snapshot };
}
