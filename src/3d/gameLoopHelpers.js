...
export const PLAYER_LOCK_ON_CONFIG = Object.freeze({ ACQUIRE_DISTANCE_METERS: 30, BREAK_DISTANCE_METERS: 38, ACQUIRE_HALF_ANGLE_DEGREES: 68, DISTANCE_SCORE_WEIGHT: 0.38, TURN_RATE_RADIANS_PER_SECOND: 11 });
...
export function evaluatePlayerLockTarget({ playerPosition, forward, entity, index = 0, maxDistanceMeters = PLAYER_LOCK_ON_CONFIG.ACQUIRE_DISTANCE_METERS, halfAngleDegrees = PLAYER_LOCK_ON_CONFIG.ACQUIRE_HALF_ANGLE_DEGREES } = {}) {
	const player = planarPosition(playerPosition), target = planarPosition(entityObject(entity)), id = entityStableId(entity, index);
...
export function createPlayerLockOnController(config = {}) {
	...
	return { update({ playerPosition, forward, candidates = [], toggleRequested = false } = {}) { const player = planarPosition(playerPosition); if (!player) { clear('invalid-player'); return snapshot(); } if (toggleRequested && lockedEntity) { clear('toggle-release'); return snapshot(); } if (lockedEntity) { if (!candidates.includes(lockedEntity)) { clear('target-removed'); return snapshot(); } const target = planarPosition(entityObject(lockedEntity)); lastDistanceMeters = target ? Math.hypot(target.x - player.x, target.z - player.z) : Infinity; if (!target || lastDistanceMeters > breakDistanceMeters) { clear('range-break'); return snapshot(); } return snapshot(); } if (!toggleRequested) return snapshot(); const selected = selectPlayerLockTarget({ playerPosition: player, forward, candidates, maxDistanceMeters: acquireDistanceMeters, halfAngleDegrees }); if (!selected) { dispatchLockOn({ locked: false, targetId: null, reason: 'no-target' }); return snapshot(); } lockedEntity = selected.entity; lockedId = selected.id; lastDistanceMeters = selected.distanceMeters; dispatchLockOn({ locked: true, targetId: lockedId, reason: 'acquired', distanceMeters: Number(lastDistanceMeters.toFixed(3)), angleDegrees: Number(selected.angleDegrees.toFixed(2)), targetPosition: Object.freeze({ ...selected.position }) }); return snapshot(); }, clear, getSnapshot: snapshot };
}
...
export function computeCameraRelativeMove(camera, controls, axes) {
	applyGamepadCameraLook(camera, controls, axes); const guarding = Boolean(axes.guarding), inputMagnitude = Math.min(1, Math.hypot(axes.forward, axes.strafe)); if (inputMagnitude === 0) return { x: 0, z: 0, guarding }; _forward.subVectors(controls.target, camera.position); _forward.y = 0; if (_forward.lengthSq() < 1e-6) _forward.set(0, 0, -1); else _forward.normalize(); _right.crossVectors(_forward, _worldUp).normalize(); _move.set(0, 0, 0).addScaledVector(_forward, axes.forward).addScaledVector(_right, axes.strafe); if (_move.lengthSq() < 1e-6) return { x: 0, z: 0, guarding }; _move.normalize().multiplyScalar(inputMagnitude); return { x: _move.x, z: _move.z, guarding };
}
