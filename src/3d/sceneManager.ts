/** Production TypeScript scene bootstrap. Legacy sceneManager.js is a compatibility shim. */
/**
...
import { createAuroraSky } from './sky.ts';
import { createStarfield } from './stars.js';
import { createDayNightLighting } from './lighting.js';
import { createFog } from './fog.ts';
import { resolveRenderQuality, configureRendererRealism, configureSunShadow, applyShadowRoles } from './renderQuality.ts';
...
export function worldToChunkCoord(worldCoord: number, chunkSizeMeters: number): number {
	return Math.round(worldCoord / chunkSizeMeters);
}

function createSceneBase(canvas: HTMLCanvasElement) {
...
export function createScene(canvas: HTMLCanvasElement) {
	const state = createSceneBase(canvas);
