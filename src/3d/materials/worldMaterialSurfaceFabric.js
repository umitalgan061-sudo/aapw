...
function cloneMaterialPreservingMaps(material) {
  ...
  // Three's Material.clone() intentionally shares textures. Keeping those exact map objects preserves
  // authored UV transforms, mip chains and texture memory while allowing per-object shader metadata.
...
vec3 worldFabricMineralStain = vec3(0.315, 0.274, 0.213);
diffuseColor.rgb = mix(diffuseColor.rgb, worldFabricMineralStain, worldFabricStainMask * ${glsl(profile.stainGain)});
...
diffuseColor.rgb = clamp(diffuseColor.rgb, vec3(0.008), vec3(0.94));`,
      ...
      .replace(
        ...
        `#include <roughnessmap_fragment>
float worldFabricRough = ${glsl(profile.roughnessBase)}
  + (worldFabricMacro - 0.5) * ${glsl(profile.roughnessMacro)}
  + (worldFabricFine - 0.5) * ${glsl(profile.roughnessFine)}
  - worldFabricDamp * 0.08 - worldFabricStreak * 0.05
  + worldFabricSnow * 0.06 + worldFabricSlopeContext * 0.015;
...
export function updateWorldMaterialSurfaceContext(root, context = {}) {
  ...
  root.traverse((mesh) => {
    ...
    for (const material of materials) {
      const fabric = material?.userData?.worldMaterialSurfaceFabric;
      if (!fabric) continue;
      // Context is shader-keyed. Reinstalling on the already-cloned generated material is cheap at
      // placement time and keeps the fragment program deterministic for the final geographic site.
      installFabricShader(material, {
        profileId: fabric.profileId,
        seed: fabric.seed,
        moisture: context.moisture ?? fabric.moisture,
        exposure: context.exposure ?? fabric.exposure,
        snow: context.snow ?? fabric.snow,
        slope: context.slope ?? fabric.slope,
      });
      updated += 1;
    }
  });
