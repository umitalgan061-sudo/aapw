import type { AssetDescriptor, AssetProvider, Backend, DeviceCapabilities, FramePlan, GameplaySnapshot, QualityTier, RuntimeState, SaveEnvelope, SaveSlot, TypedArray } from './platform.js';
import { asAssetId, asNodeId, asTick, asWorldId, normalizeBackend, normalizeQuality } from './platform.js';

export interface LegacyObjectRecord {
  id?: unknown;
  name?: unknown;
  position?: unknown;
  rotation?: unknown;
...
export interface LegacyFrameRecord {
  ...
  shadowObjectIds?: unknown;
  animatedObjectIds?: unknown;
  estimatedGpuMs?: unknown;
  cpuUploadMs?: unknown;
}

export interface LegacySaveRecord {
  slot?: unknown;
  envelope?: unknown;
  snapshot?: unknown;
}
...
export function adaptLegacyRuntime(input: LegacyRuntimeRecord): Pick<RuntimeState, 'worldId' | 'tick' | 'quality' | 'backend' | 'mode' | 'budgets' | 'capabilities'> {
  const budgets: RuntimeState['budgets'] = {
    frameMs: finite(input.budgets?.frameMs, 16.67),
    simulationMs: finite(input.budgets?.simulationMs, 5),
    renderMs: finite(input.budgets?.renderMs, 10),
    uploadMs: finite(input.budgets?.uploadMs, 2),
    streamingMs: finite(input.budgets?.streamingMs, 2),
  ...
  const defaults: DeviceCapabilities = {
    backend: normalizeBackend(input.capabilities?.backend ?? input.backend),
  ...
  return {
    ...
    tick: asTick(Math.max(0, Math.floor(finite(input.tick, 0)))),
    quality: normalizeQuality(input.quality),
    backend: normalizeBackend(input.backend ?? defaults.backend),
    mode: input.mode === 'headless' || input.mode === 'replay' ? input.mode : 'interactive',
    budgets,
    capabilities: { ...defaults, backend: normalizeBackend(input.backend ?? defaults.backend) },
  };
}
...
export function adaptLegacyFrame(input: LegacyFrameRecord): FramePlan {
  ...
  const passes = Array.isArray(input.passes) ? input.passes.filter((item): item is FramePlan['passes'][number] => typeof item === 'string' && ['depth','shadow','opaque','transparent','water','foliage','effects','post','ui'].includes(item)) : ['depth', 'opaque', 'post'];
  return {
    ...
    passes,
    visibleObjectIds: asIds(input.visibleObjectIds),
    shadowObjectIds: asIds(input.shadowObjectIds),
    animatedObjectIds: asIds(input.animatedObjectIds),
    estimatedGpuMs: Math.max(0, finite(input.estimatedGpuMs, 0)),
    cpuUploadMs: Math.max(0, finite(input.cpuUploadMs, 0)),
  };
}
...
export function adaptLegacySave(input: LegacySaveRecord): { slot: SaveSlot; envelope: SaveEnvelope } {
  ...
  if (!raw || typeof raw !== 'object') throw new TypeError('Legacy save envelope is invalid');
  const value = raw as Record<string, unknown>;
  const header = value.header;
  if (!header || typeof header !== 'object') throw new TypeError('Legacy save header is missing');
  const h = header as Record<string, unknown>;
  const version = stringValue(h.version, '1.0.0') as `${number}.${number}.${number}`;
  return { slot, envelope: { header: { format: 'aapw-save', version, schemaHash: stringValue(h.schemaHash, 'legacy-adapter'), createdAt: finite(h.createdAt, Date.now()), updatedAt: finite(h.updatedAt, Date.now()), playTimeSeconds: Math.max(0, finite(h.playTimeSeconds, 0)), worldId: asWorldId(stringValue(h.worldId, 'unknown-world')), tick: asTick(Math.max(0, Math.floor(finite(h.tick, 0)))) }, compression: value.compression === 'brotli' ? 'brotli' : value.compression === 'gzip' ? 'gzip' : 'none', checksum: stringValue(value.checksum, 'legacy'), payload: stringValue(value.payload, JSON.stringify(input.snapshot ?? {})) } };
}
...
export function adaptLegacyAsset(input: LegacyAssetRecord): AssetDescriptor {
  const kind = ['mesh','texture','material','animation','audio','shader','environment'].includes(String(input.kind)) ? String(input.kind) as AssetDescriptor['kind'] : 'mesh';
  const compression = ['ktx2','basis','draco','meshopt','webp','avif'].includes(String(input.compression)) ? String(input.compression) as AssetDescriptor['compression'] : 'none';
