...
const FBX_PACK_INSPECTOR_IDS = new Set([
  'we-name',
  'we-pos-x', 'we-pos-y', 'we-pos-z',
  'we-rot-x', 'we-rot-y', 'we-rot-z',
  'we-scale-x', 'we-scale-y', 'we-scale-z'
]);
...
export function installEditorFbxPackController(api, transformSurface) {
  ...
  function packsForRoot(root = api.getSelectedObject?.()) {
    return discoverEditorFbxPacks(root).map((node) => ({
      node,
  ...
  function onInspectorChange(event) {
    ...
    if (id === 'we-name') {
      ...
      if (id.startsWith('we-pos-')) {
        ...
        activeNode.scale[axis] = Math.max(FBX_PACK_MIN_SCALE, value);
      }
    }
  ...
  function onToastMutation() {
    ...
    Promise.resolve(promise).then((data) => {
      if (data) applySceneOverrides(data);
  ...
  function getSnapshot() {
    ...
    return Object.freeze({
      ...
      candidateCount: packsForRoot(root).length,
