...
// RUN299_FBX_PACK_INLINE_V1 — owner-requested independent static FBX pack selection and transform.
import * as THREE from 'three';

const RUN299_PACK_SCALE_FLOOR = 0.000001;

function run299IsFbxRoot(root) {
  return root?.userData?.editorFormat === 'fbx' && !root.isInstancedMesh;
}

function run299HasBoneAncestor(node, stop) {
  for (let cursor = node?.parent; cursor && cursor !== stop; cursor = cursor.parent) if (cursor.isBone) return true;
  return false;
}

function run299ContainsStaticMesh(node) {
  let found = false;
  node?.traverse?.((child) => {
    if (child.isMesh && !child.isSkinnedMesh && !run299HasBoneAncestor(child, node)) found = true;
  });
  return found;
}

function run299PackNodes(root) {
  if (!run299IsFbxRoot(root)) return [];
  let frontier = (root.children || []).filter((child) => !child.isBone && run299ContainsStaticMesh(child));
  let depth = 0;
  while (frontier.length === 1 && depth < 12) {
    const nested = frontier[0].children.filter((child) => !child.isBone && run299ContainsStaticMesh(child));
  ...
  if (frontier.length === 1) {
    ...
    frontier[0].traverse((node) => {
      if (node.isMesh && !node.isSkinnedMesh && !run299HasBoneAncestor(node, root)) meshes.push(node);
    });
...
function run299PathForNode(root, node) {
  const parts = [];
...
function run299NodeFromPath(root, path) {
...
function run299Round6(value) {
  return Number(Number(value).toFixed(6));
}

function run299TransformRecord(node) {
  return {
    name: node.name || '',
    transform: {
      position: node.position.toArray().map(run299Round6),
      rotation: [node.rotation.x, node.rotation.y, node.rotation.z].map(run299Round6),
      scale: node.scale.toArray().map(run299Round6)
    }
  };
}

export function installRun299FbxPackController(api, transformSurface) {
  ...
  function listPacks(root = api.getSelectedObject?.()) {
    return run299PackNodes(root).map((node) => ({
      path: run299PathForNode(root, node),
  ...
  function captureOverride(root, node) {
    const path = run299PathForNode(root, node);
    if (!path) return;
    if (!root.userData.editorFbxPackOverrides || typeof root.userData.editorFbxPackOverrides !== 'object') root.userData.editorFbxPackOverrides = {};
    root.userData.editorFbxPackOverrides[path] = run299TransformRecord(node);
  }

  function renderPanel() {
    const root = api.getSelectedObject?.();
    panel.hidden = !run299IsFbxRoot(root);
    list.replaceChildren();
    if (!run299IsFbxRoot(root)) {
      status.textContent = 'FBX Pack: —';
      return;
    }
    ...
    if (!packs.length) list.textContent = 'Bağımsız statik pack bulunamadı; rig/bone parçaları güvenlik için ayrılmaz.';
  ...
  function selectPack(root, path) {
    if (root !== api.getSelectedObject?.() || !run299IsFbxRoot(root)) return false;
    const node = run299NodeFromPath(root, path);
    if (!node || !run299PackNodes(root).includes(node)) return false;
    activeRoot = root;
    activeNode = node;
    activePath = path;
  ...
  function onInspectorChange(event) {
    ...
    if (id === 'we-name') activeNode.name = event.target.value.trim() || activeNode.name;
      ...
      if (id.startsWith('we-pos-')) {
        ...
        const size = Math.max(RUN299_PACK_SCALE_FLOOR, Number(state?.snapSize) || 1);
      ...
      else activeNode.scale[axis] = Math.abs(value) < RUN299_PACK_SCALE_FLOOR ? RUN299_PACK_SCALE_FLOOR : value;
    }
  ...
  function applyScenePackOverrides(data) {
    ...
    for (const record of data?.objects || []) {
      ...
      for (const pack of record.fbxPacks) {
        const node = run299NodeFromPath(root, pack.path);
        ...
        root.userData.editorFbxPackOverrides[pack.path] = run299TransformRecord(node);
        applied += 1;
      }
  ...
  function onCanvasPointerDown(event) {
    ...
    const root = api.getSelectedObject?.();
    if (!run299IsFbxRoot(root)) return;
    const candidates = run299PackNodes(root);
    ...
    if (node && candidateSet.has(node)) selectPack(root, run299PathForNode(root, node));
  }
  ...
  function getSnapshot() {
    ...
    return Object.freeze({
      ...
      overrideCount: run299IsFbxRoot(root) ? serializeOverrides(root).length : 0
    });
  }

  function onKeyDown(event) {
    if (!activeNode || event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement) return;
  ...
  selectionObserver.observe(document.getElementById('we-selection-status'), { childList: true, characterData: true, subtree: true });
  ...
  const surface = Object.freeze({ listPacks, selectPack, clearSelection, serializeOverrides, applyScenePackOverrides, getSnapshot, dispose });
  window.__WESTEROS_EDITOR_FBX_PACKS__ = surface;
  syncRootSelection();
  return surface;
}

function bootRun299FbxPackController(attempt = 0) {
  ...
  if (api && transformSurface) {
    try {
      installRun299FbxPackController(api, transformSurface);
    } catch (error) {
      console.error('[Run299FbxPackController] boot failed', error);
    }
    return;
  }
  if (attempt < 180) requestAnimationFrame(() => bootRun299FbxPackController(attempt + 1));
}

queueMicrotask(() => bootRun299FbxPackController());
