...
// Run265 owner micro-scale override. The original Run216 controller stays intact for additive-only
// history; this earlier window-capture layer safely extends ordinary-object scale precision to 1e-6.
const RUN265_MIN_EDITOR_SCALE = 0.000001;
const RUN265_SCALE_DECIMALS = 6;
const RUN265_QUICK_SHRINK_FACTOR = 0.1;

function run265ScaleText(value) {
  const numeric = Number(value);
  return Number.isFinite(numeric) ? numeric.toFixed(RUN265_SCALE_DECIMALS) : '';
}
...
export function installEditorMicroScaleOverride(api) {
  ...
  function syncBounds() {
    Object.keys(SCALE_AXIS_BY_INPUT_ID).forEach((id) => {
      const input = document.getElementById(id);
      if (!input) return;
      input.min = String(RUN265_MIN_EDITOR_SCALE);
      input.step = String(RUN265_MIN_EDITOR_SCALE);
      input.setAttribute('inputmode', 'decimal');
    });
  }
  ...
  function syncPrecision(object = api.getSelectedObject?.()) {
    ...
    Object.entries(SCALE_AXIS_BY_INPUT_ID).forEach(([id, axis]) => {
      const input = document.getElementById(id);
      if (input) input.value = run265ScaleText(object.scale?.[axis]);
    });
  }
  ...
  function onScaleChangeCapture(event) {
    ...
    object.scale[axis] = Math.max(RUN265_MIN_EDITOR_SCALE, numeric);
  ...
  function onQuickShrinkCapture(event) {
    ...
    event.stopImmediatePropagation?.();
    object.scale.set(
      Math.max(RUN265_MIN_EDITOR_SCALE, object.scale.x * RUN265_QUICK_SHRINK_FACTOR),
      Math.max(RUN265_MIN_EDITOR_SCALE, object.scale.y * RUN265_QUICK_SHRINK_FACTOR),
      Math.max(RUN265_MIN_EDITOR_SCALE, object.scale.z * RUN265_QUICK_SHRINK_FACTOR)
    );
    finishScaleMutation(object);
  ...
  const surface = Object.freeze({
    dispose,
    minimumScale: RUN265_MIN_EDITOR_SCALE,
    decimals: RUN265_SCALE_DECIMALS,
    syncBounds,
    syncPrecision
  });
...
export const EDITOR_MICRO_SCALE_POLICY = Object.freeze({
  minimumScale: RUN265_MIN_EDITOR_SCALE,
  decimals: RUN265_SCALE_DECIMALS,
  quickShrinkFactor: RUN265_QUICK_SHRINK_FACTOR
});
...
// Run265 browser-integration guard. Dynamic module evaluation can install the additive micro layer
...
function installRun265MicroScaleBoundsGuard() {
  ...
  let disposed = false;
  let queued = false;
  const expected = String(RUN265_MIN_EDITOR_SCALE);
  ...
  const surface = Object.freeze({ sync, dispose, minimumScale: RUN265_MIN_EDITOR_SCALE });
...
queueMicrotask(() => {
  try { installRun265MicroScaleBoundsGuard(); }
