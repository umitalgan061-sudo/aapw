...
export function installEditorPlacementControllerSafe(api, authoring = window.__WESTEROS_EDITOR_LIVE_AUTHORING__) {
  ...
  function syncUi() {
    ...
    ui.groundButton.disabled = busy || !selected || selected.isInstancedMesh;
  }
  ...
  async function placeSelectedAtPoint(point) {
    ...
    try {
      ...
      console.error('[EditorPlacementControllerSafe] placement failed', error);
  ...
  const selectionObserver = new MutationObserver(syncUi);
...
if (api && authoring) {
  try {
    installEditorPlacementControllerSafe(api, authoring);
  } catch (error) {
    console.error('[EditorPlacementControllerSafe] boot failed', error);
  }
}
