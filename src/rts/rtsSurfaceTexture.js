...
import { WORLD_SCALE } from '../3d/config.js';
import { ChunkManager } from '../3d/world/chunkManager.js';
...
const state = {
	status: 'idle', sourceUrl: SOURCE_URL, sourceWidth: 0, sourceHeight: 0,
	runtimeTextureSize: 0, detailOpacity: DETAIL_OPACITY, appliedChunks: 0,
...
function ensureWorldUv(mesh) {
	...
	for (let index = 0; index < position.count; index += 1) {
		...
		uv.setXY(index, clamp01((worldX + halfWidth) / WORLD_SCALE.WORLD_WIDTH_METERS), clamp01(1 - ((worldZ + halfDepth) / WORLD_SCALE.WORLD_DEPTH_METERS)));
	}
	uv.needsUpdate = true;
...
function applyToTerrainMesh(mesh) {
	...
	if (Array.isArray(mesh.material)) for (const material of mesh.material) setTextureOnMaterial(material);
	else setTextureOnMaterial(mesh.material);
...
function createRuntimeTexture(image) {
	...
	context.fillStyle = '#ffffff'; context.fillRect(0, 0, textureSize, textureSize);
	context.save(); context.globalAlpha = DETAIL_OPACITY; context.filter = DETAIL_FILTER;
	context.imageSmoothingEnabled = true; context.imageSmoothingQuality = 'high';
	context.drawImage(image, 0, 0, textureSize, textureSize); context.restore();
	const texture = new THREE.CanvasTexture(canvas);
	texture.name = 'run210-owner-surface-detail'; texture.colorSpace = THREE.SRGBColorSpace;
...
function startTextureLoad() {
	...
	image.addEventListener('load', () => {
		if (disposed) return;
		state.sourceWidth = image.naturalWidth; state.sourceHeight = image.naturalHeight;
		try { runtimeTexture = createRuntimeTexture(image); state.status = 'ready'; for (const manager of managers) applyToManager(manager); }
		catch (_error) { state.status = 'failed'; }
	}, { once: true });
	image.addEventListener('error', () => { if (!disposed) state.status = 'failed'; }, { once: true });
...
export function installRtsSurfaceTexture() {
	...
	ChunkManager.prototype.loadChunk = function loadChunkWithRtsSurfaceRun210(chunkX, chunkZ) {
		managers.add(this); const mesh = loadChunkBeforeRun210.call(this, chunkX, chunkZ); applyToTerrainMesh(mesh); return mesh;
	};
	ChunkManager.prototype.streamTowards = function streamTowardsWithRtsSurfaceRun210(centerChunkX, centerChunkZ, radius) {
		managers.add(this); const result = streamTowardsBeforeRun210.call(this, centerChunkX, centerChunkZ, radius); applyToManager(this); return result;
	};
	startTextureLoad();
	window.addEventListener('pagehide', () => {
		disposed = true; state.status = 'disposed'; runtimeTexture?.dispose(); runtimeTexture = null; managers.clear();
