...
### Iteration #06 — 2026-08-12 09:22 Europe/Istanbul
**Seçilen iyileştirme:** Temiz checkout'ta üretilen HTerrain `terrain_data` çıktısını SHA-256 manifestli, doğrulanabilir bir GitHub Actions artifact snapshot'ı olarak paketlemek.
**Neden bu seçildi:** Iteration #05, clean authoring bootstrap'ın `data.hterrain`, height/splat/detail/global-map kaynaklarını gerçekten ürettiğini kanıtladı; ancak repository hâlâ `terrain_data/.gitkeep` dışında bu çıktıları kalıcılaştırmıyor. Doğrudan binary commit veya Git LFS kararı vermeden önce gerçek dosya setini, boyutlarını ve bütünlüğünü ölçülebilir bir pipeline checkpoint'ine dönüştürmek daha küçük ve geri alınabilir adımdır.
**Yapılan değişiklikler:** Runtime, terrain scene, bootstrap ve `addons/zylann.hterrain/` hiç değiştirilmedi. Yeni `build_terrain_data_snapshot.py`, `terrain_data` altındaki tüm üretilmiş dosyaları `.gitkeep` hariç runner geçici alanına kopyalıyor; mevcut 13 temel HTerrain dosyasını zorunlu tutuyor, her dosya için SHA-256 ve byte boyutu kaydediyor ve deterministik `manifest.json` üretiyor. Iteration #06 validator aynı snapshot-ready dosya setini ve önceki global-map/detail/tint/determinism sözleşmelerini doğruluyor. CI doğrulanmış snapshot'ı 90 günlük GitHub Actions artifact'i olarak yüklüyor.
**Test sonucu:** `Terrain Polish Iteration 006` zinciri exact-main/additive-only kapsamını, Godot 4.6.3 import/editor ve temel/#05/#06 HTerrain doğrulamalarını, snapshot kopya hash'lerini ve tekrarlanan manifest determinismini, artifact upload'ını, PWA/cache/installability, mobil performans, determinism/world-safety, teknik borç, Chromium smoke/console ve final exact-main kapılarını çalıştıracak.
**Gözlem:** Snapshot, `data.hterrain`, `.uid`, `height.res`, color/splat/normal/detail/global-albedo PNG kaynakları ile bunların Godot `.import` metadata dosyalarını birlikte koruyacak. Böylece sonraki version-control/LFS kararı varsayım yerine gerçek HTerrain çıktı envanteri ve hash manifesti üzerinden verilebilir.
**Risk / Geri alma notu:** Risk düşüktür; oyun/runtime davranışı değişmez ve hiçbir binary Terrain Data dosyası bu iterasyonda repository'ye eklenmez. CI artifact'i kalıcı VCS değildir ve 90 günlük retention ile sınırlıdır; bu iterasyon yalnız güvenli ara checkpoint sağlar. Yeni workflow/validator/builder dosyaları kaldırılarak tamamen geri alınabilir.
**Bir sonraki çalıştırma için öneri:** DoD temiz kalırsa artifact'in gerçek byte boyutlarını ve dosya kararlılığını kullanarak Git/LFS veya ayrı asset-store stratejisini seç; ancak gerçek authored height/splat verisi oluşmadan sentetik starter veriyi üretim terrain'i olarak commit etme.

### Iteration #06 DoD sonucu — 2026-08-12 09:36 Europe/Istanbul
- GitHub Actions `Terrain Polish Iteration 006` run #1 PASS oldu: exact-main/additive giriş ve final kapıları, Godot 4.6.3 import/editor, `HTERRAIN_AUTHORING_VALIDATION_OK resolution=513 texture_slots=4 detail_maps=1 seed=20260811 collision=true`, Iteration #05 persistence sözleşmesi, `TERRAIN_POLISH_ITERATION_06_OK resolution=513 snapshot_files=13 global_maps=1 detail_maps=1`, PWA/cache/installability, mobil performans, determinism/world-safety, teknik borç ve tam Chromium smoke/console zinciri geçti. Aynı head için altı PR-triggered governance workflow'u da PASS oldu.
- Snapshot manifesti 13 HTerrain kaynak dosyasını toplam `1066553` byte olarak ölçtü; `height.res` tek başına `1053042` byte. Aynı kaynak üzerinde iki üretimde manifest byte-for-byte aynı kaldı ve manifest SHA-256 değeri `478aeb51818c4ef243d864d6d0ed7271fdce21c3fd16a930a32ea805c2f9ad12` oldu.
- Artifact ID `9131010544`, sıkıştırılmış boyut `6672` byte, 90 günlük son kullanım tarihi `2026-11-10T06:29:45Z` ve ZIP digest'i `641ebf39775e0a3548b9f451884b0a1d57d8ada1b7d56f69f8f7d87904f71384`. Artifact ayrıca bağımsız indirildi; ZIP SHA-256 değeri GitHub digest'iyle birebir eşleşti ve 13 kaynak dosyanın tamamı ile `manifest.json` bulundu.
- Mobil örnek 35 draw call ve 195929 triangle ile 500 / 500000 bütçelerinin altında kaldı; 14/14 terrain seat ve 14/14 yol bağlantısı PASS oldu. Runtime, terrain scene, bootstrap, tracked `terrain_data` ve HTerrain çekirdeği değiştirilmedi.
- Mevcut normalmap-baker shader compiler mesajı, UID fallback ve kapanış ObjectDB/resource uyarıları tekrar görüldü; özel ve temel doğrulamalar PASS olduğu ve addon diff'i sıfır kaldığı için çekirdeğe müdahale edilmedi.
- Iteration #06 başarılı kabul edildi. Sonraki terrain polish çalışması sentetik starter snapshot'ı üretim verisi olarak commit etmemeli; önce gerçekten authored height/splat verisi üretilip yeni snapshot'ın byte/hash büyümesi ölçülmeli, ardından ordinary Git / Git LFS / ayrı asset-store kararı gerçek veri üzerinden verilmelidir.
