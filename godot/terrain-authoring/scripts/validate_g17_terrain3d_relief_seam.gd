	...
	for edge in positions:
		for other in cross:
			for point in [Vector2(float(edge),float(other)),Vector2(float(other),float(edge))]:
				...
				var slope_jump:=maxf(absf(dx-expected_dx),absf(dz-expected_dz)); max_slope_jump=maxf(max_slope_jump,slope_jump)
				max_derivative_error=maxf(max_derivative_error,slope_jump)
