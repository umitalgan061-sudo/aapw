...
const PROBE_PATH := "res://.terrain3d-proof/g65-biome-probe.json"
const EXPECTED_POLICY := "kizil-ufuk-g65-terrain3d-biome-2026-08-13-v1"
...
func _fail(message: String) -> void:
	push_error("G65 Terrain3D biome proof failed: " + message)
	quit(1)
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G65 biome probe JSON missing"): return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G65 biome policy"): return
	...
	if not _require(int(probe["sourceGridSize"]) == 65, "proof must use deterministic 65x65 authoring source"): return

	var terrain := Terrain3D.new(); terrain.name = "G65Terrain3DBiomeProof"; get_root().add_child(terrain); terrain.region_size = 256
	...
	var suffix := OS.get_environment("G65_BIOME_PROOF_SUFFIX"); if suffix.is_empty(): suffix = "default"
	var output_dir := "user://g65-terrain3d-biome-proof-" + suffix
	...
	print("G65_TERRAIN3D_BIOME_METRICS=" + JSON.stringify(metrics)); print("SE_G65_TERRAIN3D_BIOME_VALIDATION_OK"); quit(0)
