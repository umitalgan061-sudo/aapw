...
func _nearest_int(source: Dictionary, channel: String, u: float, v: float) -> int:
	var x := clampi(int(round(clampf(u, 0.0, 1.0) * float(SOURCE_SIZE - 1))), 0, SOURCE_SIZE - 1)
	var y := clampi(int(round(clampf(v, 0.0, 1.0) * float(SOURCE_SIZE - 1))), 0, SOURCE_SIZE - 1)
	return int(source[channel][y * SOURCE_SIZE + x])
...
func _build_control_image(source: Dictionary) -> Image:
	...
	for z in IMPORT_SIZE:
		...
		for x in IMPORT_SIZE:
			var u := float(x) / 256.0
			var blend_u8 := int(round(clampf(_channel_value(source, "blend", u, v), 0.0, 1.0) * 255.0))
			var bits: int = Terrain3DUtil.enc_base(_nearest_int(source, "baseId", u, v)) | Terrain3DUtil.enc_overlay(_nearest_int(source, "overlayId", u, v)) | Terrain3DUtil.enc_blend(blend_u8)
...
func _saved_region_evidence(directory: String) -> Dictionary:
	...
	while name != "":
		if not dir.current_is_dir():
			files.push_back(name); total_bytes += FileAccess.get_file_as_bytes(directory.path_join(name)).size()
		name = dir.get_next()
...
func _write_preview(terrain: Terrain3D) -> Error:
	...
	for z in IMPORT_SIZE:
		for x in IMPORT_SIZE:
			...
			var color := terrain.data.get_color(pos); var roughness := terrain.data.get_roughness(pos); var height := terrain.data.get_height(pos)
...
func _run() -> void:
	...
	for channel in ["heights","baseId","overlayId","blend","tintR","tintG","tintB","roughness","roadCoverage","pathCoverage","snowWeight","landFactor"]:
	...
	var terrain := Terrain3D.new(); terrain.name = "G00Terrain3DRuntimeParityProof"; get_root().add_child(terrain); terrain.region_size = REGION_SIZE
	...
	var heights: Array[float] = []; var base_id: Array[int] = []; var overlay_id: Array[int] = []; var blend: Array[float] = []
	var tint_r: Array[float] = []; var tint_g: Array[float] = []; var tint_b: Array[float] = []; var roughness_values: Array[float] = []
	var road_coverage: Array = source["roadCoverage"].duplicate(); var path_coverage: Array = source["pathCoverage"].duplicate(); var snow_weight: Array = source["snowWeight"].duplicate(); var land_factor: Array = source["landFactor"].duplicate()
	var max_height_error := 0.0; var max_blend_error := 0.0; var max_color_error := 0.0; var max_roughness_error := 0.0; var id_mismatches := 0; var checksum: int = 2166136261
	for sy in SOURCE_SIZE:
		var v := float(sy) / 64.0
		for sx in SOURCE_SIZE:
			var u := float(sx) / 64.0; var pos := Vector3(float(sx * 4), 0.0, float(sy * 4))
			var actual_height := terrain.data.get_height(pos); var actual_base := terrain.data.get_control_base_id(pos); var actual_overlay := terrain.data.get_control_overlay_id(pos); var actual_blend := terrain.data.get_control_blend(pos)
			var actual_color := terrain.data.get_color(pos); var actual_roughness := terrain.data.get_roughness(pos)
			if not _require(not is_nan(actual_height) and not is_nan(actual_blend) and not is_nan(actual_color.r) and not is_nan(actual_roughness), "non-finite Terrain3D aligned sample"): return
			var expected_height := _channel_value(source, "heights", u, v); var expected_blend := _channel_value(source, "blend", u, v); var expected_color := _source_color(source, u, v)
			var expected_base := _nearest_int(source, "baseId", u, v); var expected_overlay := _nearest_int(source, "overlayId", u, v)
			if actual_base != expected_base or actual_overlay != expected_overlay: id_mismatches += 1
			max_height_error = maxf(max_height_error, absf(actual_height - expected_height)); max_blend_error = maxf(max_blend_error, absf(actual_blend - expected_blend))
			...
			heights.push_back(snappedf(actual_height, 0.000001)); base_id.push_back(actual_base); overlay_id.push_back(actual_overlay); blend.push_back(snappedf(actual_blend, 0.00000001))
			...
			for value in [actual_base, actual_overlay, int(round(clampf(actual_blend,0.0,1.0)*255.0)), int(round(clampf(actual_color.r,0.0,1.0)*255.0)), int(round(clampf(actual_color.g,0.0,1.0)*255.0)), int(round(clampf(actual_color.b,0.0,1.0)*255.0)), int(round(clampf(actual_roughness,0.0,1.0)*255.0))]: checksum = _hash_u8(checksum, int(value))
	if not _require(max_height_error <= HEIGHT_TOLERANCE, "aligned height parity exceeded tolerance"): return
	if not _require(max_blend_error <= UNIT_TOLERANCE, "aligned control blend parity exceeded tolerance"): return
	if not _require(max_color_error <= UNIT_TOLERANCE, "aligned tint parity exceeded tolerance"): return
	if not _require(max_roughness_error <= UNIT_TOLERANCE, "aligned roughness parity exceeded tolerance"): return
	if not _require(id_mismatches == 0, "aligned base/overlay texture IDs changed"): return
	...
	if not _require(boundary_probe_max_height_error <= HEIGHT_TOLERANCE, "255/256 height seam exceeded tolerance"): return
	...
	var suffix := OS.get_environment("G00_RUNTIME_PARITY_PROOF_SUFFIX"); if suffix.is_empty(): suffix = "default"
	var save_dir := "user://g00-terrain3d-runtime-parity-" + suffix; DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(save_dir)); terrain.data.save_directory(save_dir)
	...
	var bake := {"schema":"westeros-g00-terrain3d-bake-v1","policyId":"buzul-muhafizi-g00-terrain3d-threejs-runtime-parity-2026-08-13-v1","sourceMapSha256":EXPECTED_MAP_SHA,"terrain3dVersion":String(terrain.version),"lod":0,"width":SOURCE_SIZE,"height":SOURCE_SIZE,"regionCount":region_count,"savedRegionFiles":saved["files"].size(),"savedRegionBytes":int(saved["totalBytes"]),"bakedSurfaces":baked_mesh.get_surface_count(),"bakedVertices":baked_vertices.size(),"maxHeightError":snappedf(max_height_error,0.00000001),"maxBlendError":snappedf(max_blend_error,0.00000001),"maxColorError":snappedf(max_color_error,0.00000001),"maxRoughnessError":snappedf(max_roughness_error,0.00000001),"idMismatches":id_mismatches,"boundaryProbeMaxHeightError":snappedf(boundary_probe_max_height_error,0.00000001),"bakeChecksum":checksum,"heights":heights,"baseId":base_id,"overlayId":overlay_id,"blend":blend,"tintR":tint_r,"tintG":tint_g,"tintB":tint_b,"roughness":roughness_values,"roadCoverage":road_coverage,"pathCoverage":path_coverage,"snowWeight":snow_weight,"landFactor":land_factor}
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://.terrain3d-proof")); var output := FileAccess.open(BAKE_PATH, FileAccess.WRITE)
	if not _require(output != null, "could not open runtime bake JSON output"): return
	output.store_string(JSON.stringify(bake) + "\n"); output.close()
	print("G00_TERRAIN3D_RUNTIME_BAKE_METRICS=" + JSON.stringify({"regionCount":region_count,"savedRegionFiles":saved["files"].size(),"bakedVertices":baked_vertices.size(),"maxHeightError":bake["maxHeightError"],"maxBlendError":bake["maxBlendError"],"maxColorError":bake["maxColorError"],"maxRoughnessError":bake["maxRoughnessError"],"idMismatches":id_mismatches,"boundaryProbeMaxHeightError":bake["boundaryProbeMaxHeightError"],"bakeChecksum":checksum}))
