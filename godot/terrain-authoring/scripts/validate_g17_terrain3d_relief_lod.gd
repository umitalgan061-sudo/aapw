...
func _run()->void:
	...
	for lod in [0,1,2]:
		...
		var aabb:=mesh.get_aabb(); if not need(max_h>min_h and aabb.size.x>0.0 and aabb.size.z>0.0,"LOD%d bake bounds invalid"%lod): return
		levels.append({"lod":lod,"surfaces":mesh.get_surface_count(),"vertices":vertices.size(),"minHeight":snappedf(min_h,0.000001),"maxHeight":snappedf(max_h,0.000001),"checksum":checksum,"aabbX":snappedf(aabb.size.x,0.001),"aabbZ":snappedf(aabb.size.z,0.001)})
