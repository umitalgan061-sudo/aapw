...
func _build_probe() -> void:
	...
	if land_vertices <= 0 or water_vertices <= 0 or transition_vertices <= 0:
		...
		return
	if absf(minimum_height - WATER_HEIGHT) > 0.00001 or absf(maximum_height - LAND_HEIGHT) > 0.00001:
		_fail("G27 HTerrain probe height bounds drifted")
		return
	...
	if not FileAccess.file_exists(PROBE_DIRECTORY.path_join("height.res")):
		...
		return
	print("SW_G27_HTERRAIN_PROBE_OK resolution=%s land_vertices=%s water_vertices=%s transition_vertices=%s min_height=%.6f max_height=%.6f" % [RESOLUTION, land_vertices, water_vertices, transition_vertices, minimum_height, maximum_height])
	quit(0)
