...
const PROBE_PATH := "res://.terrain3d-proof/g55-probe.json"
const EXPECTED_POLICY := "kizil-ufuk-g55-terrain3d-hydrology-2026-08-12-v1"
...
func _fail(message: String) -> void:
	push_error("G55 Terrain3D proof failed: " + message)
	quit(1)
...
func _run() -> void:
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G55 policy id"):
		return
	...
	terrain.name = "G55Terrain3DProof"
	...
	for z in sample_positions:
		for x in sample_positions:
			...
			if is_nan(actual):
				_fail("Terrain3D returned NaN for imported G55 height")
				return
	...
	var suffix := OS.get_environment("G55_PROOF_SUFFIX")
	if suffix.is_empty():
		suffix = "default"
	var output_dir := "user://g55-terrain3d-proof-" + suffix
	...
	print("G55_TERRAIN3D_METRICS=" + JSON.stringify(metrics))
	print("SE_G55_TERRAIN3D_VALIDATION_OK")
	quit(0)
