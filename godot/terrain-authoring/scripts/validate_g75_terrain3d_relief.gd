...
const PROBE_PATH := "res://.terrain3d-proof/g75-relief-probe.json"
const PREVIEW_PATH := "res://.terrain3d-proof/g75-relief-imported-topdown.png"
const EXPECTED_POLICY := "kizil-ufuk-g75-terrain3d-relief-2026-08-13-v1"
...
func _fail(message: String) -> void:
	push_error("G75 Terrain3D relief proof failed: " + message)
	quit(1)
...
func _write_preview(terrain: Terrain3D, min_height: float, max_height: float) -> bool:
	...
	for z in size:
		for x in size:
			...
			preview.set_pixel(x, z, Color(0.16, 0.22, 0.10).lerp(Color(0.72, 0.62, 0.38), t))
	var absolute := ProjectSettings.globalize_path(PREVIEW_PATH)
	DirAccess.make_dir_recursive_absolute(absolute.get_base_dir())
	return preview.save_png(PREVIEW_PATH) == OK

func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G75 relief probe JSON missing"):
		return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G75 relief policy"):
		return
	...
	if not _require(int(probe["terrain3dRegionSize"]) == 256, "proof must use Terrain3D region size 256"):
		return
	if not _require(int(probe["canonicalWaterCells"]) == 0 and int(probe["canonicalLandCells"]) == 96, "G75 dry hydrology fingerprint changed"):
		return
	if not _require(int(probe["canonicalSignMismatches"]) == 0, "relief moved canonical dry-land semantics"):
		return

	var terrain := Terrain3D.new()
	terrain.name = "G75Terrain3DReliefProof"
	...
	for z in range(0, 256, 16):
		for x in range(0, 256, 16):
			...
			if is_nan(actual):
				_fail("Terrain3D returned NaN for imported G75 relief")
				return
	...
	if not _require(max_error <= MAX_HEIGHT_ERROR, "Terrain3D height roundtrip exceeded tolerance"):
		return
	if not _require(min_imported > 0.0 and max_imported > min_imported, "Terrain3D relief lost dry-land height character"):
		return
	...
	var suffix := OS.get_environment("G75_RELIEF_PROOF_SUFFIX")
	if suffix.is_empty(): suffix = "default"
	var output_dir := "user://g75-terrain3d-relief-proof-" + suffix
	...
	print("G75_TERRAIN3D_RELIEF_METRICS=" + JSON.stringify(metrics))
	print("SE_G75_TERRAIN3D_RELIEF_VALIDATION_OK")
	quit(0)
