...
const PROBE_PATH := "res://.terrain3d-proof/g07-near-detail-probe.json"
const EXPECTED_POLICY := "gunbatimi-ustasi-g07-terrain3d-near-detail-2026-08-13-v1"
...
func _fail(message: String) -> void:
	push_error("G07 Terrain3D near-detail proof failed: " + message)
	quit(1)
...
func _source_blend(probe: Dictionary, u: float, v: float) -> float:
	...
	return clampf(lerpf(lerpf(float(rows[y0][x0]), float(rows[y0][x1]), tx), lerpf(float(rows[y1][x0]), float(rows[y1][x1]), tx), ty), 0.0, 1.0)
...
func _build_control_image(probe: Dictionary) -> Image:
	...
	for z in size:
		...
		for x in size:
			...
			var bits: int = Terrain3DUtil.enc_base(int(probe["substrateTextureId"])) | Terrain3DUtil.enc_overlay(int(probe["microDetailTextureId"])) | Terrain3DUtil.enc_blend(blend_u8)
...
func _write_preview(terrain: Terrain3D) -> Error:
	...
	for z in 256:
		for x in 256:
			...
			image.set_pixel(x, z, Color(0.08,0.14,0.17,1.0).lerp(Color(0.26,0.31,0.33,1.0), clampf(blend,0.0,1.0)))
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://.terrain3d-proof"))
	return image.save_png("res://.terrain3d-proof/g07-near-detail-imported-topdown.png")
...
func _run() -> void:
	...
	var terrain := Terrain3D.new(); terrain.name = "G07Terrain3DNearDetailProof"; get_root().add_child(terrain); terrain.region_size = 256
	...
	for z in range(0, 256, 16):
		for x in range(0, 256, 16):
			...
			if not _require(terrain.data.get_control_overlay_id(pos) == int(probe["microDetailTextureId"]), "micro detail texture changed"): return
			var actual := terrain.data.get_control_blend(pos)
			if not _require(not is_nan(actual), "blend returned NaN"): return
			max_error = maxf(max_error, absf(actual-expected)); checksum = int((checksum ^ int(round(clampf(actual,0.0,1.0)*255.0))) * 16777619) & 0xffffffff; samples += 1
	...
	var suffix := OS.get_environment("G07_NEAR_DETAIL_PROOF_SUFFIX"); if suffix.is_empty(): suffix = "default"
	var out := "user://g07-terrain3d-near-detail-proof-" + suffix; DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(out)); terrain.data.save_directory(out)
	var dir := DirAccess.open(out); if not _require(dir != null, "save directory missing"): return
	...
	print("G07_TERRAIN3D_NEAR_DETAIL_METRICS=" + JSON.stringify({"terrain3dVersion":String(terrain.version),"regionCount":terrain.data.get_region_count(),"sampleCount":samples,"maxBlendError":snappedf(max_error,0.00000001),"outputChecksum":checksum,"bakedSurfaces":baked.get_surface_count(),"bakedVertices":vertices.size(),"savedRegionFiles":files,"savedRegionBytes":bytes}))
	print("SW_G07_TERRAIN3D_NEAR_DETAIL_VALIDATION_OK")
	quit(0)
