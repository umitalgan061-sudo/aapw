...
const PROBE_PATH := "res://.terrain3d-proof/g11-biome-probe.json"
const EXPECTED_POLICY := "buzul-muhafizi-g11-terrain3d-biome-2026-08-12-v1"
...
func _fail(message: String) -> void:
	push_error("G11 Terrain3D biome proof failed: " + message)
	quit(1)
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G11 biome probe JSON missing"):
		return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G11 biome policy"):
		return
	...
	terrain.name = "G11Terrain3DBiomeProof"
	...
	var suffix := OS.get_environment("G11_BIOME_PROOF_SUFFIX")
	if suffix.is_empty():
		suffix = "default"
	var output_dir := "user://g11-terrain3d-biome-proof-" + suffix
	...
	print("G11_TERRAIN3D_BIOME_METRICS=" + JSON.stringify(metrics))
	print("NW_G11_TERRAIN3D_BIOME_VALIDATION_OK")
	quit(0)
