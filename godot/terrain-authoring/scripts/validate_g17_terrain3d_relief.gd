...
const PROBE_PATH := "res://.terrain3d-proof/g17-relief-probe.json"
const PREVIEW_PATH := "res://.terrain3d-proof/g17-relief-imported-topdown.png"
...
func _fail(message: String) -> void:
	push_error("G17 Terrain3D relief proof failed: " + message)
	quit(1)
...
func _write_preview(terrain: Terrain3D, min_height: float, max_height: float) -> bool:
	...
	for z in size:
		for x in size:
			...
			preview.set_pixel(x, z, Color(0.03, 0.09, 0.12).lerp(Color(0.22, 0.42, 0.48), t))
	var absolute := ProjectSettings.globalize_path(PREVIEW_PATH)
	DirAccess.make_dir_recursive_absolute(absolute.get_base_dir())
	return preview.save_png(PREVIEW_PATH) == OK

func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G17 relief probe JSON missing"):
		return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G17 relief policy"):
		return
	...
	if not _require(float(probe["worldWidthMeters"]) > 13000.0 and float(probe["worldDepthMeters"]) > 10000.0, "physical normal scale is not full-reference extent"):
		return
	if not _require(int(probe["canonicalWaterCells"]) == 96 and int(probe["canonicalLandCells"]) == 0, "G17 marine hydrology fingerprint changed"):
		return
	if not _require(float(probe["maxHeight"]) < WATER_CEILING and float(probe["heightSpan"]) > 0.45, "G17 relief lost marine depth character"):
		return

	var height_image := _build_height_image(probe)
	var terrain := Terrain3D.new()
	terrain.name = "G17Terrain3DReliefProof"
	...
	if not _require(terrain.data.get_region_count() >= 4, "257x257 G17 relief import must create at least four Terrain3D regions"):
		return
	...
	for sy in SOURCE_SIZE:
		for sx in SOURCE_SIZE:
			...
			if is_nan(actual):
				_fail("Terrain3D returned NaN for aligned G17 relief")
				return
			...
			output_checksum = int((output_checksum ^ int(round((actual + 32.0) * 1000.0))) * 16777619) & 0xffffffff
			aligned_samples += 1
	if not _require(max_aligned_error <= MAX_ALIGNED_HEIGHT_ERROR, "Terrain3D aligned height roundtrip exceeded tolerance"):
		return
	if not _require(max_imported > min_imported and max_imported < WATER_CEILING, "Terrain3D import lost submerged relief semantics"):
		return
	...
	for z in seam_values:
		for x in seam_values:
			...
			if is_nan(actual):
				_fail("Terrain3D returned NaN across 255/256 G17 region seam")
				return
	...
	for x in seam_values:
		for z in [32.25, 96.5, 160.75, 224.5]:
			...
			if is_nan(actual_x):
				_fail("Terrain3D returned NaN on vertical G17 region seam")
				return
			...
			if is_nan(actual_z):
				_fail("Terrain3D returned NaN on horizontal G17 region seam")
				return
	...
	if suffix.is_empty():
		suffix = "default"
	var output_dir := "user://g17-terrain3d-relief-proof-" + suffix
	...
	if not _require(saved["files"].size() >= 4 and int(saved["totalBytes"]) > 0, "Terrain3D relief region resources were not persisted"):
		return
	if not _require(_write_preview(terrain, float(probe["minHeight"]), float(probe["maxHeight"])), "failed to write imported G17 top-down evidence"):
		return
	...
	print("SW_G17_TERRAIN3D_RELIEF_VALIDATION_OK")
	quit(0)
