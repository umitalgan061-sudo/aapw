...
func _run_validation() -> void:
	...
	if not bottom_ao is float or not is_equal_approx(bottom_ao, 0.35):
		_fail("Iteration #03 grass bottom AO regressed: %s" % [bottom_ao])
		return
	...
	if not tint_bottom is float or not is_equal_approx(tint_bottom, 0.55):
		_fail("Grass global-map bottom tint must be 0.55 in Iteration #04: %s" % [tint_bottom])
		return
	if not tint_top is float or not is_equal_approx(tint_top, 0.10):
		_fail("Grass global-map top tint must be 0.10 in Iteration #04: %s" % [tint_top])
		return
	if tint_bottom <= tint_top:
		_fail("Grass global-map tint gradient must remain stronger at the ground contact")
		return
