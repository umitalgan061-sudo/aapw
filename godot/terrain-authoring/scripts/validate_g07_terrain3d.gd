...
const PROBE_PATH := "res://.terrain3d-proof/g07-probe.json"
const EXPECTED_POLICY := "gunbatimi-ustasi-g07-terrain3d-hydrology-2026-08-12-v1"
const EXPECTED_SOURCE_SHA := "20702972e8f45f0fbdc4da5fa68e890a82e4e822e1d58e2f369d8bc5b9c571a1"
const MAX_HEIGHT_ERROR := 0.00001
const EXPECTED_WATER_HEIGHT := -6.0
...
func _fail(message: String) -> void:
	push_error("G07 Terrain3D proof failed: " + message)
	quit(1)
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "canonical probe JSON missing"):
		return
	var raw := FileAccess.get_file_as_string(PROBE_PATH)
	var parsed = JSON.parse_string(raw)
	if not _require(parsed is Dictionary, "probe JSON did not decode to Dictionary"):
		return
	var probe: Dictionary = parsed
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G07 policy id"):
		return
	...
	if not _require(int(probe["terrain3dRegionSize"]) == 256, "Terrain3D proof must use a real 256 region"):
		return
	if not _require(is_equal_approx(float(probe["waterHeightMeters"]), EXPECTED_WATER_HEIGHT), "G07 sea-floor height policy changed"):
		return

	var terrain := Terrain3D.new()
	terrain.name = "G07Terrain3DProof"
	...
	if not _require(absf(min_source_height - EXPECTED_WATER_HEIGHT) <= MAX_HEIGHT_ERROR, "G07 source probe minimum is not canonical sea floor"):
		return
	if not _require(absf(max_source_height - EXPECTED_WATER_HEIGHT) <= MAX_HEIGHT_ERROR, "G07 source probe synthesized non-sea relief"):
		return
	...
	for z in sample_positions:
		for x in sample_positions:
			...
			if is_nan(actual):
				_fail("Terrain3D returned NaN for imported G07 height")
				return
	...
	if not _require(max_error <= MAX_HEIGHT_ERROR, "Terrain3D persisted/imported height drift exceeded tolerance"):
		return
	if not _require(max_imported_height - min_imported_height <= MAX_HEIGHT_ERROR, "G07 Terrain3D import introduced an internal height seam"):
		return
	...
	var suffix := OS.get_environment("G07_PROOF_SUFFIX")
	if suffix.is_empty():
		suffix = "default"
	var output_dir := "user://g07-terrain3d-proof-" + suffix
	...
	print("G07_TERRAIN3D_METRICS=" + JSON.stringify(metrics))
	print("SW_G07_TERRAIN3D_VALIDATION_OK")
	quit(0)
