...
const PROBE_PATH := "res://.terrain3d-proof/g75-probe.json"
const EXPECTED_POLICY := "kizil-ufuk-g75-terrain3d-hydrology-2026-08-12-v1"
const EXPECTED_SOURCE_SHA := "20702972e8f45f0fbdc4da5fa68e890a82e4e822e1d58e2f369d8bc5b9c571a1"
const MAX_HEIGHT_ERROR := 0.00001
const EXPECTED_DRY_HEIGHT := 0.75

func _initialize() -> void:
	call_deferred("_run")

func _fail(message: String) -> void:
	push_error("G75 Terrain3D proof failed: " + message)
	quit(1)
...
func _halo_water(probe: Dictionary, cell_x: int, cell_y: int) -> float:
	...
	if local_y < 0 or local_y >= rows.size():
		_fail("halo Y sample escaped emitted owner-mask probe")
		return 0.0
	var row: Array = rows[local_y]
	if local_x < 0 or local_x >= row.size():
		_fail("halo X sample escaped emitted owner-mask probe")
		return 0.0
...
func _water_confidence(probe: Dictionary, normalized_x: float, normalized_y: float) -> float:
	...
	return clampf(lerpf(lerpf(w00, w10, tx), lerpf(w01, w11, tx), ty), 0.0, 1.0)
...
func _build_height_image(probe: Dictionary) -> Image:
	...
	for z in region_size:
		...
		for x in region_size:
			...
			image.set_pixel(x, z, Color(lerpf(dry_height, water_height, confidence), 0.0, 0.0, 1.0))
	return image
...
func _run() -> void:
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G75 policy id"):
		return
	...
	if not _require(int(probe["terrain3dRegionSize"]) == 256, "Terrain3D proof must use a real 256 region"):
		return
	if not _require(is_equal_approx(float(probe["dryHeightMeters"]), EXPECTED_DRY_HEIGHT), "G75 dry height policy changed"):
		return

	var terrain := Terrain3D.new()
	terrain.name = "G75Terrain3DProof"
	...
	var min_source_height := INF
	var max_source_height := -INF
	for z in 256:
		for x in 256:
			var source_height := height_image.get_pixel(x, z).r
			min_source_height = minf(min_source_height, source_height)
			max_source_height = maxf(max_source_height, source_height)
	if not _require(absf(min_source_height - EXPECTED_DRY_HEIGHT) <= MAX_HEIGHT_ERROR, "G75 source probe minimum is not dry land"):
		return
	if not _require(absf(max_source_height - EXPECTED_DRY_HEIGHT) <= MAX_HEIGHT_ERROR, "G75 source probe synthesized water/relief"):
		return
	...
	for z in sample_positions:
		for x in sample_positions:
			...
			if is_nan(actual):
				_fail("Terrain3D returned NaN for imported G75 height")
				return
			...
			sample_count += 1
	if not _require(max_error <= MAX_HEIGHT_ERROR, "Terrain3D persisted/imported height drift exceeded tolerance"):
		return
	if not _require(max_imported_height - min_imported_height <= MAX_HEIGHT_ERROR, "G75 Terrain3D import introduced an internal height seam"):
		return
	...
	var suffix := OS.get_environment("G75_PROOF_SUFFIX")
	if suffix.is_empty():
		suffix = "default"
	var output_dir := "user://g75-terrain3d-proof-" + suffix
	...
	var metrics := {
		...
		"heightSpan": snappedf(max_imported_height - min_imported_height, 0.00000001),
	...
	print("G75_TERRAIN3D_METRICS=" + JSON.stringify(metrics))
	print("SE_G75_TERRAIN3D_VALIDATION_OK")
	quit(0)
