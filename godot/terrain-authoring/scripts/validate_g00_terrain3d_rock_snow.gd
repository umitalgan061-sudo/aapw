...
const PROBE_PATH := "res://.terrain3d-proof/g00-rock-snow-probe.json"
const PREVIEW_PATH := "res://.terrain3d-proof/g00-rock-snow-imported-topdown.png"
const EXPECTED_POLICY := "buzul-muhafizi-g00-terrain3d-rock-snow-2026-08-13-v1"
...
func _fail(message: String) -> void:
	push_error("G00 Terrain3D rock/snow proof failed: " + message)
	quit(1)
...
func _build_control_image(probe: Dictionary) -> Image:
	...
	for z in region_size:
		...
		for x in region_size:
			var u := float(x) / float(region_size - 1)
			# Import physical snow weight so canonical water remains zero overlay.
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G00 rock/snow probe JSON missing"):
		return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G00 rock/snow policy"):
		return
	...
	terrain.name = "G00Terrain3DRockSnowProof"
	...
	if not _require(_write_imported_preview(terrain, probe) == OK, "failed to write imported G00 rock/snow preview"):
		return
	...
	var suffix := OS.get_environment("G00_ROCK_SNOW_PROOF_SUFFIX")
	if suffix.is_empty():
		suffix = "default"
	var output_dir := "user://g00-terrain3d-rock-snow-proof-" + suffix
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(output_dir))
	terrain.data.save_directory(output_dir)
	var saved := _saved_region_evidence(output_dir)
	if not _require(saved["files"].size() >= 1 and int(saved["totalBytes"]) > 0, "Terrain3D G00 rock/snow region resource was not persisted"):
		return

	print("G00_TERRAIN3D_ROCK_SNOW_METRICS=" + JSON.stringify({
	...
	print("NW_G00_TERRAIN3D_ROCK_SNOW_VALIDATION_OK")
	quit(0)
