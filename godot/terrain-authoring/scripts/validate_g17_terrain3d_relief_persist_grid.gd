...
func _run()->void:
	...
	source.data.import_images([image_from(p),null,null],Vector3.ZERO,0.0,1.0); DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(directory)); source.data.save_directory(directory)
