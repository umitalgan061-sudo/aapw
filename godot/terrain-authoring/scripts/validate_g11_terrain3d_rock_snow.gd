...
const PROBE_PATH := "res://.terrain3d-proof/g11-rock-snow-probe.json"
const EXPECTED_POLICY := "buzul-muhafizi-g11-terrain3d-rock-snow-2026-08-12-v1"
...
func _fail(message: String) -> void:
	push_error("G11 Terrain3D rock/snow proof failed: " + message)
	quit(1)
...
func _source_blend(probe: Dictionary, u: float, v: float) -> float:
	...
	var top := lerpf(float(a[2]), float(b[2]), tx)
	var bottom := lerpf(float(c[2]), float(d[2]), tx)
...
func _write_imported_preview(terrain: Terrain3D, probe: Dictionary) -> Error:
	...
	for z in region_size:
		for x in region_size:
			...
			if is_nan(blend):
				blend = 0.0
			var rock := Color(0.28, 0.31, 0.34, 1.0)
			var snow := Color(0.92, 0.95, 0.97, 1.0)
	...
	return image.save_png("res://.terrain3d-proof/g11-rock-snow-imported-topdown.png")
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G11 rock/snow probe JSON missing"):
		return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G11 rock/snow policy"):
		return
	...
	terrain.name = "G11Terrain3DRockSnowProof"
	...
	var suffix := OS.get_environment("G11_ROCK_SNOW_PROOF_SUFFIX")
	if suffix.is_empty():
		suffix = "default"
	var output_dir := "user://g11-terrain3d-rock-snow-proof-" + suffix
	...
	print("G11_TERRAIN3D_ROCK_SNOW_METRICS=" + JSON.stringify(metrics))
	print("NW_G11_TERRAIN3D_ROCK_SNOW_VALIDATION_OK")
	quit(0)
