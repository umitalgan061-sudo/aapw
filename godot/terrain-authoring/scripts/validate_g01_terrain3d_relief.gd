...
const PROBE_PATH := "res://.terrain3d-proof/g01-relief-probe.json"
const PREVIEW_PATH := "res://.terrain3d-proof/g01-relief-imported-topdown.png"
const EXPECTED_POLICY := "buzul-muhafizi-g01-terrain3d-relief-2026-08-13-v1"
...
func _fail(message: String) -> void:
	push_error("G01 Terrain3D relief proof failed: " + message)
	quit(1)
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G01 relief probe JSON missing"):
		return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G01 relief policy"):
		return
	...
	if not _require(float(probe["worldWidthMeters"]) > 13000.0 and float(probe["worldDepthMeters"]) > 10000.0, "physical normal scale is not full-reference extent"):
		return
	if not _require(int(probe["canonicalWaterCells"]) == 88 and int(probe["canonicalLandCells"]) == 8, "G01 hydrology fingerprint changed"):
		return
	if not _require(int(probe["canonicalSignMismatches"]) == 0, "relief moved canonical G01 water/land semantics"):
		return
	if not _require(float(probe["maxG00SharedSeamHeightDelta"]) <= 0.000001 and float(probe["maxG00SharedSeamNormalDelta"]) <= 0.000001, "G00/G01 shared relief seam lost parity"):
		return

	var terrain := Terrain3D.new()
	terrain.name = "G01Terrain3DReliefProof"
	...
	for z in range(0, 256, 16):
		for x in range(0, 256, 16):
			...
			if is_nan(actual):
				_fail("Terrain3D returned NaN for imported G01 relief")
				return
	...
	if not _require(max_error <= MAX_HEIGHT_ERROR, "Terrain3D height roundtrip exceeded tolerance"):
		return
	if not _require(min_imported < 0.0 and max_imported > 3.0, "Terrain3D relief lost G01 coast/land height character"):
		return
	...
	var suffix := OS.get_environment("G01_RELIEF_PROOF_SUFFIX")
	if suffix.is_empty(): suffix = "default"
	var output_dir := "user://g01-terrain3d-relief-proof-" + suffix
	...
	print("G01_TERRAIN3D_RELIEF_METRICS=" + JSON.stringify(metrics))
	print("NW_G01_TERRAIN3D_RELIEF_VALIDATION_OK")
	quit(0)
