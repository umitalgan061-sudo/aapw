...
func _source_value(probe: Dictionary, channel: int, u: float, v: float) -> float:
	...
	return lerpf(
		...
		lerpf(float(rows[y1][x0][channel]), float(rows[y1][x1][channel]), tx), ty)
...
func _control_image(probe: Dictionary) -> Image:
	...
	for z in IMPORT_SIZE:
		for x in IMPORT_SIZE:
			var blend := int(round(clampf(_source_value(probe, 1, float(x) / 256.0, float(z) / 256.0), 0.0, 1.0) * 255.0))
...
func _audit_grid(terrain: Terrain3D, probe: Dictionary) -> Dictionary:
	var max_height := 0.0; var max_blend := 0.0; var max_color := 0.0; var max_rough := 0.0
	var forbidden_overlay_samples := 0; var checksum: int = 2166136261; var samples := 0
	for z in IMPORT_SIZE:
		for x in IMPORT_SIZE:
			var u := float(x) / 256.0; var v := float(z) / 256.0; var pos := Vector3(float(x), 0.0, float(z))
			var height := terrain.data.get_height(pos); var blend := terrain.data.get_control_blend(pos)
			var color := terrain.data.get_color(pos); var rough := terrain.data.get_roughness(pos)
			if is_nan(height) or is_nan(blend) or is_nan(color.r) or is_nan(rough): return {"ok": false}
...
func _write_preview(terrain: Terrain3D) -> Error:
	...
	for z in IMPORT_SIZE:
		for x in IMPORT_SIZE:
			var pos := Vector3(float(x), 0.0, float(z)); var c := terrain.data.get_color(pos)
...
func _run() -> void:
	...
	if not _require(parsed is Dictionary, "probe invalid"): return
	var probe: Dictionary = parsed
	if not _require(String(probe.get("schema", "")) == EXPECTED_SCHEMA and String(probe.get("policyId", "")) == EXPECTED_POLICY and String(probe.get("sourceMapSha256", "")) == EXPECTED_MAP_SHA, "probe provenance drifted"): return
	if not _require(String(probe.get("geoCell", "")) == "G71" and String(probe.get("layer", "")) == "Near Detail", "cell/layer drifted"): return
	if not _require(int(probe.get("sourceGridSize", 0)) == SOURCE_SIZE and int(probe.get("terrain3dImportSize", 0)) == IMPORT_SIZE and int(probe.get("terrain3dRegionSize", 0)) == REGION_SIZE, "dimensions drifted"): return
	if not _require(int(probe.get("canonicalWaterCells", 0)) == 96 and int(probe.get("canonicalLandCells", 1)) == 0, "G71 must remain open sea"): return
	if not _require(float(probe.get("maxHeightDelta", 1)) == 0.0 and float(probe.get("maxControlDelta", 1)) == 0.0 and float(probe.get("maxRoadPathDelta", 1)) == 0.0 and float(probe.get("maxFoliageDensity", 1)) == 0.0, "Near Detail changed prior layers"): return

	var terrain := Terrain3D.new(); terrain.name = "G71Terrain3DNearDetailProof"; get_root().add_child(terrain); terrain.region_size = REGION_SIZE
	...
	if not _require(terrain.data.get_region_count() >= 4, "257 import did not create >=4 regions"): return
	...
	var suffix := OS.get_environment("G71_NEAR_DETAIL_PROOF_SUFFIX"); if suffix.is_empty(): suffix = "default"
	...
	if not _require(saved["files"].size() >= 4 and int(saved["bytes"]) > 0, "persistence failed"): return
	...
	if not _require(file != null, "metrics write failed"): return
