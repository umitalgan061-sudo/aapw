...
const PROBE_PATH := "res://.terrain3d-proof/g52-biome-probe.json"
const EXPECTED_POLICY := "safak-kartali-g52-terrain3d-biome-2026-08-12-v1"
...
func _fail(message: String) -> void:
	push_error("G52 Terrain3D biome proof failed: " + message)
	quit(1)
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G52 biome probe JSON missing"):
		return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G52 biome policy"):
		return
	...
	if not _require(int(canonical["waterCells"]) == 84 and int(canonical["landCells"]) == 12 and int(canonical["boundaryEdges"]) == 14, "G52 canonical coastline fingerprint drifted"):
		return

	var terrain := Terrain3D.new()
	terrain.name = "G52Terrain3DBiomeProof"
	...
	if not _require(terrain.data.get_region_count() >= 1, "Terrain3D biome import produced no real region"):
		return
	...
	for z in sample_positions:
		for x in sample_positions:
			...
			sample_count += 1
	if not _require(max_error <= MAX_COLOR_ERROR, "Terrain3D color-map roundtrip exceeded tolerance"):
		return

	var baked_mesh: Mesh = terrain.bake_mesh(0)
	if not _require(baked_mesh != null and baked_mesh.get_surface_count() > 0, "Terrain3D LOD0 biome bake returned no mesh"):
		return
	var arrays := baked_mesh.surface_get_arrays(0)
	var vertices: PackedVector3Array = arrays[Mesh.ARRAY_VERTEX]
	if not _require(vertices.size() > 0, "Terrain3D LOD0 biome bake returned no vertices"):
		return

	var suffix := OS.get_environment("G52_BIOME_PROOF_SUFFIX")
	if suffix.is_empty():
		suffix = "default"
	var output_dir := "user://g52-terrain3d-biome-proof-" + suffix
	var absolute_dir := ProjectSettings.globalize_path(output_dir)
	DirAccess.make_dir_recursive_absolute(absolute_dir)
	terrain.data.save_directory(output_dir)
	var saved := _saved_region_evidence(output_dir)
	if not _require(saved["files"].size() >= 1 and int(saved["totalBytes"]) > 0, "Terrain3D biome region resource was not persisted"):
		return
	...
	print("G52_TERRAIN3D_BIOME_METRICS=" + JSON.stringify(metrics))
	print("NE_G52_TERRAIN3D_BIOME_VALIDATION_OK")
	quit(0)
