...
const PROBE_PATH := "res://.terrain3d-proof/g70-near-detail-probe.json"
const PREVIEW_PATH := "res://.terrain3d-proof/g70-near-detail-imported-topdown.png"
const EXPECTED_POLICY := "safak-kartali-g70-terrain3d-near-detail-2026-08-14-v1"
...
func _fail(message: String) -> void:
	push_error("G70 Terrain3D Near Detail proof failed: " + message)
	quit(1)
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "Near Detail probe JSON missing"): return
	...
	if not _require(int(probe["canonicalWaterCells"]) == 96 and int(probe["canonicalLandCells"]) == 0, "G70 must remain 96/96 canonical sea"): return
	...
	if not _require(float(probe["maxFoliageDensity"]) == 0.0, "Near Detail invented marine foliage"): return
	...
	terrain.name = "G70Terrain3DNearDetailProof"
	...
	if not _require(terrain.data.get_region_count() >= 4, "257x257 import must span at least four regions"): return

	var max_height_error := 0.0; var max_blend_error := 0.0
	...
	for seam in [255.0, 256.0]:
		for cross in [64.0, 128.0, 192.0, 255.0, 256.0]:
			for pos in [Vector3(seam, 0.0, cross), Vector3(cross, 0.0, seam)]:
				var u: float = float(pos.x) / 256.0; var v: float = float(pos.z) / 256.0
				var expected_color := _source_color(probe, u, v)
				var actual_color := terrain.data.get_color(pos)
				...
				seam_samples += 1
	if not _require(max_seam_height_error <= MAX_SEAM_HEIGHT_ERROR, "255/256 height seam exceeded tolerance"): return
	if not _require(max_seam_material_error <= MAX_SEAM_MATERIAL_ERROR, "255/256 material seam exceeded tolerance"): return
	...
	var suffix := OS.get_environment("G70_NEAR_DETAIL_PROOF_SUFFIX")
	if suffix.is_empty(): suffix = "default"
	var output_dir := "user://g70-terrain3d-near-detail-proof-" + suffix
	...
	print("G70_TERRAIN3D_NEAR_DETAIL_METRICS=" + JSON.stringify({
	...
	print("NE_G70_TERRAIN3D_NEAR_DETAIL_VALIDATION_OK")
	quit(0)
