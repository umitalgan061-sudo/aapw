...
const PROBE_PATH := "res://.terrain3d-proof/g07-biome-probe.json"
const EXPECTED_POLICY := "gunbatimi-ustasi-g07-terrain3d-marine-biome-2026-08-12-v1"
...
func _fail(message: String) -> void:
	push_error("G07 Terrain3D marine biome proof failed: " + message)
	quit(1)
...
func _build_height_image(region_size: int) -> Image:
	...
	image.fill(Color(-6.0, 0.0, 0.0, 1.0))
	return image
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G07 biome probe JSON missing"):
		return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G07 biome policy"):
		return
	...
	terrain.name = "G07Terrain3DMarineBiomeProof"
	...
	if not _require(terrain.data.get_region_count() >= 1, "Terrain3D marine biome import produced no real region"):
		return
	...
	if not _require(baked_mesh != null and baked_mesh.get_surface_count() > 0, "Terrain3D LOD0 marine biome bake returned no mesh"):
		return
	var arrays := baked_mesh.surface_get_arrays(0)
	var vertices: PackedVector3Array = arrays[Mesh.ARRAY_VERTEX]
	if not _require(vertices.size() > 0, "Terrain3D LOD0 marine biome bake returned no vertices"):
		return

	var suffix := OS.get_environment("G07_BIOME_PROOF_SUFFIX")
	if suffix.is_empty():
		suffix = "default"
	var output_dir := "user://g07-terrain3d-biome-proof-" + suffix
	var absolute_dir := ProjectSettings.globalize_path(output_dir)
	DirAccess.make_dir_recursive_absolute(absolute_dir)
	terrain.data.save_directory(output_dir)
	var saved := _saved_region_evidence(output_dir)
	if not _require(saved["files"].size() >= 1 and int(saved["totalBytes"]) > 0, "Terrain3D marine biome region resource was not persisted"):
		return
	...
	print("G07_TERRAIN3D_BIOME_METRICS=" + JSON.stringify(metrics))
	print("SW_G07_TERRAIN3D_BIOME_VALIDATION_OK")
	quit(0)
