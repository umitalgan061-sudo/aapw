...
func _run_validation() -> void:
	...
	for value in mask:
		if value == 255:
			...
			return
	if source_land_pixels != EXPECTED_SOURCE_LAND_PIXELS or source_water_pixels != EXPECTED_SOURCE_WATER_PIXELS:
		_fail("G27 source semantic counts drifted: land=%s water=%s" % [source_land_pixels, source_water_pixels])
		return
	...
	if height_image == null:
		...
		return
	if height_image.get_size() != Vector2i(RESOLUTION, RESOLUTION) or height_image.get_format() != Image.FORMAT_RF:
		_fail("Persisted G27 HTerrain height format/resolution drifted")
		return
	...
	if land_vertices <= 0 or water_vertices <= 0 or transition_vertices <= 0:
		...
		return
	if absf(minimum_height - WATER_HEIGHT) > MAX_PERSISTENCE_ERROR or absf(maximum_height - LAND_HEIGHT) > MAX_PERSISTENCE_ERROR:
		_fail("Persisted G27 height bounds drifted")
		return
	print("SW_G27_HTERRAIN_VALIDATION_OK source_land=%s source_water=%s land_vertices=%s water_vertices=%s transition_vertices=%s max_persistence_error=%.9f min_height=%.6f max_height=%.6f" % [source_land_pixels, source_water_pixels, land_vertices, water_vertices, transition_vertices, max_persistence_error, minimum_height, maximum_height])
	quit(0)
