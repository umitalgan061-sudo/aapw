...
func _saved_region_count(directory: String) -> int:
	...
	while name != "":
		if not dir.current_is_dir() and FileAccess.get_file_as_bytes(directory.path_join(name)).size() > 0:
			count += 1
		name = dir.get_next()
	dir.list_dir_end()
	return count
...
func _run() -> void:
	...
	for sy in SOURCE_SIZE:
		for sx in SOURCE_SIZE:
			...
			if not _require(not is_nan(actual_height) and actual_height <= -2.5, "imported seafloor escaped canonical water depth"): return
	...
	var max_region_material_delta := 0.0
	for p in [Vector3(255.0, 0.0, 64.0), Vector3(64.0, 0.0, 255.0)]:
		var q := p + (Vector3(1.0, 0.0, 0.0) if p.x == 255.0 else Vector3(0.0, 0.0, 1.0))
		max_region_height_delta = maxf(max_region_height_delta, absf(terrain.data.get_height(p) - terrain.data.get_height(q)))
		var pc := terrain.data.get_color(p)
		var qc := terrain.data.get_color(q)
		max_region_material_delta = maxf(max_region_material_delta, maxf(absf(pc.r - qc.r), maxf(absf(pc.g - qc.g), absf(pc.b - qc.b))))
		max_region_material_delta = maxf(max_region_material_delta, absf(terrain.data.get_roughness(p) - terrain.data.get_roughness(q)))
	if not _require(max_region_height_delta <= HEIGHT_TOLERANCE, "255/256 height seam detected"): return
	if not _require(max_region_material_delta <= MATERIAL_TOLERANCE, "255/256 material seam detected"): return
	...
	if not _require(_saved_region_count(save_dir) >= 4, "Terrain3D persistence did not save >=4 region resources"): return
	...
	for z in IMPORT_SIZE:
		for x in IMPORT_SIZE:
			...
			preview.set_pixel(x, z, Color(c.r, c.g, c.b, terrain.data.get_roughness(pos)))
	if not _require(preview.save_png(PREVIEW_PATH) == OK, "failed to save top-down proof"): return
	...
	var metrics := {
		...
		"maxRegionMaterialDelta": max_region_material_delta,
	}
