...
const PROBE_PATH := "res://.terrain3d-proof/g75-rock-snow-probe.json"
const EXPECTED_POLICY := "kizil-ufuk-g75-terrain3d-rock-snow-2026-08-13-v1"
...
func _fail(message: String) -> void:
	push_error("G75 Terrain3D rock/snow proof failed: " + message)
	quit(1)
...
func _source_blend(probe: Dictionary, u: float, v: float) -> float:
	...
	var top := lerpf(float(a[3]), float(b[3]), tx)
	var bottom := lerpf(float(c[3]), float(d[3]), tx)
...
func _source_height(probe: Dictionary, u: float, v: float) -> float:
	...
	var c: Array = rows[y1][x0]
	var d: Array = rows[y1][x1]
	var top := lerpf(float(a[4]), float(b[4]), tx)
	var bottom := lerpf(float(c[4]), float(d[4]), tx)
...
func _write_imported_preview(terrain: Terrain3D, probe: Dictionary) -> Error:
	...
	return image.save_png("res://.terrain3d-proof/g75-rock-snow-imported-topdown.png")
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G75 rock/snow probe JSON missing"): return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G75 rock/snow policy"): return
	...
	terrain.name = "G75Terrain3DRockSnowProof"
	...
	var suffix := OS.get_environment("G75_ROCK_SNOW_PROOF_SUFFIX")
	if suffix.is_empty(): suffix = "default"
	var output_dir := "user://g75-terrain3d-rock-snow-proof-" + suffix
	...
	print("G75_TERRAIN3D_ROCK_SNOW_METRICS=" + JSON.stringify(metrics))
	print("SE_G75_TERRAIN3D_ROCK_SNOW_VALIDATION_OK")
	quit(0)
