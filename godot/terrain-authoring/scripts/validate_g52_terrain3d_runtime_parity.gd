...
const SOURCE_PATH := "res://.terrain3d-proof/g52-runtime-source.json"
const BAKE_PATH := "res://.terrain3d-proof/g52-runtime-bake.json"
const PREVIEW_PATH := "res://.terrain3d-proof/g52-runtime-bake-topdown.png"
const EXPECTED_SCHEMA := "westeros-g52-terrain3d-source-v1"
...
func _fail(message: String) -> void:
	push_error("G52 Terrain3D runtime parity proof failed: " + message)
	quit(1)
...
func _run() -> void:
	if not _require(FileAccess.file_exists(SOURCE_PATH), "G52 runtime source JSON missing"): return
	...
	terrain.name = "G52Terrain3DRuntimeParityProof"
	...
	if not _require(max_snow_error <= UNIT_TOLERANCE, "aligned Terrain3D snow-weight parity exceeded tolerance"): return
	...
	var suffix := OS.get_environment("G52_RUNTIME_PARITY_PROOF_SUFFIX")
	if suffix.is_empty(): suffix = "default"
	var save_dir := "user://g52-terrain3d-runtime-parity-" + suffix
	...
	var bake := {"schema":"westeros-g52-terrain3d-bake-v1","policyId":"safak-kartali-g52-terrain3d-threejs-runtime-parity-2026-08-13-v1","sourceMapSha256":EXPECTED_MAP_SHA,"terrain3dVersion":String(terrain.version),"lod":0,"width":SOURCE_SIZE,"height":SOURCE_SIZE,"regionCount":region_count,"savedRegionFiles":saved["files"].size(),"savedRegionBytes":int(saved["totalBytes"]),"bakedSurfaces":baked_mesh.get_surface_count(),"bakedVertices":baked_vertices.size(),"maxHeightError":snappedf(max_height_error,0.00000001),"maxSnowError":snappedf(max_snow_error,0.00000001),"maxColorError":snappedf(max_color_error,0.00000001),"maxRoughnessError":snappedf(max_roughness_error,0.00000001),"boundaryProbeMaxHeightError":snappedf(boundary_probe_max_height_error,0.00000001),"bakeChecksum":checksum,"heights":heights,"snowWeight":snow_weight,"tintR":tint_r,"tintG":tint_g,"tintB":tint_b,"roughness":roughness_values}
	...
	print("G52_TERRAIN3D_RUNTIME_BAKE_METRICS=" + JSON.stringify({"regionCount":region_count,"savedRegionFiles":saved["files"].size(),"bakedVertices":baked_vertices.size(),"maxHeightError":bake["maxHeightError"],"maxSnowError":bake["maxSnowError"],"maxColorError":bake["maxColorError"],"maxRoughnessError":bake["maxRoughnessError"],"boundaryProbeMaxHeightError":bake["boundaryProbeMaxHeightError"],"bakeChecksum":checksum}))
	print("NE_G52_TERRAIN3D_RUNTIME_BAKE_OK")
	quit(0)
