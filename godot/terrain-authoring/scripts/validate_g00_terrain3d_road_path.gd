...
const PROBE_PATH := "res://.terrain3d-proof/g00-road-path-probe.json"
const PREVIEW_PATH := "res://.terrain3d-proof/g00-road-path-imported-topdown.png"
const EXPECTED_POLICY := "buzul-muhafizi-g00-terrain3d-road-path-2026-08-13-v1"
...
func _fail(message: String) -> void:
	push_error("G00 Terrain3D road/path proof failed: " + message)
	quit(1)
...
func _build_height_image(probe: Dictionary) -> Image:
	...
	for z in region_size:
		...
		for x in region_size:
			...
			image.set_pixel(x, z, Color(_source_component(probe, u, v, 3), 0.0, 0.0, 1.0))
	return image
...
func _surface_color(texture_id: int) -> Color:
	match texture_id:
		0: return Color(0.25, 0.28, 0.31, 1.0)
		1: return Color(0.91, 0.95, 0.97, 1.0)
		2: return Color(0.61, 0.48, 0.29, 1.0)
		3: return Color(0.48, 0.39, 0.25, 1.0)
...
func _write_imported_preview(terrain: Terrain3D, probe: Dictionary) -> Error:
	...
	for z in region_size:
		for x in region_size:
			...
			if is_nan(height) or is_nan(blend): return ERR_INVALID_DATA
			if height < 0.0:
				image.set_pixel(x, z, sea)
				continue
			var base_color := _surface_color(terrain.data.get_control_base_id(pos))
			var overlay_color := _surface_color(terrain.data.get_control_overlay_id(pos))
			image.set_pixel(x, z, base_color.lerp(overlay_color, clampf(blend, 0.0, 1.0)))
...
func _run() -> void:
	if not _require(FileAccess.file_exists(PROBE_PATH), "G00 road/path probe JSON missing"): return
	...
	if not _require(String(probe["policyId"]) == EXPECTED_POLICY, "unexpected G00 road/path policy"): return
	...
	terrain.name = "G00Terrain3DRoadPathProof"
	...
	if not _require(terrain.data.get_region_count() == 1, "Terrain3D road/path import must produce exactly one region"): return

	var positions: Array[int] = []
	for coordinate in range(0, 256, 8): positions.push_back(coordinate)
	...
	for z in positions:
		for x in positions:
			...
			var expected := _expected_control(probe, u, v)
			var expected_height := _source_component(probe, u, v, 3)
			var pos := Vector3(float(x), 0.0, float(z))
			...
			if not _require(actual_base == int(expected["base"]), "Terrain3D road/path base texture ID changed"): return
			if not _require(actual_overlay == int(expected["overlay"]), "Terrain3D road/path overlay texture ID changed"): return
			if not _require(not is_nan(actual_blend) and not is_nan(actual_height), "Terrain3D road/path control/height returned NaN"): return
			...
			if bool(expected["isRoadPath"]) and actual_blend > 0.02: active_roundtrip_samples += 1
			checksum = int((checksum ^ int(round(clampf(actual_blend, 0.0, 1.0) * 255.0))) * 16777619) & 0xffffffff
			sample_count += 1
	if not _require(max_blend_error <= MAX_BLEND_ERROR, "Terrain3D road/path control-map roundtrip exceeded tolerance"): return
	if not _require(max_height_error <= MAX_HEIGHT_ERROR, "Terrain3D qualified G00 relief changed during road/path import"): return
	...
	if not _require(_write_imported_preview(terrain, probe) == OK, "failed to write imported G00 road/path preview"): return
	...
	var suffix := OS.get_environment("G00_ROAD_PATH_PROOF_SUFFIX")
	if suffix.is_empty(): suffix = "default"
	var output_dir := "user://g00-terrain3d-road-path-proof-" + suffix
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(output_dir))
	terrain.data.save_directory(output_dir)
	var saved := _saved_region_evidence(output_dir)
	if not _require(saved["files"].size() >= 1 and int(saved["totalBytes"]) > 0, "Terrain3D G00 road/path region resource was not persisted"): return

	print("G00_TERRAIN3D_ROAD_PATH_METRICS=" + JSON.stringify({
	...
	print("NW_G00_TERRAIN3D_ROAD_PATH_VALIDATION_OK")
	quit(0)
