...
function distanceToBoneMountainsPixels(normalizedX, normalizedY) {
  const px = clamp(normalizedX) * WORLD_REFERENCE_MAP.pixelWidth;
  const py = clamp(normalizedY) * WORLD_REFERENCE_MAP.pixelHeight;
...
function sampleRelief(normalizedX, normalizedY) {
  ...
  const height = sea + (dry - sea) * smoothstep(0, 1, land);
  return { water, land, bone, ridge, height };
}

function sampleSlope(normalizedX, normalizedY) {
  const dx = 4 / WORLD_REFERENCE_MAP.pixelWidth;
  const dy = 4 / WORLD_REFERENCE_MAP.pixelHeight;
  const left = sampleRelief(clamp(normalizedX - dx), normalizedY).height;
  const right = sampleRelief(clamp(normalizedX + dx), normalizedY).height;
  const up = sampleRelief(normalizedX, clamp(normalizedY - dy)).height;
  const down = sampleRelief(normalizedX, clamp(normalizedY + dy)).height;
  const gx = (right - left) / 8;
  const gy = (down - up) / 8;
  return clamp(Math.hypot(gx, gy) / 8);
}
...
export function sampleG52RockSnow(normalizedX, normalizedY) {
  ...
  const landFactor = 1 - smoothstep(0.38, 0.62, relief.water);
  const slope = sampleSlope(normalizedX, normalizedY);
  ...
  return Object.freeze({
    waterConfidence: relief.water,
    landFactor,
    heightMeters: relief.height,
    ridgeInfluence: relief.ridge,
    boneInfluence: relief.bone,
    slope,
    rockWeight,
    snowWeight,
    snowBlend,
...
function canonicalHydrologyFingerprint() {
  const bounds = G52_ROCK_SNOW_POLICY.maskBounds;
  let waterCells = 0;
  let landCells = 0;
  let boundaryEdges = 0;
...
export function measureG52RockSnow() {
  const { xMin, xMax, yMin, yMax } = G52_ROCK_SNOW_POLICY.normalizedBounds;
  const size = G52_ROCK_SNOW_POLICY.sourceGridSize;
  const rows = [];
  ...
  for (let y = 0; y < size; y += 1) {
    ...
    for (let x = 0; x < size; x += 1) {
      ...
      const sample = sampleG52RockSnow(nx, ny);
      if (sample.snowBlend > 0 && sample.snowBlend < 1) fractionalBlendSamples += 1;
      if (sample.snowBlend >= 0.5) snowDominantSamples += 1; else rockDominantSamples += 1;
  ...
  for (let i = 0; i < size; i += 1) {
    ...
    maxGuardBandBlendDelta = Math.max(
      ...
      Math.abs(sampleG52RockSnow(nx, yMin).snowBlend - sampleG52RockSnow(nx, yMin - guard).snowBlend),
      Math.abs(sampleG52RockSnow(nx, yMax).snowBlend - sampleG52RockSnow(nx, yMax + guard).snowBlend),
      Math.abs(sampleG52RockSnow(xMin, ny).snowBlend - sampleG52RockSnow(xMin - guard, ny).snowBlend),
      Math.abs(sampleG52RockSnow(xMax, ny).snowBlend - sampleG52RockSnow(xMax + guard, ny).snowBlend),
    );
  }
  ...
  for (let y = mask.yMin; y <= mask.yMax; y += 1) {
    for (let x = mask.xMin; x <= mask.xMax; x += 1) {
      if (waterCell(x, y)) {
        ...
        maxWaterSnowLeak = Math.max(maxWaterSnowLeak, sampleG52RockSnow(nx, ny).snowWeight);
      }
    }
  }
  return Object.freeze({
    policyId: G52_ROCK_SNOW_POLICY.id,
    sourceMapSha256: G52_ROCK_SNOW_POLICY.sourceMapSha256,
    geoCell: 'G52',
    layer: 'Rock/Snow',
    sourceGridSize: size,
    sourceSamples: size * size,
    terrain3dRegionSize: G52_ROCK_SNOW_POLICY.terrain3dRegionSize,
...
export function buildG52RockSnowProbe() {
  ...
  return Object.freeze({
    ...
    sourceGridSize: size,
    terrain3dRegionSize: G52_ROCK_SNOW_POLICY.terrain3dRegionSize,
    rockTextureId: G52_ROCK_SNOW_POLICY.rockTextureId,
    snowTextureId: G52_ROCK_SNOW_POLICY.snowTextureId,
    rows,
  });
}
