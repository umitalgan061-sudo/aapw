...
export const G52_ROCK_SNOW_POLICY = Object.freeze({
  id: 'safak-kartali-g52-terrain3d-rock-snow-2026-08-13-v2',
...
export function measureG52RockSnow() {
  ...
  let maxAdjacentSurfaceWeightStep = 0, maxGuardBandSurfaceWeightDelta = 0, maxWaterSnowLeak = 0, fractionalBlendSamples = 0, snowDominantSamples = 0, rockDominantSamples = 0, checksum = 2166136261;
  for (let y = 0; y < size; y += 1) { const row = [], ny = yMin + (yMax - yMin) * (y / (size - 1)); for (let x = 0; x < size; x += 1) {
    ...
    if (sample.snowWeight > 0 && sample.snowWeight < 1) fractionalBlendSamples += 1; if (sample.snowBlend >= 0.5) snowDominantSamples += 1; else rockDominantSamples += 1;
  ...
  for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) { const snowWeight = rows[y][x].snowWeight; if (x + 1 < size) maxAdjacentSurfaceWeightStep = Math.max(maxAdjacentSurfaceWeightStep, Math.abs(snowWeight - rows[y][x + 1].snowWeight)); if (y + 1 < size) maxAdjacentSurfaceWeightStep = Math.max(maxAdjacentSurfaceWeightStep, Math.abs(snowWeight - rows[y + 1][x].snowWeight)); }
  const guard = G52_ROCK_SNOW_POLICY.guardBandNormalized;
  for (let i = 0; i < size; i += 1) { const t = i / (size - 1), nx = xMin + (xMax - xMin) * t, ny = yMin + (yMax - yMin) * t; maxGuardBandSurfaceWeightDelta = Math.max(maxGuardBandSurfaceWeightDelta, Math.abs(sampleG52RockSnow(nx, yMin).snowWeight - sampleG52RockSnow(nx, yMin - guard).snowWeight), Math.abs(sampleG52RockSnow(nx, yMax).snowWeight - sampleG52RockSnow(nx, yMax + guard).snowWeight), Math.abs(sampleG52RockSnow(xMin, ny).snowWeight - sampleG52RockSnow(xMin - guard, ny).snowWeight), Math.abs(sampleG52RockSnow(xMax, ny).snowWeight - sampleG52RockSnow(xMax + guard, ny).snowWeight)); }
