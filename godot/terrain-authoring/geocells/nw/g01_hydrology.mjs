/**
 * Buzul Muhafızı / NW GeoCell G01 coast-hydrology refinement.
 *
 * G01 owns canonical source pixels x=0..192, y=128..256, normalized
 * [0, 0.125] x [0.125, 0.25]. The immutable 96x64 owner-map surface mask
...
export const G01_HYDROLOGY_POLICY = Object.freeze({
  id: 'buzul-muhafizi-g01-hydrology-2026-08-12-v1',
...
function sampleWaterAtMaskCell(cellX, cellY) {
  const x = clamp(cellX, 0, G01_HYDROLOGY_POLICY.baseMaskWidth - 1);
  const y = clamp(cellY, 0, G01_HYDROLOGY_POLICY.baseMaskHeight - 1);
  const nx = (x + 0.5) / G01_HYDROLOGY_POLICY.baseMaskWidth;
  const ny = (y + 0.5) / G01_HYDROLOGY_POLICY.baseMaskHeight;
  return isWater(classifyReferenceBaseSurface(nx, ny));
}

export function isInsideG01(normalizedX, normalizedY) {
  const { xMin, xMax, yMin, yMax } = G01_HYDROLOGY_POLICY.normalizedBounds;
  return normalizedX >= xMin && normalizedX <= xMax && normalizedY >= yMin && normalizedY <= yMax;
}

export function sampleG01WaterConfidence(normalizedX, normalizedY) {
  if (!Number.isFinite(normalizedX) || !Number.isFinite(normalizedY)) {
    throw new TypeError('normalized coordinates must be finite');
  }
  if (!isInsideG01(normalizedX, normalizedY)) return null;

  const gridX = normalizedX * G01_HYDROLOGY_POLICY.baseMaskWidth - 0.5;
  const gridY = normalizedY * G01_HYDROLOGY_POLICY.baseMaskHeight - 0.5;
...
export function measureG01Hydrology() {
  const { xMin, xMax, yMin, yMax } = G01_HYDROLOGY_POLICY.maskBounds;
  let waterCells = 0;
  let landCells = 0;
  let boundaryEdges = 0;
  ...
  for (let maskY = yMin; maskY <= yMax; maskY += 1) {
    for (let maskX = xMin; maskX <= xMax; maskX += 1) {
      ...
      const nx = (maskX + 0.5) / G01_HYDROLOGY_POLICY.baseMaskWidth;
      const ny = (maskY + 0.5) / G01_HYDROLOGY_POLICY.baseMaskHeight;
      if (sampleG01WaterConfidence(nx, ny) !== water) centreMismatches += 1;
    }
  }
  ...
  const bounds = G01_HYDROLOGY_POLICY.normalizedBounds;
  ...
  for (let sy = 0; sy <= samplesY; sy += 1) {
    ...
    for (let sx = 0; sx <= samplesX; sx += 1) {
      ...
      const confidence = sampleG01WaterConfidence(nx, ny);
  ...
  return Object.freeze({
    policyId: G01_HYDROLOGY_POLICY.id,
    geoCell: 'G01',
