/**
 * Günbatımı Ustası / SW GeoCell G15 coast-hydrology measurement.
 *
 * G15 owns source pixels x=192..384, y=640..768, normalized
 * [0.125, 0.25] x [0.625, 0.75]. The immutable 96x64 owner-map surface mask
...
export const G15_HYDROLOGY_POLICY = Object.freeze({
  id: 'gunbatimi-ustasi-g15-hydrology-2026-08-12-v1',
  sourceMapSha256: '20702972e8f45f0fbdc4da5fa68e890a82e4e822e1d58e2f369d8bc5b9c571a1',
  geoCell: 'G15', gx: 1, gy: 5,
  pixelBounds: Object.freeze({ xMin: 192, xMax: 384, yMin: 640, yMax: 768 }),
  normalizedBounds: Object.freeze({ xMin: 0.125, xMax: 0.25, yMin: 0.625, yMax: 0.75 }),
  maskBounds: Object.freeze({ xMin: 12, xMax: 23, yMin: 40, yMax: 47 }),
...
function sampleWaterAtMaskCell(cellX, cellY) {
  const x = clamp(cellX, 0, G15_HYDROLOGY_POLICY.baseMaskWidth - 1);
  const y = clamp(cellY, 0, G15_HYDROLOGY_POLICY.baseMaskHeight - 1);
  return isWater(classifyReferenceBaseSurface(
    (x + 0.5) / G15_HYDROLOGY_POLICY.baseMaskWidth,
    (y + 0.5) / G15_HYDROLOGY_POLICY.baseMaskHeight,
  ));
}

export function isInsideG15(nx, ny) {
  const b = G15_HYDROLOGY_POLICY.normalizedBounds;
  return nx >= b.xMin && nx <= b.xMax && ny >= b.yMin && ny <= b.yMax;
}

export function sampleG15WaterConfidence(nx, ny) {
  if (!Number.isFinite(nx) || !Number.isFinite(ny)) throw new TypeError('normalized coordinates must be finite');
  if (!isInsideG15(nx, ny)) return null;
  const gx = nx * G15_HYDROLOGY_POLICY.baseMaskWidth - 0.5;
  const gy = ny * G15_HYDROLOGY_POLICY.baseMaskHeight - 0.5;
...
export function measureG15Hydrology() {
  const { xMin, xMax, yMin, yMax } = G15_HYDROLOGY_POLICY.maskBounds;
  ...
  for (let y = yMin; y <= yMax; y += 1) for (let x = xMin; x <= xMax; x += 1) {
    ...
    if (sampleG15WaterConfidence((x + 0.5) / 96, (y + 0.5) / 64) !== water) centreMismatches += 1;
  }
  ...
  for (let sy = 0; sy <= samplesY; sy += 1) {
    ...
    for (let sx = 0; sx <= samplesX; sx += 1) {
      ...
      const c = sampleG15WaterConfidence(nx, ny);
  ...
  return Object.freeze({ policyId: G15_HYDROLOGY_POLICY.id, geoCell: 'G15', baseCells: 96,
