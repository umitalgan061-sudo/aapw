/**
 * Günbatımı Ustası / SW GeoCell G27 coast-hydrology refinement.
 *
 * G27 owns canonical source pixels x=384..576, y=896..1024, normalized
 * [0.25, 0.375] x [0.875, 1.0]. The immutable 96x64 owner-map surface mask
...
export const G27_HYDROLOGY_POLICY = Object.freeze({
  id: 'gunbatimi-ustasi-g27-hydrology-2026-08-12-v1',
  ...
  maskBounds: Object.freeze({ xMin: 24, xMax: 35, yMin: 56, yMax: 63 }),
  baseMaskWidth: 96,
  baseMaskHeight: 64,
...
function sampleWaterAtMaskCell(cellX, cellY) {
  const x = clamp(cellX, 0, G27_HYDROLOGY_POLICY.baseMaskWidth - 1);
  const y = clamp(cellY, 0, G27_HYDROLOGY_POLICY.baseMaskHeight - 1);
  const nx = (x + 0.5) / G27_HYDROLOGY_POLICY.baseMaskWidth;
  const ny = (y + 0.5) / G27_HYDROLOGY_POLICY.baseMaskHeight;
  return isWater(classifyReferenceBaseSurface(nx, ny));
}

export function isInsideG27(normalizedX, normalizedY) {
  const { xMin, xMax, yMin, yMax } = G27_HYDROLOGY_POLICY.normalizedBounds;
 ...
 */
export function sampleG27WaterConfidence(normalizedX, normalizedY) {
  if (!Number.isFinite(normalizedX) || !Number.isFinite(normalizedY)) {
    throw new TypeError('normalized coordinates must be finite');
  }
  if (!isInsideG27(normalizedX, normalizedY)) return null;

  const gridX = normalizedX * G27_HYDROLOGY_POLICY.baseMaskWidth - 0.5;
  const gridY = normalizedY * G27_HYDROLOGY_POLICY.baseMaskHeight - 0.5;
...
/**
 * Deterministic G27 evidence. The canonical mask has a hard 0->1 step at land/
 * water borders; quarter-cell probes quantify the continuous refinement while
 * requiring every original mask-cell centre to remain semantically identical.
 */
export function measureG27Hydrology() {
  const { xMin, xMax, yMin, yMax } = G27_HYDROLOGY_POLICY.maskBounds;
  let waterCells = 0;
  let landCells = 0;
  let boundaryEdges = 0;
  ...
  for (let maskY = yMin; maskY <= yMax; maskY += 1) {
    for (let maskX = xMin; maskX <= xMax; maskX += 1) {
      ...
      const nx = (maskX + 0.5) / G27_HYDROLOGY_POLICY.baseMaskWidth;
      const ny = (maskY + 0.5) / G27_HYDROLOGY_POLICY.baseMaskHeight;
      if (sampleG27WaterConfidence(nx, ny) !== water) centreMismatches += 1;
    }
  }
  ...
  const bounds = G27_HYDROLOGY_POLICY.normalizedBounds;
  ...
  for (let sy = 0; sy <= samplesY; sy += 1) {
    ...
    for (let sx = 0; sx <= samplesX; sx += 1) {
      ...
      const confidence = sampleG27WaterConfidence(nx, ny);
  ...
  return Object.freeze({
    policyId: G27_HYDROLOGY_POLICY.id,
    geoCell: 'G27',
