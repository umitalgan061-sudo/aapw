/**
 * Günbatımı Ustası / SW GeoCell G25 coast-hydrology measurement.
 * Source pixels x=384..576, y=640..768. Authoring/QA only.
 */
import { classifyReferenceBaseSurface } from '../../../../src/3d/world/worldReferenceSurfacePindexes.js';

export const G25_HYDROLOGY_POLICY = Object.freeze({
  id: 'gunbatimi-ustasi-g25-hydrology-2026-08-12-v1',
  sourceMapSha256: '20702972e8f45f0fbdc4da5fa68e890a82e4e822e1d58e2f369d8bc5b9c571a1',
  geoCell: 'G25', gx: 2, gy: 5,
  pixelBounds: Object.freeze({ xMin: 384, xMax: 576, yMin: 640, yMax: 768 }),
  normalizedBounds: Object.freeze({ xMin: 0.25, xMax: 0.375, yMin: 0.625, yMax: 0.75 }),
  maskBounds: Object.freeze({ xMin: 24, xMax: 35, yMin: 40, yMax: 47 }),
...
function sampleWaterAtMaskCell(cellX, cellY) {
  const x = clamp(cellX, 0, G25_HYDROLOGY_POLICY.baseMaskWidth - 1);
  const y = clamp(cellY, 0, G25_HYDROLOGY_POLICY.baseMaskHeight - 1);
  return isWater(classifyReferenceBaseSurface((x + 0.5) / 96, (y + 0.5) / 64));
}

export function isInsideG25(nx, ny) {
  const b = G25_HYDROLOGY_POLICY.normalizedBounds;
  return nx >= b.xMin && nx <= b.xMax && ny >= b.yMin && ny <= b.yMax;
}

export function sampleG25WaterConfidence(nx, ny) {
  if (!Number.isFinite(nx) || !Number.isFinite(ny)) throw new TypeError('normalized coordinates must be finite');
  if (!isInsideG25(nx, ny)) return null;
  const gx = nx * 96 - 0.5, gy = ny * 64 - 0.5;
...
export function measureG25Hydrology() {
  const { xMin, xMax, yMin, yMax } = G25_HYDROLOGY_POLICY.maskBounds;
  ...
  for (let y = yMin; y <= yMax; y += 1) for (let x = xMin; x <= xMax; x += 1) {
    ...
    if (sampleG25WaterConfidence((x + 0.5) / 96, (y + 0.5) / 64) !== water) centreMismatches += 1;
  }
  ...
  for (let sy = 0; sy <= samplesY; sy += 1) {
    ...
    for (let sx = 0; sx <= samplesX; sx += 1) {
      ...
      const c = sampleG25WaterConfidence(nx, ny);
  ...
  return Object.freeze({ policyId: G25_HYDROLOGY_POLICY.id, geoCell: 'G25', baseCells: 96,
    waterCells, landCells, boundaryEdges, centreMismatches, refinedSamples: 1617,
    fractionalSamples, hardCellMaxStep: 1, maxAdjacentStep: Number(maxAdjacentStep.toFixed(8)), confidenceChecksum });
}
