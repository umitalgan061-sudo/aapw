...
export const G75_RELIEF_POLICY = Object.freeze({
  id: 'kizil-ufuk-g75-terrain3d-relief-2026-08-13-v2',
...
export function sampleG75ReliefNormal(nx, ny) {
  ...
  const dhDnx = (sampleG75ReliefHeight(nx + e, ny) - sampleG75ReliefHeight(nx - e, ny)) / (2 * e);
  const dhDny = (sampleG75ReliefHeight(nx, ny + e) - sampleG75ReliefHeight(nx, ny - e)) / (2 * e);
  // Height derivatives above are metres per normalized owner-map unit. Convert
  // them to metres-per-metre before constructing a physical surface normal.
  // Treating one normalized map unit as one metre made otherwise modest slopes
  // nearly horizontal and caused false/unstable seam normals at GeoCell edges.
  const slopeX = dhDnx / G75_RELIEF_POLICY.worldWidthMeters;
  const slopeZ = dhDny / G75_RELIEF_POLICY.worldDepthMeters;
  const len = Math.hypot(slopeX, 1, slopeZ);
  return Object.freeze({ x: -slopeX / len, y: 1 / len, z: -slopeZ / len });
}
...
export function buildG75ReliefProbe() {
  ...
  return Object.freeze({
    ...
    worldWidthMeters: Number(G75_RELIEF_POLICY.worldWidthMeters.toFixed(6)),
    worldDepthMeters: Number(G75_RELIEF_POLICY.worldDepthMeters.toFixed(6)),
    rows,
    canonicalWaterCells,
    canonicalLandCells,
