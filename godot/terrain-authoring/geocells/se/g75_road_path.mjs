 ...
 * before the G75 intersection test. The full-reference canonical road search
 * envelope is independently bounded from KINGDOM_SEATS + the shared A* corridor
 * contract. Both prove that G75 is road-free today, so this layer must preserve
 * merged Relief + Rock/Snow exactly rather than invent a phantom track.
 */
...
import {
  G75_ROCK_SNOW_POLICY,
  sampleG75RockSnow,
} from './g75_rock_snow.mjs';
import { G75_RELIEF_POLICY } from './g75_relief.mjs';

export const G75_ROAD_PATH_POLICY = Object.freeze({
  id: 'kizil-ufuk-g75-terrain3d-road-path-2026-08-13-v2',
...
export function canonicalRoadNetworkMaxNormalizedX() {
  const paddingMapUnits = G75_ROAD_PATH_POLICY.canonicalRouteCorridorPaddingMeters
    / FULL_REFERENCE_EXTENT_PLAN.metersPerMapUnit;
  return (G75_ROAD_PATH_POLICY.canonicalMaxSeatMapX + paddingMapUnits)
    / G75_ROAD_PATH_POLICY.ownerMapWidthUnits;
}

export function canonicalRoadExclusionMarginMeters() {
  return Math.max(0, G75_ROAD_PATH_POLICY.normalizedBounds.xMin - canonicalRoadNetworkMaxNormalizedX())
    * FULL_REFERENCE_EXTENT_PLAN.widthMeters;
}

export function sampleG75RoadPath(normalizedX, normalizedY) {
  const base = sampleG75RockSnow(normalizedX, normalizedY);
  return Object.freeze({
    ...base,
    roadCoverage: 0,
    pathCoverage: 0,
...
export function measureG75RoadPath(runtimeNetwork) {
  ...
  for (let y = 0; y < size; y += 1) {
    ...
    for (let x = 0; x < size; x += 1) {
      ...
      const base = sampleG75RockSnow(nx, ny);
  ...
  return Object.freeze({
    ...
    probeGridSize: G75_ROAD_PATH_POLICY.probeGridSize,
...
export function buildG75RoadPathProbe(runtimeNetwork) {
  const bounds = G75_ROAD_PATH_POLICY.normalizedBounds;
  const size = G75_ROAD_PATH_POLICY.probeGridSize;
  const rows = [];
  for (let y = 0; y < size; y += 1) {
    ...
    for (let x = 0; x < size; x += 1) {
      ...
      const s = sampleG75RoadPath(nx, ny);
      row.push([
