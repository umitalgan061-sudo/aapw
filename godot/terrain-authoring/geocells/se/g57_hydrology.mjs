/**
 * Kızıl Ufuk / SE GeoCell G57 coast-hydrology refinement.
 *
 * G57 owns canonical source pixels x=960..1152, y=896..1024, normalized
 * [0.625, 0.75] x [0.875, 1.0]. The immutable 96x64 owner-map surface mask
 * remains the semantic source of truth. This authoring/QA primitive derives a
 * continuous water-confidence field between mask-cell centres without moving
 * canonical land/water semantics.
 */

import { classifyReferenceBaseSurface } from '../../../../src/3d/world/worldReferenceSurfacePindexes.js';

export const G57_HYDROLOGY_POLICY = Object.freeze({
  id: 'kizil-ufuk-g57-hydrology-2026-08-12-v1',
  sourceMapSha256: '20702972e8f45f0fbdc4da5fa68e890a82e4e822e1d58e2f369d8bc5b9c571a1',
  geoCell: 'G57',
  gx: 5,
  gy: 7,
  pixelBounds: Object.freeze({ xMin: 960, xMax: 1152, yMin: 896, yMax: 1024 }),
  normalizedBounds: Object.freeze({ xMin: 0.625, xMax: 0.75, yMin: 0.875, yMax: 1.0 }),
  maskBounds: Object.freeze({ xMin: 60, xMax: 71, yMin: 56, yMax: 63 }),
  baseMaskWidth: 96,
  baseMaskHeight: 64,
...
function sampleWaterAtMaskCell(cellX, cellY) {
  const x = clamp(cellX, 0, G57_HYDROLOGY_POLICY.baseMaskWidth - 1);
  const y = clamp(cellY, 0, G57_HYDROLOGY_POLICY.baseMaskHeight - 1);
  const nx = (x + 0.5) / G57_HYDROLOGY_POLICY.baseMaskWidth;
  const ny = (y + 0.5) / G57_HYDROLOGY_POLICY.baseMaskHeight;
  return isWater(classifyReferenceBaseSurface(nx, ny));
}

export function isInsideG57(normalizedX, normalizedY) {
  const { xMin, xMax, yMin, yMax } = G57_HYDROLOGY_POLICY.normalizedBounds;
  return normalizedX >= xMin && normalizedX <= xMax && normalizedY >= yMin && normalizedY <= yMax;
}

export function sampleG57WaterConfidence(normalizedX, normalizedY) {
  if (!Number.isFinite(normalizedX) || !Number.isFinite(normalizedY)) {
    throw new TypeError('normalized coordinates must be finite');
  }
  if (!isInsideG57(normalizedX, normalizedY)) return null;

  const gridX = normalizedX * G57_HYDROLOGY_POLICY.baseMaskWidth - 0.5;
  const gridY = normalizedY * G57_HYDROLOGY_POLICY.baseMaskHeight - 0.5;
...
export function measureG57Hydrology() {
  const { xMin, xMax, yMin, yMax } = G57_HYDROLOGY_POLICY.maskBounds;
  let waterCells = 0;
  let landCells = 0;
  let boundaryEdges = 0;
  ...
  for (let maskY = yMin; maskY <= yMax; maskY += 1) {
    for (let maskX = xMin; maskX <= xMax; maskX += 1) {
      ...
      const nx = (maskX + 0.5) / G57_HYDROLOGY_POLICY.baseMaskWidth;
      const ny = (maskY + 0.5) / G57_HYDROLOGY_POLICY.baseMaskHeight;
      if (sampleG57WaterConfidence(nx, ny) !== water) centreMismatches += 1;
    }
  }
  ...
  const bounds = G57_HYDROLOGY_POLICY.normalizedBounds;
  ...
  for (let sy = 0; sy <= samplesY; sy += 1) {
    ...
    for (let sx = 0; sx <= samplesX; sx += 1) {
      ...
      const confidence = sampleG57WaterConfidence(nx, ny);
  ...
  return Object.freeze({
    policyId: G57_HYDROLOGY_POLICY.id,
    geoCell: 'G57',
