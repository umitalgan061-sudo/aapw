 ...
 * G57 owns canonical source pixels x=960..1152, y=896..1024. The immutable
...
export function measureG57Hydrology() {
  ...
  for (let y = yMin; y <= yMax; y += 1) for (let x = xMin; x <= xMax; x += 1) {
    ...
    if (sampleG57WaterConfidence((x + 0.5) / 96, (y + 0.5) / 64) !== water) centreMismatches += 1;
  }
  const samplesX = 48, samplesY = 32, grid = [];
  let fractionalSamples = 0, confidenceChecksum = 2166136261;
  const b = G57_HYDROLOGY_POLICY.normalizedBounds;
  ...
  return Object.freeze({ policyId: G57_HYDROLOGY_POLICY.id, geoCell: 'G57', baseCells: 96, waterCells, landCells, boundaryEdges, centreMismatches, refinedSamples: 1617, fractionalSamples, hardCellMaxStep: 1, maxAdjacentStep: Number(maxAdjacentStep.toFixed(8)), confidenceChecksum });
}
