/**
 * Kızıl Ufuk / SE GeoCell G66 coast-hydrology refinement.
 *
 * G66 owns canonical source pixels x=1152..1344, y=768..896, normalized
 * [0.75, 0.875] x [0.75, 0.875]. The immutable 96x64 owner-map surface mask
...
export const G66_HYDROLOGY_POLICY = Object.freeze({
  id: 'kizil-ufuk-g66-hydrology-2026-08-12-v1',
  sourceMapSha256: '20702972e8f45f0fbdc4da5fa68e890a82e4e822e1d58e2f369d8bc5b9c571a1',
  geoCell: 'G66',
  gx: 6,
  gy: 6,
  pixelBounds: Object.freeze({ xMin: 1152, xMax: 1344, yMin: 768, yMax: 896 }),
  normalizedBounds: Object.freeze({ xMin: 0.75, xMax: 0.875, yMin: 0.75, yMax: 0.875 }),
  maskBounds: Object.freeze({ xMin: 72, xMax: 83, yMin: 48, yMax: 55 }),
  baseMaskWidth: 96,
  baseMaskHeight: 64,
...
function sampleWaterAtMaskCell(cellX, cellY) {
  const x = clamp(cellX, 0, G66_HYDROLOGY_POLICY.baseMaskWidth - 1);
  const y = clamp(cellY, 0, G66_HYDROLOGY_POLICY.baseMaskHeight - 1);
  const nx = (x + 0.5) / G66_HYDROLOGY_POLICY.baseMaskWidth;
  const ny = (y + 0.5) / G66_HYDROLOGY_POLICY.baseMaskHeight;
  return isWater(classifyReferenceBaseSurface(nx, ny));
}

export function isInsideG66(normalizedX, normalizedY) {
  const { xMin, xMax, yMin, yMax } = G66_HYDROLOGY_POLICY.normalizedBounds;
  return normalizedX >= xMin && normalizedX <= xMax && normalizedY >= yMin && normalizedY <= yMax;
}

export function sampleG66WaterConfidence(normalizedX, normalizedY) {
  if (!Number.isFinite(normalizedX) || !Number.isFinite(normalizedY)) {
    throw new TypeError('normalized coordinates must be finite');
  }
  if (!isInsideG66(normalizedX, normalizedY)) return null;

  const gridX = normalizedX * G66_HYDROLOGY_POLICY.baseMaskWidth - 0.5;
  const gridY = normalizedY * G66_HYDROLOGY_POLICY.baseMaskHeight - 0.5;
...
export function measureG66Hydrology() {
  const { xMin, xMax, yMin, yMax } = G66_HYDROLOGY_POLICY.maskBounds;
  let waterCells = 0;
  let landCells = 0;
  let boundaryEdges = 0;
  ...
  for (let maskY = yMin; maskY <= yMax; maskY += 1) {
    for (let maskX = xMin; maskX <= xMax; maskX += 1) {
      ...
      const nx = (maskX + 0.5) / G66_HYDROLOGY_POLICY.baseMaskWidth;
      const ny = (maskY + 0.5) / G66_HYDROLOGY_POLICY.baseMaskHeight;
      if (sampleG66WaterConfidence(nx, ny) !== water) centreMismatches += 1;
    }
  }
  ...
  const bounds = G66_HYDROLOGY_POLICY.normalizedBounds;
  ...
  for (let sy = 0; sy <= samplesY; sy += 1) {
    ...
    for (let sx = 0; sx <= samplesX; sx += 1) {
      ...
      const confidence = sampleG66WaterConfidence(nx, ny);
  ...
  return Object.freeze({
    policyId: G66_HYDROLOGY_POLICY.id,
    geoCell: 'G66',
