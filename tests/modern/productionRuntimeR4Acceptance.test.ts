...
describe('R4 production acceptance', () => {
  ...
  it('rejects network session mismatches and accepts valid deltas', () => {
    ...
    const delta = network['#unused'] as never;
    void delta;
    ...
    expect(base.entities[0]?.x).toBe(0);
    expect(next.entities[0]?.x).toBe(1);
  });
  ...
  it('keeps diagnostics serializable after multiple captures', async () => {
    ...
    for(let i=0;i<3;i+=1){
      await runtime.frame({deltaSeconds:1/60,budget:{simulationMs:1,renderMs:2,streamingMs:0,networkMs:0,totalMs:3},wallTimeMs:i*20});
