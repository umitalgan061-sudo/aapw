...
import { getR11MigrationSnapshot, R11_MIGRATION_MODULES } from '../../src/3d/modern/migrationLedgerR11.ts';
...
describe('R11 typed world-support runtime',()=>{
 it('records complete R11 ownership migration',()=>{
  const snapshot=getR11MigrationSnapshot();
  expect(snapshot.version).toBe(10);
  expect(snapshot.migratedCount).toBe(R11_MIGRATION_MODULES.length);
